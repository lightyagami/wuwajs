...
(function (global) {
  ...
  if (typeof module !== "undefined" && module.exports && !global["encoding-indexes"]) {
    global["encoding-indexes"] = require("./encoding-indexes")["encoding-indexes"];
  }
  ...
  function UTF8Encoder(options) {
    ...
    this.handler = function (stream, code_point) {
      ...
      var bytes = [(code_point >> count * 6) + offset];

      // 5. Run these substeps while count is greater than 0:
      while (count > 0) {
        // 1. Set temp to code point >> (6 × (count − 1)).
        var temp = code_point >> (count - 1) * 6;

        // 2. Append to bytes 0x80 | (temp & 0x3F).
        bytes.push(temp & 63 | 128);

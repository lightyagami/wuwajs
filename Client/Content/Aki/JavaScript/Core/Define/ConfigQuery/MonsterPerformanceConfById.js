...
const byte_buffer_1 = require("../../../RunTimeLibs/FlatBuffers/byte-buffer"),
  ...
  COMMAND = "select BinData from `MonsterPerformanceConf` where MonsterPerformanceId=?",
...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configMonsterPerformanceConfById.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configMonsterPerformanceConfById.GetConfigList"),
  ...
  GetConfigList: (o, n = !0) => {
    ...
    if (t) {
      if (n) {
        ...
        if (f) return e?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), f
      }
      if (t = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, o, ...logPair)) {
        const f = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["MonsterPerformanceId", o])) break;
          var i = void 0;
          if ([t, i] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["MonsterPerformanceId", o]), !t) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), e?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
        ...
        return n && (r = KEY_PREFIX + `#${o})`, ConfigCommon_1.ConfigCommon.SaveConfig(r, f, f.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), e?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), f
      }

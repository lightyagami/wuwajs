...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configUiDynamicTabByParentViewName.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configUiDynamicTabByParentViewName.GetConfigList"),
  ...
  GetConfigList: (i, n = !0) => {
    ...
    if (t) {
      if (n) {
        ...
        if (m) return o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }
      if (t = ConfigCommon_1.ConfigCommon.BindString(handleId, 1, i, ...logPair)) {
        const m = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["ParentViewName", i])) break;
          var a = void 0;
          if ([t, a] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["ParentViewName", i]), !t) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          a = UiDynamicTab_1.UiDynamicTab.getRootAsUiDynamicTab(new byte_buffer_1.ByteBuffer(new Uint8Array(a.buffer)));
          m.push(a)
        }
        return n && (e = KEY_PREFIX + `#${i})`, ConfigCommon_1.ConfigCommon.SaveConfig(e, m, m.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }

...
const byte_buffer_1 = require("../../../RunTimeLibs/FlatBuffers/byte-buffer"),
  ...
  COMMAND = "select BinData from `FarmGoldActivity` where ActivityId=? And InstId=?",
...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configFarmGoldActivityByActivityIdAndInstanceId.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configFarmGoldActivityByActivityIdAndInstanceId.GetConfigList"),
  CONFIG_LIST_STAT_PREFIX = "configFarmGoldActivityByActivityIdAndInstanceId.GetConfigList(";
    ...
    if (e) {
      ...
      if (e = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, t, ...logPair) && ConfigCommon_1.ConfigCommon.BindInt(handleId, 2, i, ...logPair)) {
        ...
        for (;;) {
          ...
          var d = void 0;
          if ([e, d] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["ActivityId", t], ["InstId", i]), !e) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), n?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          d = FarmGoldActivity_1.FarmGoldActivity.getRootAsFarmGoldActivity(new byte_buffer_1.ByteBuffer(new Uint8Array(d.buffer)));
          C.push(d)
        }
        return o && (a = KEY_PREFIX + `#${t}#${i})`, ConfigCommon_1.ConfigCommon.SaveConfig(a, C, C.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), n?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), C
      }

...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configMobileBattleUiSetByPanelIndex.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configMobileBattleUiSetByPanelIndex.GetConfigList"),
  CONFIG_LIST_STAT_PREFIX = "configMobileBattleUiSetByPanelIndex.GetConfigList(";
  ...
  GetConfigList: (t, e = !0) => {
    ...
    if (i) {
      if (e) {
        ...
        if (l) return o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), l
      }
      if (i = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, t, ...logPair)) {
        const l = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["PanelIndex", t])) break;
          var a = void 0;
          if ([i, a] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["PanelIndex", t]), !i) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          a = MobileBattleUiSet_1.MobileBattleUiSet.getRootAsMobileBattleUiSet(new byte_buffer_1.ByteBuffer(new Uint8Array(a.buffer)));
          l.push(a)
        }
        return e && (n = KEY_PREFIX + `#${t})`, ConfigCommon_1.ConfigCommon.SaveConfig(n, l, l.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), l
      }

...
const byte_buffer_1 = require("../../../RunTimeLibs/FlatBuffers/byte-buffer"),
  ...
  COMMAND = "select BinData from `PhysicsAssetConfig` where id = ? AND (SELECT count() from `PhysicsAssetConfig` WHERE id = ?) <= 0 OR id = ? AND (SELECT count(0) from `PhysicsAssetConfig` WHERE id = ?) >0;",
...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configPhysicsAssetConfigByIdWithDefaultId.Init"),
  getConfigStat = Stats_1.Stat.CreateNoFlameGraph("configPhysicsAssetConfigByIdWithDefaultId.GetConfig"),
...
exports.configPhysicsAssetConfigByIdWithDefaultId = {
  ...
  GetConfig: (o, i, n, t, e = !0) => {
    ...
    var s = Stats_1.Stat.CreateNoFlameGraph(CONFIG_STAT_PREFIX + `#${o}#${i}#${n}#${t})`),
      C = (s?.Start(), ConfigCommon_1.ConfigCommon.CheckStatement(handleId, ...logPair));
    if (C) {
      if (e) {
        var f = KEY_PREFIX + `#${o}#${i}#${n}#${t})`;
        const g = ConfigCommon_1.ConfigCommon.GetConfig(f);
        if (g) return s?.Stop(), getConfigStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), g
      }
      if (C = ConfigCommon_1.ConfigCommon.BindString(handleId, 1, o, ...logPair) && ConfigCommon_1.ConfigCommon.BindString(handleId, 2, i, ...logPair) && ConfigCommon_1.ConfigCommon.BindString(handleId, 3, n, ...logPair) && ConfigCommon_1.ConfigCommon.BindString(handleId, 4, t, ...logPair) && 0 < ConfigCommon_1.ConfigCommon.Step(handleId, !0, ...logPair, ["Id", o], ["Id", i], ["Id", n], ["Id", t])) {
        f = void 0;
        if ([C, f] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["Id", o], ["Id", i], ["Id", n], ["Id", t]), C) {
          ...
          return e && (C = KEY_PREFIX + `#${o}#${i}#${n}#${t})`, ConfigCommon_1.ConfigCommon.SaveConfig(C, g)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), s?.Stop(), getConfigStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), g
        }
      }
      ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair)
    }
    s?.Stop(), getConfigStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop()
  }
};

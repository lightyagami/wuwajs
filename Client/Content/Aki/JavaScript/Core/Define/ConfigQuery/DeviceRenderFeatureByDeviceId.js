...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configDeviceRenderFeatureByDeviceId.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configDeviceRenderFeatureByDeviceId.GetConfigList"),
  CONFIG_LIST_STAT_PREFIX = "configDeviceRenderFeatureByDeviceId.GetConfigList(";
  ...
  GetConfigList: (e, i = !0) => {
    ...
    if (n) {
      if (i) {
        ...
        if (a) return o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), a
      }
      if (n = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, e, ...logPair)) {
        const a = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["DeviceId", e])) break;
          var r = void 0;
          if ([n, r] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["DeviceId", e]), !n) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
        ...
        return i && (t = KEY_PREFIX + `#${e})`, ConfigCommon_1.ConfigCommon.SaveConfig(t, a, a.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), a
      }

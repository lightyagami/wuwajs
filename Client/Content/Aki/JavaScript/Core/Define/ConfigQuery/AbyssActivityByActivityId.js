...
let handleId = 0;
const initStat = Stats_1.Stat.CreateNoFlameGraph("configAbyssActivityByActivityId.Init"),
  getConfigStat = Stats_1.Stat.CreateNoFlameGraph("configAbyssActivityByActivityId.GetConfig"),
    ...
    if (n) {
      ...
      if (n = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, i, ...logPair) && 0 < ConfigCommon_1.ConfigCommon.Step(handleId, !0, ...logPair, ["ActivityId", i])) {
        ...
        if ([n, e] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["ActivityId", i]), n) {
          const C = AbyssActivity_1.AbyssActivity.getRootAsAbyssActivity(new byte_buffer_1.ByteBuffer(new Uint8Array(e.buffer)));
          return t && (n = KEY_PREFIX + `#${i})`, ConfigCommon_1.ConfigCommon.SaveConfig(n, C)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), o?.Stop(), getConfigStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), C
        }
      }

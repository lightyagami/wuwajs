...
let handleId = 0;
const initStat = Stats_1.Stat.CreateNoFlameGraph("configSkillTreeByNodeIndex.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configSkillTreeByNodeIndex.GetConfigList"),
    ...
    if (n) {
      ...
      if (n = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, o, ...logPair)) {
        ...
        for (;;) {
          ...
          var r = void 0;
          if ([n, r] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["NodeGroup", o]), !n) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), i?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          r = SkillTree_1.SkillTree.getRootAsSkillTree(new byte_buffer_1.ByteBuffer(new Uint8Array(r.buffer)));
          C.push(r)
        }
        return e && (t = KEY_PREFIX + `#${o})`, ConfigCommon_1.ConfigCommon.SaveConfig(t, C, C.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), i?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), C
      }

...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configBuffItemByPublicCdGroup.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configBuffItemByPublicCdGroup.GetConfigList"),
  ...
  GetConfigList: (o, t = !0) => {
    ...
    if (n) {
      ...
      if (n = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, o, ...logPair)) {
        const C = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["PublicCdGroup", o])) break;
          var f = void 0;
          if ([n, f] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["PublicCdGroup", o]), !n) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), i?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          f = BuffItem_1.BuffItem.getRootAsBuffItem(new byte_buffer_1.ByteBuffer(new Uint8Array(f.buffer)));
          C.push(f)
        }
        return t && (e = KEY_PREFIX + `#${o})`, ConfigCommon_1.ConfigCommon.SaveConfig(e, C, C.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), i?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), C
      }

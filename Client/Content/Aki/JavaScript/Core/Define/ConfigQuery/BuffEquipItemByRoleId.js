...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configBuffEquipItemByRoleId.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configBuffEquipItemByRoleId.GetConfigList"),
    ...
    if (n) {
      ...
      if (n = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, o, ...logPair)) {
        ...
        for (;;) {
          ...
          var f = void 0;
          if ([n, f] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["RoleId", o]), !n) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), t?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          f = BuffEquipItem_1.BuffEquipItem.getRootAsBuffEquipItem(new byte_buffer_1.ByteBuffer(new Uint8Array(f.buffer)));
          C.push(f)
        }
        return i && (e = KEY_PREFIX + `#${o})`, ConfigCommon_1.ConfigCommon.SaveConfig(e, C, C.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), t?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), C
      }

...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configMappingBySheetNameAndFieldName.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configMappingBySheetNameAndFieldName.GetConfigList"),
  CONFIG_LIST_STAT_PREFIX = "configMappingBySheetNameAndFieldName.GetConfigList(";
  ...
  GetConfigList: (n, i, o = !0) => {
    ...
    if (t) {
      if (o) {
        ...
        if (m) return e?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }
      if (t = ConfigCommon_1.ConfigCommon.BindString(handleId, 1, n, ...logPair) && ConfigCommon_1.ConfigCommon.BindString(handleId, 2, i, ...logPair)) {
        const m = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["SheetName", n], ["FieldName", i])) break;
          var g = void 0;
          if ([t, g] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["SheetName", n], ["FieldName", i]), !t) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), e?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          g = Mapping_1.Mapping.getRootAsMapping(new byte_buffer_1.ByteBuffer(new Uint8Array(g.buffer)));
          m.push(g)
        }
        return o && (a = KEY_PREFIX + `#${n}#${i})`, ConfigCommon_1.ConfigCommon.SaveConfig(a, m, m.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), e?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }

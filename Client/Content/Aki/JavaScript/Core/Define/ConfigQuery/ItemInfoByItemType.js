...
const initStat = Stats_1.Stat.CreateNoFlameGraph("configItemInfoByItemType.Init"),
  getConfigListStat = Stats_1.Stat.CreateNoFlameGraph("configItemInfoByItemType.GetConfigList"),
  ...
  GetConfigList: (o, t = !0) => {
    ...
    if (e) {
      if (t) {
        ...
        if (m) return n?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }
      if (e = ConfigCommon_1.ConfigCommon.BindInt(handleId, 1, o, ...logPair)) {
        const m = new Array;
        for (;;) {
          if (1 !== ConfigCommon_1.ConfigCommon.Step(handleId, !1, ...logPair, ["ItemType", o])) break;
          var f = void 0;
          if ([e, f] = ConfigCommon_1.ConfigCommon.GetValue(handleId, 0, ...logPair, ["ItemType", o]), !e) return ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), n?.Stop(), getConfigListStat?.Stop(), void ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop();
          f = ItemInfo_1.ItemInfo.getRootAsItemInfo(new byte_buffer_1.ByteBuffer(new Uint8Array(f.buffer)));
          m.push(f)
        }
        return t && (i = KEY_PREFIX + `#${o})`, ConfigCommon_1.ConfigCommon.SaveConfig(i, m, m.length)), ConfigCommon_1.ConfigCommon.Reset(handleId, ...logPair), n?.Stop(), getConfigListStat?.Stop(), ConfigCommon_1.ConfigCommon.AllConfigStatementStat.Stop(), m
      }

"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.PhysicsAssetConfig = undefined;
const GameUtils_1 = require("../../../Game/GameUtils");
class PhysicsAssetConfig {
  constructor() {
    this.J7 = null;
    this.z7 = 0;
  }
  get Id() {
    return this.id();
  }
  get PhysicsAssetPath() {
    return this.physicsassetpath();
  }
  get BoneNames() {
    return GameUtils_1.GameUtils.ConvertToArray(this.bonenamesLength(), this.bonenames, this);
  }
  __init(t, s) {
    this.z7 = t;
    this.J7 = s;
    return this;
  }
  static getRootAsPhysicsAssetConfig(t, s) {
    return (s || new PhysicsAssetConfig()).__init(t.readInt32(t.position()) + t.position(), t);
  }
  id(t) {
    var s = this.J7.__offset(this.z7, 4);
    var s = s ? this.J7.__string(this.z7 + s, t) : null;
    if (typeof s == "string" && GameUtils_1.GameUtils.IsOptimizeDbString) {
      GameUtils_1.GameUtils.InternalizedString(s);
    }
    return s;
  }
  physicsassetpath(t) {
    var s = this.J7.__offset(this.z7, 6);
    var s = s ? this.J7.__string(this.z7 + s, t) : null;
    if (typeof s == "string" && GameUtils_1.GameUtils.IsOptimizeDbString) {
      GameUtils_1.GameUtils.InternalizedString(s);
    }
    return s;
  }
  GetBonenamesAt(t) {
    return this.bonenames(t);
  }
  bonenames(t, s) {
    var i = this.J7.__offset(this.z7, 8);
    var i = i ? this.J7.__string(this.J7.__vector(this.z7 + i) + t * 4, s) : null;
    if (typeof i == "string" && GameUtils_1.GameUtils.IsOptimizeDbString) {
      GameUtils_1.GameUtils.InternalizedString(i);
    }
    return i;
  }
  bonenamesLength() {
    var t = this.J7.__offset(this.z7, 8);
    if (t) {
      return this.J7.__vector_len(this.z7 + t);
    } else {
      return 0;
    }
  }
}
exports.PhysicsAssetConfig = PhysicsAssetConfig;
//# sourceMappingURL=PhysicsAssetConfig.js.map
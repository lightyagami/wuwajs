"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.RiskHarvestDifficulty = undefined;
const GameUtils_1 = require("../../../Game/GameUtils");
class RiskHarvestDifficulty {
  constructor() {
    this.J7 = null;
    this.z7 = 0;
  }
  get Id() {
    return this.id();
  }
  get LevelGroup() {
    return GameUtils_1.GameUtils.ConvertToArray(this.levelgroupLength(), this.levelgroup, this);
  }
  get TimeRatio() {
    return this.timeratio();
  }
  get MonsterRatio() {
    return this.monsterratio();
  }
  __init(t, i) {
    this.z7 = t;
    this.J7 = i;
    return this;
  }
  static getRootAsRiskHarvestDifficulty(t, i) {
    return (i || new RiskHarvestDifficulty()).__init(t.readInt32(t.position()) + t.position(), t);
  }
  id() {
    var t = this.J7.__offset(this.z7, 4);
    if (t) {
      return this.J7.readInt32(this.z7 + t);
    } else {
      return 0;
    }
  }
  GetLevelgroupAt(t) {
    return this.levelgroup(t);
  }
  levelgroup(t) {
    var i = this.J7.__offset(this.z7, 6);
    if (i) {
      return this.J7.readInt32(this.J7.__vector(this.z7 + i) + t * 4);
    } else {
      return 0;
    }
  }
  levelgroupLength() {
    var t = this.J7.__offset(this.z7, 6);
    if (t) {
      return this.J7.__vector_len(this.z7 + t);
    } else {
      return 0;
    }
  }
  levelgroupArray() {
    var t = this.J7.__offset(this.z7, 6);
    if (t) {
      return new Int32Array(this.J7.bytes().buffer, this.J7.bytes().byteOffset + this.J7.__vector(this.z7 + t), this.J7.__vector_len(this.z7 + t));
    } else {
      return null;
    }
  }
  timeratio() {
    var t = this.J7.__offset(this.z7, 8);
    if (t) {
      return this.J7.readInt32(this.z7 + t);
    } else {
      return 0;
    }
  }
  monsterratio() {
    var t = this.J7.__offset(this.z7, 10);
    if (t) {
      return this.J7.readInt32(this.z7 + t);
    } else {
      return 0;
    }
  }
}
exports.RiskHarvestDifficulty = RiskHarvestDifficulty;
//# sourceMappingURL=RiskHarvestDifficulty.js.map
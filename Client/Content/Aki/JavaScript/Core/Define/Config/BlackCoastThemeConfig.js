"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.BlackCoastThemeConfig = undefined;
const GameUtils_1 = require("../../../Game/GameUtils");
class BlackCoastThemeConfig {
  constructor() {
    this.J7 = null;
    this.z7 = 0;
  }
  get ActivityId() {
    return this.activityid();
  }
  get ItemId() {
    return this.itemid();
  }
  get WeaponPreviewId() {
    return GameUtils_1.GameUtils.ConvertToArray(this.weaponpreviewidLength(), this.weaponpreviewid, this);
  }
  __init(t, i) {
    this.z7 = t;
    this.J7 = i;
    return this;
  }
  static getRootAsBlackCoastThemeConfig(t, i) {
    return (i || new BlackCoastThemeConfig()).__init(t.readInt32(t.position()) + t.position(), t);
  }
  activityid() {
    var t = this.J7.__offset(this.z7, 4);
    if (t) {
      return this.J7.readInt32(this.z7 + t);
    } else {
      return 0;
    }
  }
  itemid() {
    var t = this.J7.__offset(this.z7, 6);
    if (t) {
      return this.J7.readInt32(this.z7 + t);
    } else {
      return 0;
    }
  }
  GetWeaponpreviewidAt(t) {
    return this.weaponpreviewid(t);
  }
  weaponpreviewid(t) {
    var i = this.J7.__offset(this.z7, 8);
    if (i) {
      return this.J7.readInt32(this.J7.__vector(this.z7 + i) + t * 4);
    } else {
      return 0;
    }
  }
  weaponpreviewidLength() {
    var t = this.J7.__offset(this.z7, 8);
    if (t) {
      return this.J7.__vector_len(this.z7 + t);
    } else {
      return 0;
    }
  }
  weaponpreviewidArray() {
    var t = this.J7.__offset(this.z7, 8);
    if (t) {
      return new Int32Array(this.J7.bytes().buffer, this.J7.bytes().byteOffset + this.J7.__vector(this.z7 + t), this.J7.__vector_len(this.z7 + t));
    } else {
      return null;
    }
  }
}
exports.BlackCoastThemeConfig = BlackCoastThemeConfig;
//# sourceMappingURL=BlackCoastThemeConfig.js.map
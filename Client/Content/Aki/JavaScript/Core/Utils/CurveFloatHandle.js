...
exports.CurveFloatHandle = exports.RichCurveKeyHandle = undefined;
const UE = require("ue");
const MathUtils_1 = require("./MathUtils");
const MAX_flt = 3.402823466e+38;
class RichCurveKeyHandle {
  ...
  ToUeRichCurveKey() {
    if (this.Joh) {
      ...
      this.Joh = new UE.RichCurveKey(this.InterpMode, this.TangentMode, this.TangentWeightMode, this.Time, this.Value, this.ArriveTangent, this.ArriveTangentWeight, this.LeaveTangent, this.LeaveTangentWeight);
    }
    return this.Joh;
  }
}
...
class CurveFloatHandle {
  ...
  MapRangeClampedTimeAndValue(t, s, i, h) {
    ...
    var r;
    var l;
    var o = (h[1] - h[0]) / (i[1] - i[0]) / ((s[1] - s[0]) / (t[1] - t[0]));
    for (const n of this.FloatCurveKeys) {
      ...
      if (n.Time === s[0]) {
        ...
        if (n.TangentWeightMode === 3) {
          n.TangentWeightMode = 2;
        } else if (n.TangentWeightMode === 1) {
          n.TangentWeightMode = 0;
        }
      } else {
        if (n.TangentWeightMode !== 0 && n.TangentWeightMode !== 2) {
          a = Math.atan(n.ArriveTangent);
          e = Math.cos(a) * n.ArriveTangentWeight;
          a = Math.sin(a) * n.ArriveTangentWeight;
          e = MathUtils_1.MathUtils.Lerp(s[0], s[1], MathUtils_1.MathUtils.GetRangePct(t[0], t[1], e));
          a = MathUtils_1.MathUtils.Lerp(h[0], h[1], MathUtils_1.MathUtils.GetRangePct(i[0], i[1], a));
      ...
      if (n.Time === s[1]) {
        ...
        } else if (n.TangentWeightMode === 2) {
          n.TangentWeightMode = 0;
        }
      } else {
        if (n.TangentWeightMode !== 0 && n.TangentWeightMode !== 1) {
          e = Math.atan(n.LeaveTangent);
          a = Math.cos(e) * n.LeaveTangentWeight;
          l = Math.sin(e) * n.LeaveTangentWeight;
          r = MathUtils_1.MathUtils.Lerp(s[0], s[1], MathUtils_1.MathUtils.GetRangePct(t[0], t[1], a));
          l = MathUtils_1.MathUtils.Lerp(h[0], h[1], MathUtils_1.MathUtils.GetRangePct(i[0], i[1], l));
  ...
  ToUeCurveFloat() {
    this.Zoh ||= new UE.CurveFloat();
    var t = this.Zoh.FloatCurve;
    this.Zoh.bIsEventCurve = this.IsEventCurve;
    t.Keys.Empty();
    for (const s of this.FloatCurveKeys) {
      t.Keys.Add(s.ToUeRichCurveKey());
    }

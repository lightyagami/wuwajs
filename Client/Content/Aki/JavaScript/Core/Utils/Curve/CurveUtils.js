"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.CurveUtils = undefined;
const UE = require("ue");
const Log_1 = require("../../Common/Log");
const CubicCurve_1 = require("./CubicCurve");
const CubicCurveWithStartSlope_1 = require("./CubicCurveWithStartSlope");
const FloatCurve_1 = require("./FloatCurve");
const LinearCurve_1 = require("./LinearCurve");
const PowerCurve_1 = require("./PowerCurve");
const PowerCurve2_1 = require("./PowerCurve2");
const PowerCurve3_1 = require("./PowerCurve3");
const SquaredCurve_1 = require("./SquaredCurve");
const curveTypeMap = {
  [0]: LinearCurve_1.LinearCurve,
  1: CubicCurve_1.CubicCurve,
  2: CubicCurveWithStartSlope_1.CubicCurveWithStartSlope,
  3: SquaredCurve_1.SquaredCurve,
  4: PowerCurve_1.PowerCurve,
  5: PowerCurve2_1.PowerCurve2
};
class CurveUtils {
  static CreateCurve(e, ...r) {
    if (e === 0) {
      return this.DefaultLinear;
    } else {
      return new curveTypeMap[e](...r);
    }
  }
  static CreateCurveByStruct(e) {
    if (!e) {
      return this.DefaultLinear;
    }
    if (e instanceof UE.SFloatCurve) {
      if (e.FloatCurve) {
        return new FloatCurve_1.FloatCurve(e.FloatCurve);
      } else {
        if (Log_1.Log.CheckError()) {
          Log_1.Log.Error("Core", 57, "浮点曲线参数不合法");
        }
        return this.DefaultLinear;
      }
    }
    switch (e.CurveType) {
      case 0:
        return this.DefaultLinear;
      case 5:
        return new CubicCurve_1.CubicCurve(e.N);
      case 6:
        if (e.N === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("Core", 6, "三阶 - 反S函数不接受0作为N");
          }
          return this.DefaultLinear;
        } else {
          return new CubicCurve_1.CubicCurve(1 / e.N);
        }
      case 3:
        if (e.N === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("Core", 6, "三阶 - 凹函数不接受0作为N");
          }
          return this.DefaultLinear;
        } else {
          return new CubicCurveWithStartSlope_1.CubicCurveWithStartSlope(1 / e.N);
        }
      case 4:
        return new CubicCurveWithStartSlope_1.CubicCurveWithStartSlope(e.N);
      case 1:
        if (e.N === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("Core", 6, "二阶 - 凹函数不接受0作为N");
          }
          return this.DefaultLinear;
        } else {
          return new SquaredCurve_1.SquaredCurve(1 / e.N);
        }
      case 2:
        return new SquaredCurve_1.SquaredCurve(e.N);
      case 7:
        return new PowerCurve3_1.PowerCurve3(e.N);
      case 8:
        if (e.N === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("Core", 6, "幂函数 - 凸函数不接受0作为N");
          }
          return this.DefaultLinear;
        } else {
          return new PowerCurve3_1.PowerCurve3(1 / e.N);
        }
      case 9:
        if (e.N === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("Core", 6, "幂函数 - S函数不接受0作为N");
          }
          return this.DefaultLinear;
        } else {
          return new PowerCurve_1.PowerCurve(1 / e.N);
        }
      case 10:
        return new PowerCurve_1.PowerCurve(e.N);
      case 11:
        return new PowerCurve2_1.PowerCurve2(e.N);
      default:
        if (Log_1.Log.CheckError()) {
          Log_1.Log.Error("Core", 6, "无效曲线类型", ["Type", e.CurveType]);
        }
        return this.DefaultLinear;
    }
  }
}
(exports.CurveUtils = CurveUtils).DefaultLinear = new LinearCurve_1.LinearCurve();
CurveUtils.DefaultPara = new SquaredCurve_1.SquaredCurve(2);
CurveUtils.DefaultCubic = new CubicCurve_1.CubicCurve(0); //# sourceMappingURL=CurveUtils.js.map
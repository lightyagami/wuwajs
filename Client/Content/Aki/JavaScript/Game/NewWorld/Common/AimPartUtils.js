...
class AimPart {
  ...
  InitSceneItem(t) {
    this.SceneItemHit = this.OwnerBase.Entity.GetComponent(155);
    this.BoneNameString = t.BoneName ?? "";
  ...
  GetAimPointLocation(t) {
    if (this.OwnerCharacter) {
      AimPart.tga.FromUeTransform(this.OwnerCharacter.Actor.Mesh.D_GetSocketTransform(this.BoneName));
    } else {
      if (!this.SceneItemHit) {
        return false;
      }
      if (this.BoneNameString) {
        let t = this.OwnerBase.GetActorInSceneInteraction(this.BoneNameString);
        if (!t && !(t = this.OwnerBase.Owner)) {
          return false;
        }
    ...
    AimPart.tga.TransformPosition(this.Offset, t);
    return true;
  }
}

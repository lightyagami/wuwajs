...
class SensoryInfoController {
  constructor() {
    this._A = 0, this.SensoryInfoType = 0, this.MaxSensoryRange = 0, this.Wrr = new Map
  }
  Tick(t) {
  ...
  HandleEntities(t, s, o) {
    ...
    for (const n of t) {
      ...
      if (n && n.Entity?.Active && r && i !== o) {
        ...
        for (const f of this.Wrr) e > f[1].SensoryRange || !f[1].CheckEntity(n.Entity) || f[1].EnterRange(n.Entity)
      }
    }
    for (const c of this.Wrr) c[1].ExitRange()
  }
  AddSensoryInfo(t) {
    this.SensoryInfoType |= t.SensoryInfoType, this.MaxSensoryRange = Math.max(this.MaxSensoryRange, t.SensoryRange);
    var s = ++this._A;
    return this.Wrr.set(s, t), s
  }
  RemoveSensoryInfo(t) {
    return !!this.Wrr.has(t) && (this.Wrr.delete(t), this.UpdateInfoType(), this.UpdateSensoryRange(), !0)
  }
  UpdateInfoType() {
  ...
  UpdateSensoryRange() {
    ...
    for (const t of this.Wrr) this.MaxSensoryRange = Math.max(this.MaxSensoryRange, t[1].SensoryRange)
  }
  Clear() {

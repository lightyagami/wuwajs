...
class DamageAccumulation extends ExtraEffectBase_1.BuffEffect {
  constructor() {
    super(...arguments), this.rXo = 0, this.nXo = 0, this.sXo = 0, this.aXo = [], this.hXo = 0, this.lXo = -0, this.ine = void 0
  }
  InitParameters(t) {
    var t = t.ExtraEffectParameters,
      e = (this.nXo = Number(t[0]), this.sXo = Number(t[1]), this.aXo = t[2].split("#").map(t => Number(t)), (t[3] ?? "").split("#").map(t => Number(t)));
  ...
  static ApplyEffects(t, e, s, i) {
    var r, a, h = [s.OwnerBuffComponent, i.OwnerBuffComponent];
    for ([r, a] of h.entries()) {
      var c = h[1 - r];
      for (const f of a.BuffEffectManager.FilterById(19)) f.Check(e, c) && f.Execute(r, t)
    }
  }
  ...
  _Xo(t) {
    if (this.cXo(t)) switch (t) {
      case 0: {
        let t = 0;
        var e;
        (t = this.ine ? (e = this.lXo * CharacterAttributeTypes_1.DIVIDED_TEN_THOUSAND, this.OwnerEntity.GetComponent(173)?.GetBaseValue(this.ine) * e) : this.lXo) < this.rXo && this.mXo();
        break
      }
      default:
        this.mXo()
  ...
  CXo() {
    ...
    if (t?.IsValid())
      for (const e of this.aXo) this.OwnerBuffComponent.AddIterativeBuff(e, t, void 0, !0, `因为其它buff额外效果而移除（前置buff Id=${this.BuffId}, handle=${this.ActiveHandleId}）`)
  }
  dXo() {

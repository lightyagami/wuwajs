...
exports.CharacterMoveComponent = undefined;
const UE = require("ue");
...
let CharacterMoveComponent = CharacterMoveComponent_1 = class CharacterMoveComponent extends BaseMoveComponent_1.BaseMoveComponent {
  constructor() {
    ...
    this.OnLand = () => {
      this.GroundedTimeUe = UE.GameplayStatics.GetTimeSeconds(this.ActorComp.Actor);
    ...
    this.OnStateInherit = (t, e) => {
      ...
      var s;
      var h;
      if (t?.Valid && (i = t.GetComponent(178))?.Valid) {
        h = t.GetComponent(62);
        s = this.Entity.GetComponent(62);
        if (h && s && (h = h.GetAutoMovingConfig()).GetAutoMovingState()) {
          ...
          s.SetAutoMovingConfig(h);
          h.ResetAutoMovingState("切人");
        }
        ...
        if (!e) {
          this.jB1(i);
          s = t.GetComponent(205);
          h = t.GetComponent(175);
          if (i.CharacterMovement.MovementMode === 0 || s?.HasTag(-2100129479)) {
            this.ActorComp.Actor.KuroSetMovementMode({
              Mode: 1,
              CustomMode: 0,
              Context: "[CharacterMoveComponent.OnStateInherit] MOVE_None"
            });
          } else if (i.CharacterMovement.MovementMode !== 5 && this.UnifiedStateComponent?.MoveState !== CharacterUnifiedStateTypes_1.ECharMoveState.Roll && h?.PositionState !== CharacterUnifiedStateTypes_1.ECharPositionState.Ride && !this.WalkOnWaterComp?.BlockMoveModeInherit(i.CharacterMovement)) {
            this.ActorComp.Actor.KuroSetMovementMode({
    ...
    this.OnSpeedRatioAttributeChanged = (t, e, i) => {
      ...
      var h = this.Entity.GetComponent(178);
      if (h?.Valid) {
        h.ResetMaxSpeed(s);
      }
    };
    ...
    this.InitWalkableFloorAngle = MAX_WALK_FLOOR_ANGLE;
  }
  ...
  jB1(t) {
    ...
    t.Entity.GetComponent(113)?.DumpVelocityCacheInfo("战斗换人");
    if (Log_1.Log.CheckDebug()) {
  ...
  OnStart() {
    ...
    this.AttributeComponent = this.Entity.GetComponent(173);
    this.TagComponent = this.Entity.GetComponent(205);
    this.DeathComponent = this.Entity.GetComponent(15);
    this.UnifiedStateComponent = this.Entity.GetComponent(101);
    ...
    this.MovementData = DataTableUtil_1.DataTableUtil.GetDataTableRow(this.ActorComp.Actor.DtBaseMovementSetting, CharacterNameDefines_1.CharacterNameDefines.NORMAL.toString());
  ...
  InitStepUpParams() {
    if (this.CharacterMovement) {
      ...
      this.InitWalkableFloorAngle = this.CharacterMovement.WalkableFloorAngle;
    }
  }
  ...
  ResetWalkableFloorAngle() {
    if (this.CharacterMovement) {
      this.CharacterMovement.SetWalkableFloorAngle(this.InitWalkableFloorAngle);
    }
  }
  ...
  b71() {
    return !!CharacterMoveComponent_1.EnableKuroAsyncRootMotion && !!this.yku();
  }
...
CharacterMoveComponent = CharacterMoveComponent_1 = __decorate([(0, RegisterComponent_1.RegisterComponent)(178)], CharacterMoveComponent);

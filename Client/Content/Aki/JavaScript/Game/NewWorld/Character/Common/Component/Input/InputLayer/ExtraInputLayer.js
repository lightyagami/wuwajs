...
class ExtraInputLayer extends InputLayer_1.InputLayer {
  ...
  Init(s) {
    var s = s.Entity;
    var t = s.GetComponent(220);
    var e = t?.GetCharacterLoadTypeList();
    var n = t?.GetFightInfo()?.BpInputMap;
    let u = "";
    if (e && n) {
      for (const r of e) {
        var a = n.Get(r);
        if ((u = a?.AssetPathName.toString() ?? "") && u !== "None") {
          this.I4d = true;
          break;
        }
      }
    ...
    if (this.I4d) {
      if (u) {
        const i = s.GetComponent(3).Actor;
        ResourceSystem_1.ResourceSystem.LoadAsync(u, UE.Class, s => {
          this.Aia = i.AddComponentByClass(s, false, MathUtils_1.MathUtils.DefaultTransform, false);
          this.Aia.OwnerActor = i;
        });
      } else if (Log_1.Log.CheckError()) {
        Log_1.Log.Error("Input", 67, "[ExtraInputLayer]加载BpInput失败", ["EntityId", s.Id]);
      }
    }
  }
  Clear() {

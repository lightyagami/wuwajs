...
let CharacterSelfCenterComponent = class CharacterSelfCenterComponent extends EntityComponent_1.EntityComponent {
  constructor() {
    ...
    this.tZ = false;
    this.W8c = t => {
      var t = EntitySystem_1.EntitySystem.GetComponent(t, 293);
      if (t?.$8c) {
  ...
  OnStart() {
    ...
    this.vHr = this.Entity.GetComponent(183);
    this.H8c = this.Entity.GetComponent(56);
    this.uZr = this.Entity.GetComponent(16);
    this.hed = this.Entity.GetComponent(126);
    this.led = new Set();
    this._ed = new Set();
  ...
  SetSelfCenterTimeDilation(t, e = true) {
    ...
    if (e) {
      ...
      if (i) {
        for (const s of i) {
          EntitySystem_1.EntitySystem.GetComponent(s, 293)?.SetSelfCenterTimeDilation(t, e);
        }
      }
      i = this.EIe.CustomServerEntityIds;
      if (i.length > 0) {
        for (const h of i) {
          EntitySystem_1.EntitySystem.GetComponent(ModelManager_1.ModelManager.CreatureModel.GetEntityId(h), 293)?.SetSelfCenterTimeDilation(t, e);
        }
      }
    }
    this.$8c = t !== 1 && e;
...
CharacterSelfCenterComponent = __decorate([(0, RegisterComponent_1.RegisterComponent)(293)], CharacterSelfCenterComponent);

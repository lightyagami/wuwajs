...
class ExtraEffectDestroyBullet extends ExtraEffectBase_1.BuffEffect {
  constructor() {
    super(...arguments), this.XDl = void 0, this.YDl = 0, this.zDl = !1, this.e6c = 0, this.Tq_ = 0
  }
  CheckExecutable() {
  ...
  OnRemoved() {
    if (0 !== this.YDl && this.XDl) {
      ...
      if (e) {
        ...
        if (e) {
          var i, r = (0 === this.e6c ? t.GetComponent(1) : ModelManager_1.ModelManager.SceneTeamModel.GetCurrentEntity?.Entity?.GetComponent(1))?.ActorLocationProxy;
          if (this.Tq_ <= 0 || !r)
            for (const o of e) o?.Valid && this.XDl.includes(o.GetBulletInfo().BulletRowName) && s.DestroyBullet(o.Id, this.zDl, 4);
          else
            for (const a of e) a?.Valid && this.XDl.includes(a.GetBulletInfo().BulletRowName) && (i = a.GetComponent(1)?.ActorLocationProxy) && Vector_1.Vector.DistSquared(i, r) <= this.Tq_ && s.DestroyBullet(a.Id, this.zDl, 4)
        } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 20, "无法获取子弹集合", ["Buff", this.BuffId])
      } else Log_1.Log.CheckError() && Log_1.Log.Error("Battle", 20, "无法获取子弹拥有者", ["Buff", this.BuffId])
    }
  }
}

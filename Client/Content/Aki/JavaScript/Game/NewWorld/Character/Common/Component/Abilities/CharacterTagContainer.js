...
class TagContainer {
  constructor() {
    this.PQo = new Map, this.xQo = new Map, this.wQo = new Map, this.BQo = new Set, this.bQo = new Set, this.qQo = void 0
  }
  GetAllExactTags() {
  ...
  BindTsTagContainer(t) {
    if (this.qQo = t) {
      ...
      Log_1.Log.CheckDebug() && Log_1.Log.Debug("Character", 19, "绑定UE Actor并复制Tag", ["tags", this.xQo])
    }
  }
  Clear() {
    return this.xQo.clear(), this.PQo.clear(), this.wQo.clear(), this.BQo.clear(), this.bQo.clear(), !(this.qQo = void 0)
  }
  ClearObject() {
    return this.Clear()
  }
  GQo(t, a) {
    ...
    const e = this.xQo.get(t) ?? 0,
      i = Math.max(0, e + a);
    if (a = i - e, e === i) return TagContainer.w__.Stop(), TagContainer.P__.Stop(), [];
    ...
    for (var n = [
      ...
      i <= 0 ? this.wQo.delete(r) : this.wQo.set(r, i), n.push([r, i + s, e + s, !1]), r = GameplayTagUtils_1.GameplayTagUtils.GetParentTag(r)
    }
    TagContainer.w__.Stop(), TagContainer.U__.Start();
    t = GameplayTagUtils_1.GameplayTagUtils.GetGameplayTagById(t);
    return t && this.qQo?.UpdateTagMap(t, a), TagContainer.U__.Stop(), TagContainer.P__.Stop(), n
  }
  NQo(t) {
    if (t && !(t.length <= 0)) {
      ...
      for (var [a, e, i, r] of t) {
        if (r)
          for (const n of this.bQo) try {
            n(a, e, i)
          } catch (t) {
            t instanceof Error ? Log_1.Log.CheckError() && Log_1.Log.ErrorWithStack("Character", 19, "执行Tag监听回调时出错", t, ["error", t.message]) : Log_1.Log.CheckError() && Log_1.Log.Error("Character", 19, "执行Tag监听回调时出错", ["error", t])
          }
        for (const s of this.BQo) try {
          s(a, e, i)
        } catch (t) {
          t instanceof Error ? Log_1.Log.CheckError() && Log_1.Log.ErrorWithStack("Character", 19, "执行Tag监听回调时出错", t, ["error", t.message]) : Log_1.Log.CheckError() && Log_1.Log.Error("Character", 19, "执行Tag监听回调时出错", ["error", t])
        }
  ...
  RemoveTag(t, a) {
    ...
    if (e) {
      ...
      if (0 < this.wQo.get(a)) {
        var r = [];
        for (const o of e.keys()) GameplayTagUtils_1.GameplayTagUtils.IsChildTag(o, a) && r.push(o);
        for (const g of r) {
          const s = e.get(g) ?? 0;
          e.delete(g);
          var n = this.GQo(g, -s);
  ...
  RemoveExactTag(a, e) {
    ...
    if (i) {
      var r = i.get(e) ?? 0;
      let t = void 0;
      0 < r && (t = this.GQo(e, -r), i.delete(e)), 0 === i.size && this.PQo.delete(a), this.NQo(t)
    }
  }

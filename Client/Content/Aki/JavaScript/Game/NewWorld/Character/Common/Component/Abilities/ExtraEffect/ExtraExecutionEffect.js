...
class BuffExecution extends ExtraEffectBase_1.BuffEffectBase {
  ...
  TryExecute(t, ...e) {
    this.Level = t.Level;
    var i, s = (this.Buff = t).GetOwnerBuffComponent();
    return !(!s || (i = t.GetInstigatorBuffComponent(), this.OwnerBuffComponent = s, i && (this.InstigatorEntityId = t.GetInstigator().Id), !this.Check({})) || (this.IsInLoop = !0, this.OnExecute(...e), this.IsInLoop = !1, this.OwnerBuffComponent = void 0, this.Buff = void 0))
  }
  GetEffectTarget() {
...
class ReviveExecution extends PeriodExecution {
  ...
  InitParameters(t) {
    this.HealRate = Number(t.ExtraEffectParameters[0] ?? 0) * CharacterAttributeTypes_1.DIVIDED_TEN_THOUSAND, this.HealValue = Number(t.ExtraEffectParameters[1] ?? 0)
  }
  OnExecute() {
    var t = this.OwnerBuffComponent?.GetEntity(),
      e = t?.GetComponent(0).GetPlayerId() ?? 0;
    2 !== ModelManager_1.ModelManager.SceneTeamModel.GetCurrentGroupLivingState(e) && (t?.CheckGetComponent(191))?.ExecuteRevive()
  }
}
exports.ReviveExecution = ReviveExecution;
class AddFormationAttributeExecution extends PeriodExecution {
  constructor() {
    super(...arguments), this.AttributeId = 0, this.AddRate = -0, this.AddValue = 0
  }
  InitParameters(t) {
    this.AttributeId = Number(t.ExtraEffectParameters[0]), this.AddRate = AbilityUtils_1.AbilityUtils.GetLevelValue(t.ExtraEffectGrowParameters1, this.Level, 0) * CharacterAttributeTypes_1.DIVIDED_TEN_THOUSAND, this.AddValue = AbilityUtils_1.AbilityUtils.GetLevelValue(t.ExtraEffectGrowParameters2, this.Level, 0)
  }
  OnExecute() {
  ...
  OnExecute() {
    ...
    if (t) {
      ...
      var e = CharacterAttributeTypes_1.EAttributeId.Proto_Energy,
        i = this.AddValue;
      const h = i * a;
      if (t.AddBaseValue(e, h), t.Entity?.CheckGetComponent(175)?.IsInGame) {
        var s = CharacterDamageCalculations_1.ENERGY_SHARE_RATE / CharacterAttributeTypes_1.PER_TEN_THOUSAND;
        for (const u of ModelManager_1.ModelManager.SceneTeamModel.GetTeamEntities(!0)) {
          var r = u?.Entity?.CheckGetComponent(175),
            o = u?.Entity?.CheckGetComponent(173);
          if (r && !r.IsInGame && o) {
...
class DamageExecution extends PeriodExecution {
  ...
  InitParameters(e) {
    ...
    for (let t = 0; t < e.ExtraEffectParameters.length; t++) this.DamageIdList[t] = e.ExtraEffectParameters[t].split("#").map(t => BigInt(t))
  }
  OnExecute() {
    ...
    if (this.OwnerBuffComponent) {
      var e = this.OwnerBuffComponent.GetEntity()?.CheckGetComponent(19),
        ...
        s = this.InstigatorEntity?.Valid ? this.InstigatorEntity.Entity : this.OwnerEntity;
      if (e && i && s)
        for (const r of t) e.ExecuteBuffDamage({
          DamageDataId: r,
...
class CdReduceExecution extends PeriodExecution {
  constructor() {
    super(...arguments), this.KXo = void 0, this.QXo = 0, this.XXo = void 0, this.$Xo = void 0
  }
  InitParameters(t) {
    var e = t.ExtraEffectParameters;
    this.KXo = e[0]?.split("#") ?? [], this.QXo = Number(e[1] ?? 0), this.XXo = t.ExtraEffectGrowParameters1, this.$Xo = t.ExtraEffectGrowParameters2
  }
  OnExecute() {
...
class ExtendBuffDurationExecution extends PeriodExecution {
  constructor() {
    super(...arguments), this.jQo = [], this.YXo = void 0, this.JXo = void 0, this.zXo = void 0
  }
  InitParameters(t) {
    var e = t.ExtraEffectParameters;
    this.TargetType = Number(e[0]), this.jQo = e[1].split("#").map(t => Number(t)), this.zXo = e[2]?.split("#").map(t => Number(t)), this.YXo = t.ExtraEffectGrowParameters1, this.JXo = t.ExtraEffectGrowParameters2
  }
  OnExecute() {
    ...
    if (t?.HasBuffAuthority()) {
      var e = AbilityUtils_1.AbilityUtils.GetLevelValue(this.YXo, this.Level, 0) * CharacterAttributeTypes_1.DIVIDED_TEN_THOUSAND,
        i = AbilityUtils_1.AbilityUtils.GetLevelValue(this.JXo, this.Level, 0) * TimeUtil_1.TimeUtil.Millisecond;
      for (const o of this.jQo) {
        ...
        if (s) {
          ...
          if (0 < r) {
            ...
            this.zXo?.[0] && (t = Math.max(t, this.zXo[0] * TimeUtil_1.TimeUtil.Millisecond)), this.zXo?.[1] && (t = Math.min(t, this.zXo[1] * TimeUtil_1.TimeUtil.Millisecond)), s.SetDuration(t)
          }
        }
      }
    }
...
class AddBuffToAdjacentRoleExecution extends InitExecution {
  constructor() {
    super(...arguments), this.ApplyBuffId = [], this.Distance = 0, this.ApplyRoleFormationType = void 0
  }
  CheckExecutable() {
    return this.OwnerBuffComponent?.HasBuffAuthority() ?? !1
  }
  InitParameters(t) {
    this.Distance = Number(t.ExtraEffectParameters[0]), this.ApplyRoleFormationType = 1 < t.ExtraEffectParameters.length ? t.ExtraEffectParameters[1].split("#").map(t => Number(t)) : [0, 1, 2], this.ApplyBuffId = 2 < t.ExtraEffectParameters.length ? t.ExtraEffectParameters[2].split("#").map(t => Number(t)) : [this.BuffId]
  }
  OnExecute(t) {
    if (t) {
      var e = this.OwnerBuffComponent?.GetEntity(),
        i = e?.GetComponent(0).GetPlayerId() ?? 0,
        t = ModelManager_1.ModelManager.CreatureModel.GetScenePlayerData(i)?.GetLocation();
      if (e && t)
        for (const r of ModelManager_1.ModelManager.SceneTeamModel.GetTeamItemsInRange(t, this.Distance))
          if (r.GetPlayerId() !== i) {
            ...
            if (s && s.Entity.GetComponent(0).IsRole() && s.Id !== e.Id)
              for (const o of this.ApplyBuffId) s.Entity.GetComponent(174).AddIterativeBuff(o, this.Buff, void 0, !1, `Buff${this.BuffId}的额外效果导致的共享添加`)
          }
    }
  }
}
...
class PhantomAssistExecution extends InitExecution {
  constructor() {
    super(...arguments), this.ZXo = Protocol_1.Aki.Protocol.Summon.x3s.Proto_ESummonTypeDefault, this.e$o = 0, this.wmo = 0, this.t$o = 2
  }
  CheckExecutable() {
    return this.OwnerBuffComponent?.HasBuffAuthority() ?? !1
  }
  InitParameters(t) {
    ...
    this.ZXo = Number(t[0]), this.e$o = Number(t[1]), this.wmo = Number(t[2]), this.t$o = Number(t[3])
  }
  OnExecute(t) {
    var e = this.OwnerBuffComponent.GetEntity(),
      e = e ? PhantomUtil_1.PhantomUtil.GetSummonedEntity(e, this.ZXo, this.e$o) : void 0;
    e && e.Entity.GetComponent(40).BeginSkill(this.wmo, {
...
class ExecuteBulletOrBuff extends PeriodExecution {
  constructor() {
    super(...arguments), this.GoalType = 0, this.Ids = [], this.Times = void 0, this.BulletPosType = 0, this.ProcessBulletId = (e, i, t) => {
      ...
      if (r) {
        ...
        for (let t = 0; t < i; t++) ControllerHolder_1.ControllerHolder.BulletController.CreateBulletCustomTarget(r, String(e), s, {
          SyncType: 1,
          CreateOnAuthority: !1
        }, o)
      } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 19, "尝试执行添加子弹的额外效果时找不到对应的buff施加者", ["buffId", this.BuffId], ["持有者", this.OwnerBuffComponent?.GetDebugName()])
    }, this.o$o = (t, e, i) => {
      i.AddIterativeBuff(t, this.Buff, e, !0, `瞬间型额外效果迭代添加（前置buff Id=${this.BuffId}）`)
    }, this.r$o = (t, e, i) => {
  ...
  InitParameters(t) {
    t = t.ExtraEffectParameters;
    this.TargetType = 0, this.GoalType = Number(t[0]), this.Ids = t[1].split("#").map(t => Number(t)), this.Times = t[2]?.split("#").map(t => Number(t)) ?? [], this.BulletPosType = Number(t[3] ?? 0)
  }
  OnExecute() {
  ...
  GetBuffHolderSkillTarget() {
    ...
    return t ? t.Entity.CheckGetComponent(209) : this.OwnerBuffComponent
  }
  ExecutePerId(i) {
    for (let e = 0; e < this.Ids.length; e++) {
      let t = this.GetEffectTarget();
      (t = (0, RegisterComponent_1.isComponentInstance)(t, 199) ? t.GetCurrentBuffComponent() : t) && i(this.Ids[e], this.n$o(e), t)
    }
  }
  n$o(t) {
    var e = this.Times;
...
class ExecuteAddBuffByStackCount extends PeriodExecution {
  ...
  InitParameters(t) {
    ...
    for (let t = 0; t < e.length; t++) this.Ids[t] = e[t].split("#").map(t => Number(t))
  }
  OnExecute() {
    var t = this.Buff.StackCount,
    ...
    if (e)
      for (const i of t) e.AddIterativeBuff(i, this.Buff, 1, !0, `因为其它瞬间型buff额外效果迭代添加（前置buff Id=${this.BuffId}）`);
    else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 19, "尝试执行添加buff的额外效果时找不到对应的buff接收者", ["buffId", this.BuffId], ["持有者", this.Buff?.GetOwnerDebugName()], ["施加者", this.Buff.InstigatorId])
  }
}
...
class ExecuteAddBulletByStackCount extends PeriodExecution {
  ...
  InitParameters(t) {
    ...
    for (let t = 1; t < e.length; t++) this.Ids[t - 1] = e[t].split("#").map(t => t.trim())
  }
  OnExecute() {
    var t = this.Buff.StackCount,
      t = AbilityUtils_1.AbilityUtils.GetLevelValue(this.Ids, t, []),
    ...
    if (i && e) {
      ...
      for (const r of t) ControllerHolder_1.ControllerHolder.BulletController.CreateBulletCustomTarget(i, r, e, {
        SyncType: 1,
        CreateOnAuthority: !1
      }, s)
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 19, "尝试执行添加子弹的额外效果时找不到对应的buff施加者或接收者", ["buffId", this.BuffId], ["持有者", this.Buff?.GetOwnerDebugName()], ["施加者", this.Buff.InstigatorId])
  }
  GetBulletTarget() {
    switch (this.BulletPosType) {
      case 0:
...
class ExecuteAddBulletByTagStackCount extends PeriodExecution {
  constructor() {
    super(...arguments), this.BulletCreateInfos = void 0, this.BulletPosType = 0, this.TagIdGetStackCount = 0
  }
  InitParameters(t) {
    this.TargetType = 0;
    var e = t.ExtraEffectParameters;
    this.BulletPosType = Number(e[0]), this.TagIdGetStackCount = GameplayTagUtils_1.GameplayTagUtils.GetTagIdByName(e[1]), this.BulletCreateInfos = new Array(e.length - 2);
    for (let t = 2; t < e.length; t++) {
      var i = e[t].split("#").map(t => t.trim());
      i.length < 3 ? Log_1.Log.CheckError() && Log_1.Log.Error("BuffItem", 20, "ExecuteAddBulletByTagStackCount的参数个数错误", ["BuffId", this.BuffId], ["参数", e]) : this.BulletCreateInfos[t - 2] = i
    }
  }
  OnExecute() {
    var t = (this.OwnerEntity?.GetComponent(205))?.GetTagCount(this.TagIdGetStackCount);
    if (!t || t < 0) Log_1.Log.CheckDebug() && Log_1.Log.Debug("BuffItem", 20, "ExecuteAddBulletByTagStackCount指定的Tag在持有者身上不存在", ["BuffId", this.BuffId], ["持有者", this.Buff.GetOwnerDebugName()], ["Tag", GameplayTagUtils_1.GameplayTagUtils.GetNameByTagId(this.TagIdGetStackCount)], ["StackCount", t]);
    else {
      ...
      } else Log_1.Log.CheckError() && Log_1.Log.Error("BuffItem", 20, "ExecuteAddBulletByTagStackCount尝试执行添加子弹的额外效果时找不到对应的buff施加者或接收者", ["BuffId", this.BuffId], ["持有者", this.Buff?.GetOwnerDebugName()], ["BulletCreateInfos", this.BulletCreateInfos])
    }
  }
  CompareStackCount(t, e, i) {
    switch (t) {
      ...
      case "!=":
        return e !== i;
      default:
        return Log_1.Log.CheckError() && Log_1.Log.Error("BuffItem", 20, "ExecuteAddBulletByTagStackCount的比较运算符错误", ["buffId", this.BuffId], ["operator", t]), !1
    }
  }
  GetBulletTarget() {
    switch (this.BulletPosType) {
...
class QteExecution extends InitExecution {
  ...
  InitParameters(t) {
    this.$fa = Number(t.ExtraEffectParameters?.[0] ?? 0), t.ExtraEffectParameters && !StringUtils_1.StringUtils.IsEmpty(t.ExtraEffectParameters[1]) && (this.xtc = 0 < Number(t.ExtraEffectParameters[1]))
  }
  OnExecute(t) {
          ...
          if (i !== r.Id) {
            ...
            if (i) {
              if (this.xtc || !i.ChangeRole && !i.ChangeRoleOnQte) {
                e = 0 < this.$fa, i = r.GetComponent(205);
                ...
                try {
                  ...
                  t instanceof Error ? Log_1.Log.CheckError() && Log_1.Log.ErrorWithStack("Battle", 48, "执行Qte额外效果出现异常", t, ["CreatureDataId", s.GetCreatureDataId()], ["Error", t.message]) : Log_1.Log.CheckError() && Log_1.Log.Error("Battle", 48, "执行Qte额外效果出现异常", ["CreatureDataId", s.GetCreatureDataId()], ["Error", t])
                }
                e || i.TagContainer.UpdateExactTag(2, 2014048239, -1)
              }
            } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 48, "执行Qte额外效果时，Buff持有者无Qte配置", ["buffId", this.BuffId], ["持有者", this.OwnerBuffComponent?.GetDebugName()])
          }
        } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 48, "执行Qte额外效果时，编队当前角色不合法", ["buffId", this.BuffId], ["持有者", this.OwnerBuffComponent?.GetDebugName()])
      }
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("Battle", 48, "执行Qte额外效果时，Buff持有者不合法", ["buffId", this.BuffId], ["持有者", this.OwnerBuffComponent?.GetDebugName()])
  }
}
exports.QteExecution = QteExecution;
class PeriodAddBuffToAdjacentEntity extends PeriodExecution {
  constructor() {
    super(...arguments), this.nJo = 0, this.axl = [], this.jQo = [], this.otc = !1, this.eue = [], this.lxl = "", this.hxl = !1
  }
  InitParameters(t) {
    t = t.ExtraEffectParameters;
    !t || t.length < 2 || (this.nJo = Number(t[0]), this.jQo = t[1].split("#").map(t => Number(t.trim())), this.axl = t[2] ? t[2].split("#").map(t => Number(t.trim())) : [0, 2], this.hxl = this.axl.some(t => 0 === t || 3 === t || 2 === t), this.lxl = `Buff${this.BuffId}的光环额外效果导致的共享添加`)
  }
  CheckExecutable() {
  ...
  OnExecute() {
    ...
    if (t && i) {
      ...
      if (this.hxl && !r) this.otc || (CombatLog_1.CombatLog.Warn("Buff", e, "光环效果指定了施加者阵营判断，但施加者不合法", ["handle", this.Buff?.Handle], ["buffId", this.BuffId]), this.otc = !0);
      else {
        var o = r?.GetEntityCamp(),
          a = (ModelManager_1.ModelManager.CreatureModel.GetEntitiesInRangeWithLocation(t, this.nJo, 62, this.eue), i.GetEntityCamp());
        for (const l of this.eue) {
          let t = !1;
          var h = l.Entity;
          if (h && h.Valid && h.IsInit) {
            var u = h?.GetComponent(0),
              n = h?.GetComponent(174);
            if (h && u && n) {
              var c = u.GetEntityType() === Protocol_1.Aki.Protocol.kks.Proto_Player ? 0 : u.GetEntityCamp();
              for (const f of this.axl)
                if (this.CheckCamp(f, h.Id, c, s?.Id, o, e.Id, a)) {
                  t = !0;
                  break
                } if (t)
                for (const d of this.jQo) n.AddIterativeBuff(d, this.Buff, void 0, !1, this.lxl)
            }
          }
        }
      }
...
class ConvertBuffToAnother extends PeriodExecution {
  ...
  InitParameters(t) {
    t = t.ExtraEffectParameters ?? [];
    this.sK_ = t[0]?.split("#").map(t => Number(t)) ?? [], this.aK_ = Number(t[1] ?? 0)
  }
  OnExecute() {
    ...
    if (e) {
      ...
      0 !== t && 0 < this.aK_ && e.AddIterativeBuff(this.aK_, this.Buff, t, !0, "Buff转换效果")
    }
  }
}
...
class StartBattleQte extends PeriodExecution {
  ...
  InitParameters(t) {
    0 < t.ExtraEffectParameters.length && (this.BattleQteId = Number(t.ExtraEffectParameters[0]))
  }
  OnExecute() {
    var t, e;
    void 0 !== this.BattleQteId && (this.Buff?.GetOwner())?.GetComponent(3)?.IsAutonomousProxy && (t = ModelManager_1.ModelManager.CharacterModel?.GetHandleByEntity(this.Buff?.GetInstigator()), e = this.Buff?.MessageId, t) && void 0 !== e && ControllerHolder_1.ControllerHolder.BattleQteController.StartBattleQte(this.BattleQteId, e, t, 2)
  }
  GetDebugEffectString() {
...
class ChangeBuffStackCount extends PeriodExecution {
  constructor() {
    super(...arguments), this.BuffIds = [], this.StackChange = [], this.StackDurationRefreshPolicy = [], this.StackPeriodResetPolicy = []
  }
  InitParameters(t) {
    this.BuffIds = t.ExtraEffectParameters[0].split("#").map(t => Number(t)), this.StackChange = t.ExtraEffectParameters[1].split("#").map(t => Number(t)), this.StackDurationRefreshPolicy = t.ExtraEffectParameters[2].split("#").map(t => Number(t)), this.StackPeriodResetPolicy = t.ExtraEffectParameters[3].split("#").map(t => Number(t))
  }
  OnExecute() {
    for (let t = 0; t < this.BuffIds.length; t++) this.OwnerBuffComponent?.ChangeBuffStack(this.BuffIds[t], this.StackDurationRefreshPolicy[t], this.StackPeriodResetPolicy[t], this.StackChange[t], "buff 79号额外效果")
  }
  GetDebugEffectString() {
    var e = [];

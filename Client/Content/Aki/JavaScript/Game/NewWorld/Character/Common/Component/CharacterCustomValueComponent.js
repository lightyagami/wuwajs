      ...
      if (a.ReturnType === 0 && a.NumSource === 0 || a.ReturnType === 1 && a.VecSource === 0 || a.ReturnType === 2 && a.RotSource === 0) {
        ...
        if (a.ReturnType === 1) {
          if (a.VecSource === 3) {
            ...
            _ = this.Entity.GetComponent(40)?.SkillTarget;
            if (!_?.Valid) {
              if (Log_1.Log.CheckDebug()) {
                Log_1.Log.Debug("Battle", 20, "[自定义值]技能目标不存在", ["Key", r], ["Actor", this.Entity.GetComponent(1)?.Owner?.GetName()]);
              }
            ...
            l = this.Entity.GetComponent(40)?.SkillTarget;
            if (!l?.Valid) {
              if (Log_1.Log.CheckDebug()) {
                Log_1.Log.Debug("Battle", 20, "[自定义值]技能目标不存在", ["Key", r], ["Actor", this.Entity.GetComponent(1)?.Owner?.GetName()]);
              }
...
CharacterCustomValueComponent = CharacterCustomValueComponent_1 = __decorate([(0, RegisterComponent_1.RegisterComponent)(282)], CharacterCustomValueComponent);

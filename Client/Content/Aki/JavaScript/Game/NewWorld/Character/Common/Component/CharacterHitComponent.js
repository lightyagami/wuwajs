...
let CharacterHitComponent = CharacterHitComponent_1 = class CharacterHitComponent extends BaseHitComponent_1.BaseHitComponent {
  constructor() {
    ...
    this.RVr = () => {
      ...
      if (this.DoubleHitInAirEffect.Valid) {
        i = (t = this.Entity.GetComponent(189)).GetLastUpdateVelocity();
        ...
        if (t.Valid) {
          ...
          t.SetGravityScale(this.DoubleHitInAirEffect.GravityScaleUp, this.DoubleHitInAirEffect.GravityScaleDown, this.DoubleHitInAirEffect.GravityScaleTop, this.DoubleHitInAirEffect.VelocityTop, this.DoubleHitInAirEffect.Duration, false, this.iVr);
  ...
  OnStart() {
    ...
    this.oVr = this.Entity.GetComponent(60);
    this.tVr = this.Entity.GetComponent(73);
    this.vHr = this.Entity.GetComponent(133);
    this.EVr = this.Entity.GetComponent(184);
    this.SVr = this.Entity.GetComponent(217);
    ...
    if (this.vHr) {
      this.F1t = (t, i) => {
        ...
        var e = (this.vHr?.FreezeTimeScale ?? 1) * this.TimeDilation;
  ...
  PY1() {
    ...
    if (s !== Protocol_1.Aki.Protocol.kks.Proto_Player && ((i = e?.GetPbEntityInitData()) && ((t = (i = (0, IComponent_1.getComponent)(i.ComponentsData, "AttributeComponent"))?.HardnessModeId) && (this.HardnessModeId = t), t = i?.RageModeId) && (this.RageModeId = t), this.RefreshHardnessModeConfig(), this.RefreshRageModeConfig(), (i = e?.GetEntityPropertyConfig()) && i.受击映射索引ID > 0 && (this.BeHitMapping = FightLibrary_1.FightLibrary.GetHitMapConfig(i.受击映射索引ID)), s === Protocol_1.Aki.Protocol.kks.Proto_Monster)) {
      this.wY1 = new CharacterHitOptimize_1.CharacterHitOptimize();
    }
  }
  OnEnd() {
  ...
  GetAcceptedNewBeHitAndReset() {
    ...
    if (this.AcceptedNewBeHit) {
      this.FVr(false);
      this.Entity.GetComponent(188).MainAnimInstance.AddForceUpdateSlotNameWhenMontageBlend(DEFALUT_SLOT_NAME);
    }
    return t;
  }
  FVr(t) {
  ...
  ReceiveOnHit(i, t, e, s, h, r, o, a, n, _, l) {
    if (this.kVr(forbidHitTagIds)) {
      ...
      if (e) {
        ...
        if (!!this.EnterFk && (!(o = t.GetComponent(1))?.Valid || !(this.Hte.ActorLocationProxy.Subtraction(o.ActorLocationProxy, this.BeHitDirect), this.BeHitDirect.Normalize()))) {
  ...
  OnHit(t, i, e, s, h, r, o, a = false) {
    ...
    this.ehl = false;
    this.sVr = a;
    this.AY1 = t.ShouldOptimize && !!this.wY1?.IsInCooling();
    CharacterHitComponent_1.xY1.Start();
    this.JVr();
    this.zVr(i);
    CharacterHitComponent_1.xY1.Stop();
    ...
    this.iwr(i);
    CharacterHitComponent_1.BY1.Start();
    this.n6r();
    CharacterHitComponent_1.BY1.Stop();
    CharacterHitComponent_1.kY1.Start();
    this.a6r(i);
    if (!this.EnterFk) {
      this.gs1(t);
    }
    CharacterHitComponent_1.kY1.Stop();
    this.vJl(i);
    CharacterHitComponent_1.OY1.Start();
    this.zTm();
    CharacterHitComponent_1.OY1.Stop();
    CharacterHitComponent_1.qY1.Start();
    if (this.rVr) {
      this.BroadcastEvent(this.rVr);
    } else {
      CombatLog_1.CombatLog.Error("Hit", this.Entity, "HitData为空");
    }
    CharacterHitComponent_1.qY1.Stop();
    CharacterHitComponent_1.GY1.Start();
    this.ProcessOnHitMaterial();
    CharacterHitComponent_1.GY1.Stop();
  ...
  u6r(t) {
    ...
    if (this.CounterAttackInfoInternal.QTE弹刀忽略角度距离检测) {
      t = t.GetBulletInfo();
      t = this.iVr.GetComponent(42).GetSkillInfo(t.BulletInitParams.SkillId);
      if (t && t.SkillGenre === 4) {
        return true;
      }
    }
    ...
    for (let t = 0; t < h; t++) {
      var o = s.Get(t);
      if (BulletConstant_1.BulletConstant.OpenHitActorLog && Log_1.Log.CheckDebug()) {
        Log_1.Log.Debug("Test", 20, "检查弹反 配置部位", ["部位", o.toString()]);
      }
      if (o.op_Equality(e)) {
        r = true;
        break;
    ...
    this.xVr.FromUeVector(this.iVr.GetComponent(1).ActorLocationProxy);
    if (r) {
    ...
    var a = Math.cos(this.CounterAttackInfoInternal.最大触发夹角 * MathUtils_1.MathUtils.DegToRad);
    var n = this.CounterAttackInfoInternal.最大触发距离;
    if (BulletConstant_1.BulletConstant.OpenHitActorLog && Log_1.Log.CheckDebug()) {
      Log_1.Log.Debug("Test", 20, "检查弹反 最大距离和最大触发夹角", ["当前距离", i], ["最大触发距离", n], ["夹角cos值", t], ["最大触发夹角cos值", a]);
    }
    return i < n && a < t;
  }
  c6r(t) {
  ...
  iwr(t) {
    if (!this.AY1) {
      ...
      const a = e.TimeScale;
      let i = this.sVr ? a.AttackerTimeScaleOnHitWeakPoint : a.TimeScaleOnAttack;
      const n = i.时间膨胀时长;
      if (a.TimeScaleOnAttackIgnoreAttacker) {
        if (n > 0) {
          BulletUtil_1.BulletUtil.SetTimeScale(t.GetBulletInfo(), i.优先级, i.时间膨胀值, i.时间膨胀变化曲线, n, 1);
        }
      } else if (n > 0) {
        if (e.Logic.Type === 2 && this.xoa?.替换近战子弹顿帧) {
          i = this.xoa.顿帧;
        }
        this.iVr.GetComponent(133).SetTimeScale(i.优先级, i.时间膨胀值, i.时间膨胀变化曲线, n, 1, false, true);
        var s = a.CharacterCustomKeyTimeScale;
        var h = s?.length ?? 0;
        for (let t = 0; t < h; t++) {
          var r = s[t];
          var o = ModelManager_1.ModelManager.BulletModel.GetEntityIdByCustomKey(this.iVr.Id, r, this.rVr.BulletId.toString());
          var o = ModelManager_1.ModelManager.CharacterModel.GetHandle(o);
          if (o?.Valid) {
            o.Entity.GetComponent(133)?.SetTimeScale(i.优先级, i.时间膨胀值, i.时间膨胀变化曲线, n, 1, false, true);
          } else if (Log_1.Log.CheckWarn()) {
            Log_1.Log.Warn("Character", 20, "", ["自定义连携顿帧单位key", r], ["子弹ID", this.rVr.BulletId]);
          }
        }
      }
      if (this.vHr && !e.Base.ContinuesCollision && !this.OVr(-648310348)) {
        const a = this.rVr.ReBulletData.TimeScale;
        t = this.sVr ? a.VictimTimeScaleOnHitWeakPoint : a.TimeScaleOnHit;
        const n = t.时间膨胀时长;
        BulletUtil_1.BulletUtil.SetVictimTimeScale(this.rVr.BulletEntityId, this.Entity.Id, this.vHr, t.优先级, t.时间膨胀值, t.时间膨胀变化曲线, n, 2, a.RemoveHitTimeScaleOnDestroy);
      }
    }
  }
  gs1(t) {
    var i;
    var e;
    if (ModelManager_1.ModelManager.CharacterModel?.EnabledSelfCentered && (i = t.Attacker?.GetComponent(314)) && !MathUtils_1.MathUtils.IsNearlyEqual(i.SelfCenterTimeDilation, 1)) {
      if ((e = t.ReBulletData.TimeScale.TimeScaleEffectImmune * BattleUiDefine_1.SECOND_TO_MILLISECOND) > TimerSystem_1.MIN_TIME) {
        this.Entity.GetComponent(314)?.SetBeHitTimeDilation(i.SelfCenterTimeDilation, e);
  ...
  WVr() {
    ...
    if (t && t.size > 0) {
      ...
      var h = this.rVr.Attacker?.GetComponent(54);
      let e = 2;
      if ((0, RegisterComponent_1.isComponentInstance)(h, 201)) {
        e = h.CurrentPriority;
      }
      ...
      for ([o, a] of t) {
        var l = EffectSystem_1.EffectSystem.SpawnEffect(GlobalData_1.GlobalData.World, this.oHo.ToUeTransform(), o, "[CharacterHitComponent.ProcessHitEffect]", r, undefined, undefined, a === 1 ? n : undefined);
  ...
  g6r(t, i) {
    ...
    for (const o of t) {
      ...
      var h = this.v6r(i, false, s, o.Index);
      if (!s) {
  ...
  v6r(t, i, e, s = -1, h = 1) {
    ...
    var n = t.DamageId;
    var _ = t.Target;
    if (!(n < 1) && _ && (r = t.Target.GetComponent(20), a = t.DirectTarget.GetComponent(43), r) && a) {
      o = EntitySystem_1.EntitySystem.Get(t.BulletEntityId)?.GetBulletInfo().ContextId;
      a = a.CurrentSkill;
      return r?.ExecuteBulletDamage(t.BulletEntityId, {
        ...
        PartId: s,
        ExtraRate: h,
        CounterSkillMessageId: this.IsTriggerCounterAttack ? a?.MNc : undefined,
        BulletId: t.BulletId,
        CounterSkillId: this.IsTriggerCounterAttack ? Number(a?.SkillId) : undefined,
        CounterType: this.cVr
      }, o);
    } else {
      return 0;
    }
  ...
  C6r(t) {
    ...
    var i = this.iVr.GetComponent(61)?.GetAttributeHolder();
    if (i) {
      t.Attacker = i;
    }
    t.Target = this.Entity.GetComponent(61)?.GetAttributeHolderExceptVisionSummon() ?? this.Entity;
    return t;
  }
  n6r() {
    if (!this.AY1 && !CameraController_1.CameraController.Model.IsModeEnabled(2) && !CameraController_1.CameraController.Model.IsModeEnabled(1) && this.rVr.IsShaking) {
      var i = this.rVr.ReBulletData.Render;
  ...
  a6r(i) {
    ...
    if (this.kVr(forbidHitTagIds)) {
      ...
      if (s) {
        ...
        var t = s.被击动作 === 12;
        var o = this.OVr(1922078392);
        if (h && !r && !t || o || t && this.OVr(-1525421570)) {
          this.HVr(i);
          this.cMc();
        } else {
          ...
          if (this.oVr && !this.oVr.CheckSwitchHitState(t, true)) {
            ...
            if (this.BeHitAnim !== 7 && this.BeHitAnim !== 12) {
              this.L6r(s);
            }
            this.wY1?.UpdateLastHitTime();
  ...
  zTm() {
    if (this.IsTriggerCounterAttack || !this.AY1) {
      let t = 0;
      var i;
      if ((t = this.cVr !== 2 || this.fVr ? t : this.VisionCounterAttackInfoInternal.对策事件ID) !== 0 || !!this.ehl) {
        if (i = this.Entity.GetComponent(81)) {
          this.LF_ = false;
          i.StateMachineGroup?.TriggerBeHitStateMachine(true, this.BeHitAnim, t);
          if (!this.LF_) {
  ...
  BroadcastEvent(t) {
    ...
    var s = e.BulletInitParams.SkillId;
    var h = e.BulletInitParams.SkillContextId;
    var r = this.iVr?.GetComponent(42);
    var h = {
      Attacker: this.iVr,
      Target: this.Entity,
      BulletId: t.BulletId,
      ...
      BattleFlags: r?.GetSkill(s)?.BattleContext?.BattleFlags ?? []
    };
    if (this.iVr && (SceneTeamController_1.SceneTeamController.EmitEvent(this.iVr, EventDefine_1.EEventName.CharHitLocal, t, h), e = this.iVr.GetComponent(0)) && (r = e.IsVision() ? this.iVr.GetComponent(61)?.GetAttributeHolder() ?? this.iVr : this.iVr)) {
    ...
    this.Entity.GetComponent(81)?.Tick(0);
  }
  L6r(t) {
    ...
    this.FVr(true);
    this.Entity.GetComponent(186).ExitAimStatus();
    ...
    if (this.OVr(-1732582420)) {
      ...
      if (this.OVr(-1898186757) || this.OVr(-823677393)) {
        if (this.BeHitAnim === 4) {
          i = t.地面受击滞空;
          e = i.滞空时间 + i.到滞空点时间;
          if (this.R6r(e)) {
  ...
  U6r(t, i) {
    ...
    var r;
    var o;
    var a = this.Entity.GetComponent(189);
    if (a.Valid) {
      e = this.Hte;
      ...
      h = this.iVr.GetComponent(1);
      r = t.滞空高度限制;
      if (a.IsStandardGravity) {
        if ((o = h.ActorLocationProxy.Z + r) < this.bVr.Z) {
          this.bVr.Z = o;
        }
      } else {
        this.bVr.Subtraction(h.ActorLocationProxy, this.Ml1);
        if (r < (o = this.Ml1.DotProduct(a.GravityUp))) {
          a.GravityUp.Multiply(o - r, this.Ml1);
          this.bVr.SubtractionEqual(this.Ml1);
        }
      }
      a.BeginWhirlpool(s, t.到滞空点时间, this.bVr, e.ActorLocationProxy, i, t.到滞空点曲线);
    }
  }
  R6r(t) {
    return t > 0;
  ...
  P6r(t, i) {
    this.ActiveStiff(-1);
    var e;
    var s = this.Entity.GetComponent(189);
    if (s.Valid) {
      e = this.Hte;
      ...
      if (s.GetWhirlpoolEnable() && s.GetWhirlpoolCancelByHit()) {
        s.EndWhirlpool("被AirHit取消了");
      }
      ...
      s.SetGravityScale(t.上升标量, t.下落标量, t.弧顶标量, t.速度阈值, e, t.到达弧顶清除速度, this.iVr);
      if (t.落地反弹.Z > 0) {
  ...
  A6r(t) {
    ...
    var o = t.命中硬直时间;
    var t = t.地面受击移动曲线;
    if (r > 0 && (i = this.EVr?.GetCurrentValue(EAttributeId.Proto_Mass) ?? MASS_RATE, e = e.op_Multiply(MASS_RATE / i), (i = this.Entity.GetComponent(189)).Valid)) {
      if (i.GetWhirlpoolEnable() && i.GetWhirlpoolCancelByHit()) {
        i.EndWhirlpool("被GroundHit取消了");
      }
  ...
  ZVr() {
    ...
    var t = this.iVr.CheckGetComponent(222);
    switch (this.cVr) {
      case 1:
  ...
  x6r(t) {
    ...
    this.b6r(t, i);
    this.q6r(i);
    if (this.iVr.GetComponent(1).IsAutonomousProxy) {
      this.G6r(i);
    }
    this.N6r();
  ...
  w6r(t) {
    ...
    this.b6r(t, i);
    this.q6r(i);
    if (this.iVr.GetComponent(1).IsAutonomousProxy && !this.OVr(1161958668)) {
      this.G6r(i);
    }
    this.N6r();
  }
  ...
  q6r(t) {
    var i = t.被击者顿帧;
    this.vHr?.SetTimeScale(i.优先级, i.时间膨胀值, i.时间膨胀变化曲线, i.时间膨胀时长, 4, false, true);
    i = t.攻击者顿帧;
    this.iVr.GetComponent(133).SetTimeScale(i.优先级, i.时间膨胀值, i.时间膨胀变化曲线, i.时间膨胀时长, 3, false, true);
  }
  G6r(t) {
    var i;
  ...
  N6r() {
    var t;
    if (!this.kVr(forbidHitTagIds)) {
      if ((t = this.Entity.GetComponent(188)).Valid) {
  ...
  cMc() {
    ...
    if (this.IsTriggerCounterAttack) {
      ...
      CombatMessage_1.CombatNet.Send(28413, this.Entity, i);
    }
  }
  I6r() {
    var t;
  ...
  ProcessOnHitMaterial() {
    if (!this.AY1 && ModelManager_1.ModelManager.BulletModel.OpenHitMaterial && this.hXs) {
...
CharacterHitComponent.xY1 = Stats_1.Stat.Create("OnHit_CounterAttack");
CharacterHitComponent.UY1 = Stats_1.Stat.Create("OnHit_RemoveByTag");
CharacterHitComponent.DY1 = Stats_1.Stat.Create("OnHit_ProcessDamage");
CharacterHitComponent.BY1 = Stats_1.Stat.Create("OnHit_CameraEffect");
CharacterHitComponent.kY1 = Stats_1.Stat.Create("OnHit_ProcessMain");
CharacterHitComponent.qY1 = Stats_1.Stat.Create("OnHit_BroadcastEvent");
CharacterHitComponent.GY1 = Stats_1.Stat.Create("OnHit_OnHitMaterial");
CharacterHitComponent.OY1 = Stats_1.Stat.Create("OnHit_BeHitStateMachine");
...
CharacterHitComponent = CharacterHitComponent_1 = __decorate([(0, RegisterComponent_1.RegisterComponent)(66)], CharacterHitComponent);

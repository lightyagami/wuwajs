...
class AddBuffTrigger extends ExtraEffectPassiveEffects_1.PassiveEffects {
  ...
  InitParameters(e) {
    ...
    this.EventType = Number(e[0]), this.TargetType = Number(e[1]), this.BuffIds = e[2].split("#").map(e => Number(e))
  }
  OnExecute() {
    ...
    if (e && s) {
      ...
      if (t?.IsValid())
        for (const r of this.BuffIds) e.AddIterativeBuff(r, t, void 0, !0, `因为触发其它buff额外效果而添加（前置buff Id=${this.BuffId}, handle=${this.ActiveHandleId}）`, this.ExecuteContext?.BulletMessageId)
    }
  }
}

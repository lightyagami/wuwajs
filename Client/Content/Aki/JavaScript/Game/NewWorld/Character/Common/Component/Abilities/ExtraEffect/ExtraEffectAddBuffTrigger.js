"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.AddBuffTrigger = undefined;
const Macro_1 = require("../../../../../../../Core/Preprocessor/Macro");
const ExtraEffectPassiveEffects_1 = require("./ExtraEffectPassiveEffects");
class AddBuffTrigger extends ExtraEffectPassiveEffects_1.PassiveEffects {
  constructor() {
    super(...arguments);
    this.BuffIds = [];
  }
  InitParameters(e) {
    e = e.ExtraEffectParameters;
    this.EventType = Number(e[0]);
    this.TargetType = Number(e[1]);
    this.BuffIds = e[2].split("#").map(e => Number(e));
  }
  OnExecute() {
    var e = this.GetEffectTarget();
    var s = this.InstigatorBuffComponent;
    if (e && s) {
      var t = this.OwnerBuffComponent.GetBuffByHandle(this.ActiveHandleId);
      if (t?.IsValid()) {
        for (const r of this.BuffIds) {
          e.AddIterativeBuff(r, t, undefined, true, `因为触发其它buff额外效果而添加（前置buff Id=${this.BuffId}, handle=${this.ActiveHandleId}）`, this.ExecuteContext?.BulletMessageId);
        }
      }
    }
  }
}
exports.AddBuffTrigger = AddBuffTrigger;
//# sourceMappingURL=ExtraEffectAddBuffTrigger.js.map
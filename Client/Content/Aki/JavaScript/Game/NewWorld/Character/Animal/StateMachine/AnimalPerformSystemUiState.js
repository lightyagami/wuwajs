...
class AnimalPerformSystemUiState extends AnimalPerformStateBase_1.AnimalPerformStateBase {
  ...
  OnEnter(t) {
    if (this.EcologicalInterface?.IsValid() && this.uKo) {
      ...
      if (t = this.Owner.GetComponent(215)) {
        this.mKo(t);
      }
  ...
  OnExit(t) {
    if (this.EcologicalInterface?.IsValid()) {
      this.Owner.GetComponent(207)?.SetInteractionState(true, "AnimalPerformSystemUiState OnExit");
      this.EcologicalInterface.SystemUiEnd();
      this.Owner.GetComponent(215)?.RemoveTag(1819982634);
      this._Ko = false;
      this.uKo = undefined;
    }
  }

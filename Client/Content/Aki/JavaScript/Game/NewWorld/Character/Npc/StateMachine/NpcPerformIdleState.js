...
class NpcPerformIdleState extends NpcPerformBaseState_1.NpcPerformBaseState {
  constructor() {
    ...
    this._tr = (t, i) => {
      ...
      this.etr = false;
      this.utr();
      this.Owner.Entity.GetComponent(188).AnyIdleLoopMontagePlaying = false;
    };
    this.ctr = false;
    this.mtr = false;
    this.dtr = false;
    ...
    this.ptr = () => {
      ...
      var i;
      this.vtr();
      this.Owner.Entity.GetComponent(188)?.PauseAi("StalkAlert");
      this.dtr = true;
    ...
    this.Oer = () => {
      this.vtr();
      this.Owner.Entity.GetComponent(188)?.ResumeAi("StalkAlert");
      this.dtr = false;
    };
    this.xei = () => {
      this.vtr();
      this.Owner.Entity?.GetComponent(188)?.ResumeAi("LeaveLogicRange");
      this.dtr = false;
    };
    this.Mtr = () => {
      this.Owner.Entity?.GetComponent(188)?.PauseAi("LeaveLogicRange");
      this.dtr = true;
    };
  }
  get NpcMoveComp() {
  ...
  StartFromBornState() {
    this.Etr();
    this.Ore();
    if (!this.itr && !(this._Ll?.RunTask(), this.Owner.Entity.GetComponent(188)?.IsInPlot) && !this.dtr && !this.gqn?.IsMoving) {
      if (this.Ker) {
        this.Itr(true);
      } else {
  ...
  OnEnter(t) {
    if (t === 0) {
      ...
      (t = this.Owner.Entity.GetComponent(188))?.ResumeAi("NpcPerformIdleState");
      this.Ore();
  ...
  OnUpdate(t) {
    if (!this.itr && !(this.XWa(), this.Owner.Entity.GetComponent(188)?.IsInPlot) && !this.dtr && !this.InteractRequestWaiting) {
  ...
  OnExit(t) {
    ...
    this.kre();
    this.Owner.Entity.GetComponent(188)?.PauseAi("NpcPerformIdleState");
  }
  OnDestroy() {
    this.YWa();
    this._Ll?.Dispose();
  ...
  Itr(r = false) {
    if (!this.Xaa && !this.Wer) {
      const n = this.Owner.Entity.GetComponent(188);
  ...
  Etr() {
    ...
    if (e?.IsEnabled() && ((t = e.AiController?.AiBase?.SubBehaviorConfigs?.get("AiSense")) && (i = this.Owner.Entity.GetComponent(122), t = AiSenseById_1.configAiSenseById.GetConfig(Number(t))) && (t = Math.max(t.SenseDistanceRange.Max, 0), i.SetLogicRange(t), this.ctr = t > 0), i = e.AiController?.AiBase?.SubBehaviorConfigs?.get("AiAlert")) && AiAlertById_1.configAiAlertById.GetConfig(Number(i))) {
      this.mtr = true;
    }
  }
  Ore() {
    ...
    if (this.ctr && !this.mtr) {
      ...
      if (!this.Owner.Entity.GetComponent(122)?.IsInLogicRange) {
        this.Mtr();
      }
    }
  }
  ...
  OnPlayerInteractTurnActionStart() {
    this.Owner.Entity.GetComponent(188)?.PauseAi("PlayerInteractTurnAction");
  ...
  OnPlayerInteractTurnActionEnd() {
    ...
    this.TurnActionController.OnTurnToDefaultForwardEndHandle = () => {
      if (this?.Owner?.Valid) {
        this.Owner.Entity.GetComponent(188)?.ResumeAi("PlayerInteractTurnAction");

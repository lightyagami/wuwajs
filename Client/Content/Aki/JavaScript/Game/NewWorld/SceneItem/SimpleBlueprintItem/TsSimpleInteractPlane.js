...
const UE = require("ue"),
  ...
  drawPoints = [new UE.Vector(0, -1, -1), new UE.Vector(0, -1, 1), new UE.Vector(0, 1, -1), new UE.Vector(0, 1, 1)],
...
class TsSimpleInteractPlane extends TsSimpleInteractBase_1.default {
  ...
  CheckLegal() {
    ...
    for (let t = 0; t < e; ++t) {
      if (this.TmpLocation.X = 0, this.TmpLocation.Y = r, this.TmpLocation.Z = -this.PlaneHalfHeight, this.SelfTransform.TransformPositionNoScale(this.TmpLocation, this.StartLocation), this.TmpLocation.Z = this.PlaneHalfHeight, this.SelfTransform.TransformPositionNoScale(this.TmpLocation, this.EndLocation), this.LineTrace.WorldContextObject = this, TraceElementCommon_1.TraceElementCommon.SetStartLocation(this.LineTrace, this.StartLocation), TraceElementCommon_1.TraceElementCommon.SetEndLocation(this.LineTrace, this.EndLocation), TraceElementCommon_1.TraceElementCommon.LineTrace(this.LineTrace, PROFILE_KEY_CEHCK_LEGAL)) return !1;
      r += a
    }
    return !0
  }
  OnDraw() {
    ...
    for (let i = 0; i < s; ++i) {
      this.TmpLocation.Y = drawPoints[i].Y * this.PlaneHalfWidth, this.TmpLocation.Z = drawPoints[i].Z * this.PlaneHalfHeight, this.SelfTransform.TransformPositionNoScale(this.TmpLocation, this.StartLocation);
      for (let t = i + 1; t < s; ++t) this.TmpLocation.Y = drawPoints[t].Y * this.PlaneHalfWidth, this.TmpLocation.Z = drawPoints[t].Z * this.PlaneHalfHeight, this.SelfTransform.TransformPositionNoScale(this.TmpLocation, this.EndLocation), UE.KismetSystemLibrary.D_DrawDebugLine(this, this.StartLocation.ToUeVector(), this.EndLocation.ToUeVector(), h, DRAW_TIME, DEFAULT_THICKNESS)
    }
    this.SelfTransform.TransformPositionNoScale(forwardOffset, this.EndLocation), UE.KismetSystemLibrary.D_DrawDebugArrow(this, this.D_K2_GetActorLocation(), this.EndLocation.ToUeVector(), DEFAULT_ARROW_SIZE, redColor, DRAW_TIME, DEFAULT_THICKNESS)
  }
  SetText(t) {
    this.Text.HorizontalAlignment = 1, this.Text.SetWorldSize(TEXT_SIZE), this.Text.SetTextRenderColor(textColor), this.Text.Text = "Plane " + this.TypeId
  }
  OnGetBestTransform(t, i, h, s) {
    this.UpdateData();
    var e = this.TmpResult;
    return this.ActorLocation.FromUeVector(t.D_K2_GetActorLocation()), this.ActorLocation.Subtraction(this.SelfLocation, this.SelfToActor), e.Success = this.SelfToActor.DotProduct(this.SelfForward) > s, e.Success && (this.MoveOffset.FromUeVector(i), this.IsHorizontalPlane ? (i = this.GetBestTransformHorizontal(h, s), e.Location = i[0].ToUeVectorOld(), e.Rotator = i[1].ToUeRotator(), e.SquaredOffsetLength = i[2]) : (i = this.GetBestTransformNotHorizontal(h, s), e.Location = i[0].ToUeVectorOld(), e.Rotator = i[1].ToUeRotator(), e.SquaredOffsetLength = i[2]), this.LineTrace.WorldContextObject = t, TraceElementCommon_1.TraceElementCommon.SetStartLocation(this.LineTrace, this.ActorLocation), TraceElementCommon_1.TraceElementCommon.SetEndLocation(this.LineTrace, e.Location), e.Success = !TraceElementCommon_1.TraceElementCommon.LineTrace(this.LineTrace, PROFILE_KEY), e.Success), e
  }
  GetBestTransformHorizontal(t, i) {
    ...
    if (this.FindHitMatrixAndCircle(_, l, h, s, n, e, a)) {
      this.TmpResultLocation.X = i, this.TmpResultLocation.Subtraction(this.ActorLocation, this.TmpVector1), MathUtils_1.MathUtils.LookRotationForwardFirst(this.SelfBackward, this.TmpVector1, this.TmpResultRotator);
    ...
    const E = MathUtils_1.MathUtils.Square(this.TmpResultLocation.Z - (this.ActorLocation.Z + this.MoveOffset.Z)) + n + M - 2 * Math.sqrt(n * M);
    return [this.TmpResultLocation, this.TmpResultRotator, E]
  }
  FindHitMatrixAndCircle(i, h, s, e, a, r, o) {
    ...
    for (let t = 0; t < 2; ++t) {
      ...
      0 < E && (l = e - (E = Math.sqrt(E)), Math.abs(l) < h && (M = MathUtils_1.MathUtils.Square(n - r) + MathUtils_1.MathUtils.Square(l - o)) < _ && (_ = M, this.TmpResultLocation.Y = n, this.TmpResultLocation.Z = l), l = e + E, Math.abs(l) < h) && (M = MathUtils_1.MathUtils.Square(n - r) + MathUtils_1.MathUtils.Square(l - o)) < _ && (_ = M, this.TmpResultLocation.Y = n, this.TmpResultLocation.Z = l)
    }
    for (let t = 0; t < 2; ++t) {
      var U, c, m = 0 === t ? h : -h,
        C = a - MathUtils_1.MathUtils.Square(e - m);
      0 < C && (U = s - (C = Math.sqrt(C)), Math.abs(s) < i && (c = MathUtils_1.MathUtils.Square(U - r) + MathUtils_1.MathUtils.Square(m - o)) < _ && (_ = c, this.TmpResultLocation.Y = U, this.TmpResultLocation.Z = m), U = s + C, Math.abs(U) < h) && (c = MathUtils_1.MathUtils.Square(U - r) + MathUtils_1.MathUtils.Square(m - o)) < _ && (_ = c, this.TmpResultLocation.Y = U, this.TmpResultLocation.Z = m)
    }
    return _ < 10 * a
  }
  GetBestTransformNotHorizontal(t, i) {
    this.TmpVector1.DeepCopy(this.SelfToActor), this.TmpVector1.Z += this.MoveOffset.Z;

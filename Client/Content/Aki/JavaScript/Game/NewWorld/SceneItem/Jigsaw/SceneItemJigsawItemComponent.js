...
exports.SceneItemJigsawItemComponent = undefined;
const UE = require("ue");
...
let SceneItemJigsawItemComponent = SceneItemJigsawItemComponent_1 = class SceneItemJigsawItemComponent extends EntityComponent_1.EntityComponent {
  constructor() {
    ...
    this.gIe = (t, e) => {
      ...
      var s = 741712776;
      var n = 1488947861;
      var o = this.Lie.HasTag(s) || this.Lie.HasTag(n);
      var h = this.Lie.HasTag(i);
      var r = o ? 2142861976 : -628734864;
      if (t === i && e) {
        for (const a of this.Fcn) {
          this.fcn.AddTagsByIndex(a, t);
          this.fcn.AddTagsByIndex(a, r);
        }
      }
      if ((t === s || t === n) && !e && h) {
        for (const c of this.Fcn) {
          this.fcn.RemoveTagsByIndex(c, 2142861976);
        }
      }
      if (t === i) {
    ...
    switch (this.Config.FillCfg.Type) {
      case IComponent_1.EFillType.Fixed:
        ...
        for (const n of i.Config.Pieces) {
          if (n.InitState === IAction_1.EJigsawPieceState.Correct) {
            this.Fcn.push(new SceneItemJigsawBaseComponent_1.JigsawIndex(n.Index.RowIndex, n.Index.ColumnIndex));
          }
        }
        break;
  ...
  GetActiveBlockOffset(t) {
    var e = [];
    for (const i of this.Fcn) {
      e.push(this.Kcn(new SceneItemJigsawBaseComponent_1.JigsawIndex(i.Row - this.Vcn.Row, i.Col - this.Vcn.Col), t));
    }
    return e;
  }
  RotateSelf() {
  ...
  Kcn(t, e) {
    ...
    var i = t.Row * cosValue[e] - t.Col * sinValue[e];
  ...
  GetBlockLocationByIndex(t) {
    var e;
    var i;
    var s = this.Config.FillCfg;
    if (s.Type !== IComponent_1.EFillType.Fixed) {
      return this.Hte.ActorLocationProxy;
    } else if (t.Row >= s.Config.Row || t.Col >= s.Config.Column) {
      return undefined;
    } else {
      i = (e = this.Vcn).Row - t.Row;
      e = e.Col - t.Col;
      t = s.Config.Size;
      s = Vector2D_1.Vector2D.Create(t, t).MultiplyEqual(Vector2D_1.Vector2D.Create(i, e));
      t = new UE.VectorDouble(s.X, -s.Y, 0);
      (i = Vector_1.Vector.Create(0, 0, 0)).FromUeVector(this.Hte.ActorTransform.TransformPosition(t));
      return i;
    }
  }
  OnPutDownToBase(t) {
    ...
    for (const i of this.Fcn) {
      ...
      switch (t.GetBlockStateByIndex(e)) {
        case 1:
  ...
  OnFinish() {
    for (const t of this.Fcn) {
      this.fcn.AddTagsByIndex(t, 754952868);
    }
  }
  GetNextMoveTargetOnHit(t) {
    if (this.PutDownBase?.Valid) {
      return [(t = this.PutDownBase.GetNextPosByDirection(this.PutDownIndex, t, this.Entity)).GetKey() !== this.PutDownIndex.GetKey(), this.PutDownBase.GetBlockLocationByIndex(t), t];
    }
  }
  OnMove(t) {
  ...
  GetAllActivatedBlockPos() {
    ...
    for (const i of this.Fcn) {
      ...
      if (e) {
        t.push(Vector_1.Vector.Create(e));
      }
    }
    return t;
  }
};
SceneItemJigsawItemComponent = SceneItemJigsawItemComponent_1 = __decorate([(0, RegisterComponent_1.RegisterComponent)(149)], SceneItemJigsawItemComponent);

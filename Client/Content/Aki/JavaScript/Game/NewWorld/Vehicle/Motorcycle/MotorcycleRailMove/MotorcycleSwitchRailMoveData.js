...
class MotorcycleSwitchRailMoveData extends MotorcycleRailMoveDataBase_1.MotorcycleRailMoveDataBase {
  ...
  OnEnter(t) {
    ...
    if (this.oTf.Duration <= 0) {
      if (Log_1.Log.CheckError()) {
        Log_1.Log.Error("MotorRailMove", 39, "[MotorcycleSwitchRailMoveData] OnEnter Failed: Duration有误", ["SplineId", this.RelatedRail?.GetRailSplineId()], ["Duration", this.oTf.Duration]);
      }
      return false;
    }
    ...
    if (!(i = (i = MotorcycleRailMoveUtils_1.MotorcycleRailMoveUtils.CalcRailMoveTargetAdvanceBySpeed(this.AJf, this.oTf.Duration, this.oTf.MinSpeedAlongRail, this.oTf.MaxSpeedAlongRail)) && MotorcycleRailMoveUtils_1.MotorcycleRailMoveUtils.ExecCheckList(this.AJf, this.aTf.EnterRailCondition, t, true, this.constructor.name))) {
      this.AJf.IsForward = false;
      i = (i = MotorcycleRailMoveUtils_1.MotorcycleRailMoveUtils.CalcRailMoveTargetAdvanceBySpeed(this.AJf, this.oTf.Duration, this.oTf.MinSpeedAlongRail, this.oTf.MaxSpeedAlongRail)) && MotorcycleRailMoveUtils_1.MotorcycleRailMoveUtils.ExecCheckList(this.AJf, this.aTf.EnterRailCondition, t, true, this.constructor.name);
    }
    if (!i) {
  ...
  LRm() {
    if (this.ojo.EnableBlockingCheck && this.ayf && this.fqf > this.ojo.MaxBlockingTimeOut) {
      ...
      if (Log_1.Log.CheckWarn()) {
        Log_1.Log.Warn("MotorRailMove", 39, "[MotorcycleSwitchRailMoveData] Finish move on blocking", ["BlockingTime", this.fqf], ["CurrentLocation", this.I1e]);
      }
    } else {
    ...
    if (this.ojo.EnableBlockingCheck && !this.IsFinishMove && (i = this.OwnerEntity.GetComponent(247), s = this.OwnerEntity.GetComponent(249), i && s && s.VehicleMovement)) {
      ...
      if (this.ayf) {
        ...
        if (Log_1.Log.CheckDebug()) {
          Log_1.Log.Debug("MotorRailMove", 39, "[MotorcycleSwitchRailMoveData] Blocked", ["BlockingTime", this.fqf], ["CurrentLocation", this.I1e]);
        }
      } else {
        this.fqf = 0;
      }

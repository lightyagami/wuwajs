...
class FishingBoatConfig extends GongduolaConfig_1.GongduolaConfig {
  ...
  DeepCopy() {
    var t = new FishingBoatConfig(this.VehicleEntity, undefined);
  ...
  SetBaseStateMoveConfig(t) {
    ...
    let s = 1;
    i = this.VehicleEntity?.GetComponent(176);
    if (i) {
  ...
  RefreshSprintConfig(t, i, s) {
    t = this.fu_ * t;
    this.SprintMaxSpeed = t <= this.SprintExceedLimitSpeed ? t : this.SprintExceedLimitSpeed;
    this.SprintExceedLimitDuration = i <= s ? i : s;
    this.SprintDuration = s;
    this.SprintBrakeAcceleration = 3000;
  }
}

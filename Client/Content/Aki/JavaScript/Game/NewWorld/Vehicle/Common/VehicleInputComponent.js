"use strict";
var VehicleInputComponent_1, __decorate = this && this.__decorate || function(t, s, e, i) {
  var n, h = arguments.length,
    r = h < 3 ? s : null === i ? i = Object.getOwnPropertyDescriptor(s, e) : i;
  if ("object" == typeof Reflect && "function" == typeof Reflect.decorate) r = Reflect.decorate(t, s, e, i);
  else
    for (var u = t.length - 1; 0 <= u; u--)(n = t[u]) && (r = (h < 3 ? n(r) : 3 < h ? n(s, e, r) : n(s, e)) || r);
...
let VehicleInputComponent = VehicleInputComponent_1 = class VehicleInputComponent extends EntityComponent_1.EntityComponent {
  ...
  HandlePressEvent(t, s) {
    ModelManager_1.ModelManager.BattleInputModel?.GetInputEnable(t) ? this.InputEvents.push(new CharacterInputComponent_1.InputEvent(t, 1, s)) : Log_1.Log.CheckDebug() && Log_1.Log.Debug("Battle", 17, "该战斗输入被禁用，不执行按下操作", ["action", t])
  }
  HandleReleaseEvent(t, s) {
    ModelManager_1.ModelManager.BattleInputModel?.GetInputEnable(t) ? (this.InputEvents.push(new CharacterInputComponent_1.InputEvent(t, 2, s)), VehicleInputComponent_1.HoldPressMap.set(t, !1), VehicleInputComponent_1.HoldActionMap.delete(t)) : Log_1.Log.CheckDebug() && Log_1.Log.Debug("Battle", 17, "该战斗输入被禁用，不执行放开操作", ["action", t])
  }
  HandleHoldEvent(t, s) {
    this.InputEvents.push(new CharacterInputComponent_1.InputEvent(t, 3, s)), VehicleInputComponent_1.HoldActionMap.set(t, !0)
  }
  HandleInputAxis(t, s) {
    let e = s;
  ...
  PreProcessInput(t, s) {
    if (Info_1.Info.AxisInputOptimize) {
      this.NextFrameClear && (this.NextFrameClear = !1, this.AxisValues.clear());
      for (const e of this.W$a) this.AxisValues.has(e) && this.AxisValues.delete(e);
      this.W$a.clear()
  ...
  PostProcessInput(i, t) {
    ...
    const n = new Array;
    if (this.InputEvents.forEach((t, s) => {
        var e = this.GetCommand(i, t);
        e && 0 !== e.CommandType && n.push(new CharacterInputComponent_1.InputCommand(t.Action, t.State, e, s))
      }), 0 < this.TestInputEvent.length) {
      let t = this.InputEvents.length;
      for (const h of this.TestInputEvent) {
        var s = this.GetCommand(i, h);
        s && n.push(new CharacterInputComponent_1.InputCommand(h.Action, h.State, s, t)), t++
      }
      this.TestInputEvent.length = 0
    }
    var e = this.GetBestInputCommand(n);
    this.CacheInputs(e), this.InputEvents.length = 0, 3 === e?.State && VehicleInputComponent_1.HoldPressMap.set(e.Action, !0), void 0 !== e && (ModelManager_1.ModelManager.SundryModel.SceneCheckOn && Log_1.Log.CheckDebug() && Log_1.Log.Debug("Input", 6, "ReceiveInput", ["BestInputCommand", JSON.stringify(e)]), this.ExecuteInputCommand(e, "PostProcessInput"))
  }
  ClearInputAxis(t) {
    Info_1.Info.AxisInputOptimize && (t || this.AxisValues.clear(), this.NextFrameClear = t)
  }
  ClearSingleAxisInput(t, s) {
    Info_1.Info.AxisInputOptimize && (s ? this.W$a.add(t) : this.AxisValues.has(t) && this.AxisValues.set(t, 0))
  }
  GetCameraInput() {
  ...
  UnRegisterInputHandler() {
    InputController_1.InputController.RemoveInputHandler(this), this.LastMovementInputTime = INVALID_INPUT_TIME, this.InputEvents.length = 0, this.InputCaches.length = 0, this.AxisValues.clear(), this.LongPressLeaveCondition = !1, VehicleInputComponent_1.HoldPressMap.clear(), VehicleInputComponent_1.HoldActionMap.clear()
  }
  SetVehicleRelatedInputEnable(t, s) {
    const e = t?.GetComponent(205);
    e && (s ? this.PassengerInputForbidTagArray.forEach(t => {
      e.RemoveTag(t)
  ...
  ValidateInputCaches() {
    ...
    for (let t = this.InputCaches.length - 1; 0 <= t; t--) {
      ...
      0 === this.LimitInputArray.length && s - e.Time > i && this.InputCaches.splice(t, 1)
    }
  }
  GetCacheTime(s, e) {
    if (this.BpInputComp) {
      let t = void 0;
      if (this.CacheTimes.has(s) || (t = this.BpInputComp.GetUnrealCacheConfig(s), this.CacheTimes.set(s, t)), t = t || this.CacheTimes.get(s)) {
        switch (e) {
          case 1:
            return t.按下;
          case 3:
      ...
      case 1:
        if (0 < this.LimitInputArray.length)
          for (const n of this.LimitInputArray)
            if (n.ForbidExecuteCommand && n.Action === s.Action && n.State === s.State) return;
        this.DispatchPressEvent(s.Action, s.Time), e = this.HandlePress(s.Action, s.Time);
        break;
      case 2:
        if (0 < this.LimitInputArray.length) {
          for (const h of this.LimitInputArray)
            if (h.ReleaseHoldCache && h.Action === s.Action && 3 === h.State) {
              for (let t = this.InputCaches.length - 1; 0 <= t; t--) {
                var i = this.InputCaches[t];
                i.Action === s.Action && 3 === i.State && this.InputCaches.splice(t, 1)
              }
              break
            } for (const r of this.LimitInputArray)
            if (r.ForbidExecuteCommand && r.Action === s.Action && r.State === s.State) return
        }
        this.DispatchReleaseEvent(s.Action, s.Time), e = this.HandleRelease(s.Action, s.Time);
        break;
      case 3:
        if (s.Action === InputEnums_1.EInputAction.跳跃 && this.IsEnableLongPressLeave) {
          if (!!!this.CheckIfCanLeave()) return void(this.LongPressLeaveCondition = !1);
          this.LongPressLeaveCondition || (this.InvalidHoldTime = s.Time, EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.SkillLongPressStart, s.Action)), this.LongPressLeaveCondition = !0
        }
        if (!this.ShouldTriggerHoldEvent(s.Action, s.Time - this.InvalidHoldTime, t)) return void(s.Action = InputEnums_1.EInputAction.None);
        if (0 < this.LimitInputArray.length)
          for (const u of this.LimitInputArray)
            if (u.ForbidExecuteCommand && u.Action === s.Action && u.State === s.State) return;
  ...
  CacheInputs(t) {
    ...
    this.InputEvents.forEach((s, t) => {
      if (t !== h && s.Action !== InputEnums_1.EInputAction.None) {
        for (let t = this.InputCaches.length - 1; 0 <= t; t--) {
          var e = this.InputCaches[t];
          e.Action === s.Action && e.State === s.State && this.InputCaches.slice(t, 1)
        }
        if (0 < this.LimitInputArray.length)
          for (const i of this.LimitInputArray) i.Action === s.Action && i.State === s.State && this.InputCaches.push(new CharacterInputComponent_1.InputCache(s.Action, s.State, s.Time, n));
        else this.GetCacheTime(s.Action, s.State) !== ZERO_TIME && this.InputCaches.push(new CharacterInputComponent_1.InputCache(s.Action, s.State, s.Time, n))
      }
    })
  }
    ...
    if (this.ActorComp) {
      if (this.BpInputComp) switch (t) {
        case InputEnums_1.EInputAction.跳跃:
          this.BpInputComp.跳跃按下事件(s), this.IsEnableLongPressLeave && (this.LongPressLeaveCondition = !0, this.InvalidHoldTime = 0, this.CheckIfCanLeave()) && EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.SkillLongPressStart, t);
          break;
  ...
  DispatchReleaseEvent(t, s) {
    if (this.ActorComp) {
      if (this.BpInputComp) switch (t) {
        case InputEnums_1.EInputAction.跳跃:
          this.BpInputComp.跳跃抬起事件(s), this.IsEnableLongPressLeave && EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.SkillLongPressEnd, t);
          break;
        case InputEnums_1.EInputAction.攀爬:
  ...
  ShouldTriggerHoldEvent(t, s, e) {
    ...
    return n !== NULL_CONFIG_TIME && !(s < n || !i && (VehicleInputComponent_1.HoldPressMap.has(t) && VehicleInputComponent_1.HoldPressMap.get(t) || !(n < s - e)))
  }
  GetHoldConfig(t) {
    if (!this.BpInputComp) return [!1, NULL_CONFIG_TIME];
    let s = void 0;
    return this.HoldConfigs.has(t) || (s = this.BpInputComp.GetUnrealHoldConfig(t), this.HoldConfigs.set(t, s)), (s = s || this.HoldConfigs.get(t)) ? [s.连续触发, s.触发时间] : [!1, NULL_CONFIG_TIME]
  }
  HandleHold(t, s) {
    if (this.ActorComp) {
      if (this.BpInputComp) switch (t) {
  ...
  UpdateVehicleInputDirectAndFacing() {
    this.UpdateMoveCache(), this.ActorComp.SetInputDirect(this.GetWorldMoveDirectionCache(), !0), this.SetInputFacingFromInputDirect()
  }
  SetInputFacingFromInputDirect(t = !0) {
    GravityUtils_1.GravityUtils.GetPlanarSizeSquared2dForActor(this.ActorComp, this.ActorComp.InputDirectProxy) > MathUtils_1.MathUtils.SmallNumber ? this.ActorComp.SetInputFacing(this.ActorComp.InputDirectProxy) : t && this.ActorComp.SetInputFacing(this.ActorComp.ActorForwardProxy)
  }
  ...
  UpdateMoveCache() {
    this.GetMoveVector(this.MoveVectorCache), 1 < this.MoveVectorCache.SizeSquared2D() && this.MoveVectorCache.Normalize(), this.IsSmallInput(this.MoveVectorCache) ? Time_1.Time.Now - this.LastMovementInputTime > MOVE_VECTOR_CACHE_TIME && (this.MoveDirectionCache.DeepCopy(this.MoveVectorCache), this.MoveDirectionCache.Normalize(), this.LastMovementInputTime = INVALID_INPUT_TIME) : (this.MoveDirectionCache.DeepCopy(this.MoveVectorCache), this.MoveDirectionCache.Normalize(), this.LastMovementInputTime = Time_1.Time.Now)
  }
  ...
  GetMoveVector(t) {
    t.X = this.QueryInputAxis(InputEnums_1.EInputAxis.MoveForward) ?? 0, t.Y = this.QueryInputAxis(InputEnums_1.EInputAxis.MoveRight) ?? 0, t.Z = 0
  }
  IsSmallInput(t) {
  ...
  CheckIfCanLeave() {
    return "Gongduola" !== this.VehicleType || !!this.Entity.GetComponent(237)?.CheckIfCanLeave()
  }
...
VehicleInputComponent.HoldPressMap = new Map, VehicleInputComponent.HoldActionMap = new Map, VehicleInputComponent = VehicleInputComponent_1 = __decorate([(0, RegisterComponent_1.RegisterComponent)(240)], VehicleInputComponent), exports.VehicleInputComponent = VehicleInputComponent;

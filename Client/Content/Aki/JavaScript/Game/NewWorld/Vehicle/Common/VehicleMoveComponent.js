...
var __decorate = this && this.__decorate || function(t, i, s, h) {
  var e, o = arguments.length,
    r = o < 3 ? i : null === h ? h = Object.getOwnPropertyDescriptor(i, s) : h;
  if ("object" == typeof Reflect && "function" == typeof Reflect.decorate) r = Reflect.decorate(t, i, s, h);
  else
    for (var n = t.length - 1; 0 <= n; n--)(e = t[n]) && (r = (o < 3 ? e(r) : 3 < o ? e(i, s, r) : e(i, s)) || r);
...
let VehicleMoveComponent = class VehicleMoveComponent extends EntityComponent_1.EntityComponent {
  constructor() {
    super(...arguments), this.ActorComp = void 0, this.AnimComp = void 0, this.TagComponent = void 0, this.AudioComp = void 0, this.UeMovementMgrComp = void 0, this.VehicleMovement = void 0, this.UeMovementDisableHandle = 0, this.AutoUpdateUeMovementTickState = !0, this.IsMoving = !1, this.HasMoveInput = !1, this.IsSpecialMove = !1, this.IsMovePath = !1, this.DeltaTimeSeconds = 0, this.IsStopInternal = !1, this.ForceSpeed = Vector_1.Vector.Create(INVALID_FORCE_SPEED, INVALID_FORCE_SPEED, INVALID_FORCE_SPEED), this.Speed = 0, this.AimYawRate = 0, this.Acceleration = Vector_1.Vector.Create(), this.PreviousAimYaw = 0, this.PreviousVelocity = Vector_1.Vector.Create(), this.TurnRate = 1, this.CanMoveWithDistanceInternal = !0, this.CanMoveFromInputInternal = !0, this.AdditiveTurnYaw = 0, this.AdditiveTurnYawCache = 0, this.AdditiveTurnDuration = 0, this.AdditiveTurnElapsedTime = 0, this.CannotResponseInputCount = 0, this.CanResponseInputTasks = new Array, this.GravityDirectInternal = Vector_1.Vector.Create(0, 0, -1), this.GravityUpInternal = Vector_1.Vector.Create(0, 0, 1), this.IsStandardGravityInternal = !0, this.DebugCurve = void 0, this.IsSummoningPerform = !1, this.TmpVector = Vector_1.Vector.Create(), this.TmpVector2 = Vector_1.Vector.Create(), this.TmpRotator = Rotator_1.Rotator.Create(), this.TmpTrans = Transform_1.Transform.Create(), this.TmpQuat = Quat_1.Quat.Create(), this.TmpQuat2 = Quat_1.Quat.Create(), this.OnResponseInputTagsChanged = (t, i) => {
      i ? (0 === this.CannotResponseInputCount && (this.HasMoveInput = !1), ++this.CannotResponseInputCount) : --this.CannotResponseInputCount
    }, this.OnEnterVehicle = t => {
      t.IsDriver && t.IsRolePassenger() && (this.AutoUpdateUeMovementTickState = !1, this.UeMovementDisableHandle) && (this.UeMovementMgrComp.Enable(this.UeMovementDisableHandle, "进入载具主动启用移动组件Tick"), this.UeMovementDisableHandle = 0)
    }, this.OnLeaveVehicle = t => {
      t.IsDriver && t.IsRolePassenger() && (this.AutoUpdateUeMovementTickState = !0)
    }
  }
  ...
  OnStart() {
    if (this.ActorComp = this.Entity.GetComponent(234), this.AnimComp = this.Entity.GetComponent(235), this.TagComponent = this.Entity.GetComponent(205), this.AudioComp = this.Entity.GetComponent(242), this.UeMovementMgrComp = this.Entity.GetComponent(244), this.UeMovementDisableHandle = this.UeMovementMgrComp.Disable("载具出生时默认关闭移动组件"), this.VehicleMovement = this.ActorComp.Actor.GetComponentByClass(UE.KuroVehicleMovementComponent.StaticClass()), !this.VehicleMovement) return !1;
    if (this.GravityDirectInternal.FromUeVector(this.VehicleMovement.Kuro_GetGravityDirect()), this.IsStandardGravityInternal = Math.abs(this.GravityDirectInternal.Z + 1) < MathUtils_1.MathUtils.SmallNumber, this.IsStandardGravityInternal && this.GravityDirectInternal.Set(0, 0, -1), this.GravityDirectInternal.UnaryNegation(this.GravityUpInternal), this.VehicleMovement.GravityScale = 2, this.InitGravityDirect(), this.CannotResponseInputCount = 0, this.TagComponent)
      for (const t of cannotResponseInputTag) this.TagComponent.HasTag(t) && ++this.CannotResponseInputCount, this.CanResponseInputTasks.push(this.TagComponent.ListenForTagAddOrRemove(t, this.OnResponseInputTagsChanged));
    return EventSystem_1.EventSystem.AddWithTarget(this.Entity, EventDefine_1.EEventName.OnVehicleBeenEntered, this.OnEnterVehicle), EventSystem_1.EventSystem.AddWithTarget(this.Entity, EventDefine_1.EEventName.OnVehicleBeenLeaved, this.OnLeaveVehicle), !0
  }
  OnActivate() {
    var t = this.ActorComp.CreatureData.ComponentDataMap.get("pAc")?.pAc;
    t && ControllerHolder_1.ControllerHolder.SyncSplineMoveController.SyncVehicleMoveAlongPath(this.Entity, t.dTs)
  }
  OnTick(i) {
    if (this.DrawDebugCurve(), super.OnTick(i), this.CanMoveWithDistanceInternal = this.Entity.DistanceWithCamera <= 7e3, this.ActorComp && (this.DeltaTimeSeconds = i * MathUtils_1.MathUtils.MillisecondToSecond, !this.IsSpecialMove)) {
      this.IsStopInternal ? this.Speed = 0 : this.Speed = this.ActorComp.ActorVelocityProxy.Size2D(), this.IsMoving = this.Speed > MIN_MOVE_SPEED, this.IsMovePath || this.AudioComp?.UpdateVehicleMoveSound(this.Speed, this.ActorComp.Owner), this.UpdateUeMovementDisableState();
      var s = 1 < this.Entity.GetTickInterval() && this.AnimComp?.Valid && this.ActorComp.Owner.WasRecentlyRenderedOnScreen();
      let t = void 0;
      s && (t = this.AnimComp.GetMeshTransform()), this.CanResponseInput() ? (this.SetInfoVar(), this.CacheVar()) : this.HasMoveInput = !1, this.ActorComp.IsMoveAutonomousProxy && (s && this.IsMoving && this.AnimComp.SetModelBuffer(t, i), this.ActorComp.Actor.AddMovementInput(this.ActorComp.InputDirect, 1, !1))
    }
  }
  OnEnd() {
    for (const t of this.CanResponseInputTasks) t.EndTask();
    return this.CanResponseInputTasks.length = 0, this.StopMove(), EventSystem_1.EventSystem.RemoveWithTarget(this.Entity, EventDefine_1.EEventName.OnVehicleBeenEntered, this.OnEnterVehicle), EventSystem_1.EventSystem.RemoveWithTarget(this.Entity, EventDefine_1.EEventName.OnVehicleBeenLeaved, this.OnLeaveVehicle), !0
  }
  SetInfoVar() {
    this.DeltaTimeSeconds > MathUtils_1.MathUtils.SmallNumber && (this.Acceleration.DeepCopy(this.ActorComp.ActorVelocityProxy), this.Acceleration.SubtractionEqual(this.PreviousVelocity), this.Acceleration.DivisionEqual(this.DeltaTimeSeconds), this.AimYawRate = Math.abs(this.ActorComp.ActorRotationProxy.Yaw - this.PreviousAimYaw) / this.DeltaTimeSeconds), this.HasMoveInput = GravityUtils_1.GravityUtils.GetPlanarSizeSquared2dForActor(this.ActorComp, this.ActorComp.InputDirectProxy) > MathUtils_1.MathUtils.SmallNumber
  }
  CacheVar() {
  ...
  ApplyForceSpeedAndRecordSpeed() {
    this.ForceSpeed.X !== INVALID_FORCE_SPEED && (this.ForceSpeed.ContainsNaN() ? Log_1.Log.CheckError() && Log_1.Log.Error("Movement", 6, "ForceSpeed Nan.", ["V", this.ForceSpeed]) : this.VehicleMovement.Velocity = this.ForceSpeed.ToUeVectorOld(), this.ActorComp.ResetCachedVelocityTime(), this.ForceSpeed.X = INVALID_FORCE_SPEED)
  }
  SetForceSpeed(t) {
  ...
  MoveAlongPath(i) {
    ...
    var t;
    s?.IsValid() && (t = i.StartFromNearest ? this.FindNearestNextPoint(s.CurveInfo.SplineCurve) : 0, s.JumpToPoint(t), !i.ForceToFirstPoint && (s.CurveInfo.SplineCurve.GetTransformAtSplineIndex(t, 1, this.TmpTrans), t = s.CurveInfo.SplineConfig?.TransitionSpeed || s.CurveInfo.SplineCurve.GetSplineLength() / s.CurveInfo.TotalTime, (t = VehiclePathMoveController_1.VehiclePathMoveController.CreateMoveToTask(this.Entity, this.TmpTrans, t))?.IsValid()) ? (t.CurveInfo.SplineId = -i.SplineId, t.NeedSync = i.NeedSync ?? !0, t.SimulateRotation &&= i.SimulateRotation ?? !0, t.OnMoveEndHandle = t => {
      t ? (VehiclePathMoveController_1.VehiclePathMoveController.AddSplineMoveTask(s), this.DebugCurve = s.CurveInfo?.SplineCurve, i.OnArriveStartPointHandle && i.OnArriveStartPointHandle(t)) : i.OnMoveEndHandle && i.OnMoveEndHandle(!1)
    }, s.NeedSync = i.NeedSync ?? !0, s.SimulateRotation = i.SimulateRotation ?? !0, s.EnableDynamicGravity(!!i.DynamicGravity), s.OnMoveEndHandle = t => {
      i.OnMoveEndHandle && i.OnMoveEndHandle(t), this.DebugCurve = void 0
    }, VehiclePathMoveController_1.VehiclePathMoveController.AddSplineMoveTask(t), this.DebugCurve = t.CurveInfo?.SplineCurve) : (VehiclePathMoveController_1.VehiclePathMoveController.AddSplineMoveTask(s), this.DebugCurve = s.CurveInfo?.SplineCurve))
  }
  StopMove() {
  ...
  FindNearestNextPoint(i) {
    ...
    for (let t = 0; t < i.WorldPositionList.length - 1; t++) {
      o.DeepCopy(i.WorldPositionList[t]), r.DeepCopy(i.WorldPositionList[t + 1]), this.TmpVector.Set(r.X, r.Y, r.Z), this.TmpVector.SubtractionEqual(o);
      var n = this.TmpVector.Size();
      this.TmpVector2.Set(e.X, e.Y, e.Z), this.TmpVector2.SubtractionEqual(r), 0 < this.TmpVector.DotProduct(this.TmpVector2) || (this.TmpVector2.Set(e.X, e.Y, e.Z), this.TmpVector2.SubtractionEqual(o), this.TmpVector.DotProduct(this.TmpVector2) < 0) || this.TmpVector.DotProduct(this.ActorComp.ActorForwardProxy) < 0 || (this.TmpVector.CrossProduct(this.TmpVector2, this.TmpVector), (n = this.TmpVector.Size() / n) < h && (h = n, s = t + 1))
    }
    return s
  }
  DrawDebugCurve() {
    if (this.DebugCurve && this.ActorComp?.Actor.CapsuleComponent?.bKuroMoveDebugLog) {
      var i = this.DebugCurve?.GetSplineLength(),
        s = i / 100;
      for (let t = 0; t < i; t += s) this.DebugCurve.GetTransformAtDistanceAlongSpline(t, 1, this.TmpTrans), UE.KismetSystemLibrary.D_DrawDebugPoint(this.ActorComp.Actor, this.TmpTrans.GetLocation().ToUeVector(), 10, splineDebugColor, .1)
    }
  }
};
VehicleMoveComponent = __decorate([(0, RegisterComponent_1.RegisterComponent)(236)], VehicleMoveComponent), exports.VehicleMoveComponent = VehicleMoveComponent;

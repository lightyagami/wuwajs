...
class SimplePool {
  ...
  PreloadAdd(t) {
    t || Log_1.Log.CheckError() && Log_1.Log.Error("Pool", 17, "无效对象", ["target", t]), this.p7.push(t)
  }
  Release(t) {
    t || Log_1.Log.CheckError() && Log_1.Log.Error("Pool", 17, "无效对象", ["target", t]), this.p7.push(t)
  }
...
class BulletPool {
  static Init() {
    for (let t = 0; t < PRE_ADD_COUNT; t++) {
      ...
      EntitySystem_1.EntitySystem.Init(l), EntitySystem_1.EntitySystem.DeSpawn(l), BulletPool.BulletEntityPool.Put(l)
    }
    for (let t = 0; t < PRE_ADD_COUNT; t++) {
    ...
    for (let t = 0; t < PRE_ADD_COUNT; t++) this.VectorPool.PreloadAdd(Vector_1.Vector.Create());
    for (let t = 0; t < PRE_ADD_COUNT; t++) this.RotatorPool.PreloadAdd(Rotator_1.Rotator.Create());
    for (let t = 0; t < PRE_ADD_COUNT; t++) this.BulletHitTempResultPool.PreloadAdd(new BulletHitActorData_1.BulletHitTempResult)
  }
  static Clear() {
    this.BulletEntityPool.Clear(), this.BulletHitActorDataPool.Clear(), this.BulletConditionResultPool.Clear(), this.VectorPool.Clear(), this.RotatorPool.Clear()
  }
  static CreateBulletEntity() {
    let t = BulletPool.BulletEntityPool.Get(KEY_BULLET_ENTITY);
    return t ? EntitySystem_1.EntitySystem.Respawn(t) : (t = BulletPool.BulletEntityPool.Create(KEY_BULLET_ENTITY), EntitySystem_1.EntitySystem.Init(t)), t
  }
  ...
  static RecycleVector(t) {
    this.yjo--, BulletConstant_1.BulletConstant.OpenPoolCheck ? t.Set(NaN, NaN, NaN) : BulletPool.VectorPool.Release(t)
  }
  ...
  static RecycleRotator(t) {
    this.Ijo--, BulletConstant_1.BulletConstant.OpenPoolCheck ? t.Set(NaN, NaN, NaN) : BulletPool.RotatorPool.Release(t)
  }
  static CreateBulletHitTempResult() {
    let t = BulletPool.BulletHitTempResultPool.Get();
    return t = t || new BulletHitActorData_1.BulletHitTempResult, this.Tjo++, t
  }
  static RecycleBulletHitTempResult(t) {
    this.Tjo--, BulletPool.BulletHitTempResultPool.Release(t)
  }
  static CheckAtFrameEnd() {
    0 !== this.yjo && (Log_1.Log.CheckError() && Log_1.Log.Error("Bullet", 17, "当前帧子弹申请的Vector没有回收", ["VectorCount", this.yjo]), this.yjo = 0), 0 !== this.Ijo && (Log_1.Log.CheckError() && Log_1.Log.Error("Bullet", 17, "当前帧子弹申请的Rotator没有回收", ["RotatorCount", this.Ijo]), this.Ijo = 0), 0 !== this.Tjo && (Log_1.Log.CheckError() && Log_1.Log.Error("Bullet", 17, "当前帧子弹申请的BulletHitTempResultCount没有回收", ["BulletHitTempResultCount", this.Tjo]), this.Tjo = 0)
  }
}(exports.BulletPool = BulletPool).BulletEntityPool = new ProxyLru_1.ProxyLru(PRE_ADD_COUNT, t => EntitySystem_1.EntitySystem.Create(BulletEntity_1.BulletEntity)), BulletPool.BulletHitActorDataPool = new Pool_1.Pool(CAPACITY, () => new BulletHitActorData_1.BulletHitActorData), BulletPool.BulletConditionResultPool = new Pool_1.Pool(CAPACITY, () => new BulletCollisionInfo_1.BulletConditionResult), BulletPool.VectorPool = new SimplePool, BulletPool.yjo = 0, BulletPool.RotatorPool = new SimplePool, BulletPool.Ijo = 0, BulletPool.BulletHitTempResultPool = new SimplePool, BulletPool.Tjo = 0;

...
class MapMarkContainer {
  constructor() {
    this.vlh = new MapMarkPreemptiveFrameQueue_1.MapMarkPreemptiveFrameQueue(50), this.rUi = new Map, this.Mlh = new Map, this.nUi = new Map, this.hUi = new Map, this.sUi = new Set, this.aUi = new Set, this.qDl = new Set
  }
  Tick() {
    this.vlh.Process()
  }
  ClearMarkItems(e, t) {
    if (e) {
      ...
      if (r) {
        for (var [, a] of r) this.hUi.delete(a.MarkId), a.Destroy(t), this.LUi(a);
        r.clear()
      }
      r = this.Mlh.get(e);
      if (r) {
    ...
    } else this.nIl(this.rUi, t), this.nIl(this.Mlh, t), this.sUi.clear(), this.rUi.clear(), this.Mlh.clear(), this.nUi.clear(), this.vlh.Dispose()
  }
  AddMarkItem(t, r) {
    var e;
    if (r)
      if (r.IsTracked && 12 !== r.MarkType && (e = {
          MarkType: r.MarkType,
          MarkId: r.MarkId,
          Track: !0
        }, ModelManager_1.ModelManager.MapModel.IsEqualToCurTrack(e) || ModelManager_1.ModelManager.MapModel.SetCurTrackMark(e), this.sUi.add(r)), r.IsInConsistentDistrict()) MapLogger_1.MapLogger.Debug(63, "标记系统->MapMarkContainer.AddMarkItem, 被添加到跨地图列表，将不会显示在地图上", ["markType", t], ["markId", r.MarkId], ["MapType", r.MapType], ["InstanceDungeonId", r.InstanceDungeonId], ["MapId", r.MapId]), this.Slh(t, r);
      else {
        let e = this.GetMarkItemsByType(t, !1);
        e || (e = new Map, this.rUi.set(t, e)), e.has(r.MarkId) ? ModelManager_1.ModelManager.WorldMapModel.EnableDebug && MapLogger_1.MapLogger.ErrorOnce(r.MarkId, 63, "重复添加标记_MarkMgr", ["MarkId", r.MarkId]) : (e.set(r.MarkId, r), this.TUi(r), MapLogger_1.MapLogger.Debug(63, "标记系统->MapMarkContainer.AddMarkItem", ["markType", t], ["markId", r.MarkId], ["MapType", r.MapType]), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.AddMapMark, r))
      }
  }
  Slh(t, r) {
    if (r) {
  ...
  RemoveMarkItem(e, t) {
    ...
    if (r && 0 !== r.size) {
      var a = r.get(t);
      if (r.delete(t), this.hUi.delete(t), a) return this.LUi(a), MapLogger_1.MapLogger.Debug(63, "标记系统->MapMarkContainer.RemoveMarkItem", ["markType", e], ["markId", t]), a
    }
  }
  nIl(e, t) {
    for (var [, r] of e)
  ...
  TUi(e) {
    var t, r = e.WorldPosition;
    r && (r = MapUtil_1.MapUtil.ConvertWorldPositionToIndex(r), (t = this.nUi.get(r)) ? this.DO_(t, e) : (t = (new Set).add(e), this.nUi.set(r, t)), e.GridId = r)
  }
  DO_(e, t) {
    e.has(t) ? MapLogger_1.MapLogger.ErrorOnce(t.MarkId, 63, "重复添加标记到格子集合中_MarkMarkContainer", ["MarkId", t.MarkId]) : e.add(t)
  }
  LUi(e) {
    var t = e.GridId,
      t = this.nUi.get(t);
  ...
  GetMarkItemById(a, e = !1) {
    ...
    return void 0 === r && e && (e = this.GetAllDiffMapMarkItems(), r = t(e)), r
  }
  ...
  GetMarkItemsByClickPosition(e) {
    var e = MapUtil_1.MapUtil.UiPosition2WorldPosition(e),
      e = MapUtil_1.MapUtil.ConvertWorldPositionToIndex(e),
      t = [];
  ...
  UpdateNearbyMarkItem(e, t, r) {
    this.vlh.Flush();
    var a, e = MapUtil_1.MapUtil.ConvertWorldPositionToIndex(e),
      i = MapUtil_1.MapUtil.GetAroundIndex(e);
    for (const o of i) {
      var s = this.nUi.get(o);
      if (s)
        for (const h of s) this.hUi.has(h.MarkId) || 11 === h.MarkType || this.hUi.set(h.MarkId, h)
    }
    this.GDl();
    for ([, a] of this.hUi) {
      t(a);
      var n = a.GridId;
      this.qDl.has(a.MarkId) || a.IsCanShowView && i.has(n) || (this.aUi.add(a), this.hUi.delete(a.MarkId))
    }
    for (const k of this.sUi) t(k);
    if (0 !== this.aUi.size) {
      for (const f of this.aUi) f.IsDestroy || r(f);
      this.aUi.clear()
    }
    for (const _ of i) {
      var M = this.nUi.get(_);
      if (M)
        for (const p of M) t(p)
    }
  }
  GDl() {
    this.qDl.clear();
  ...
  FindNearbyMarkItems(r, e, a) {
    ...
    const i = [],
      s = e * e;
    for (const n of t) this.nUi.get(n)?.forEach(e => {
      var t;
      (a?.(e) ?? !0) && (t = 9 !== r.MarkType && 22 !== r.MarkType || !MathUtils_1.MathUtils.IsNearlyZero(r.WorldPosition.Z) ? Vector_1.Vector.DistSquared(e.WorldPosition, r.WorldPosition) : Vector2D_1.Vector2D.DistSquared(Vector2D_1.Vector2D.Create(e.WorldPosition.X, e.WorldPosition.Y), Vector2D_1.Vector2D.Create(r.WorldPosition.X, r.WorldPosition.Y))) <= s && i.push([e, t])
    });
    return 0 < i.length && i.sort((e, t) => e[1] - t[1]), i
  }
  RemoveDynamicMark(e, t) {
    var r = this.GetMarkItem(e, t);
    r && (this.TrackMapMark(e, r.MarkId, !1), this.hUi.has(t) && this.hUi.delete(t), void 0 !== (r = this.RemoveMarkItem(e, t))) && r.Destroy()
  }
  TrackMapMark(t, r, a, i = !1) {
    ...
    if (n || s) {
      let e = 0;
      s && (e = s.TrackSource, s.IconPath = n?.IconPath ?? s.IconPath), n && (e = n.TrackSource);
      s = ModelManager_1.ModelManager.TrackModel.IsTracking(e, r);
      i || s !== a ? a ? n && (i = n.MarkItemEntity.GetComponent(15)?.Config, TrackController_1.TrackController.StartTrack({
  ...
  GetTrackMenuMarkList() {
    const t = [];
    return this.GetMarkItemsByType(11, !1)?.forEach(e => {
      t.push(e)
  ...
  GetNavigateMarkList() {
    ...
    this.GetMarkItemsByType(11, !1)?.forEach(e => {
      ...
      var t = ModelManager_1.ModelManager.MapModel.GetDungeonLocateWorldMapId(e.InstanceDungeonOrMapConfigId);
      ConfigManager_1.ConfigManager.WorldMapConfig.IsMapInWorld(t ?? e.MapId) && r.push(e)
    });
    var e = ModelManager_1.ModelManager.QuestNewModel.GetCurTrackedQuest(),
      t = e?.GetCurrentActiveChildQuestNode()?.NodeId ?? 0,

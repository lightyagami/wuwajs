...
class ShipTowerStageData {
  constructor() {
    this.Id = 0, this.OrderIndex = 1, this.StageType = 0, this.Season = 0, this.IsEndLess = !1, this.InstIds = [], this.PreLevel = [], this.TitleKey = "", this.DescKey = "", this.TargetScoreList = [], this.ScoreStageList = [], this.CurrentScore = 0, this.PassScore = 0, this.PassReward = 0, this.cs_ = [], this.TeamDataList = [], this.ProtoIsUnLocked = !1, this.NewChallengeScore = 0, this.IsHaveProtoData = !1, this.LastIsPass = !1, this.LastScore = 0, this.IsNeedSureScore = !1, this.BelongToSeason = 0, this.ProtoIsPassed = !1, this.TeamRecommendList = []
  }
  Init(e) {
    this.Id = e.Id, this.Season = e.Season, this.IsEndLess = e.EndLess, this.InstIds = e.InstIds, this.PreLevel = e.PreLevel, this.PassScore = e.PassScore, this.PassReward = e.LevelPassReward, this.TitleKey = e.Title, this.DescKey = e.Desc, this.TargetScoreList = e.TargetScore, this.ScoreStageList = e.ScoreStage, this.BelongToSeason = e.Season, this.us_()
  }
  us_() {
    this.IsEndLess ? this.StageType = 2 : 0 === this.Season ? this.StageType = 0 : this.StageType = 1, this.InstIds.forEach((e, t) => {
      var s = new ShipTowerTeamData_1.ShipTowerTeamData;
      s.Init(e, t, this.Id), this.TeamDataList.push(s)
    })
  }
  IsUnLocked() {
    return this.IsHaveProtoData ? this.ProtoIsUnLocked : 0 === this.PreLevel.length || this.PreLevel.every(e => ModelManager_1.ModelManager.ShipTowerModel.GetStageDataById(e).IsPassed())
  }
  IsPassed() {
  ...
  OpenViewStageDesc() {
    var e;
    this.IsUnLocked() ? ModelManager_1.ModelManager.ShipTowerModel.OpenViewDesc({
      StageId: this.Id
    }) : (e = ShipTowerDefine_1.shipTowerTextKey.StageNotUnLock, e = ConfigManager_1.ConfigManager.TextConfig.GetMultiTextByKey(e, e), ScrollingTipsController_1.ScrollingTipsController.ShowTipsByText(e))
  }
  ...
  ZH_() {
    var e, t;
    if (!this.IsEndLess) return e = ShipTowerDefine_1.shipTowerTextKey.PassTarget, t = ShipTowerDefine_1.shipTowerTextKey.ScoreTarget, {
  ...
  e9_() {
    ...
    return this.TargetScoreList.forEach((e, t) => {
  ...
  GetPassUnlockBuffList() {
    return 0 < this.PassReward && 0 === this.cs_.length && ConfigManager_1.ConfigManager.RewardConfig.GetDropPackage(this.PassReward)?.DropPreview.forEach((e, t) => {
      this.cs_.push([{
        ItemId: t,
        IncId: 0
      }, 0])
  ...
  UpdateOtherTeamRoleToModel(t) {
    ModelManager_1.ModelManager.ShipTowerModel.ClearOtherTeamRoleData(), this.TeamDataList.forEach(e => {
      e.Index !== t && (e.UpdateOtherTeamRoleToShipTowerModel(), e.UpdateRoleIndexInAllTeam())
    })
  }
  UpdateAllTeamRoleToModel() {
    ModelManager_1.ModelManager.ShipTowerModel.ClearAllTeamRoleData(), this.TeamDataList.forEach(e => {
  ...
  UpdateOtherTeamRoleRepeat(t, s) {
    ...
    let r = !1;
    return this.TeamDataList.forEach(e => {
      e.Index !== t && (r = e.UpdateOtherTeamRoleRepeat(i) || r, e.RemoveRoleIdEdit(s))
    }), this.UpdateOtherTeamRoleToModel(t), r
  }
  StartChallenge() {
  ...
  Cs_() {
    this.CurrentScore = 0, this.TeamDataList.forEach(e => {
  ...
  ProtoUpdateDataBase(e) {
    this.IsHaveProtoData = !0, this.ProtoIsUnLocked = e.MT_, this.ProtoIsPassed = e.nA_;
    const s = this.TeamDataList[0],
      i = this.TeamDataList[1];
    e.UL_?.RL_.forEach((e, t) => {
      s.ProtoSetRole(e, t)
    }), s.ProtoSetBuff(e.UL_?.AL_), s.UpdateCurrentScore(e.PL_), e.DL_?.RL_.forEach((e, t) => {
      i.ProtoSetRole(e, t)
    }), i.ProtoSetBuff(e.DL_?.AL_), i.UpdateCurrentScore(e.xL_)
  }
  ProtoNotifyUpdateData(e) {
    this.SaveLastData(), this.ProtoUpdateDataBase(e), this.Cs_(), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.ShipTowerStageUpdate, this.Id)
  }
  SaveLastData() {
    this.LastIsPass = this.IsPassed(), this.LastScore = this.CurrentScore, this.IsNeedSureScore = 0 < this.CurrentScore
  }
  CheckPass(e = 0) {
  ...
  UpdateNewChallengeScore() {
    this.NewChallengeScore = 0, this.TeamDataList.forEach(e => {
  ...
  ProtoTeamEditFromResult(e) {
    var t = this.TeamDataList[0],
      ...
      i = (t.ProtoTeamEditFromResult(e.sA_), s.ProtoTeamEditFromResult(e.aA_), e.GL_ + e.FL_),
      e = e.NL_ + e.VL_;
    t.ProtoSetNewChallengeScore(i), s.ProtoSetNewChallengeScore(e), this.UpdateNewChallengeScore()
  }
  ...
  ProtoUpdateTeamRecommendList(e) {
    this.TeamRecommendList.length = 0, e.Iuc.forEach((e, t) => {
      const s = [],
        i = [];
      e.Muc?.RL_.forEach(e => {
        s.push({
  ...
  UseTeamRecommend(e) {
    var t = this.TeamDataList[0],
      s = this.TeamDataList[1];
    return t.CopyIdsToEdit(e.RoleIdList1.map(e => e.Id)), s.CopyIdsToEdit(e.RoleIdList2.map(e => e.Id)), t.CopyBuffIdToEdit(e.Buff1, this.Id), s.CopyBuffIdToEdit(e.Buff2, this.Id), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.ShipTowerTeamRecommendApplyFinish, this.Id), !0
  }

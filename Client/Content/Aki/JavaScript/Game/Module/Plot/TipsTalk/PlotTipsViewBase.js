...
class PlotTipsViewBase extends UiPanelBase_1.UiPanelBase {
  constructor() {
    ...
    this.Eto = (e = false, t = true) => {
      if (this.Izm !== e && (!!e || !this.IsHideOrHiding)) {
        if (this.Izm = e) {
          if (t) {
  ...
  async OpenAsync(e) {
    ...
    if (this.m3d?.ViewName) {
      ...
      if (t.IsHideOrHiding) {
        if (Log_1.Log.CheckInfo()) {
          Log_1.Log.Info("Plot", 26, "[PlotViewHud] 父界面已经隐藏，attach时子界面主动隐藏");
        }
        this.Hide();
      }
    }
  ...
  Rbn(e, t = false) {
    if (Log_1.Log.CheckInfo()) {
      Log_1.Log.Info("Plot", 26, "[PlotTips] 开始延迟完成字幕", ["delay", e], ["isPause", t]);
    }
  ...
  Abn() {
    if (Log_1.Log.CheckInfo()) {
      Log_1.Log.Info("Plot", 26, "[PlotTips] 暂停字幕");
    }
    this.ybn?.Pause();
    if (this.lZi !== AudioSystem_1.INVALID_AUDIO_EVENT_VALUE) {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("Plot", 26, "[PlotTips] 暂停音频播放");
      }
  ...
  EZi() {
    ...
    if (this.lZi === AudioSystem_1.INVALID_AUDIO_EVENT_VALUE) {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("Plot", 26, "[PlotTips] 语音播放", ["mediaName", i]);
      }
      this.Ybn++;
      const s = this.Ybn;
      this.lZi = AudioSystem_1.AudioSystem.PostEvent(this.OverrideAudioEventName ?? e, this.m3d?.AudioAttachActor, {
        ...
        CallbackHandler: (e, t) => {
          if (Log_1.Log.CheckInfo()) {
            Log_1.Log.Info("Plot", 26, "[PlotTips] 语音播放完成回调", ["mediaName", i], ["callbackType", e]);
          }
          if (e === 0 && s === this.Ybn) {
            this.lZi = AudioSystem_1.INVALID_AUDIO_EVENT_VALUE;
            this.Rbn(this.Ebn.CaptionParams?.IntervalTime || ModelManager_1.ModelManager.PlotModel.PlotGlobalConfig.AudioEndWaitTimePrompt, this.Izm);
          }
        }
      });
    } else {
      if (Log_1.Log.CheckDebug()) {
        Log_1.Log.Debug("Plot", 26, "[PlotTips] 恢复语音播放", ["mediaName", i]);
      }
      AudioSystem_1.AudioSystem.ExecuteAction(this.lZi, 2, {
        TransitionDuration: BREAK_TIME
      });
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("Plot", 26, "[PlotTips] 恢复音频播放", ["mediaName", i]);
      }
    }
    return true;
  }
  pZi() {
    ...
    if (this.lZi === AudioSystem_1.INVALID_AUDIO_EVENT_VALUE) {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("Plot", 26, "[PlotTips] 语气播放", ["event", s]);
      }
      this.Ybn++;
      const o = this.Ybn;
      this.lZi = AudioSystem_1.AudioSystem.PostEvent(s, undefined, {
        ...
        CallbackHandler: (e, t) => {
          if (Log_1.Log.CheckInfo()) {
            Log_1.Log.Info("Plot", 26, "[PlotTips] 语气播放完成回调", ["event", s]);
          }
          if (e === 0 && o === this.Ybn) {
            this.lZi = AudioSystem_1.INVALID_AUDIO_EVENT_VALUE;
            this.Rbn(this.Ebn.CaptionParams?.IntervalTime || ModelManager_1.ModelManager.PlotModel.PlotGlobalConfig.AudioEndWaitTimePrompt, this.Izm);

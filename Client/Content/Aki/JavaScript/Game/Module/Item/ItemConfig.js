...
class ItemConfig extends ConfigBase_1.ConfigBase {
  GetConfig(e) {
    ...
    if (i) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("Item", 16, "获取物品配置错误", ["itemConfigId", e])
  }
  ...
  GetSpecialItemConfig(e) {
    ...
    if (i) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("Item", 37, "获取特殊物品配置错误", ["specialItemId", e])
  }
  GetMainTypeConfig(e) {
    var i = ItemMainTypeById_1.configItemMainTypeById.GetConfig(e);
    if (i) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("Item", 16, "获取物品主类型配置错误", ["mainType", e])
  }
  GetQualityConfig(e) {
    var i = QualityInfoById_1.configQualityInfoById.GetConfig(e);
    if (i) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("Item", 16, "获取获取物品品质配置错误", ["qualityId", e])
  }
  GetItemListMaxSize() {
    var e = CommonParamById_1.configCommonParamById.GetIntConfig("item_list_max_size");
    return e || Log_1.Log.CheckError() && Log_1.Log.Error("Reward", 8, '外入包列表最大数量无法找到, 请检测c.参数字段"item_list_max_size"'), e
  }
  GetPriorItemListMaxSize() {
    var e = CommonParamById_1.configCommonParamById.GetIntConfig("item_prior_list_max_size");
    return e || Log_1.Log.CheckError() && Log_1.Log.Error("Reward", 5, '外入包列表最大数量无法找到, 请检测c.参数字段"item_prior_list_max_size"'), e
  }
  GetItemName(e) {
  ...
  GetItemDesc(e) {
    ...
    if (e) {
      ...
      if (e) return e = e.Name, MultiTextLang_1.configMultiTextLang.GetLocalTextNew(e)
    }
  }
  GetItemAttributeDesc(e) {
    e = this.GetConfig(e);
    if (e) return MultiTextLang_1.configMultiTextLang.GetLocalTextNew(e.AttributesDescription)
  }
}

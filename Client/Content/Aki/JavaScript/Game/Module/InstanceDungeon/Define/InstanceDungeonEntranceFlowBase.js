...
class InstanceDungeonEntranceFlowBase {
  ...
  Rai() {
    ++this.Dai, this.Dai < 0 || this.Dai >= this.Lai.length ? Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "副本进入流程执行失败，当前步数与总步数不匹配！", ["CurrentStep", this.Dai]) : this.Lai[this.Dai]()
  }
  Reset() {
    this.Dai = -1
  }

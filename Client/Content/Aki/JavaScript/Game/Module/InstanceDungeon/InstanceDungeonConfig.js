...
class InstanceDungeonConfig extends ConfigBase_1.ConfigBase {
  constructor() {
    super(...arguments), this.thi = new Map, this._ec = new Map, this.cec = () => {
      return ModelManager_1.ModelManager.TowerModel.CheckInTower() ? !this.uec() : !ModelManager_1.ModelManager.ShipTowerModel?.CheckInBattleShipTower() || !UiManager_1.UiManager.IsViewOpen("ShipTowerDescView")
    }, this.mec = () => this.cec(), this.uec = () => !!UiManager_1.UiManager.IsViewOpen("TeamRoleSelectView") || !!UiManager_1.UiManager.IsViewOpen("MultiTeamRoleSelectView")
  }
  OnInit() {
    return this._ec.set("RoleRootView", this.cec), this._ec.set("WeaponRootView", this.mec), !0
  }
  ihi(e) {
    let n = this.thi.get(e);
    return n || (n = new Array, (n = Array.from(this.GetConfig(e).RecommendLevel)).sort((e, n) => e[0] - n[0]), this.thi.set(e, n)), n
  }
  GetConfig(e) {
    var n = InstanceDungeonById_1.configInstanceDungeonById.GetConfig(e);
    if (n) return n;
    Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "获取副本配置错误", ["id", e])
  }
  GetCountConfig(e) {
  ...
  GetTitleConfig(e) {
    ...
    if (n) return n;
    Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "获取副本标题配置错误", ["id", e])
  }
  GetTrialRoleConfig(e) {
    var n = InstanceTrialRoleConfigById_1.configInstanceTrialRoleConfigById.GetConfig(e);
    if (n) return n;
    Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 48, "获取副本试用角色配置错误", ["id", e])
  }
  GetGameplayModeConfig(e) {
    var n = InstanceGameplayModeById_1.configInstanceGameplayModeById.GetConfig(e);
    if (n) return n;
    Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 48, "获取副本玩法模式配置错误", ["id", e])
  }
  ...
  GetRecommendLevel(e, n) {
    e = this.ihi(e);
    if (!e) return Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "推荐等级区间配置错误"), 0;
    let r = 0;

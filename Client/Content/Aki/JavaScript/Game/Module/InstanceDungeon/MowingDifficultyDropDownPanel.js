...
exports.MowingDifficultyDropDownPanel = exports.DropDownTitle = exports.DropDownItem = undefined;
const UE = require("ue");
...
class DropDownItem extends DropDownItemBase_1.DropDownItemBase {
  ...
  OnShowDropDownItemBase(t) {
    ...
    var t = StringUtils_1.StringUtils.Format(MultiTextLang_1.configMultiTextLang.GetLocalTextNew("MowingPointMultiply") ?? "", (t.Magnification / 100).toString());
...
class MowingDifficultyDropDownPanel extends UiPanelBase_1.UiPanelBase {
  constructor() {
    ...
    this.f_i = (t, e) => {
      var i;
      var o = ActivityMowingController_1.ActivityMowingController.GetMowingActivityData();
      if (o) {
        if (this.m_i) {
          i = TakeWeedsDifficultyById_1.configTakeWeedsDifficultyById.GetConfig(e.Id).RecommendedLevel.toString();
          LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(3), "RecommendLevel", i);
          ActivityMowingController_1.ActivityMowingController.RequestSetDifficultyAll(o.Id, e.Id);
  ...
  async p_i() {
    this.c_i = new CommonDropDown_1.CommonDropDown(this.GetItem(2), this.d_i, this.C_i);
  ...
  v_i() {
    ...
    if (!t || t.DifficultyOptions.length === 0) {
      return this.u_i;
    }
    var e = [];
    for (const i of t.DifficultyOptions) {
      e.push(TakeWeedsDifficultyById_1.configTakeWeedsDifficultyById.GetConfig(i));
    }
    return e;
  }
  RefreshByInstanceId(t) {
    ...
    if (t) {
      this.c_i?.InitScroll(this.v_i(), this.g_i, t.GetLevelDiffIndex(this.NUe));
    } else if (Log_1.Log.CheckError()) {
      Log_1.Log.Error("Activity", 37, "当前没有割草活动数据");
    }
  }

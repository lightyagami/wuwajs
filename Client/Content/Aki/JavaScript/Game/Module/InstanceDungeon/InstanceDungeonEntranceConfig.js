...
class InstanceDungeonEntranceConfig extends ConfigBase_1.ConfigBase {
  constructor() {
    super(...arguments), this.hhi = new Map, this.lhi = void 0, this.MXa = void 0
  }
  get SXa() {
    if (!this.MXa) {
      this.MXa = new Map;
  ...
  OnInit() {
    return this.hhi.set(1, new InstanceDungeonEntranceFlowNormal_1.InstanceDungeonEntranceFlowNormal), this.hhi.set(2, new InstanceDungeonEntranceFlowSkipEditFormation_1.InstanceDungeonEntranceFlowSkipEditFormation), this.hhi.set(6, new InstanceDungeonEntranceFlowRoguelike_1.InstanceDungeonEntranceFlowRoguelike), this.hhi.set(8, new InstanceDungeonEntranceFlowTowerDefence_1.InstanceDungeonEntranceFlowTowerDefense), this.hhi.set(9, new InstanceDungeonEntranceFlowAttached_1.InstanceDungeonEntranceFlowAttached), this.hhi.set(10, new InstanceDungeonEntranceFlowFarmGold_1.InstanceDungeonEntranceFlowFarmGold), this.hhi.set(12, new InstanceDungeonEntranceFlowMowingRisk_1.InstanceDungeonEntranceFlowMowingRisk), this.hhi.set(13, new InstanceDungeonEntranceFlowAbyss_1.InstanceDungeonEntranceFlowAbyss), !0
  }
  GetConfig(n) {
    var e = InstanceDungeonEntranceById_1.configInstanceDungeonEntranceById.GetConfig(n);
    if (e) return e;
    Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "获取副本入口配置错误", ["id", n])
  }
  GetInstanceDungeonEntranceFlowId(n) {
    let e = this.GetConfig(n)?.FlowId;
    return e || (Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "获取副本入口流程错误", ["flowId", e]), e = 1), e
  }
  GetInstanceDungeonEntranceFlow(n) {
    let e = this.GetConfig(n)?.FlowId;
    return e || (Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 16, "获取副本入口流程错误", ["flowId", e]), e = 1), this.hhi.get(e)
  }
  OnEditBattleViewClose() {
    for (var [, n] of this.hhi) n.OnEditBattleViewClose()
  }
  ...
  CheckMarkIdIsTowerEntrance(n) {
    ...
    return !!n && (4 === (n = this.GetConfig(n))?.FlowId || 3 === n?.FlowId || 5 === n?.FlowId)
  }
  ...
  GetEntranceIdByInstanceId(n) {
    ...
    return void 0 === e ? (Log_1.Log.CheckError() && Log_1.Log.Error("InstanceDungeon", 64, "未找到副本入口，请检查副本表配置，instanceId: " + n), 0) : e.Id
  }
  GetEntranceMarkIdMap() {
    if (!this.lhi) {
      this.lhi = new Map;
      for (const n of InstanceDungeonEntranceAll_1.configInstanceDungeonEntranceAll.GetConfigList()) n.MarkId && this.lhi.set(n.MarkId, n.Id)
    }
    return this.lhi
  }
}

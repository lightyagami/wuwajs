...
class WeaponResonanceView extends UiTabViewBase_1.UiTabViewBase {
  ...
  OnStart() {
    ...
    if (this.sVd().MaterialPlaceType === 1) {
      this.iVd = true;
    }
    if (this.iVd) {
      this.rVd = new MediumItemGrid_1.MediumItemGrid();
      this.rVd.Initialize(this.GetItem(5).GetOwner());
      this.oVd();
    } else {
      this.iko();
    }
  ...
  rko(e, t) {
    let i = 0;
    if (this.iVd || this.zOo.GetCurrentSelectedData()) {
  ...
  nVd() {
    ...
    if (this.iVd) {
      return {
        ItemId: e = this.sVd().AlternativeConsume[0],
        IncId: 0,
  ...
  oVd() {
    ...
    this.rVd.Apply(t);
    this.rVd.BindOnCanExecuteChange(() => false);
    this.rVd.BindOnExtendToggleClicked(() => {

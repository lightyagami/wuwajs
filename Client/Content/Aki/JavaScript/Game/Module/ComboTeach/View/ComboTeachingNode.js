...
class ComboTeachingNode extends AutoAttachItem_1.AutoAttachItem {
  ...
  InitNode() {
    ...
    if (e.KeyID[s]?.length === 0 || e.KeyID[s] === undefined) {
      ...
      if (t && (i = Global_1.Global.BaseCharacter.GetEntityIdNoBlueprint(), i = EntitySystem_1.EntitySystem.Get(i).GetComponent(43).GetSkillInfo(Number(t)))) {
  ...
  OnBulletHit(t) {
    ...
    if (i === 9 || i === 1) {
      i = {
        HitSkillId: t.Attacker.GetComponent(43)?.CurrentSkill?.SkillId ?? 0,
        BulletId: t.BulletId
      };

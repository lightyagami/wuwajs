...
class LoadingView extends LoadingViewBase_1.LoadingViewBase {
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UITexture], [1, UE.UIText], [2, UE.UIText], [3, UE.UITexture], [4, UE.UIButtonComponent], [5, UE.UIText], [6, UE.UISprite], [7, UE.UILayoutBase]];
    this.BtnBindInfo = [[4, this.ChangeShowTips.bind(this)]];
  }
  OnStart() {
    super.OnStart();
    this.GetButton(4).RootUIComp.SetUIActive(this.ShowData.GetTipCount() > 1);
  ...
  Ovi() {
    ...
    i?.OnLateUpdate.Bind(() => {
      i?.OnLateUpdate.Unbind();
      i?.GetRootComponent()?.SetAlpha(1);
    });
  }

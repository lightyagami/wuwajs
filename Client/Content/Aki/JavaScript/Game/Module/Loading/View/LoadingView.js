...
class LoadingView extends LoadingViewBase_1.LoadingViewBase {
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [
      ...
      [4, this.ChangeShowTips.bind(this)]
    ]
  }
  OnStart() {
    super.OnStart(), this.GetButton(4).RootUIComp.SetUIActive(1 < this.ShowData.GetTipCount()), this.RootItem?.SetAlpha(1)
  }
  OnAfterShow() {
  ...
  Ovi() {
    ...
    i?.GetRootComponent()?.SetAlpha(0), i?.OnLateUpdate.Bind(() => {
      i?.OnLateUpdate.Unbind(), i?.GetRootComponent()?.SetAlpha(1)
    })
  }
  UpdateShowTipsUi(i, t) {
    LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(1), t), LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(2), i)
  }

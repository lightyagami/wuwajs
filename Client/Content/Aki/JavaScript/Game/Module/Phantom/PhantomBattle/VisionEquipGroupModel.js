...
exports.VisionEquipGroupModel = exports.VisionAssembleAttrData = exports.VisionEquipGroupData = undefined;
...
class VisionEquipGroupModel extends ModelBase_1.ModelBase {
  constructor() {
    ...
    this.dP_ = (e, t) => {
      ...
      if (o && i) {
        if (r === a) {
          return (t.IfPercentage ? 0 : 1) - (e.IfPercentage ? 0 : 1);
        } else {
          return r - a;
        }
      } else if (o) {
  ...
  GetVisionFetterDataByIncIdList(t) {
    ...
    var e = ConfigManager_1.ConfigManager.PhantomBattleConfig.GetFetterMapResultBySuitMap(e);
    const i = new Array();
    e.forEach((e, a) => {
      e.forEach((e, t) => {
    ...
    for (let e = 0; e < o; e++) {
      ...
      if (i) {
        for (const p of i.GetPhantomMainProp()) {
          var n;
          var s = ConfigManager_1.ConfigManager.PhantomBattleConfig.GetPhantomMainPropertyItemId(p.Yws);
          var u = s.AddType === CommonComponentDefine_1.RATIO;
          let e = false;
          for (const h of a) {
            if (h.AttrId === s.PropId && h.IfPercentage === u) {
              h.CurrentValue += p.e5n;
              e = true;
              break;
            }
          }
        ...
        for (const m of i.GetPhantomSubProp()) {
          var l;
          var f = ConfigManager_1.ConfigManager.PhantomBattleConfig.GetPhantomSubPropertyById(m.Yws);
          var _ = f.AddType === CommonComponentDefine_1.RATIO;
          let e = false;
          for (const c of a) {
            if (c.AttrId === f.PropId && c.IfPercentage === _) {
              c.CurrentValue += m.e5n;
              e = true;
              break;
            }
          }
  ...
  GetLocalTipsState() {
    return LocalStorage_1.LocalStorage.GetPlayer(LocalStorageDefine_1.ELocalStoragePlayerKey.VisionTopTipsState) === 1;
  }
  GetVisionGroupFirstOpenState() {
    var e = ModelManager_1.ModelManager.FunctionModel.IsOpen(10075);
    return (LocalStorage_1.LocalStorage.GetPlayer(LocalStorageDefine_1.ELocalStoragePlayerKey.FirstOpenVisionGroup, true) ?? true) && e;
  }

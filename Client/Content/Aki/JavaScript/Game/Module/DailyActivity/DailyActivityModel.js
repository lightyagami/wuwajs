...
class DailyActivityModel extends ModelBase_1.ModelBase {
  constructor() {
    super(...arguments), this.wkt = 0, this.Bkt = 0, this.bkt = new Map, this.qkt = [], this.Gkt = 0, this.Nkt = void 0, this.AreaId = 0
  }
  get ActivityValue() {
    return this.wkt
  }
  ...
  InitGoalData() {
    ...
    for (const s of t) {
      var e = [];
      for (const a of ConfigManager_1.ConfigManager.DailyActivityConfig.GetDropShowInfo(s.DropId).entries()) e.push([{
        IncId: 0,
        ItemId: a[0]
      }, a[1]]);
      var i = {
  ...
  RefreshActivityInfo(t) {
    for (const i of t) {
      ...
      e && (e.Achieved = !0, e.State = this.GetActivityGoalState(e.Goal, !0))
    }
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DailyActivityRewardTake, t), this.EmitDailyActivityStateCheck()
  }
  RefreshActivityValue(t) {
    this.wkt = t, this.bkt.forEach((t, e) => {
      t.State = this.GetActivityGoalState(t.Goal, t.Achieved)
    }), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DailyActivityValueChange, this.wkt), this.EmitDailyActivityStateCheck()
  }
  GetActivityRewardById(t) {
    return this.bkt.get(t).Rewards
  }
  RefreshDailyActivityData(s) {
    this.wkt = s?.wxs ?? 0, this.Gkt = Number(MathUtils_1.MathUtils.LongToBigInt(s?.rEs ?? 0)), this.bkt.forEach((t, e) => {
      var i = s.xxs.includes(t.Id);
      t.Achieved = i, t.State = this.GetActivityGoalState(t.Goal, i)
    }), this.AreaId = s.p6n, this.qkt = [];
    for (const a of s.cMs) {
      var t = new DailyActivityTaskItem_1.DailyActiveTaskData,
        e = (t.TaskId = a.s5n, t.CurrentProgress = a.lMs, t.TargetProgress = a.j6n, t.IsFunctionUnlock = a.bxs, a.dMs ? a.mMs ? t.TaskState = 3 : t.TaskState = 1 : t.TaskState = 2, ConfigManager_1.ConfigManager.DailyActivityConfig.GetActivityTaskConfigById(a.s5n)),
        i = (t.Sort = e.SortRank, []);
      for (const r of e.TaskReward.entries()) i.push([{
        ItemId: r[0],
        IncId: 0
      }, r[1]]);
      t.RewardItemList = i, this.qkt.push(t)
    }
    this.SortTaskDataList(), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DailyActivityRefresh), this.EmitDailyActivityStateCheck()
  }
  UpdateDailyActivityData(t) {
    ...
    for (const a of t.cMs) {
      ...
      i.CurrentProgress = a.lMs, a.dMs ? a.mMs ? i.TaskState = 3 : i.TaskState = 1 : i.TaskState = 2
    }
    this.SortTaskDataList(), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DailyActivityTaskUpdate), this.EmitDailyActivityStateCheck()
  }
  SortTaskDataList() {

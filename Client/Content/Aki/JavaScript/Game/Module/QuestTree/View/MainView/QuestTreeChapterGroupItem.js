...
class QuestTreeChapterGroupItem extends GridProxyAbstract_1.GridProxyAbstract {
  ...
  OnStart() {
    this.KMd = [this.GetItem(6), this.GetItem(7), this.GetItem(8), this.GetItem(9), this.GetItem(10)];
    this.XMd = [this.GetItem(1), this.GetItem(2), this.GetItem(3), this.GetItem(4), this.GetItem(5)];
    this.YMd = [];
    for (let e = 0; e < QuestTreeDefine_1.QUEST_TREE_VIEW_CHAPTER_GROUP_LENGTH; e++) {
      this.YMd.push(new QuestTreeChapterItem_1.QuestTreeChapterItem());
    }
  }
  Refresh(e, t, s) {
    this.fLc(e);
  ...
  async fLc(e) {
    if (e.length !== 0 && !(e.length > QuestTreeDefine_1.QUEST_TREE_VIEW_CHAPTER_GROUP_LENGTH)) {
      ...
      for (let e = 0; e < QuestTreeDefine_1.QUEST_TREE_VIEW_CHAPTER_GROUP_LENGTH; e++) {
        ...
        if (r) {
          s = this.YMd[e];
          t.push(s.LoadOrRefresh(r, this.KMd[e]));
          this.XMd[e].SetUIActive(true);
        } else {
          this.XMd[e].SetUIActive(false);
        }
      }
      await Promise.all(t);
    }

...
exports.FriendModel = exports.LocalFriendApplication = undefined;
...
class FriendModel extends ModelBase_1.ModelBase {
  constructor() {
    ...
    this.n8t = new Set();
    this.s8t = new Set();
    this.ApplyCdTime = CommonParamById_1.configCommonParamById.GetIntConfig("apply_valid_time");
  }
  OnClear() {
    this.t8t = undefined;
    this.i8t = undefined;
  ...
  LoadLocalFriendApplication() {
    var e;
    var t = LocalStorage_1.LocalStorage.GetPlayer(LocalStorageDefine_1.ELocalStoragePlayerKey.LocalFriendApplication) ?? new Map();
    for (const i of this.z6t.values()) {
      if (t.has(i.ApplyPlayerData.PlayerId) && (e = t.get(i.ApplyPlayerData.PlayerId)).CreatedTime === i.ApplyCreatedTime) {
        i.Fresh = e.Fresh;
        if (!i.Fresh) {
  ...
  GetFriendApplyListIds() {
    ...
    for (const t of this.z6t.values()) {
      if (t.ApplyPlayerData?.CanShowInFriendList()) {
  ...
  GetRecentlyTeamIds() {
    ...
    for (const t of this.RecentlyTeamList.values()) {
      if (t.PlayerData?.CanShowInFriendList()) {
        e.push(t.PlayerData.PlayerId);
      }
    }
    e.sort((e, t) => {
      e = this.GetRecentlyTeamData(e);
      t = this.GetRecentlyTeamData(t);
      if (e.TeamTime > t.TeamTime) {
        return 1;
      } else {
        return -1;
      }
  ...
  GetFriendDataInApplicationById(e) {
    if (this.z6t.has(e)) {
      return this.z6t.get(e).ApplyPlayerData;
    }
  }
  ...
  DeleteFriend(e) {
    if (this.J6t.has(e)) {
      this.J6t.delete(e);
      EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.OnRemoveFriend, e);
    }
  ...
  AddFriendApplication(e) {
    ...
    if (e.ApplyPlayerData.CanShowInFriendList()) {
  ...
  DeleteFriendApplication(e) {
    if (this.z6t.has(e) && (this.z6t.delete(e), this.FreshFriendApplicationIds.has(e))) {
      ...
      switch (r) {
        case "FriendView":
          switch (this.FilterState) {
            ...
            default:
              if (Log_1.Log.CheckError()) {
                Log_1.Log.Error("Friend", 27, "所属页签错误！", ["页签filter", this.FilterState]);
              }
              return;
          }
        case "FriendSearchView":
          if (this.IsMyFriend(i)) {
            return this.GetFriendById(i);
          } else {
            return this.GetFriendSearchResultById(i);
          }
  ...
  ClearTestFriendData() {
    if (this.TestDataLoaded) {
      for (const e of this.J6t.keys()) {
        if (this.J6t.get(e).Debug) {
          this.J6t.delete(e);
        }
      }
      for (const t of this.z6t.keys()) {
        if (this.z6t.get(t).ApplyPlayerData.Debug && (this.z6t.delete(t), this.FreshFriendApplicationIds.has(t))) {
          this.FreshFriendApplicationIds.delete(t);
        }
      }
      for (const i of this.e8t.keys()) {
        if (this.e8t.get(i).GetBlockedPlayerData.Debug) {
          this.e8t.delete(i);
        }
      }
      for (const r of this.Z6t.keys()) {
        if (this.Z6t.get(r).Debug) {
          this.Z6t.delete(r);
        }
      }
      this.TestDataLoaded = false;
  ...
  async InitRecentlyTeamDataByResponse(e) {
    if (e) {
      ...
      await Promise.all(e);
      EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.UpdateRecentlyTeamDataEvent);
    }
  }
  ...
  GetApplyViewDataList(e) {
    ...
    if (e) {
      for (const r of e) {
        if (this.z6t.has(r)) {
          t.push(this.z6t.get(r));
        }
      }
    } else {
      for (var [, i] of this.z6t) {
        t.push(i);
      }
    }
    return t;

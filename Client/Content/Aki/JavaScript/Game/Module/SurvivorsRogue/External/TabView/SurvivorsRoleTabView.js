...
class SurvivorsRoleTabView extends SurvivorsTabViewBase_1.SurvivorsTabViewBase {
  ...
  async InitSubComponents() {
    var e = [];
    this.zqd = new SurvivorsRoleTabDetail_1.SurvivorsRoleTabDetail();
    e.push(this.zqd.CreateThenShowByActorAsync(this.GetItem(4).GetOwner()));
  ...
  OnTriggerSequenceStartEvent() {
    this.zqd?.Refresh(this.CurrentItemId, true, SHOW_ATTRIBUTE_NUM);
  }
  OnBeforeShow() {
  ...
  OnSelectItem(e, t = true) {
    ...
    this.zqd.Refresh(e.Id, t, SHOW_ATTRIBUTE_NUM);
    this.Jqd(r.TrialRoleId);
    if (t) {

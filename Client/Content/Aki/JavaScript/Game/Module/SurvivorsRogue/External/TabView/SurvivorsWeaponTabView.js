...
class SurvivorsWeaponTabView extends SurvivorsTabViewBase_1.SurvivorsTabViewBase {
  constructor() {
    super(...arguments);
    this.kkd = undefined;
    this.Okd = () => new SurvivorsWeaponDetailItem_1.SurvivorsWeaponDetailItem();
    this.CreateLoopItem = () => {
  ...
  async InitSubComponents() {
    this.kkd = new GenericScrollViewNew_1.GenericScrollViewNew(this.GetScrollViewWithScrollbar(4), this.Okd, this.GetItem(6).GetOwner());
  }
  OnTriggerSequenceStartEvent() {
    this.kkd?.GetGenericLayout().PlayGridAnim();
  }
  OnSelectItem(e, t = true) {
    ...
    this.kkd.RefreshByData(this.qkd(e), () => {
      for (const e of this.kkd.GetScrollItemList()) {

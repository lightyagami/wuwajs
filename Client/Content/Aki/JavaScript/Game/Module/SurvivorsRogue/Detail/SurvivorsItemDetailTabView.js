...
class SurvivorsItemDetailTabView extends UiPanelBase_1.UiPanelBase {
  ...
  async OnBeforeStartAsync() {
    ...
    this.Spt = new LoopScrollView_1.LoopScrollView(this.GetLoopScrollViewComponent(1), this.GetItem(2).GetOwner(), this.Qld);
  }
  OnStart() {
...
class SurvivorsItemGrid extends GridProxyAbstract_1.GridProxyAbstract {
  constructor() {
    ...
    this.Jgt = i => {
      if (this.eTt) {
        this.eTt(this.GridIndex, this.kFd);
      }
    };
  }
  OnRegisterComponent() {
  ...
  Refresh(i, t, e) {
    this.kFd = i.ConfigId;
    i = ConfigManager_1.ConfigManager.SurvivorsRogueConfig.GetSurvivorsItem(this.kFd);

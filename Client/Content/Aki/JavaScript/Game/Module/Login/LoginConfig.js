...
class LoginConfig extends ConfigBase_1.ConfigBase {
  ...
  GetLoginFailResetTime() {
    ...
    return 0 === e && Log_1.Log.CheckError() && Log_1.Log.Error("Login", 8, "登录失败次数重置参数错误", ["loginFailResetTime", e]), e
  }
  GetLoginFailParam(n) {
    var o = CommonParamById_1.configCommonParamById.GetStringConfig("login_fail_params"),
      r = o.split(/[,|]/g);
    if (0 === r.length || r.length % 2 != 0) return Log_1.Log.CheckError() && Log_1.Log.Error("Login", 8, "登录失败重试参数错误, 请检查个数", ["params", o]), 0;
    let i = 0;
    for (let e = 0; e < r.length; e += 2) {
      var t = Number(r[e]),
        a = Number(r[e + 1]);
      if (isNaN(t) || t <= 0 || isNaN(a) || a <= 0) return Log_1.Log.CheckError() && Log_1.Log.Error("Login", 8, "登录失败重试参数错误, ", ["params", o]), 0;
      t <= n && (i = a)
    }
    return i
  }

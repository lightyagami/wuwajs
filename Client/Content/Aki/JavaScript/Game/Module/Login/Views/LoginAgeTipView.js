"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.LoginAgeTipView = exports.ELoginShowType = undefined;
const UE = require("ue");
const ConfigManager_1 = require("../../../Manager/ConfigManager");
const ModelManager_1 = require("../../../Manager/ModelManager");
const UiViewBase_1 = require("../../../Ui/Base/UiViewBase");
const UiLayer_1 = require("../../../Ui/UiLayer");
const UiManager_1 = require("../../../Ui/UiManager");
var ELoginShowType;
(function (e) {
  e[e.AgeTip = 0] = "AgeTip";
  e[e.UserAgreement = 1] = "UserAgreement";
  e[e.PrivacyAgreement = 2] = "PrivacyAgreement";
  e[e.ChildPrivacyAgreement = 3] = "ChildPrivacyAgreement";
  e[e.LoginNotice = 4] = "LoginNotice";
})(ELoginShowType = exports.ELoginShowType ||= {});
class LoginAgeTipView extends UiViewBase_1.UiViewBase {
  constructor() {
    super(...arguments);
    this.BEi = () => {
      UiLayer_1.UiLayer.SetShowNormalMaskLayer(false);
      UiManager_1.UiManager.CloseView("LoginAgeTipView");
    };
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIText], [1, UE.UIText], [2, UE.UIText], [3, UE.UIButtonComponent], [4, UE.UIText]];
    this.BtnBindInfo = [[3, this.BEi]];
  }
  OnStart() {
    this.ChildPopView?.PopItem.OverrideBackBtnCallBack(() => {
      this.BEi();
    });
    var e = this.OpenParam;
    this.bEi(e);
    this.qEi(e);
  }
  bEi(e) {
    switch (e) {
      case ELoginShowType.AgeTip:
        this.GetText(0).SetUIActive(true);
        this.GetText(0).SetText(ConfigManager_1.ConfigManager.TextConfig.GetTextById("AgeTip"));
        break;
      case ELoginShowType.UserAgreement:
      case ELoginShowType.PrivacyAgreement:
      case ELoginShowType.ChildPrivacyAgreement:
        this.GEi(ConfigManager_1.ConfigManager.TextConfig.GetTextById(ELoginShowType[e]));
        break;
      case ELoginShowType.LoginNotice:
        if (ModelManager_1.ModelManager.LoginModel.LoginNotice) {
          this.GEi(ModelManager_1.ModelManager.LoginModel.LoginNotice.content);
        }
    }
  }
  GEi(e) {
    let i = e;
    for (let e = 0; e < 3; e++) {
      var a = i.indexOf("\n", 6000);
      if (a === -1) {
        if (i.length < 6000) {
          this.GetText(0 + e).SetUIActive(true);
          this.GetText(0 + e).SetText(i);
        }
        break;
      }
      var r = i.substr(0, a);
      this.GetText(0 + e).SetUIActive(true);
      this.GetText(0 + e).SetText(r);
      i = i.substr(a + 1);
    }
  }
  qEi(e) {
    let i = undefined;
    switch (e) {
      case ELoginShowType.AgeTip:
        i = "AgeTipTitle";
        break;
      case ELoginShowType.UserAgreement:
        i = "UserTitle";
        break;
      case ELoginShowType.PrivacyAgreement:
        i = "PrivacyTitle";
        break;
      case ELoginShowType.ChildPrivacyAgreement:
        i = "ChildPrivacyTitle";
        break;
      case ELoginShowType.LoginNotice:
        if (ModelManager_1.ModelManager.LoginModel.LoginNotice) {
          this.GetText(4).SetText(ModelManager_1.ModelManager.LoginModel.LoginNotice.Title);
        }
        return;
    }
    e = ConfigManager_1.ConfigManager.TextConfig.GetTextById(i);
    this.GetText(4).SetText(e);
  }
}
exports.LoginAgeTipView = LoginAgeTipView;
//# sourceMappingURL=LoginAgeTipView.js.map
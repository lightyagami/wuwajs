...
class RacingBetsConfirmBoxView extends UiViewBase_1.UiViewBase {
  constructor() {
    super(...arguments), this.ButtonList = [], this.Config = void 0, this.ConfirmBoxData = void 0, this.SelectedIndex = -1, this.ButtonComponentList = new Array, this.NXs = void 0, this.fea = void 0, this.OnClose = () => {
  ...
  async OnBeforeStartAsync() {
    this.GetText(4).SetText(""), this.ButtonComponentList.push(this.GetButton(1)), this.ButtonComponentList.push(this.GetButton(2));
  ...
  async i3e(t, i, s) {
    ...
    return await e.CreateByActorAsync(t.GetOwner()), this.Config.ButtonText.length > i && (e.SetClickFunction(s), i + 1 === this.Config.DelayButtonIndex && 0 < this.Config.DelayTime ? e.SetTimer(this.Config.ButtonText[i], this.Config.DelayTime, this.ConfirmBoxData.CanClickDuringTimer) : (t = this.ConfirmBoxData.GetBtnText(i), StringUtils_1.StringUtils.IsBlank(t) ? e.SetTextById(this.Config.ButtonText[i]) : e.SetText(t))), this.ConfirmBoxData.InteractionMap.has(i) && (s = this.ConfirmBoxData.InteractionMap.get(i), e.SetBtnCanClick(s)), this.Config.ButtonText.length >= i + 1 && await e.ShowAsync(), e
  }
  dbt() {

...
class BattlePassConfig extends ConfigBase_1.ConfigBase {
  ...
  GetBattlePassData(e) {
    ...
    if (a) return a;
    Log_1.Log.CheckError() && Log_1.Log.Error("Temp", 10, "获取战令配置错误，BattlePass表格里没有这个id", ["battlePassId", e])
  }
  GetBattlePassTask(e) {
    var a = BattlePassTaskByTaskId_1.configBattlePassTaskByTaskId.GetConfig(e);
    if (a) return a;
    Log_1.Log.CheckError() && Log_1.Log.Error("Temp", 10, "获取战令配置错误，BattlePassTask表格里没有这个id", ["taskId", e])
  }
  GetBattlePassUnlock(e) {
    ...
    if (a) return a;
    Log_1.Log.CheckError() && Log_1.Log.Error("Temp", 53, "获取战令配置错误，BattlePassTask表格里没有这个id", ["taskId", e])
  }
  GetBattlePassUnlockReward(e, a) {
    ...
    if (s)
      for (var [t, B] of s.UnlockReward) {
        ...
        a.push(t)
      } else Log_1.Log.CheckError() && Log_1.Log.Error("Temp", 53, "BattlePassUnlockPop里没有该type", ["type", e])
  }
}

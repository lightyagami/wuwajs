...
exports.ExploreProgressModel = undefined;
...
class ExploreProgressModel extends ModelBase_1.ModelBase {
  ...
  InitializeCurrentCountryIdAndAreaId() {
    ...
    if (this.SelectedCountryId <= 0 && (this.SelectedCountryId = ExploreProgressDefine_1.DEFAULT_COUNTRY_ID, Log_1.Log.CheckInfo())) {
      Log_1.Log.Info("ExploreProgress", 63, "初始化所有探索度区域数据时，找不到所在国家则选中皇龙", ["CountryId", this.SelectedCountryId]);
    }
    var e = MapUtil_1.MapUtil.GetWorldMapLevelOneAreaId();
    if (this.GetExploreAreaData(e)) {
      this.SelectedAreaId = e;
    } else {
      e = this.GetExploreCountryData(this.SelectedCountryId).GetExploreAreaDataList();
      this.SelectedAreaId = e[0].AreaId;
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("ExploreProgress", 63, "初始化所有探索度区域数据时，若玩家当前所在区域没有在对应区域探索数据，则选中当前国家的第一个区域", ["AreaId", this.SelectedAreaId]);
      }
    }
  }
  ...
  GetAllAreaDataListSortCountryState() {
    ...
    var t = [];
    for ([, e] of this.YVt) {
      for (const r of e.GetStateDataList()) {
        t.push(...r.ExploreAreaDataList);
      }
    }
    return t;
  }
  ...
  UpdateAreaStageRewardDataList(e) {
    ...
    for (const r of e) {
      ...
      if (t !== undefined) {
        this.GetExploreAreaData(t)?.UpdateAchievedStageReward(r);
      }
      this.DOl.add(r);
    }
    this.POl();
  ...
  GetOnlinePlayerIndexListByAreaId(r) {
    ...
    this.OnlinePlayersAreaMap.forEach((e, t) => {
      if (ConfigManager_1.ConfigManager.AreaConfig.GetLevelOneAreaId(e) === r && (e = ModelManager_1.ModelManager.OnlineModel?.GetCurrentTeamListById(t))?.PlayerNumber) {
  ...
  SetTrackTaskAreaId(e, t) {
    this.TrackTaskAreaId = e;
    this.TrackTaskIconPath = t;
  }
  ...
  SetTrackExploreAreaItemData(e, t) {
    var r;
    if (t) {
      if (r = this.GetExploreAreaData(e)) {
  ...
  D7l() {
    var e = MapUtil_1.MapUtil.GetWorldMapLevelOneAreaId();
    var e = this.GetExploreAreaData(e);

...
class MapPlayPointDetailView extends UiViewBase_1.UiViewBase {
  ...
  async OnBeforeStartAsync() {
    this.DNl = this.OpenParam, this.tNl = new MapExplorePlayProgressPanel_1.MapExplorePlayProgressPanel, await this.tNl.Init(this.GetItem(0))
  }
  OnBeforeShow() {
    var e = this.DNl.ExploreAreaItemData,
      ...
      s = (this.GetText(1).SetText(s.toString()), this.GetText(2).SetText(i.toString()), this.GetText(3).SetText(t.toString()), this.GetText(4).SetText(r.toString()), this.GetText(5).SetText(e.GetPlayDetailTitle()), this.GetText(6).ShowTextNew(e.GetNameId()), e.HasSpecialPlayPoint());
    this.GetItem(8).SetUIActive(s), s && this.GetText(7).ShowTextNew(e.SpecialPlayerDesc)
  }
}

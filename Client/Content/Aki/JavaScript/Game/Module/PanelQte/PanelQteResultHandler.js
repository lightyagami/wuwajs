...
class PanelQteResultHandler {
  Handle(r) {
    Log_1.Log.CheckDebug() && Log_1.Log.Debug("PanelQte", 17, "通用界面QTE结算", ["qteId", r.QteId], ["success", r.Success]);
  ...
  LOi(e, r, a) {
    switch (e) {
      ...
      case 2: {
        const a = this.TOi();
        (a?.GetComponent(175))?.PositionState === CharacterUnifiedStateTypes_1.ECharPositionState.Climb ? (t = a?.GetComponent(177))?.Valid && t.ClimbDash() : a?.GetComponent(40)?.BeginSkill(RUSH_SKILL_ID, {
  ...
  DOi() {
    ...
    if (!e?.EntityHandle?.Entity?.GetComponent(205)?.HasTag(-1697149502)) {
      ...
      for (let r = 1; r < t; r++) {
        ...
        e >= t && (e -= t);
        var l = a[e];
        if (0 === l?.CanGoBattle()) return void CooperationController_1.CooperationController.TryCooperate(l.GetCreatureDataId())
      }
    }
  }
  TOi() {

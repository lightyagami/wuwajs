"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.PanelQteController = undefined;
const Log_1 = require("../../../Core/Common/Log");
const EventDefine_1 = require("../../Common/Event/EventDefine");
const EventSystem_1 = require("../../Common/Event/EventSystem");
const Global_1 = require("../../Global");
const ModelManager_1 = require("../../Manager/ModelManager");
const UiControllerBase_1 = require("../../Ui/Base/UiControllerBase");
const UiManager_1 = require("../../Ui/UiManager");
const PanelQteContext_1 = require("./PanelQteContext");
const disableInputTagIds = [-542518289, -541178966, -732810197, 581080458, -1802431900, -469423249, 766688429, -1752099043, -1697149502];
class PanelQteController extends UiControllerBase_1.UiControllerBase {
  static StartAnimNotifyQte(e, t, a) {
    var n;
    if (this.vOi()) {
      (n = new PanelQteContext_1.PanelQteContext()).QteId = e;
      n.PreMessageId = a;
      n.Source = 0;
      n.SourceMeshComp = t;
      n.SourceActor = t.GetOwner();
      return this.MOi(n);
    } else {
      return -1;
    }
  }
  static StartBuffQte(e, t, a, n, r) {
    var i;
    if (this.vOi() && n && n.Id === Global_1.Global.BaseCharacter?.EntityId) {
      (i = new PanelQteContext_1.PanelQteContext()).QteId = e;
      i.Source = 1;
      i.PreMessageId = r;
      i.SourceBuffId = t;
      i.SourceBuffHandleId = a;
      i.SourceActor = n.GetComponent(1)?.Owner;
      i.SourceEntityHandle = ModelManager_1.ModelManager.CharacterModel?.GetHandleByEntity(n);
      i.IsInitSourceEntity = true;
      return this.MOi(i);
    } else {
      return -1;
    }
  }
  static StartLevelSequenceQte(e, t) {
    var a;
    if (this.vOi()) {
      (a = new PanelQteContext_1.PanelQteContext()).QteId = e;
      a.Source = 2;
      a.SourceActor = t;
      return this.MOi(a);
    } else {
      return -1;
    }
  }
  static StartLevelEventQte(e) {
    var t;
    if (this.vOi()) {
      (t = new PanelQteContext_1.PanelQteContext()).QteId = e;
      t.Source = 3;
      return this.MOi(t);
    } else {
      return -1;
    }
  }
  static StopQte(e, t = false) {
    if (!ModelManager_1.ModelManager.PanelQteModel.StopQte(e)) {
      return false;
    }
    var a = ModelManager_1.ModelManager.PanelQteModel;
    if (a.IsHideAllBattleUi) {
      ModelManager_1.ModelManager.BattleUiModel.ChildViewData.ShowBattleView(6);
      a.IsHideAllBattleUi = false;
    } else if (n = a.HideBattleUiChildren) {
      ModelManager_1.ModelManager.BattleUiModel.ChildViewData.SetChildrenVisible(6, n, true);
      a.HideBattleUiChildren = undefined;
    }
    if (a.DisableFightInput) {
      let e = undefined;
      var n = ModelManager_1.ModelManager.BattleUiModel.GetCurRoleData();
      if (e = n?.EntityHandle === a.CurRoleEntity ? n.GameplayTagComponent : a.CurRoleEntity.Entity.GetComponent(205)) {
        for (const r of disableInputTagIds) {
          e.RemoveTag(r);
        }
      } else if (Log_1.Log.CheckError()) {
        Log_1.Log.Error("PanelQte", 17, "界面qte结束时，场上角色已经被销毁了");
      }
      a.CurRoleEntity = undefined;
      a.DisableFightInput = false;
    }
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.PanelQteEnd, e, ModelManager_1.ModelManager.PanelQteModel.IsQteSuccess());
    if (!t) {
      ModelManager_1.ModelManager.PanelQteModel.HandleResult();
    }
    return true;
  }
  static vOi() {
    return !ModelManager_1.ModelManager.PanelQteModel.IsInQte && !!ModelManager_1.ModelManager.GameModeModel.WorldDone && !!ModelManager_1.ModelManager.BattleUiModel.GetCurRoleData();
  }
  static MOi(e) {
    var t = ModelManager_1.ModelManager.PanelQteModel.GetPanelQteConfig(e.QteId);
    if (!t) {
      return -1;
    }
    e.Config = t;
    var a = ModelManager_1.ModelManager.PanelQteModel;
    var n = a.StartQte(e);
    var r = e.Config.ViewType;
    switch (r) {
      case 0:
        if (UiManager_1.UiManager.IsViewOpen("FrozenQteView")) {
          UiManager_1.UiManager.CloseView("FrozenQteView");
        }
        UiManager_1.UiManager.OpenView("FrozenQteView", n);
        break;
      case 1:
        if (UiManager_1.UiManager.IsViewOpen("InteractQteView")) {
          UiManager_1.UiManager.CloseView("InteractQteView");
        }
        UiManager_1.UiManager.OpenView("InteractQteView", n);
        a.DisableFightInput = true;
        break;
      case 2:
        e.BuffIndex = Math.floor(Math.random() * t.MaxSuccessCount);
        if (UiManager_1.UiManager.IsViewOpen("YouHuQteView")) {
          UiManager_1.UiManager.CloseView("YouHuQteView");
        }
        UiManager_1.UiManager.OpenView("YouHuQteView", n);
        break;
      case 3:
        if (UiManager_1.UiManager.IsViewOpen("FreeRunningQteView")) {
          UiManager_1.UiManager.CloseView("FreeRunningQteView");
        }
        UiManager_1.UiManager.OpenView("FreeRunningQteView", n);
        a.DisableFightInput = true;
        break;
      default:
        if (Log_1.Log.CheckError()) {
          Log_1.Log.Error("PanelQte", 17, "QTE界面类型没有实现", ["", r]);
        }
    }
    if (t.HideAllBattleUi) {
      a.IsHideAllBattleUi = true;
      ModelManager_1.ModelManager.BattleUiModel.ChildViewData.HideBattleView(6, [20]);
    } else {
      a.IsHideAllBattleUi = false;
      var i = t.HideUIElement.Num();
      if (i > 0) {
        var o = [];
        for (let e = 0; e < i; e++) {
          o.push(t.HideUIElement.Get(e));
        }
        ModelManager_1.ModelManager.BattleUiModel.ChildViewData.SetChildrenVisible(6, o, false);
        a.HideBattleUiChildren = o;
      } else {
        a.HideBattleUiChildren = undefined;
      }
    }
    if (a.DisableFightInput) {
      var l = ModelManager_1.ModelManager.BattleUiModel.GetCurRoleData();
      if (l) {
        var _ = l.EntityHandle;
        var M = l.GameplayTagComponent;
        if (M) {
          for (const s of disableInputTagIds) {
            M.AddTag(s);
          }
          a.CurRoleEntity = _;
        }
      }
    }
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.PanelQteStart, n);
    return n;
  }
  static OnAddEvents() {
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.OnTeamLivingStateChange, PanelQteController.t$s);
  }
  static OnRemoveEvents() {
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.OnTeamLivingStateChange, PanelQteController.t$s);
  }
}
(exports.PanelQteController = PanelQteController).t$s = (e, t, a) => {
  if (e && t === 1 && a === 2 && (e = ModelManager_1.ModelManager.PanelQteModel.GetContext())?.QteHandleId) {
    PanelQteController.StopQte(e.QteHandleId, true);
  }
};
//# sourceMappingURL=PanelQteController.js.map
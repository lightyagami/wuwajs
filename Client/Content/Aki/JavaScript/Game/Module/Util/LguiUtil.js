...
const puerts_1 = require("puerts"),
  ...
  skillFormatRegex = new RegExp(`{<${ACTION_ID_MATCH}><${SKILL_ID_MATCH}>}`, "g"),
  dtSkillFormatRegex = new RegExp(`{<${ACTION_ID_MATCH}><${ROLE_ID_MATCH}><${SKILL_ID_MATCH}>}`, "g"),
  exploreFormatRegex = new RegExp(`{<${ACTION_ID_MATCH}><${EXPLORE_ID_MATCH}>}`, "g"),
  phantomFormatRegex = new RegExp(`{<${ACTION_ID_MATCH}><${PHANTOM_ID_MATCH}>}`, "g"),
...
class LguiUtil {
  ...
  static async LoadPrefabByAsync(t, r, i = GlobalData_1.GlobalData.World) {
    ...
    return ResourceSystem_1.ResourceSystem.LoadAsync(t, UE.PrefabAsset, (t, e) => {
  ...
  static DuplicateActor(t, e) {
    var r, i;
    return Stats_1.Stat.Enable ? ((r = Stats_1.Stat.CreateNoFlameGraph("LGUI DuplicateActor " + LguiUtil.GetActorFullPath(t))).Start(), i = UE.LGUIBPLibrary.DuplicateActor(t, e), r.Stop(), i) : UE.LGUIBPLibrary.DuplicateActor(t, e)
  }
  ...
  static SetLocalTextNew(e, t, ...r) {
    e && (e.Clear(), r.forEach(t => {
      "number" == typeof t ? Number.isInteger(t) ? e.AddIntArgs(t) : e.AddFloatArgs(t) : t instanceof TableTextArgNew ? e.AddFormatTableInfoNew(t.TextKey) : e.AddStringArgs(t)
    }), e.ShowTextNew(t))
  }
  static TrySetLocalTextNew(t, e, ...r) {
    StringUtils_1.StringUtils.IsEmpty(e) ? t?.SetUIActive(!1) : (t?.SetUIActive(!0), this.SetLocalTextNew(t, e, ...r))
  }
  static ReplaceWildCard(t) {
    var e;
    t?.IsValid() ? t.GetRichText() ? (e = t.GetText(), e = LguiUtil.ConvertToPcKeyIconRichText(e), e = LguiUtil.ConvertToGamepadKeyIconRichText(e), e = LguiUtil.ConvertToActionIconRichText(e), e = LguiUtil.ConvertToDataTableSkillIconRichText(e), e = LguiUtil.ConvertToSkillIconRichText(e), e = LguiUtil.ConvertToToExploreIconRichText(e), e = LguiUtil.ConvertToToPhantomIconRichText(e), e = LguiUtil.ConvertToToPlatformIconRichText(e), t.SetText(e)) : Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "替换富文本图标失败，因为此文本不是富文本", ["uiText", t.GetText()]) : Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "替换富文本时，UiText已经失效")
  }
  ...
  static GGo(t, e, r) {
    ...
    if (i) {
      ...
      if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, `<texture=${i}>`);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "按键配置了空的图标路径", ["pcKeyId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应Pc按键配置", ["pcKeyId", r])
  }
  static NGo(t, e, r) {
    ...
    if (i) {
      i = i.KeyIconPath;
      if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, `<texture=${i}>`);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "按键配置了空的图标路径", ["gamepadKeyId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应Gamepad按键配置", ["pcKeyId", r])
  }
  static kGo(t, e, r) {
    ...
    if (i) {
      var i = i.Icon,
        o = `<texture=${i}>`;
      if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, o);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "技能配置了空的图标路径", ["skillId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应技能", ["skillId", r])
  }
  static OGo(t, e, r) {
    ...
    if (i) {
      ...
      if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, `<texture=${i}>`);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "Action配置了空的图标路径", ["actionId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应ActionBinding", ["actionId", r])
  }
  static FGo(t, e, r) {
    ...
    if (i) {
      i = i.BattleViewIcon;
      if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, `<texture=${i}>`);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "探索幻象图标路径为空", ["phantomId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应探索幻象", ["phantomId", r])
  }
  static VGo(t, e, r) {
    ...
    if (i) {
      ...
      if (i) {
        i = i.BattleViewIcon;
        if (!StringUtils_1.StringUtils.IsEmpty(i)) return t.replace(e, `<texture=${i}>`);
        Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "战斗幻象图标路径为空", ["phantomId", r])
      } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应战斗幻象技能", ["phantomId", r])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应战斗幻象", ["phantomId", r])
  }
  static HGo(e, r, i) {
    var o = ConfigManager_1.ConfigManager.InputSettingsConfig.GetPlatformIconConfig(i);
    if (o) {
      let t = o.IconPath;
      if (Info_1.Info.IsInTouch() && (t = o.MobileIconPath), !StringUtils_1.StringUtils.IsEmpty(t)) return o = `<texture=${t}>`, e.replace(r, o);
      Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "多端图标路径为空", ["iconId", i])
    } else Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "找不到对应多端平台图标配置", ["iconId", i])
  }
  static qGo(t, e) {
    t = t.match(e)[0];
  ...
  static GetActorFullPath(t) {
    ...
    return UE.LGUIBPLibrary.GetFullPathOfActor(GlobalData_1.GlobalData.World, t, e), (0, puerts_1.$unref)(e)
  }
  ...
  static ClearAttachChildren(e) {
    for (let t = e.AttachChildren.Num() - 1; 0 <= t; t--) UE.LGUIBPLibrary.DeleteActor(e.AttachChildren.Get(t).GetOwner())
  }
  static LoadAndSetText(i, o, a, _) {
    ...
    a.forEach((t, r) => {
      ResourceSystem_1.ResourceSystem.LoadAsync(t, UE.PrefabAsset, (t, e) => {
        g[r] = t, (++s >= a.length || s >= a.length) && (g.forEach((t, e) => {
  ...
  static SetActorIsPermanent(t, e, r) {
    t ? t.IsValid() ? UE.KuroStaticLibrary.SetActorPermanent(t, e, r) : Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "无缝切换传入Actor异常,Actor IsValid") : Log_1.Log.CheckWarn() && Log_1.Log.Warn("LguiUtil", 10, "无缝切换传入Actor异常,Actor为空")
  }
  ...
  static ConvertPointerPositionToLguiCenterPosition(t, e) {
    var r, i, o = UiLayer_1.UiLayer.UiRootItem.GetCanvasScaler();
    o && (r = UiLayer_1.UiLayer.UiRootItem.GetWidth(), i = UiLayer_1.UiLayer.UiRootItem.GetHeight(), e.Set(t.X, t.Y), t = o.ConvertPositionFromViewportToLGUICanvas(e.ToUeVector2D()), e.FromUeVector2D(t), e.X = MathCommon_1.MathCommon.Clamp(e.X, 0, r), e.Y = MathCommon_1.MathCommon.Clamp(e.Y, 0, i), e.X -= r / 2, e.Y -= i / 2)
  }
  static ConvertPointerPositionToLguiPosition(t, e) {
    var r, i, o = UiLayer_1.UiLayer.UiRootItem.GetCanvasScaler();
    o && (r = UiLayer_1.UiLayer.UiRootItem.GetWidth(), i = UiLayer_1.UiLayer.UiRootItem.GetHeight(), e.Set(t.X, t.Y), t = o.ConvertPositionFromViewportToLGUICanvas(e.ToUeVector2D()), e.FromUeVector2D(t), e.X = MathCommon_1.MathCommon.Clamp(e.X, 0, r), e.Y = MathCommon_1.MathCommon.Clamp(e.Y, 0, i))
  }
}

...
exports.PhantomArenaCardOutlookRewardView = undefined;
const UE = require("ue");
...
class PhantomArenaCardOutlookRewardView extends UiViewBase_1.UiViewBase {
  constructor() {
    ...
    this.dV1 = () => {
      if (ModelManager_1.ModelManager.PhantomArenaModel.CardOutlookUnlockQueue.length > 0) {
        this.ShowNext();
      } else {
        this.CloseMe();
      }
    ...
    this.vCu = e => {
      if (e === "Change" && this.IsWaitingChange) {
  ...
  async OnBeforeStartAsync() {
    LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(0), "PhantomBattle_1090");
  ...
  OnStart() {
    ...
    this.CardId = ModelManager_1.ModelManager.PhantomArenaModel.CardOutlookUnlockQueue.shift();
    this.RefreshByCardId(this.CardId);
  }
  OnAddEventListener() {

"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.SmallItemGridCoolDownComponent = undefined;
const UE = require("ue");
const TimeUtil_1 = require("../../../../Common/TimeUtil");
const ModelManager_1 = require("../../../../Manager/ModelManager");
const LguiUtil_1 = require("../../../Util/LguiUtil");
const SmallItemGridComponent_1 = require("./SmallItemGridComponent");
class SmallItemGridCoolDownComponent extends SmallItemGridComponent_1.SmallItemGridComponent {
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UISprite], [1, UE.UIText]];
  }
  OnRefresh(e) {
    var t;
    var i = e.CoolDown;
    var e = e.TotalCdTime;
    if (i && e) {
      this.GetSprite(0).SetFillAmount(i / e);
      this.SetActive(true);
      e = this.GetText(1);
      if (i < ModelManager_1.ModelManager.SmallItemGridModel.ItemGridCoolDownSecond) {
        LguiUtil_1.LguiUtil.SetLocalTextNew(e, "ItemCdTime_Second", i.toFixed(1));
      } else {
        t = TimeUtil_1.TimeUtil.CalculateRemainingTime(i).TimeValue;
        if (i < TimeUtil_1.TimeUtil.Hour) {
          LguiUtil_1.LguiUtil.SetLocalTextNew(e, "ItemCdTime_Minute", t);
        } else if (i < TimeUtil_1.TimeUtil.OneDaySeconds) {
          LguiUtil_1.LguiUtil.SetLocalTextNew(e, "ItemCdTime_Hour", t);
        } else {
          LguiUtil_1.LguiUtil.SetLocalTextNew(e, "ItemCdTime_Day", t);
        }
      }
    } else {
      this.SetActive(false);
    }
  }
  GetResourceId() {
    return "UiItem_ItemBCD";
  }
  GetLayoutLevel() {
    return 1;
  }
}
exports.SmallItemGridCoolDownComponent = SmallItemGridCoolDownComponent;
//# sourceMappingURL=SmallItemGridCoolDownComponent.js.map
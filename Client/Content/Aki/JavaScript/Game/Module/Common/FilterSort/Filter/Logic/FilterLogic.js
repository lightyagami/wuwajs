...
class FilterLogic {
  ...
  WLt(e, r) {
    ...
    if (i) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("Filter", 10, "传入的筛选项id查找不到对应方法", ["数据类型", e], ["筛选表格类型", r])
  }
  KLt(e, r, i, t) {
    var n = this.WLt(r, i);
    if (!n) return e;
  ...
  GetFilterList(e, i, r, t) {
    var n = [];
    let F = [];
    var l, o, a = this.jLt(i);
    0 === a.length && (F = e);
    for (const L of a)
      for (const b of e)(L(b) ? n : F).push(b);
    if (r) {
      let e = F;
      var u, y, _, s = [];
      let r = !1;
      for ([u, y] of t) y.size <= 0 || (r = !0, _ = this.QLt(e, i, u, y), e = _.UnFindList, s.push(..._.FindList));
  ...
  GetFilterItemDataList(e, r) {
    ...
    if (t) {
      var n = ConfigManager_1.ConfigManager.FilterConfig.GetFilterConfig(r),
        r = t(i.IdList);
      for (const F of r) F.SetIsShowIcon(n.IsShowIcon), F.NeedChangeColor = i.NeedChangeColor;
      return r
    }
    Log_1.Log.CheckError() && Log_1.Log.Error("Filter", 10, "传入的筛选表格类型未进行枚举定义以及方法实现", ["EFilterType", e])
  }
}

...
class SpecialEnergyBarSlot extends SpecialEnergyBarBase_1.SpecialEnergyBarBase {
  constructor() {
    super(...arguments), this.SlotNum = 0, this.SlotItemList = [], this.IsMorph = !1, this.ForceHideBottomLine = !1, this.ForceEffectBasePercent = -1
  }
  OnRegisterComponent() {
    this.SlotNum = this.Config.SlotNum, 0 === this.SlotNum && Log_1.Log.CheckError() && Log_1.Log.Error("Battle", 17, "槽型的能量条，分段数量不能为0", ["id", this.Config.Id]);
    for (let t = 0; t <= this.SlotNum + 1; t++) this.ComponentRegisterInfos.push([t, UE.UIItem])
  }
  async OnBeforeStartAsync() {
    var e = [];
    for (let t = 0; t < this.SlotNum; t++) e.push(this.InitSlotItem(this.GetItem(t)));
  ...
  OnStart() {
    if (this.Config) {
      var e = 0 < this.ForceEffectBasePercent ? this.ForceEffectBasePercent : effectBasePercents[this.SlotNum - 1];
      ...
      if (this.Config.EffectColor) {
        ...
        let t = s;
        this.Config.PointColor && (t = UE.Color.FromHex(this.Config.PointColor));
        for (const h of this.SlotItemList) h.SetBarColor(s), h.SetPointColor(t), h.SetFullEffectColor(i, this.IsMorph)
      }
      let t = !0;
      (this.ForceHideBottomLine || this.IsMorph && this.Config?.IconPath) && (t = !1), this.GetItem(this.SlotNum + 1)?.SetUIActive(t), this.RefreshBarPercent(!0)
    }
  }
  RefreshBarPercent(t = !1) {
    var e = this.PercentMachine.GetCurPercent(),
      s = this.GetKeyEnable();
    for (let t = 0; t < this.SlotItemList.length; t++) this.SlotItemList[t].UpdatePercent(e * this.SlotNum - t, s);
  ...
  UpdateFullEffectOffsetBySlotWidth() {
    for (const t of this.SlotItemList) t.SetFullEffectOffsetX(t.GetRootItem().Width / 2)
  }

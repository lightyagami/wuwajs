...
class SpecialEnergyBarFuLuoLuo extends SpecialEnergyBarBase_1.SpecialEnergyBarBase {
    ...
    this.l1d = () => {
      if (!(this.ewu.length < SPECIAL_ENERGY_COUNT)) {
        ...
        if (e !== 0) {
          ...
          this._1d(i, SPECIAL_ENERGY_COUNT);
        }
      }
    };
    this.hOd = () => {
  ...
  AddEvents() {
    ...
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.FuLuoLuoAddDuplicatedEnergy, this.l1d);
  }
  RemoveEvents() {
    super.RemoveEvents();
    if (this.Jh) {
    ...
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.FuLuoLuoAddDuplicatedEnergy, this.l1d);
  }
  OnBeforeDestroy() {
    if (this.wju) {
      this.wju?.Destroy();
  ...
  n4l(t = false) {
    ...
    if (t) {
      ...
      this.Dpd(this.TagComponent?.HasTag(disableTag) ?? false);
    } else {
  ...
  GetKeyEnable() {
    return !(this.ewu.length < SPECIAL_ENERGY_COUNT) && !this.Apd;
  }
  Dpd(t) {
    if (this.Apd !== t) {
      this.Apd = t;
  ...
  jju() {
    if (!this.Dju) {
      ...
      this.AYc();
      this.Uju = TimerSystem_1.TimerSystem.Delay(this.hOd, FIRST_NOTE_TIME, undefined, undefined, true);

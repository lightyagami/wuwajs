...
class SkillButtonPanel extends BattleChildViewPanel_1.BattleChildViewPanel {
  constructor() {
    ...
    this.GHd = Stats_1.Stat.Create("[SkillButton]RefreshSkillItemLayoutStat");
    this.Det = undefined;
    this.$Qe = false;
    this.FHd = [];
  ...
  async FZe(t, e, i) {
    ...
    if (!i) {
      s?.SetOnVisibleChangedCallback(this.$Hd);
    }
    this.lZe.push(s);
    return s;
  }
  ...
  AddEvents() {
    ...
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.OnSkillButtonExtraEffectRefresh, this.Z6d);
  ...
  RemoveEvents() {
    ...
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.OnSkillButtonExtraEffectRefresh, this.Z6d);
  ...
  HHd() {
    if (Info_1.Info.IsInKeyBoard() && this.FHd.length !== 0) {
      this.GHd.Start();
      ...
      for (let t = this.lZe.length - 1; t >= 0; t--) {
        ...
        if (e < SECOND_LAYOUT_START_COUNT) {
          n.SetSkillItemLayout(this.FHd[0]);
        } else if (e === SECOND_LAYOUT_START_COUNT) {
          if (this.NHd <= this.w$d || s) {
            n.SetSkillItemLayout(this.FHd[1]);
          } else {
            n.SetSkillItemLayout(this.FHd[0]);
          }
        } else if (e > SECOND_LAYOUT_START_COUNT) {
  ...
  WHd() {
    this.VHd ||= TimerSystem_1.TimerSystem.Next(this.QHd);
  }
  KHd() {
    if (this.VHd) {
      if (TimerSystem_1.TimerSystem.Has(this.VHd)) {
        TimerSystem_1.TimerSystem.Remove(this.VHd);
      }
      this.VHd = undefined;
    }
  }

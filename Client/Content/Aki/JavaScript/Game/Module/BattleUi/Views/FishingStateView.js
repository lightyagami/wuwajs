...
class FishingStateView extends BattleVisibleChildView_1.BattleVisibleChildView {
  constructor() {
    super(...arguments), this.osn = void 0, this.x5e = [], this.hXe = (t, i, e) => {
      this.bNe()
    }
  }
  OnRegisterComponent() {
  ...
  OnStart() {
    ...
    this.osn = t.GetEntityHandle()?.Entity?.GetComponent(172), this.Ore(), this.bNe()
  }
  Reset() {
    this.kre(), this.osn = void 0, super.Reset()
  }
  Ore() {
    this.osn && (this.osn.AddListener(EAttributeId.Proto_Life, this.hXe), this.osn.AddListener(EAttributeId.l5n, this.hXe))
  }
  kre() {
    this.osn && (this.osn.RemoveListener(EAttributeId.Proto_Life, this.hXe), this.osn.RemoveListener(EAttributeId.l5n, this.hXe))
  }
  bNe() {
    if (this.osn) {
      var i = this.osn?.GetCurrentValue(EAttributeId.l5n) ?? 0,
        e = this.osn?.GetCurrentValue(EAttributeId.Proto_Life) ?? 0;
      Log_1.Log.CheckDebug() && Log_1.Log.Debug("Battle", 17, "捕鱼船血量变化", ["curHp", e], ["maxHp", i]);
      for (let t = this.x5e.length; t < i; t++) this.qF_();
      for (let t = 0; t < this.x5e.length; t++) t < i ? (this.x5e[t].SetUiActive(!0), this.x5e[t].SetHpVisible(t < e)) : this.x5e[t].SetUiActive(!1)
    }
  }
  qF_() {
    var t = this.GetItem(0),
      i = this.GetItem(1);
    let e = void 0;
    e = 0 === this.x5e.length ? i.GetOwner() : LguiUtil_1.LguiUtil.DuplicateActor(i.GetOwner(), t);

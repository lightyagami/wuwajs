...
exports.SpecialEnergyBarLuoKeKe = undefined;
const UE = require("ue");
...
const NUM = 3;
const EFFECT_TIME = 1000;
const ghostTags = [1459704896, -1325526953, -1895287750];
class SpecialEnergyBarLuoKeKe extends SpecialEnergyBarBase_1.SpecialEnergyBarBase {
    ...
    this.L4l = (t, s) => {
      if (s) {
        ...
        if (Log_1.Log.CheckDebug()) {
          Log_1.Log.Debug("Battle", 17, "【能量条】洛可可鬼头状态更新", ["", this.I4l]);
        }
      } else {
        this.A4l();
      }
  ...
  AddEvents() {
    super.AddEvents();
    for (const t of ghostTags) {
      this.ListenForTagAddOrRemoveChanged(t, this.L4l);
    }
  }
  OnStart() {
  ...
  x4l(s = false) {
    ...
    if (this.b4l !== e) {
      if (Log_1.Log.CheckDebug()) {
        Log_1.Log.Debug("Battle", 17, "【能量条】洛可可鬼头数量更新", ["new", e], ["old", this.b4l], ["inGhost", this.I4l]);
      }
      if (e > this.b4l) {
        for (let t = 0; t < NUM; t++) {
          var h = t < e;
          this.T5l[t].SetUIActive(h);
          if (s && h) {
            this.T5l[t].SetAlpha(1);
          }
        }
        if (!s) {
      ...
      if (this.I4l) {
        for (let t = 0; t < NUM; t++) {
          this.Rdt.SetFullEffectVisible(t, false);
        }
      } else {
        for (let t = 0; t < NUM; t++) {
          var r = t < i;
    ...
    if (this.T4l !== i) {
      if (i < this.T4l) {
        if (Log_1.Log.CheckDebug()) {
          Log_1.Log.Debug("Battle", 17, "【能量条】洛可可鬼头使用", ["new", i], ["old", this.T4l], ["inGhost", this.I4l]);
        }
        for (let t = i; t < this.T4l; t++) {
          this.b5l[t].SetUIActive(true);
  ...
  Tick(t) {
    super.Tick(t);
    this.Rdt?.Tick(t);
    if (this.Fdt && this.L5l <= Time_1.Time.Now) {
      for (const s of this.b5l) {
        s.SetUIActive(false);
      }
      this.Fdt = false;
    }

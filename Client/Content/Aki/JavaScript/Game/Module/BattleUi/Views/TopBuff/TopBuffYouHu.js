...
class TopBuffYouHu extends TopBuffContainer_1.TopBuffContainer {
  constructor() {
    super(...arguments), this.x5e = [], this.Zmt = t => {
      this.qYt()
    }
  }
  async OnInitAsync() {
    ...
    for (let t = 0; t < 4; t++) {
      ...
      s.push(o.CreateByResourceIdAsync("UiItem_BuffItemYouhu", this.ParentItem)), this.x5e.push(o)
    }
  ...
  qYt() {
    ...
    if (t) {
      ...
      for (const i of tagIds) {
        ...
        for (let t = 0; t < e; t++)
          if (this.x5e[s].RefreshByTag(i), this.x5e[s].SetVisible(1, !0), ++s >= o) return
      }
      for (let t = s; t < o; t++) this.x5e[t].SetVisible(1, !1)
    } else
      for (const s of this.x5e) s.SetVisible(1, !1)
  }
  OnDestroy() {
    this.kre();

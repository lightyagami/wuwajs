"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.TopBuffYouHu = undefined;
const TopBuffContainer_1 = require("./TopBuffContainer");
const TopBuffItemYouHu_1 = require("./TopBuffItemYouHu");
const tagIds = [-1776747611, -2102795492, 166342785, 789955629];
class TopBuffYouHu extends TopBuffContainer_1.TopBuffContainer {
  constructor() {
    super(...arguments);
    this.x5e = [];
    this.Zmt = t => {
      this.qYt();
    };
  }
  async OnInitAsync() {
    var s = [];
    for (let t = 0; t < 4; t++) {
      var o = new TopBuffItemYouHu_1.TopBuffItemYouHu();
      s.push(o.CreateByResourceIdAsync("UiItem_BuffItemYouhu", this.ParentItem));
      this.x5e.push(o);
    }
    await Promise.all(s);
    this.Ore();
    this.qYt();
  }
  SetVisible(t) {
    for (const s of this.x5e) {
      s.SetVisible(0, t);
    }
  }
  Ore() {
    for (const t of tagIds) {
      this.ListenForTagCountChanged(t, this.Zmt);
    }
  }
  kre() {}
  qYt() {
    var t = this.RoleData?.GameplayTagComponent;
    if (t) {
      let s = 0;
      var o = this.x5e.length;
      for (const i of tagIds) {
        var e = t.GetTagCount(i);
        for (let t = 0; t < e; t++) {
          this.x5e[s].RefreshByTag(i);
          this.x5e[s].SetVisible(1, true);
          if (++s >= o) {
            return;
          }
        }
      }
      for (let t = s; t < o; t++) {
        this.x5e[t].SetVisible(1, false);
      }
    } else {
      for (const s of this.x5e) {
        s.SetVisible(1, false);
      }
    }
  }
  OnDestroy() {
    this.kre();
    for (const t of this.x5e) {
      t.Destroy();
    }
  }
}
exports.TopBuffYouHu = TopBuffYouHu;
//# sourceMappingURL=TopBuffYouHu.js.map
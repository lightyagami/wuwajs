...
class PartStatePanel {
  constructor() {
    this.smt = new Map, this.amt = (t, e, a) => {
      var r = EntitySystem_1.EntitySystem.Get(t);
      if (FNameUtil_1.FNameUtil.IsNothing(e)) r && (a ? this.ActivatePartStateByRole(r) : this.DestroyPartStateFromRole(r));
      else if (r) {
        var i, s = r.GetComponent(69).Parts;
        if (!(s.length <= 0))
          for (const o of s) o.BoneName.op_Equality(e) && (a ? this.ActivatePartState(r, o) : (i = o.Index, this.DestroyPartState(t, i)))
      }
    }
  }
  ...
  OnCreateEntity(t) {
    if (t) {
      ...
      if (e) {
        e = e.Parts;
        if (e && 0 !== e.length) {
          var a, r = t.GetComponent(1).Owner.Mesh;
          for (const i of e) i.IsPartStateVisible && (a = i.PartSocketName, r.DoesSocketExist(a) ? this.ActivatePartState(t, i) : Log_1.Log.CheckInfo() && Log_1.Log.Info("Battle", 17, "[BattleView]激活部位血条时找不到部位插槽:", ["SocketName", a]))
        }
      }
    }
  }
  ActivatePartStateByRole(t) {
    if (t) {
      ...
      if (0 !== e.length)
        for (const a of e) this.ActivatePartState(t, a)
    }
  }
  ...
  Tick(t) {
    ...
    for (const e of this.smt.values())
      for (const a of e.values()) a.Tick(t);
  ...
  hmt(t, e) {
    ...
    return r ? r.set(e.Index, t) : ((r = new Map).set(e.Index, t), this.smt.set(a, r)), t
  }
  GetPartState(t, e) {
    t = this.smt.get(t);
    if (t) return t.get(e)

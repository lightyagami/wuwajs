"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.LevelPlayPrepareTimer = undefined;
const EventDefine_1 = require("../../../Common/Event/EventDefine");
const EventSystem_1 = require("../../../Common/Event/EventSystem");
const TimeUtil_1 = require("../../../Common/TimeUtil");
const ControllerHolder_1 = require("../../../Manager/ControllerHolder");
const UiManager_1 = require("../../../Ui/UiManager");
const GeneralLogicTreeController_1 = require("../GeneralLogicTreeController");
const LogicTreeTimerBase_1 = require("./LogicTreeTimerBase");
class LevelPlayPrepareTimer extends LogicTreeTimerBase_1.LogicTreeTimerBase {
  constructor() {
    super(...arguments);
    this.MYt = -0;
    this.GP_ = -0;
    this.yYt = false;
    this.$Ge = e => {
      if (e === "PrepareCountdownFloatTips") {
        this.TYt();
      }
    };
  }
  StartShowTimer(e, r) {
    this.MYt = e;
    this.GP_ = r;
    ControllerHolder_1.ControllerHolder.GenericPromptController.ShowPromptByItsType(13);
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.CloseView, this.$Ge);
  }
  EndShowTimer() {
    if (EventSystem_1.EventSystem.Has(EventDefine_1.EEventName.CloseView, this.$Ge)) {
      EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.CloseView, this.$Ge);
    }
    var e = UiManager_1.UiManager.GetViewByName("PrepareCountdownFloatTips");
    if (e && !e.ClosePromise?.IsPending()) {
      UiManager_1.UiManager.CloseView("PrepareCountdownFloatTips");
    }
  }
  TYt() {
    if (!this.yYt) {
      this.yYt = true;
      EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.CloseView, this.$Ge);
      GeneralLogicTreeController_1.GeneralLogicTreeController.RequestTimerEnd(this.TreeId, this.TimerType);
    }
  }
  GetRemainTime() {
    var e = (this.MYt - TimeUtil_1.TimeUtil.GetServerStopTimeStamp()) / 1000;
    var r = this.GP_ !== 0 ? (this.MYt - this.GP_) / 1000 : -1;
    return Math.max(e, r, 0);
  }
}
exports.LevelPlayPrepareTimer = LevelPlayPrepareTimer;
//# sourceMappingURL=LevelPlayPrepareTimer.js.map
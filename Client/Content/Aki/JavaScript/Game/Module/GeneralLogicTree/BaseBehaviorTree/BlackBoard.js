...
class Blackboard extends BehaviorTreeTagComponent_1.BehaviorTreeTagContainer {
  ...
  AddModifyTrackAreaConfig(i, e) {
    var t = this.ModifyTrackAreaConfig.findIndex((e, t) => e.SourceOfAdd === i);
    if (t < 0) {
      this.ModifyTrackAreaConfig.push(new GeneralLogicTreeDefine_1.ModifyTrackAreaConfig(i, e));
    } else {
      this.ModifyTrackAreaConfig[t].TrackConfig = e;
    }
  }
  RemoveModifyTrackAreaConfig(i) {
    var e = this.ModifyTrackAreaConfig.findIndex((e, t) => e.SourceOfAdd === i);
    if (!(e < 0)) {
      this.ModifyTrackAreaConfig.splice(e, 1);
    }
  }
  GetModifyTrackAreaConfig() {
    if (this.ModifyTrackAreaConfig.length !== 0) {
      return this.ModifyTrackAreaConfig[this.ModifyTrackAreaConfig.length - 1];
    }
  }

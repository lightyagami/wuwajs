...
exports.TowerNormalView = undefined;
const UE = require("ue");
...
class TowerNormalView extends UiViewBase_1.UiViewBase {
  constructor() {
    ...
    this.YDo = () => {
      ...
      UiManager_1.UiManager.OpenView("TowerRewardView", undefined, (e, t) => {
        this.AddChildViewById(t);
      });
    };
    this.JDo = () => {
      if (UiManager_1.UiManager.GetViewByName("TowerVariationView")) {
        this.CloseMe();
      } else {
  ...
  OnRegisterComponent() {
    ...
    this.BtnBindInfo = [[2, this.YDo], [3, this.JDo], [10, this.zDo], [4, this.QDo], [5, this.$Do], [18, this.Vcl]];
  }
  async eRo() {
    if (UiManager_1.UiManager.GetViewByName("TowerVariationView")) {
  ...
  OnStart() {
    ...
    this.GetItem(8).SetUIActive(ModelManager_1.ModelManager.TowerModel.GetDifficultyIsClear(TowerData_1.HIGH_RISK_DIFFICULTY));
    this.GetItem(20).SetUIActive(ModelManager_1.ModelManager.TowerModel.GetDifficultyIsClear(TowerData_1.OVERLOCK_RISK_DIFFICULTY));
  ...
  OnBeforeShow() {
    RedDotController_1.RedDotController.BindRedDot("TowerReward", this.GetItem(6));
    RedDotController_1.RedDotController.BindRedDot("TowerRewardByDifficulties", this.GetItem(11), undefined, 1);
    RedDotController_1.RedDotController.BindRedDot("TowerRewardByDifficulties", this.GetItem(12), undefined, 2);
    RedDotController_1.RedDotController.BindRedDot("TowerRewardByDifficulties", this.GetItem(13), undefined, 3);
    RedDotController_1.RedDotController.BindRedDot("TowerRewardByDifficulties", this.GetItem(22), undefined, 4);
    ModelManager_1.ModelManager.TowerModel.CurrentSelectDifficulties = this.KDo;
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerReward);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerRewardByDifficulties, 1);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerRewardByDifficulties, 2);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerRewardByDifficulties, 3);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerRewardByDifficulties, 4);
  }
  OnBeforeDestroy() {
  ...
  OnAfterHide() {
    RedDotController_1.RedDotController.UnBindGivenUi("TowerReward", this.GetItem(6));
    RedDotController_1.RedDotController.UnBindGivenUi("TowerRewardByDifficulties", this.GetItem(11), 1);
    RedDotController_1.RedDotController.UnBindGivenUi("TowerRewardByDifficulties", this.GetItem(12), 2);
    RedDotController_1.RedDotController.UnBindGivenUi("TowerRewardByDifficulties", this.GetItem(13), 3);
    RedDotController_1.RedDotController.UnBindGivenUi("TowerRewardByDifficulties", this.GetItem(22), 4);
  }
  XDo(e) {
    ...
    if (this.KDo === TowerData_1.OVERLOCK_RISK_DIFFICULTY) {
      this.HDo.RefreshByData([]);
      this.Ncl.RefreshByData(e);
      if (!ModelManager_1.ModelManager.TowerModel.CurrentTowerLock) {
        ModelManager_1.ModelManager.TowerModel.SetOverLockHasShow();
      }
    } else {
      this.Ncl.RefreshByData([]);
      this.HDo.RefreshByData(e);
    }
    this.ZDo();
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerReward);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RedDotTowerRewardByDifficulties, this.KDo);
  }
  Hcl() {
    switch (this.KDo) {
      case TowerData_1.LOW_RISK_DIFFICULTY:
        ModelManager_1.ModelManager.TowerModel.CurrentTowerLock = false;
      ...
      default:
        if (Log_1.Log.CheckError()) {
          Log_1.Log.Error("CycleTower", 5, "逆境深塔选择难度时异常", ["难度:", this.KDo]);
        }
    }
  }
}

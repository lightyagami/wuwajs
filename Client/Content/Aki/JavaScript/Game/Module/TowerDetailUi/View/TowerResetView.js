...
class TowerResetView extends UiViewBase_1.UiViewBase {
  constructor() {
    super(...arguments), this.TDo = -1, this.tFe = void 0, this.sbi = () => {
      return new TowerResetItem_1.TowerResetItem
    }, this.eDo = () => {
      this.CloseMe()
    }, this.sOt = () => {
      TowerController_1.TowerController.TowerResetRequest(this.TDo), this.CloseMe()
    }
  }
  OnRegisterComponent() {
  ...
  SHe() {
    ...
    if (e) {
      ...
      this.GetText(1).SetText("" + r), this.GetText(2).SetText("" + (r - e.Star))
    } else Log_1.Log.CheckError() && Log_1.Log.Error("CycleTower", 5, "重置爬塔成绩时，无法获取到爬塔数据", ["TowerId: ", this.TDo])
  }
}

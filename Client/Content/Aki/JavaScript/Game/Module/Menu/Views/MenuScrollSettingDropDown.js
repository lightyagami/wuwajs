...
class MenuScrollSettingDropDown extends MenuScrollSettingBaseItem_1.MenuScrollSettingBaseItem {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIText], [1, UE.UIItem], [2, UE.UIItem], [3, UE.UIText], [4, UE.UISprite]];
  }
  async OnBeforeStartAsync() {
    this.hbi = new CommonDropDown_1.CommonDropDown(this.GetItem(1), t => new OneTextDropDownItem_1.OneTextDropDownItem(t), t => new OneTextTitleItem_1.OneTextTitleItem(t));
    await this.hbi.Init();
  ...
  _bi() {
    ...
    var e = this.lbi.GetDefaultIndex(this.Data);
  ...
  async ClearAsync() {
    var t = [];
    for (const e of this.hbi.GetDropDownItemList()) {
      t.push(e.DestroyAsync());
    }
    await Promise.all(t);
  }

...
class VehicleSmBlackBoard {
  constructor(t, i, s, e, h, r, o, a, n, u, c, d, l, _, m) {
    this.TeamId = t;
    this.MemberId = i;
    this.CreatureDataId = s;
    this.PbDataId = e;
    ...
    this.RQf.Actor = this.RoadNetworkNavigationComponent.GetActor();
  }
  get StartRoadway() {
  ...
  InitTrace() {
    ...
    var s = (i?.Size.X ?? 0) + Math.abs(i?.Center.X ?? 0);
    var e = (VehicleStreamDefine_1.DISTANCE_BETWEEN_PLAYER + VehicleStreamDefine_1.MAX_BRAKING_DISTANCE_WHEN_CHECKOBSTRUCTION) * 100 + t + s;
    this.BoxTrace.SetBoxHalfSize(e / 2, this.VehicleSize.Y / 2, this.VehicleSize.Z / 2);
    this.BoxTrace.SetDrawDebugTrace(0);
    if (i) {
      this.BoxTracePosition.Set(this.BoxTrace.HalfSizeX - s, 0, this.VehicleSize.Z / 2);
    } else {
  ...
  GetHeadDistanceAndRoadway(t, i) {
    var s = t + this.RootCenterToHead;
    if (s > this.CurrentSplineLength) {
      if (this.NextRoadway && this.NextRoadway.RoadSpline) {
        i.Distance = s - this.CurrentSplineLength;
  ...
  OpenAudio(t, i) {
    if (this.IsInPlayerRange) {
      t = AudioSystem_1.AudioSystem.PostEvent(t, this.RoadNetworkNavigationComponent.GetActor());
      if (i) {
        this.AudioHandleSet.add(t);
      }
      return t;
    } else {
      return 0;
  ...
  UpdateRtpc(t) {
    var i;
    if (this.IsInPlayerRange) {
      i = this.CurrentSpeed > this.v5f ? this.AcceleratedSpeedConfig : -this.AcceleratedSpeedConfig;
      this.v5f = MathCommon_1.MathCommon.Clamp(this.v5f + i * t * TimeUtil_1.TimeUtil.Millisecond, 0, this.CurrentSpeed);
      if (!(Math.abs(this.m2f - this.v5f) < VehicleStreamDefine_1.CHANGE_VEHICLE_SPEED_TOLERENCE) || this.m2f !== 0 && this.v5f === 0) {
        this.m2f = this.v5f;
        AudioSystem_1.AudioSystem.SetRtpcValue("vehicle_speed", this.v5f, this.RQf);
      }
    }
  }
  ModelCenterOffsetX() {

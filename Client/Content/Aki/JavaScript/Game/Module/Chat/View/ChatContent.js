...
class ChatContent extends UiPanelBase_1.UiPanelBase {
  ...
  Dke() {
    ...
    if (this.vBd.ContentType === Protocol_1.Aki.Protocol.p8n.DIs && (e = this.GetText(1), t = this.vBd.Content, e.SetText(t), s.SetUIActive(true), r.SetUIActive(false), Log_1.Log.CheckInfo())) {
      Log_1.Log.Info("Chat", 5, "表情:" + this.vBd.Content, ["expressionItemActive", r?.IsUIActiveSelf()]);
    }
    if (this.vBd.ContentType === Protocol_1.Aki.Protocol.p8n.Proto_Emoji) {
      e = this.GetTexture(4);
      t = Number(this.vBd.Content);
  ...
  Nxa() {
    if (PlatformSdkManagerNew_1.PlatformSdkManagerNew.GetPlatformSdk()?.NeedShowThirdPartyId()) {
      let e = this.vBd.PsOnlineId;
      var i = ModelManager_1.ModelManager.PersonalModel;
      var s = this.vBd.SenderPlayerId;
  ...
  Oac() {
    var e = this.vBd.SenderPlayerId;
    var t = this.GetText(6);
    var i = this.vBd.ChatRoomType;
    if (i === 2 || i === 3) {
  ...
  sSt() {
    ...
    var t = this.vBd.TimeStamp;
    var i = this.vBd.LastTimeStamp;

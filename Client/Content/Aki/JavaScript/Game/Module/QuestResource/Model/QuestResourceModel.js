...
class QuestResourceModel extends ModelBase_1.ModelBase {
  ...
  OnInit() {
    if (VideoResUpdate_1.VideoResUpdate.GetIsSeparateVideo()) {
      ...
      for (const a of e) {
        ...
        if (!isNaN(i)) {
          if (this.Zqm.has(a.QuestId)) {
            this.Zqm.get(a.QuestId).add(i);
          } else {
            (r = new Set()).add(i);
            this.Zqm.set(a.QuestId, r);
          }
        }
      }
  ...
  PIm(e) {
    ...
    var o = this.wIm;
    this.wIm = undefined;
    ControllerHolder_1.ControllerHolder.ConfirmBoxController.CloseNetWorkConfirmBoxView(this.RIm);
    o?.SetResult(e);
  }
  async AIm() {
    if (this.wIm) {

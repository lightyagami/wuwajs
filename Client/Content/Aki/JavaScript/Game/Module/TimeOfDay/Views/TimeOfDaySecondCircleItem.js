...
exports.TimeOfDaySecondCircleItem = undefined;
const UE = require("ue");
...
class TimeOfDaySecondCircleItem extends AutoAttachExhibitionItem_1.AutoAttachExhibitionItemAbstract {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIItem], [1, UE.UIText], [2, UE.UIText], [3, UE.UIButtonComponent], [4, UE.UITexture], [5, UE.UIItem]];
    this.BtnBindInfo = [[3, () => {
      this.MLo();
    }]];
  }
  ...
  gsi() {
    if (this.rai) {
      ...
      var _ = this.GetItem(0);
      this.Iot.Yaw = R;
      _.SetUIRelativeRotation(this.Iot.ToUeRotator());
    }
  }
  SetData(E) {
    this.Pe = E;
  ...
  OnMoveItem(E) {
    ...
    var e = t.RelativeScale3D;
    let D = 0;
    let i = 0;
    if (_ > BORDER_LEFT && _ < BORDER_MIDDLE) {
      D = (_ - BORDER_LEFT) / (BORDER_MIDDLE - BORDER_LEFT);
      i = MathUtils_1.MathUtils.Lerp(BORDER_ALPHA, 1, D);
    } else if (_ > BORDER_LEFT_HIDE && _ < BORDER_LEFT) {
      D = (_ - BORDER_LEFT_HIDE) / (BORDER_LEFT - BORDER_LEFT_HIDE);
      i = MathUtils_1.MathUtils.Lerp(0, BORDER_ALPHA, D);
    } else if (_ <= BORDER_LEFT_HIDE) {
      i = 0;
    }
    if (_ >= BORDER_MIDDLE && _ < BORDER_RIGHT) {
      D = (_ - BORDER_MIDDLE) / (BORDER_RIGHT - BORDER_MIDDLE);
      i = MathUtils_1.MathUtils.Lerp(1, BORDER_ALPHA, D);
    } else if (_ >= BORDER_RIGHT && _ < BORDER_RIGHT_HIDE) {
      D = (_ - BORDER_RIGHT) / (BORDER_RIGHT_HIDE - BORDER_RIGHT);
      i = MathUtils_1.MathUtils.Lerp(BORDER_ALPHA, 0, D);
    } else if (_ >= BORDER_RIGHT_HIDE) {
      i = 0;
    }
    this.GetItem(5).SetAlpha(i);
    i = _ > STONE2_BORDER_LEFT && _ < BORDER_MIDDLE ? (D = (_ - STONE2_BORDER_LEFT) / (BORDER_MIDDLE - STONE2_BORDER_LEFT), MathUtils_1.MathUtils.Lerp(0, 1, D)) : _ >= BORDER_MIDDLE && _ < STONE2_BORDER_RIGHT ? (D = (_ - BORDER_MIDDLE) / (STONE2_BORDER_RIGHT - BORDER_MIDDLE), MathUtils_1.MathUtils.Lerp(1, 0, D)) : 0;
    this.GetTexture(4).SetAlpha(i);
    if (_ >= LEFT_RANGE && _ <= RIGHT_RANGE) {
      if (_ >= LEFT_RANGE && _ <= MIDDLE_RANGE) {
        D = _ - LEFT_RANGE;
        R = MathUtils_1.MathUtils.Lerp(ANIMAL_SCALE, 1, D * 10);
        R = new UE.Vector(R, R, R);
        t.SetUIItemScale(R);
      } else {
        D = _ - MIDDLE_RANGE;
        R = MathUtils_1.MathUtils.Lerp(1, ANIMAL_SCALE, D * 10);
        _ = new UE.Vector(R, R, R);
        t.SetUIItemScale(_);
      }
    } else if (e.X !== this.nai.X) {
      t.SetUIItemScale(this.nai);
    }
  }
  OnUnSelect() {}
  OnSelect() {

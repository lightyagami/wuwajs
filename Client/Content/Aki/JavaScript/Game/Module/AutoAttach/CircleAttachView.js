...
class CircleAttachView extends AutoAttachBaseView_1.AutoAttachBaseView {
  ...
  ReloadItems(t, e, i = 0) {
    if (t < this.ShowItemNum) Log_1.Log.CheckError() && Log_1.Log.Error("UiCommon", 27, "组件数据长度需要大于等于展示长度");
    else {
      var s, r = this.Items.length;
      for (let t = 0; t < r; t++) this.Items[t].SetUiActive(!1);
      for (let t = 0; t < this.ShowItemNum + 1; t++) t >= this.Items.length && (s = LguiUtil_1.LguiUtil.DuplicateActor(this.SourceActor, this.ControllerItem), (s = this.CreateItemFunction(s, t, this.ShowItemNum)).SetSourceView(this), this.Items.push(s)), this.Items[t].SetItemIndex(t), this.Items[t].SetUiActive(!0), this.Items[t].SetData(e), this.Items[t].InitItem();

...
class ComposeConfig extends ConfigBase_1.ConfigBase {
  ...
  GetSynthesisFormulaByFormulaItemId(e) {
    ...
    return o || Log_1.Log.CheckError() && Log_1.Log.Error("Compose", 49, "合成配方获取失败，请检查合成配方配置表是否正确", ["FormulaItemId=", e]), o
  }
  GetSynthesisFormulaById(e) {
    var o = SynthesisFormulaById_1.configSynthesisFormulaById.GetConfig(e);
    return o || Log_1.Log.CheckError() && Log_1.Log.Error("Compose", 49, "合成配方获取失败，请检查合成配方配置表是否正确", ["Id=", e]), o
  }
  GetComposeListByType(e) {
    var o = SynthesisFormulaByFormulaType_1.configSynthesisFormulaByFormulaType.GetConfigList(e);
    return o || Log_1.Log.CheckError() && Log_1.Log.Error("Compose", 49, "获取对应类型合成数据失败，请检查合成配方配置表是否正确", ["FormulaType=", e]), o
  }
  GetExchangeList() {
    var e = MaterialReplaceAll_1.configMaterialReplaceAll.GetConfigList();
    return e || Log_1.Log.CheckError() && Log_1.Log.Error("Compose", 5, "获取置换数据列表失败，请检查合成表是否正确"), e
  }
  GetComposeLevel() {
    var e = SynthesisLevelAll_1.configSynthesisLevelAll.GetConfigList();
    return e || Log_1.Log.CheckError() && Log_1.Log.Error("Compose", 49, "获取制药证书相关配置失败"), e
  }
  GetConditionInfo(e) {

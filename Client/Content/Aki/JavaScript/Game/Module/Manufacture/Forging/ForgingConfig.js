...
class ForgingConfig extends ConfigBase_1.ConfigBase {
  ...
  GetForgeFormulaByFormulaItemId(e) {
    ...
    return r || Log_1.Log.CheckError() && Log_1.Log.Error("Forging", 49, "锻造配方获取失败，请检查锻造配方配置表是否正确", ["FormulaItemId=", e]), r
  }
  GetForgeFormulaById(e) {
    var r = ForgeFormulaById_1.configForgeFormulaById.GetConfig(e);
    return r || Log_1.Log.CheckError() && Log_1.Log.Error("Forging", 49, "锻造配方获取失败，请检查锻造配方配置表是否正确", ["Id=", e]), r
  }
  GetForgeList() {
    var e = ForgeFormulaAll_1.configForgeFormulaAll.GetConfigList();
    return e || Log_1.Log.CheckError() && Log_1.Log.Error("Forging", 49, "获取对应类型锻造数据列表失败，请检查锻造配方配置表是否正确"), e
  }
  GetForgeListByType(e) {
    var r = ForgeFormulaByTypeId_1.configForgeFormulaByTypeId.GetConfigList(e);
    return r || Log_1.Log.CheckError() && Log_1.Log.Error("Forging", 49, "获取对应类型锻造数据失败，请检查锻造配方配置表是否正确", ["TypeId=", e]), r
  }
}

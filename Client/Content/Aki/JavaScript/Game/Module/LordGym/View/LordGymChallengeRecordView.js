...
class LordGymChallengeRecordView extends UiViewBase_1.UiViewBase {
  constructor() {
    super(...arguments), this.wSi = 1, this.vDt = LordGymDefine_1.NONE_FILTER_TYPE, this.BFl = void 0, this.BSi = void 0, this.bSi = void 0, this.IJl = 0, this.qFl = 0, this.qSi = (i, e) => {
  ...
  async OnBeforeStartAsync() {
    var i;
    this.IJl = this.OpenParam, this.IJl && (i = ConfigManager_1.ConfigManager.LordGymConfig.GetLordGymEntranceLordList(this.IJl), i = ConfigManager_1.ConfigManager.LordGymConfig.GetLordGymConfig(i[0]), this.qFl = i.FilterType), this.BSi = new CommonDropDown_1.CommonDropDown(this.GetItem(5), this.qSi, this.GSi), await this.BSi.Init()
  }
  OnStart() {
    this.BFl = ConfigManager_1.ConfigManager.LordGymConfig.GetAllLordGymFilterTypeConfig(), this.BSi.InitScroll(this.BFl, i => i), this.BSi?.SetOnSelectCall(this.NSi), this.BSi?.SetShowType(1), this.bSi = new GenericScrollViewNew_1.GenericScrollViewNew(this.GetScrollViewWithScrollbar(1), this.VSi);
    let e = 0;
    if (this.qFl)
  ...
  HSi() {
    ...
    if (t?.filter(i => !i.IsDebug), i = this.vDt === LordGymDefine_1.NONE_FILTER_TYPE ? ConfigCommon_1.ConfigCommon.ToList(t) : t?.filter(i => i.FilterType === this.vDt), this.IJl) {
  ...
  OSi() {
    this.GetButton(3)?.SetSelfInteractive(this.wSi !== MIN_DIFFICULTY), this.GetButton(4)?.SetSelfInteractive(this.wSi !== MAX_DIFFICULTY);
    var i = this.HSi();
    i && this.bSi.RefreshByData(i), LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(6), "LordGymDifficulty", this.wSi)
  }
}

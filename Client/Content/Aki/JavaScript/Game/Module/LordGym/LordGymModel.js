...
class LordGymModel extends ModelBase_1.ModelBase {
  ...
  GetMaxDifficultyLordGymEntranceCanFight(e) {
    ...
    if (e) {
      ...
      for (const n of e.LordGymList) {
        var t = ConfigManager_1.ConfigManager.LordGymConfig?.GetLordGymConfig(n);
        1 < t.Difficulty && this.GetLordGymIsUnLock(n) && this.GetLordGymIsFinish(n - 1) && t.Difficulty > r && (r = t.Difficulty)
      }
      return r
    }
  }
  ...
  GetGymEntranceAllFinish(r) {
    ...
    for (const e of r.LordGymList)
      if (!this.GetLordGymIsFinish(e)) return !1;
    return !0
  }
  GetGymCanFightMaxLevelWithoutLockCondition(e) {
    ...
    if (e) {
      ...
      for (const n of e.LordGymList) {
        var t = ConfigManager_1.ConfigManager.LordGymConfig?.GetLordGymConfig(n);
        1 < t.Difficulty && this.GetLordGymIsUnLock(n) && t.Difficulty > r && (r = t.Difficulty)
      }
      return r
    }
  }
  GetLordGymCurrencyRewardAndTotalCount(r) {
    ...
    for (const s of r)
      for (const f of n.GetLordGymEntranceConfig(s).LordGymList) {
        ...
        for (const h of i) h[0].ItemId === PayShopDefine_1.LORD_GYM_CURRENCY_ID && (a && (e += h[1]), t += h[1])
      }
    return [e, t]
  }
  IsChallenging() {
  ...
  RecordNewLordGymEntrance(r) {
    this.NewLordGymEntranceIdRecord && !this.IsNewLordGymEntranceRecord(r) && (this.NewLordGymEntranceIdRecord.push(r), LocalStorage_1.LocalStorage.SetPlayer(LocalStorageDefine_1.ELocalStoragePlayerKey.NewLordGymEntranceIdRecord, this.NewLordGymEntranceIdRecord))
  }

"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.RoguelikeShopDetail = exports.RoguelikeShopAttrItem = undefined;
const UE = require("ue");
const Protocol_1 = require("../../../../Core/Define/Net/Protocol");
const ConfigManager_1 = require("../../../Manager/ConfigManager");
const ModelManager_1 = require("../../../Manager/ModelManager");
const UiPanelBase_1 = require("../../../Ui/Base/UiPanelBase");
const AsyncTask_1 = require("../../../World/Task/AsyncTask");
const TaskSystem_1 = require("../../../World/Task/TaskSystem");
const GenericLayout_1 = require("../../Util/Layout/GenericLayout");
const LguiUtil_1 = require("../../Util/LguiUtil");
const RoguelikeController_1 = require("../RoguelikeController");
const CommonSelectItem_1 = require("./CommonSelectItem");
class RoguelikeShopAttrItem extends UiPanelBase_1.UiPanelBase {
  constructor() {
    super();
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIText], [1, UE.UIItem]];
  }
  Update(e) {
    var i = ConfigManager_1.ConfigManager.RoguelikeConfig.GetRogueAffixConfig(e.Id);
    LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(0), i.AffixDesc);
    this.GetItem(1).SetUIActive(e.IsUnlock);
  }
}
exports.RoguelikeShopAttrItem = RoguelikeShopAttrItem;
class RoguelikeShopDetail extends UiPanelBase_1.UiPanelBase {
  constructor() {
    super(...arguments);
    this.ylo = [];
    this.Sui = undefined;
    this.Fao = () => {
      return new CommonSelectItem_1.CommonElementItem();
    };
    this.ilo = () => {
      RoguelikeController_1.RoguelikeController.RogueChooseDataResultRequest(6);
    };
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIText], [1, UE.UIText], [2, UE.UIText], [3, UE.UIText], [4, UE.UIItem], [5, UE.UIItem], [6, UE.UIItem], [7, UE.UIButtonComponent], [8, UE.UITexture], [9, UE.UIText], [10, UE.UIText], [11, UE.UIHorizontalLayout]];
    this.BtnBindInfo = [[7, this.ilo]];
  }
  OnStart() {
    this.Sui = new GenericLayout_1.GenericLayout(this.GetHorizontalLayout(11), this.Fao);
  }
  Refresh(s) {
    var e = new AsyncTask_1.AsyncTask("RoguelikeShopDetail.Refresh", async () => {
      if (s.RoguelikeGainDataType === Protocol_1.Aki.Protocol.a8s.hxs) {
        var e = ConfigManager_1.ConfigManager.RoguelikeConfig.GetRoguePhantomConfig(s.ConfigId);
        LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(0), e.PokemonName);
        var i = ConfigManager_1.ConfigManager.PhantomBattleConfig.GetPhantomItemById(e.PhantomItem);
        if (!i) {
          return true;
        }
        var t = ConfigManager_1.ConfigManager.PhantomBattleConfig.GetPhantomSkillBySkillId(i.SkillId);
        if (!t) {
          return true;
        }
        i = ConfigManager_1.ConfigManager.PhantomBattleConfig?.GetPhantomSkillDescExByPhantomSkillIdAndQuality(i.SkillId, e.Quality);
        LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(3), t.DescriptionEx, ...i);
        this.GetItem(4).SetUIActive(true);
        const r = [];
        s.AffixEntryList.forEach((e, i) => {
          if (!this.ylo[i]) {
            this.ylo[i] = new RoguelikeShopAttrItem();
            r.push(this.ylo[i].CreateThenShowByActorAsync(LguiUtil_1.LguiUtil.CopyItem(this.GetItem(5), this.GetItem(4)).GetOwner()));
          }
        });
        await Promise.all(r);
        s.AffixEntryList.forEach((e, i) => {
          this.ylo[i].Update(e);
        });
      } else {
        e = ConfigManager_1.ConfigManager.RoguelikeConfig.GetRogueBuffConfig(s.ConfigId);
        LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(0), e.BuffName);
        LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(3), e.BuffDesc, ...e.BuffDescParam);
        this.GetItem(4).SetUIActive(false);
      }
      LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(2), "RoguelikeShopItemType" + s.RoguelikeGainDataType.toString());
      var t = s.IsDiscounted() ? s.CurrentPrice : s.OriginalPrice;
      var i = ConfigManager_1.ConfigManager.RoguelikeConfig?.GetRogueCurrencyConfig(s.ShopItemCoinId);
      var e = ModelManager_1.ModelManager.RoguelikeModel.GetRoguelikeCurrency(s.ShopItemCoinId);
      if (s.IsDiscounted()) {
        LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(10), "RogueShopOriginPriceDiscount", s.OriginalPrice.toString());
      } else {
        this.GetText(10).SetText("");
      }
      this.GetText(9).SetText(t.toString());
      this.GetText(9).useChangeColor = e < t;
      this.SetTextureByPath(i?.IconSmall ?? "", this.GetTexture(8));
      this.GetItem(6).SetUIActive(!s.IsSell);
      this.GetButton(7).RootUIComp.SetUIActive(!s.IsSell);
      var e = s.GetSortElementInfoArrayByCount();
      if (e.length > 0) {
        t = e[0];
        i = new Array(t.Count).fill(t.ElementId);
        await this.Sui?.RefreshByDataAsync(i);
      }
      return true;
    });
    TaskSystem_1.TaskSystem.AddTask(e);
    TaskSystem_1.TaskSystem.Run();
  }
}
exports.RoguelikeShopDetail = RoguelikeShopDetail;
//# sourceMappingURL=RoguelikeShopDetail.js.map
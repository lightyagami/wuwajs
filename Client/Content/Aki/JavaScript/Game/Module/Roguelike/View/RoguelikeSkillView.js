...
exports.RoguelikeSkillView = undefined;
const UE = require("ue");
...
class RoguelikeSkillView extends UiViewBase_1.UiViewBase {
  constructor() {
    ...
    this.OnSelectSkill = e => {
      ...
      ModelManager_1.ModelManager.RoguelikeModel.SelectSkillId = e.Data.Id;
  ...
  OnStart() {
    ...
    this.ScrollView.BindLateUpdate(e => {
      var i;
      if (this.SelectColumn > 0 && this.ExecuteCount === 2) {
  ...
  OnAddEventListener() {
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.RoguelikeSelectSkill, this.OnSelectSkill);
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.RoguelikeTalentLevelUp, this.OnSkillLevelUp);
  }
  OnRemoveEventListener() {
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.RoguelikeSelectSkill, this.OnSelectSkill);
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.RoguelikeTalentLevelUp, this.OnSkillLevelUp);
  }
  BuildSkillTreeConfig() {
    ...
    for (const t of e) {
      ...
      if (ModelManager_1.ModelManager.RoguelikeModel.RoguelikeSkillDataMap.get(t.Id) === 0 && t.Column > this.SelectColumn) {
        this.SelectColumn = t.Column;
      }
      this.SkillTreeConfigList[t.Column].push(t);
    }
    for (const s of this.SkillTreeConfigList) {
      s?.sort((e, i) => e.Row - i.Row);
    }

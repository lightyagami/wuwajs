...
exports.PhantomSelectView = undefined;
const UE = require("ue");
...
class PhantomSelectView extends RogueSelectBaseView_1.RogueSelectBaseView {
  constructor() {
    ...
    this.PhantomSelectItemLayout = undefined;
    this.IsShowChooseTips = false;
    this.ConfirmBtn = () => {
    ...
    this.RefreshBtnEnableClick = e => {
      if (e !== undefined) {
        for (const t of this.PhantomSelectItemLayout.GetLayoutItemList()) {
          if (t.RogueGainEntry?.ConfigId !== e) {
            t.SetToggleUnChecked();
          }
        }
      }
      const t = this.GetCommonSelectItem();
      this.ButtonItem.SetEnableClick(t !== undefined);
    };
    ...
    this.RoguelikeChooseDataResult = (e, t, i, o) => {
      if (i && o === this.RoguelikeChooseData?.Index) {
        if (this.IsShowChooseTips) {
          i = this.GetCommonSelectItem();
          UiManager_1.UiManager.CloseAndOpenView(this.Info.Name, "RoguePhantomSelectResultView", new RogueSelectResult_1.RogueSelectResult(e, t, i?.RogueGainEntry));
        } else {
  ...
  OnRegisterComponent() {
    ...
    if (this.Info.Name === "RoguePhantomReplaceView") {
  ...
  OnStart() {
    ...
    this.PhantomSelectItemLayout = new GenericLayout_1.GenericLayout(this.GetHorizontalLayout(1), this.CreatePhantomSelectItem);
  }
  OnBeforeDestroy() {
  ...
  RefreshTopPanel() {
    ...
    this.TopPanel.RefreshSelectTipsText(RoguelikeDefine_1.ROGUELIKEVIEW_6_TEXT);
  ...
  GetGuideUiItemAndUiItemForShowEx(e) {
    if (e.length === 2 || isNaN(Number(e[0]))) {
      var t = Number(e[0]);
      var i = this.PhantomSelectItemLayout?.GetItemByIndex(t);
      if (i) {
        if (e[1] === "All") {
          return [i, i];
        }

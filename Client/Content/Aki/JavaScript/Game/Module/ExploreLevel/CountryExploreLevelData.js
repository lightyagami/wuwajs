...
exports.CountryExploreLevelData = undefined;
...
class CountryExploreLevelData {
  ...
  GetVisibleExploreScoreDataList() {
    ...
    for (const a of this.rVt.values()) {
      for (var [t, r] of a) {
        ...
        var o = a.get(o);
        if (!o || !!o.GetIsReceived()) {
          if (r.CanReceive() || r.GetIsReceived() || t > r.GetAreaProgress()) {
            e.push(r);
          }
        }
      }
  ...
  CanLevelUp() {
    let e = 0;
    for (const r of this.nVt) {
      if (r.CanReceive()) {
        e += r.Score;
      }
    }
    var t = this.GetCurrentExploreLevelRewardData();
    return e !== 0 && !(this.eVt >= t.GetMaxExploreScore()) && this.eVt + e >= t.GetMaxExploreScore();
  }
}

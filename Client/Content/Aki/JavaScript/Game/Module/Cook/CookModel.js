...
class CookModel extends ModelBase_1.ModelBase {
  constructor() {
    super(...arguments), this.Xqt = -1, this.LastExp = 0, this.CurrentInteractCreatureDataLongId = void 0, this.$qt = 0, this.Yqt = 0, this.Jqt = void 0, this.zqt = new Map, this.Zqt = void 0, this.eGt = (t, e) => {
      var i = t.IsUnLock ? 1 : 0,
        o = e.IsUnLock ? 1 : 0;
      return 1 == i && i == o ? t.IsMachining === e.IsMachining ? t.Quality === e.Quality ? t.ItemId - e.ItemId : t.Quality - e.Quality : e.IsMachining - t.IsMachining : 0 == i && i == o ? t.Quality === e.Quality ? t.ItemId - e.ItemId : t.Quality - e.Quality : o - i
    }, this.tGt = 0, this.iGt = void 0, this.oGt = void 0, this.rGt = void 0, this.nGt = void 0, this.sGt = void 0, this.aGt = void 0, this.hGt = void 0, this.lGt = (t, e) => t.IsBuff === e.IsBuff ? t.RoleId - e.RoleId : t.IsBuff ? -1 : 1, this._Gt = 0, this.E0 = void 0, this.uGt = void 0
  }
  OnInit() {
    return !0
  }
  ...
  CheckHasItemTimeoutStateChangedCore() {
    var t, e, i, o, r, s, n = this.Jqt,
      a = this.zqt;
    for ([t, e] of a) a.set(t, e - 1);
    let h = !1;
    for (const l of n) 0 !== l.ExistStartTime && 0 !== l.ExistEndTime && (i = l.ItemId, o = TimeUtil_1.TimeUtil.IsInTimeSpan(l.ExistStartTime, l.ExistEndTime) ? 1 : 3, a.has(i) && a.get(i) === o - 1 || (h = !0), a.set(i, o));
  ...
  CreateCookingDataList(t) {
    ...
    for (const h of t) {
      ...
      i.has(h.s5n) ? ((r = i.get(h.s5n)).CookCount = h.DVn, r.IsNew = ModelManager_1.ModelManager.NewFlagModel.HasNewFlag(LocalStorageDefine_1.ELocalStoragePlayerKey.CookerLevelKey, h.s5n), r.LastRoleId = h.cPs, r.IsCook = 0, r.LimitTotalCount = h.dPs, r.LimitedCount = h.mPs, r.DataId = s.FoodItemId, r.ExistStartTime = MathUtils_1.MathUtils.LongToNumber(h.CPs) * TimeUtil_1.TimeUtil.Millisecond, r.ExistEndTime = MathUtils_1.MathUtils.LongToNumber(h.gPs) * TimeUtil_1.TimeUtil.Millisecond, r.IsUnLock = !0) : (r = {
        MainType: 0,
        SubType: 0,
        UniqueId: 0,
        ItemId: h.s5n,
        ...
        IsUnLock: !0
      }, this.Jqt.push(r)), r.IsCook = this.CheckCanCook(h.s5n) ? 1 : 0
    }
  }
  UpdateCookingDataList(t) {
    ...
    for (const i of t)
      for (const o of this.Jqt) {
        var e;
        i.s5n === o.ItemId && (o.CookCount = i.DVn, o.LimitedCount = i.mPs, o.IsUnLock || (e = ConfigManager_1.ConfigManager.CookConfig.GetCookFormulaById(i.s5n), o.IsNew = !0, o.IsUnLock = !0, o.DataId = e.FoodItemId, o.ExistStartTime = MathUtils_1.MathUtils.LongToNumber(i.CPs) * TimeUtil_1.TimeUtil.Millisecond, o.ExistEndTime = MathUtils_1.MathUtils.LongToNumber(i.gPs) * TimeUtil_1.TimeUtil.Millisecond, ModelManager_1.ModelManager.NewFlagModel.AddNewFlag(LocalStorageDefine_1.ELocalStoragePlayerKey.CookerLevelKey, i.s5n), ControllerHolder_1.ControllerHolder.GenericPromptController.ShowPromptByCode("FormulaLearned")), o.IsCook = this.CheckCanCook(i.s5n) ? 1 : 0)
      }
  }
  UpdateCookingDataByServerConfig(t) {
    for (const r of t) {
      var e = MathUtils_1.MathUtils.LongToNumber(r.CPs) * TimeUtil_1.TimeUtil.Millisecond,
        i = MathUtils_1.MathUtils.LongToNumber(r.gPs) * TimeUtil_1.TimeUtil.Millisecond,
        o = this.Jqt.findIndex(t => t.ItemId === r.s5n); - 1 !== o && (this.Jqt[o].ExistStartTime = e, this.Jqt[o].ExistEndTime = i)
    }
  }
  UnlockCookMenuData(t) {
    var e = ConfigManager_1.ConfigManager.CookConfig.GetCookFormulaById(t);
    let i = 0;
    for (; i < this.Jqt.length && (6e4 !== this.Jqt[i].SubType || this.Jqt[i].ItemId !== e.FormulaItemId); i++);
    this.Jqt.splice(i, 1)
  }
  GetCookingDataList() {
    return this.Jqt
  ...
  GetCookingDataById(t) {
    for (const e of this.Jqt)
      if (t === e.ItemId) return e
  }
  GetCookRoleId(t) {
    t = this.GetCookingDataById(t);
    return t?.LastRoleId ? t.LastRoleId : ModelManager_1.ModelManager.PlayerInfoModel.GetPlayerRoleId() ?? 1502
  }
    ...
    for (const s of t)
      for (const n of this.Zqt)
        if (s.s5n === n.ItemId) {
          let t = [];
          var i = [],
            o = ConfigManager_1.ConfigManager.CookConfig.GetCookProcessedById(s.s5n);
          if (s.fPs) {
          ...
          n.IsUnLock = t.length === o.InterationId.length, n.InteractiveList = t, n.UnlockList = i, n.IsMachining = CookController_1.CookController.CheckCanProcessed(s.s5n) ? 1 : 0, e && (n.IsNew = e, ModelManager_1.ModelManager.NewFlagModel.AddNewFlag(LocalStorageDefine_1.ELocalStoragePlayerKey.CookerLevelKey, s.s5n), s.fPs) && ControllerHolder_1.ControllerHolder.GenericPromptController.ShowPromptByCode("FormulaLearned");
          break
        } this.Zqt.sort(this.eGt)
  }
  ...
  CreateCookerInfo(t) {
    ...
    for (const e of ConfigManager_1.ConfigManager.CookConfig.GetCookLevel()) this.iGt.set(e.Id, e)
  }
  UpdateCookerInfo(t) {
    let e = 0;
    this.oGt && (this.LastExp = this.oGt.TotalProficiencys, e = t.uPs - this.oGt.TotalProficiencys), this.oGt = {
      CookingLevel: t._Ps,
  ...
  GetDropIdByLevel(t) {
    t += 1;
    return this.GetCookerMaxLevel() < t ? -1 : this.GetCookLevelByLevel(t).DropIds
  }
  CreateTmpMachiningItemList(t) {
    this.rGt || (this.rGt = new Array), this.rGt.length = 0, this.nGt || (this.nGt = new Array), this.nGt.length = 0, this.sGt || (this.sGt = new Map), this.sGt.clear(), this.rGt.length = 0, this.nGt.length = 0;
  ...
  CheckCanProcessedNew(t) {
    if (0 === this.sGt.size) return CookController_1.CookController.CheckCanProcessed(t);
    let e = !0;
  ...
  UpdateCookRoleItemDataList() {
    ...
    for (const t of ModelManager_1.ModelManager.RoleModel.GetRoleList()) this.hGt.push({
      RoleId: t.GetRoleId(),
  ...
  GetCookRoleItemDataList(t) {
    ...
    for (const e of this.hGt) e.ItemId = t, e.IsBuff = CookController_1.CookController.CheckIsBuff(e.RoleId, t);
    return this.hGt.sort(this.lGt)
  }
  GetCookMaterialList(t, e) {
    var i = new Array;
    if (0 === e)
      for (const r of ConfigManager_1.ConfigManager.CookConfig.GetCookFormulaById(t).ConsumeItems) i.push({
        L8n: r.ItemId,
        UVn: r.Count,
        K6n: !0
      });
      ...
      for (const s of ConfigManager_1.ConfigManager.CookConfig.GetCookProcessedById(t).ConsumeItemsId) i.push({
        L8n: s.ItemId,
        UVn: s.Count,
  ...
  UpdateCookItemList(t) {
    ...
    for (const e of t) this.uGt.push({
      ItemId: e.L8n,
      ItemNum: e.UVn
    })
  }

...
class InfoDisplayCircleItem extends AutoAttachExhibitionItem_1.AutoAttachExhibitionItemAbstract {
  ...
  RefreshItem() {
    this.rai = this.Pe[this.GetShowItemIndex()], this.Aqe(), this.GetRootItem().SetHierarchyIndex(0)
  }
  OnMoveItem(t) {
    var e, i = this.GetAttachItem().ExhibitionView.GetWidth(),
      s = this.GetRootItem(),
      i = (s.GetAnchorOffsetX() + i / 2) / i,
      r = s.RelativeScale3D;
    let h = 0;
    i >= LEFT_RANGE && i <= RIGHT_RANGE ? (i >= LEFT_RANGE && i <= MIDDLE_RANGE ? (h = i - LEFT_RANGE, e = MathUtils_1.MathUtils.Lerp(ANIMAL_SCALE, 1, 10 * h), e = new UE.Vector(e, e, e), s.SetUIItemScale(e)) : (h = i - MIDDLE_RANGE, e = MathUtils_1.MathUtils.Lerp(1, ANIMAL_SCALE, 10 * h), i = new UE.Vector(e, e, e), s.SetUIItemScale(i)), this.GetRootItem().SetHierarchyIndex(FRONT_HIERACHY)) : r.X !== this.nai.X && s.SetUIItemScale(this.nai)
  }
  SetData(t) {
    this.Pe = t
  }

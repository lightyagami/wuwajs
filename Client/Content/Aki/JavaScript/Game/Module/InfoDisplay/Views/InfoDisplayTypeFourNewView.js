...
class InfoDisplayTypeFourNewView extends UiTickViewBase_1.UiTickViewBase {
  constructor() {
    super(...arguments), this.sai = void 0, this.qrc = void 0, this.Grc = !1, this.Frc = 0, this.Nrc = !1, this.I5t = () => {
      this.Vrc()
    }, this.wvo = () => {
      this.Vrc()
    }, this.jrc = () => {
  ...
  OnStart() {
    ...
    this.sai = new InfoDisplayAudioPlayerWithProgress_1.InfoDisplayAudioPlayerWithProgress, this.qrc = this.GetUiNiagara(5), this.SHe(A), this.$rc(A)
  }
  OnTick(A) {
  ...
  async $rc(A) {
    ...
    await this.sai.CreateThenShowByActorAsync(this.GetItem(2).GetOwner()), await this.sai.InitAudioPlayer(A), this.Hrc(!0), this.sai.SetOnAudioEnd(this.wvo), this.sai.SetOnPlay(this.jrc), this.sai.SetOnPause(this.FIr)
  }
  SHe(A) {
    var i = ConfigManager_1.ConfigManager.InfoDisplayModuleConfig.GetInfoDisplayTitle(A),
      i = (this.GetText(4).SetText(i), ConfigManager_1.ConfigManager.InfoDisplayModuleConfig.GetInfoDisplayDesc(A));
  ...
  Qrc(A, i) {
    var _;
    A && (_ = this.Nrc ? NIAGARA_PARAM_WPO_ACTIVE - i * (NIAGARA_PARAM_WPO_ACTIVE - NIAGARA_PARAM_WPO_INACTIVE) : NIAGARA_PARAM_WPO_INACTIVE + i * (NIAGARA_PARAM_WPO_ACTIVE - NIAGARA_PARAM_WPO_INACTIVE), A.SetNiagaraVarFloat(NIAGARA_PARAM_WPO_KEY, _), _ = this.Nrc ? NIAGARA_PARAM_DISSOLVE_ACTIVE - i * (NIAGARA_PARAM_DISSOLVE_ACTIVE - NIAGARA_PARAM_DISSOLVE_INACTIVE) : NIAGARA_PARAM_DISSOLVE_INACTIVE + i * (NIAGARA_PARAM_DISSOLVE_ACTIVE - NIAGARA_PARAM_DISSOLVE_INACTIVE), A.SetNiagaraVarFloat(NIAGARA_PARAM_DISSOLVE_KEY, _))
  }
  Wrc(i) {
    if (this.Grc) {
      this.Frc += i;
      let A = this.Frc / NIAGARA_INTERP_TIME;
      1 <= A && (this.Grc = !1, this.Frc = 0, A = 1), this.Qrc(this.qrc, A)
    }
  }
}

...
class RoleFavorLockComponent extends RoleFavorViewComponentBase_1.RoleFavorViewComponentBase {
  constructor() {
    ...
    this.Sui = undefined;
    this.huo = [];
    this.Lfd = () => new RoleFavorLockItem_1.RoleFavorLockItem();
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIVerticalLayout], [1, UE.UIText], [2, UE.UIText], [3, UE.UIItem], [4, UE.UIItem], [5, UE.UIItem]];
  }
  OnStart() {
    this.Sui = new GenericLayout_1.GenericLayout(this.GetVerticalLayout(0), this.Lfd, this.GetItem(5).GetOwner());
  }
  OnBeforeDestroy() {
    this._uo();
    this.huo = [];

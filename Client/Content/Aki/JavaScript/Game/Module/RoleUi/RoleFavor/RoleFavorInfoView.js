...
class RoleFavorInfoView extends UiViewBase_1.UiViewBase {
  constructor() {
    ...
    this.Tcd = () => {
      ...
      switch (this.Ecd.FavorContentType) {
        case 3:
          e = "FavorAction";
          break;
        case 1:
    ...
    this.tuo = () => {
      for (const e of this.k_o.GetLayoutItemList()) {
        for (const i of e.GetContentItemList()) {
          i.BindToggleFunction(this.bcd);
          i.BindButtonFunction(this.Rcd);
          this.F_o.push(i);
        }
      }
    };
    this.bcd = (e, i, t) => {
      if (e) {
        ...
        this.j_o = this.H_o;
        this.Z_o();
        this.OnContentItemToggleClick(t, this.Ecd);
      }
    };
    this.Rcd = (e, i) => {
      ...
      if (e.FavorContentType === 1 || r === 2) {
        if ((t = this.wcd[e.FavorContentType])?.PlayContent) {
          t.PlayContent(e, i);
        }
      }
    };
  ...
  async OnBeforeStartAsync() {
    ...
    this.k_o = new GenericLayout_1.GenericLayout(this.GetVerticalLayout(1), this.Lcd);
    this.euo();
    await this.k_o.RefreshByDataAsync(this.Mcd);
    this.tuo();
    if (this.Mcd.length > 0) {
      this.Ecd = this.Mcd[0].GetContentDataByIndex(0);
  ...
  OnBeforeShow() {
    ...
    if (this.Ecd && this.Ecd.FavorContentType === 4) {
  ...
  ShowItemByData(e) {
    ...
    } else if (i = this.wcd[e.FavorContentType]) {
      i.ShowItem(e);
    }
  }
  HandleLockItemData() {
    var e = this.Ecd.ConfigId;
    var i = this.Ecd.FavorContentType;
    let t = undefined;
    ...
    if (i === 3) {
      ...
      var s = r.GetFavorItemState(this.Ecd.ConfigId, i);
      if (s === 1) {
  ...
  ShowLockItem() {
    var e;
    this.z_o();
    this.Y_o.SetData(this.Ecd);
    if (this.Ecd?.FavorContentType === 4) {
      e = this.Ecd;
      this.$_o.SetData(e);
  ...
  OnContentItemToggleClick(e, i) {
    ...
    if (i.FavorContentType !== 1 && t !== 2) {
      this.HandleLockItemData();
    } else if ((t = this.wcd[i.FavorContentType]) && (t.ShowItem(i), t.PlayContent)) {
      t.PlayContent(i, e);
    }
  }
  OnAddEventListener() {

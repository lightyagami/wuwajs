...
class RoleDevWeaponViewItem extends UiPanelBase_1.UiPanelBase {
  ...
  async OnBeforeStartAsync() {
    this.gSd = new RoleDevWeaponDevItem_1.RoleDevWeaponDevItem();
    this.CSd = new RoleDevWeaponRecommendItem_1.RoleDevWeaponRecommendItem();
    await Promise.all([this.gSd.CreateThenShowByActorAsync(this.GetItem(0).GetOwner()), this.CSd.CreateThenShowByActorAsync(this.GetItem(1).GetOwner())]);
    this.gSd.OnClickBtnSwitch = () => {
      this.SetType(2);
    };
    this.CSd.OnClickBtnSwitch = () => {
      this.SetType(1);
    };
  }
  ...
  Refresh() {
    ...
    if (e === 1) {
      this.gSd.Refresh(this.Pe.DevItemData);
      this.GetItem(0).SetUIActive(true);
      this.GetItem(1).SetUIActive(false);
    }
    if (e === 2) {
      this.CSd.Refresh(this.Pe.RecommendItemData);

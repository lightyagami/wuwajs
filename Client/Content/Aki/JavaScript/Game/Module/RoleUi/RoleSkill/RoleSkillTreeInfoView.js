...
class RoleSkillTreeInfoView extends UiViewBase_1.UiViewBase {
  constructor() {
    ...
    this.edo = () => {
      ...
      var i = ConfigManager_1.ConfigManager.RoleSkillConfig.GetRoleSkillTreeConsume(this.Mmo, i + 1);
      if (i) {
        for (var [t, e] of i) {
      ...
      i = this.bmo.NodeType;
      if (i === 1 || i === 2) {
        RoleController_1.RoleController.SendPbUpLevelSkillRequest(this.dFe, this.Mmo);
      } else {
        RoleController_1.RoleController.SendRoleActivateSkillRequest(this.dFe, this.Mmo);
      }
    };
    this.tdo = () => {
  ...
  Update(i, t) {
    this.dFe = i;
    this.Mmo = t;
    this.bmo = ConfigManager_1.ConfigManager.RoleSkillConfig.GetSkillTreeNode(this.Mmo);
    this.wmo = this.bmo.SkillId;
    this.B9l = ModelManager_1.ModelManager.RoleModel.GetUpgradeSkillIdIfUpgraded(this.wmo, i);
    ...
    this.Nmo?.RefreshUiAsync(e.GetRoleId(), e.IsTrialRole(), true);
    this.Refresh();
    if (e.IsTrialRole()) {
      this.rdo();
    }
  ...
  Refresh() {
    if (!this.InAsyncLoading()) {
      switch (ConfigManager_1.ConfigManager.RoleSkillConfig.GetSkillTreeNode(this.Mmo).NodeType) {
        case 4:
          this.ndo();
          break;
        case 3:
  ...
  cdo() {
    var i;
    var t = this.Bmo.GetSkillTreeNodeState(this.bmo, this.dFe);
  ...
  RefreshRoleBackgroundMusicSwitchItem() {
    ...
    if (this.d1o) {
      ...
      if (i.BackgroundMusicSwitch && t !== undefined && !t.IsTrialRole() && t.GetRoleConfig().EnableOperateSelfBgm) {

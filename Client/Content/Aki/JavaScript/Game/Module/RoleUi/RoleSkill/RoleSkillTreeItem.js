...
class RoleSkillTreeItem extends UiPanelBase_1.UiPanelBase {
  ...
  GetSkillItemByIndex(e) {
    if (this.Edo) {
      var i = this.Edo.GetSkillTreeNodeConfig();
      if (i && e === i.NodeIndex) {
        return this.Edo;
      }
    }
    for (const r of this.Mdo) {
      for (const l of r.GetSkillNodeItems()) {
        if (l) {
          var t = l.GetSkillTreeNodeConfig();
          if (t && e === t.NodeIndex) {
            return l;
          }
        }
    ...
    for (const n of this.vdo.GetSkillNodeItems()) {
      if (n) {
        var s = n.GetSkillTreeNodeConfig();
        if (s && e === s.NodeIndex) {
          return n;
        }
      }

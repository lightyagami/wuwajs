...
const StrengthItemForAimisi_1 = require("./Strength/StrengthItemForAimisi");
const strengthItemConfigMap = new Map([[1, StrengthItem_1.StrengthItem], [2, FlyStrengthItem_1.FlyStrengthItem], [3, MotorcycleStrengthItem_1.MotorcycleStrengthItem], [4, StrengthItemForAimisi_1.StrengthItemForAimisi]]);
...
const swapIndex = [1, 0];
const specialNormalIndex = [2, 1];
const specialSwapIndex = [1, 2];
class StrengthUnit extends HudUnitBase_1.HudUnitBase {
  ...
  AddStrengthItem(t, i) {
    if (this.NRl.has(t)) {
      ...
      if (this.VRl[i] !== 0) {
        if (i === 0) {
          if (Log_1.Log.CheckError()) {
            Log_1.Log.Error("HudUnit", 17, "该位置已有其他体力条", ["oldStrengthItemType", this.VRl[i]], ["newStrengthItemType", t]);
          }
          return;
        }
        var e = this.NRl.get(this.VRl[i]);
        if (e) {
          if (Log_1.Log.CheckDebug()) {
            Log_1.Log.Debug("HudUnit", 17, "该位置已有其他体力条, 销毁旧体力条", ["oldStrengthItemType", this.VRl[i]], ["newStrengthItemType", t]);
          }
          e.Destroy();
          this.NRl.delete(this.VRl[i]);
        }
      }
      var e = strengthItemConfigMap.get(t);
      if (e) {
        this.VRl[i] = t;
        i = this.x5e[i];
        (e = new e()).Init(i, this.Wst, this.HRl);
        this.NRl.set(t, e);
  ...
  SetStandardBarMode(t = true) {
    var i;
    if (this.DMg !== t) {
      this.DMg = t;
      (i = (t, i) => {
        t = this.VRl[t];
        this.NRl.get(t)?.SetEnableStrengthItem(i);
      })(normalIndex[0], t);
      i(specialNormalIndex[0], !t);
    }
  }
  SwapPlace(t) {
    if (this.FRl !== t) {
  ...
  jRl() {
    ...
    if (t) {
      ...
      let i = 0;
      let e = true;
      for (const r of t = this.DMg ? this.FRl ? swapIndex : normalIndex : this.FRl ? specialSwapIndex : specialNormalIndex) {
        var s = this.x5e[r];
        var h = this.VRl[r];
        if (this.NRl.get(h)?.GetUiVisible()) {
          s.SetAnchorOffsetX(ITEM_OFFSET_X_BASE + ITEM_OFFSET_X_INTERVAL * i);
          s.SetAlpha(e ? 1 : 0.3);
  ...
  WRl(t) {
    ...
    if (this.GetActive() && this.n$t && this.n$t.Actor?.IsValid() && (i = this.n$t.ActorLocation, HudUnitUtils_1.HudUnitUtils.PositionUtil.ProjectWorldToScreen(i, this.jma))) {
      i = this.jma.X;
      e = this.jma.Y;
      if (Math.abs(i - this.Rii) > MAX_POS_OFFSET || Math.abs(e - this.Uii) > MAX_POS_OFFSET) {
        this.Rii = i;
        this.Uii = e;
        this.SetAnchorOffset(this.Rii, this.Uii);
      } else {
        this.Aii = this.jii(t, i, this.Rii, this.Aii);
        this.Pii = this.jii(t, e, this.Uii, this.Pii);
        i = this.Aii * t;
        e = this.Pii * t;
        if (!(i < MIN_DELTA_OFFSET) || !(i > -MIN_DELTA_OFFSET) || !(e < MIN_DELTA_OFFSET) || !(e > -MIN_DELTA_OFFSET)) {
          this.Rii += i;
          this.Uii += e;
          this.SetAnchorOffset(this.Rii, this.Uii);
        }

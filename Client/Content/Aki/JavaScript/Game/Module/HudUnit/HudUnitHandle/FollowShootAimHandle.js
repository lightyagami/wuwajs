...
class FollowShootAimHandle extends HudUnitHandleBase_1.HudUnitHandleBase {
              ...
              if (!(h <= 0)) {
                for (let i = 0; i < h; i++) {
                  ...
                  if (_ !== Protocol_1.Aki.Protocol.kks.Proto_Player && _ !== Protocol_1.Aki.Protocol.kks.Proto_Npc) {
                    if (_ === Protocol_1.Aki.Protocol.kks.Proto_SceneItem && a.GetBaseInfo()?.Category.MechanismType === "PortalCreater" && t.Entity.GetComponent(165) !== undefined) {
  ...
  iKa(t) {
    if (t !== this.sDe) {
      ...
      if (t?.Valid) {
        this.sDe = t;
        this.Xte = this.sDe.Entity?.GetComponent(217);
        this.c$e();

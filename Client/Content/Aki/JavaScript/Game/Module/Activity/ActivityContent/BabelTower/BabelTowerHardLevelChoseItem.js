"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.BabelTowerHardLevelChoseItem = undefined;
const UE = require("ue");
const MathUtils_1 = require("../../../../../Core/Utils/MathUtils");
const StringUtils_1 = require("../../../../../Core/Utils/StringUtils");
const TimeUtil_1 = require("../../../../Common/TimeUtil");
const ConfigManager_1 = require("../../../../Manager/ConfigManager");
const ModelManager_1 = require("../../../../Manager/ModelManager");
const RedDotController_1 = require("../../../../RedDot/RedDotController");
const UiManager_1 = require("../../../../Ui/UiManager");
const LevelSequencePlayer_1 = require("../../../Common/LevelSequencePlayer");
const GridProxyAbstract_1 = require("../../../Util/Grid/GridProxyAbstract");
const GenericLayout_1 = require("../../../Util/Layout/GenericLayout");
const LguiUtil_1 = require("../../../Util/LguiUtil");
class BabelTowerHardLevelChoseItem extends GridProxyAbstract_1.GridProxyAbstract {
  constructor() {
    super(...arguments);
    this.gQl = false;
    this.dEc = false;
    this.yq = 0;
    this.tFe = undefined;
    this.OnClickButtonCallBack = undefined;
    this.fbc = undefined;
    this.nqe = e => {
      if (this.gQl) {
        this.OnClickButtonCallBack?.(this.yq);
      }
      this.fbc.StopCurrentSequence();
    };
    this.ifc = e => {
      UiManager_1.UiManager.OpenView("BabelTowerResetView", this.yq);
    };
    this.nFe = () => new RoleItem();
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIButtonComponent], [3, UE.UIText], [2, UE.UIText], [1, UE.UITexture], [5, UE.UIItem], [4, UE.UIText], [12, UE.UIItem], [6, UE.UIItem], [7, UE.UIText], [8, UE.UIHorizontalLayout], [9, UE.UIItem], [10, UE.UIItem], [11, UE.UIButtonComponent], [13, UE.UIItem], [14, UE.UIText], [15, UE.UITexture], [16, UE.UITexture], [17, UE.UIItem]];
    this.BtnBindInfo = [[0, this.nqe], [11, this.ifc]];
  }
  OnStart() {
    this.tFe = new GenericLayout_1.GenericLayout(this.GetHorizontalLayout(8), this.nFe);
    this.fbc = new LevelSequencePlayer_1.LevelSequencePlayer(this.RootItem);
  }
  Refresh(e, t, i) {
    this.GetText(2).SetText(i + 1 + "");
    this.yq = e.ELl;
    this.GetItem(9).SetUIActive(false);
    this.GetItem(10).SetUIActive(false);
    let s = 1;
    for (const o of e.Dt1) {
      const r = ConfigManager_1.ConfigManager.BabelTowerConfig.GetBabelTowerBuff(o);
      if (s === 1) {
        this.GetItem(9).SetUIActive(true);
        this.SetTextureByPath(r.Texture, this.GetTexture(15));
      }
      if (s === 2) {
        this.GetItem(10).SetUIActive(true);
        this.SetTextureByPath(r.Texture, this.GetTexture(16));
      }
      s++;
    }
    const r = ConfigManager_1.ConfigManager.BabelTowerConfig.GetBabelTowerLevelConfig(e.ELl);
    LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(3), r.NameText);
    if (StringUtils_1.StringUtils.IsEmpty(r.DesText)) {
      this.GetText(4).SetUIActive(false);
    } else {
      this.GetText(4).SetUIActive(true);
      LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(4), r.DesText);
    }
    this.SetTextureByPath(r.Texture, this.GetTexture(1));
    i = e.dMs;
    this.GetItem(5).SetUIActive(i);
    this.GetItem(6).SetUIActive(!i);
    this.GetButton(11).RootUIComp.SetUIActive(i);
    if (i) {
      this.GetText(7).SetText(e.jX_ + "");
      this.tFe?.RefreshByData(e.xt1);
    }
    i = MathUtils_1.MathUtils.LongToNumber(e.yzs);
    this.gQl = i <= TimeUtil_1.TimeUtil.GetServerTimeStamp();
    this.GetItem(12).SetUIActive(this.gQl);
    this.GetItem(13).SetUIActive(!this.gQl);
    if (!this.gQl) {
      e = TimeUtil_1.TimeUtil.GetRemainTimeDataFormat((i - TimeUtil_1.TimeUtil.GetServerTimeStamp()) * TimeUtil_1.TimeUtil.Millisecond);
      this.GetText(14).SetText(e.CountDownText);
    }
    if (this.dEc) {
      RedDotController_1.RedDotController.UnBindGivenUi("BabelTowerNewLevel", this.GetItem(17), this.yq);
    } else {
      this.dEc = true;
    }
    RedDotController_1.RedDotController.BindRedDot("BabelTowerNewLevel", this.GetItem(17), undefined, this.yq);
    if (ModelManager_1.ModelManager.BabelTowerModel.LevelChoseHandle === this.yq) {
      this.PlayLoopSequence();
    } else {
      this.StopLoopSequence();
    }
  }
  OnBeforeDestroy() {
    RedDotController_1.RedDotController.UnBindGivenUi("BabelTowerNewLevel", this.GetItem(17), this.yq);
  }
  DisableButton() {
    this.GetButton(11).RootUIComp.SetUIActive(false);
    this.GetItem(12).SetUIActive(false);
  }
  PlayLoopSequence() {
    if (this.fbc.IsPlayingSequence("Loop_item")) {
      this.fbc.ReplaySequenceByKey("Loop_item");
    } else {
      this.fbc.PlayLevelSequenceByName("Loop_item");
    }
  }
  StopLoopSequence() {
    this.fbc.StopCurrentSequence(false, true);
  }
  IsPlayingSequence() {
    return this.fbc.IsPlayingSequence("Loop_item");
  }
}
exports.BabelTowerHardLevelChoseItem = BabelTowerHardLevelChoseItem;
class RoleItem extends GridProxyAbstract_1.GridProxyAbstract {
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIButtonComponent], [2, UE.UITexture]];
  }
  Refresh(e, t, i) {
    var s = ConfigManager_1.ConfigManager.RoleConfig.GetRoleConfig(e);
    this.SetRoleIcon(s.Card, this.GetTexture(2), e);
  }
}
//# sourceMappingURL=BabelTowerHardLevelChoseItem.js.map
...
exports.DockyardItemBlockData = undefined;
...
class DockyardItemBlockData {
  constructor(t) {
    ...
    this.Data = undefined;
    this.Data = t;
    this.fYl = DockyardPanelUtil_1.DockyardPanelUtil.RotateOriginalPosData(this.EYl, t.Rotate);
  ...
  IYl(t, i) {
    let s = 0;
    let e = 0;
    e = (this.IsInUpOrDown() ? (s = this.EYl.length, this.EYl[0]) : (s = this.EYl[0].length, this.EYl)).length;
  ...
  RYl() {
    if (this.Rotate === Protocol_1.Aki.Protocol.mR_.Proto_No) {
      this.MYl.Set(this.SYl.X, this.SYl.Y);
    }
    if (this.Rotate === Protocol_1.Aki.Protocol.mR_.Proto_DirectionDown) {
      this.MYl.Set(this.SYl.Y, 1 - this.SYl.X);
    }
    if (this.Rotate === Protocol_1.Aki.Protocol.mR_.Proto_DirectionLeft) {
      this.MYl.Set(1 - this.SYl.X, 1 - this.SYl.Y);
    }
    if (this.Rotate === Protocol_1.Aki.Protocol.mR_.Proto_DirectionUp) {
  ...
  GetTopLeftOffsetByRotate() {
    this.wPr.Set(0, 1);
    this.MYl = this.RYl();
    return this.wPr.SubtractionEqual(this.MYl);
  }
  RotateData() {

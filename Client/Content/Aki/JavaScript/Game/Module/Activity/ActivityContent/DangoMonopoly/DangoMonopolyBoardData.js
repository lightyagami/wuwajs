...
class DangoMonopolyBoardData {
  constructor(t, i) {
    this.Id = 0, this.GroupId = 0, this.Index = 0, this.IsRewarded = !1, this.RewardItemId = 0, this.RewardItemCount = 0, this.FinishTitle = "", this.FinishDesc = "", this.GridList = [], this.GridMap = new Map, this.ActivityData = void 0, this.UnlockTime = 0, this.RecordRollDiceTimes = 0, this.RecordTriggerBuffMap = new Map, this.OwnedBuffIdList = [], this.Id = t, this.Index = i
  }
  static Create(t, i) {
  ...
  AU(t) {
    this.RewardItemId = t.ItemId, this.RewardItemCount = t.ItemNum, this.GroupId = t.BoardGroupId, this.FinishTitle = t.FinishTitle, this.FinishDesc = t.FinishDesc, ConfigManager_1.ConfigManager.ActivityDangoMonopolyConfig.GetGridList(t.GridGroupId).forEach((t, i) => {
  ...
  IsFinish() {
    ...
    if (t?.IsEqualIndex(this.Index) && !this.GetCurrentGridData()?.IsLessIndex(this.GridList.length - 1)) return !0;
    return !1
  }
  IsRunning() {
  ...
  GetDangoBuffShowList() {
    ...
    return this.GridList.forEach(t => {
      var i, r, e = t.GetDangoData();
      e && (i = e.Id, r = t.GetAddPropertyConfig(), s.push({
        DangoId: i,
        PropertyId: r?.Id ?? 0,
        GridId: t.Id,
  ...
  GetFinishGridNum() {
    var t;
    return this.IsFinish() ? this.GridList.length : (t = this.GetCurrentGridData()) ? t.GetPosition() : 0
  }
  InitStartMoveDango(t) {
    ...
    return this.ActivityData?.UpdateTargetGrid(s?.Id), Log_1.Log.CheckDebug() && Log_1.Log.Debug("DangoMonopoly", 69, "InitStartMoveDango=>初始化移动", ["骰子最终结果(包含特性之后)", t], ["实际移动步数", e], ["起点Index", i], ["目标格子Id", s.Id], ["目标格子Index", r], ["目标格子位置", s.GetPosition()]), e
  }
  MoveDangoOneStep() {
    var t, i;
    return !!this.IsRunning() && (i = this.ActivityData?.RunningGridData?.Id ?? 0, t = this.ActivityData?.TargetGridData?.Id ?? 0, i = Math.min(i + 1, t), this.ActivityData?.UpdateRunningGrid(i), !0)
  }
  IsMoveToTarget() {
  ...
  GetUnlockRemainTime() {
    return this.UnlockTime <= 0 ? 0 : Math.max(0, this.UnlockTime - TimeUtil_1.TimeUtil.GetServerTime())
  }
  UpdateUnlockTime(t) {
  ...
  AddRecordTriggerBuff(t) {
    var i;
    t <= 0 || (i = this.RecordTriggerBuffMap.get(t) ?? 0, this.RecordTriggerBuffMap.set(t, i + 1), Log_1.Log.CheckDebug() && Log_1.Log.Debug("DangoMonopoly", 69, "添加触发的特性记录", ["棋盘id", this.Id], ["特性id", t], ["次数", i + 1]))
  }
  ClearRecordTriggerBuff() {
    this.RecordTriggerBuffMap.clear(), Log_1.Log.CheckDebug() && Log_1.Log.Debug("DangoMonopoly", 69, "清理触发的特性记录", ["棋盘id", this.Id])
  }
  GetRecordTriggerBuffTotalTimes() {
    let r = 0;
    return this.RecordTriggerBuffMap.forEach((t, i) => {

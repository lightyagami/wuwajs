...
class ActivitySubViewBeginnerBook extends ActivitySubViewBase_1.ActivitySubViewBase {
  constructor() {
    super(...arguments), this.mNe = void 0, this.dNe = !0, this.CNe = void 0, this.gNe = void 0, this.sGe = () => {
  ...
  OnStart() {
    this.GetText(0).SetText(this.CNe.GetTitle()), this.gNe = new GenericLayout_1.GenericLayout(this.GetVerticalLayout(2), this.sGe, this.GetItem(3).GetOwner());
    var i = 0 === ModelManager_1.ModelManager.PlayerInfoModel.GetPlayerGender();
    this.GetItem(4)?.SetUIActive(i), this.GetItem(5)?.SetUIActive(!i), this.mNe = this.GetText(1), this.dNe = 0 !== this.CNe.EndShowTime, this.mNe.SetUIActive(0 !== this.CNe.EndShowTime), this.fNe()
  }
  async OnBeforeShowSelfAsync() {
    await ActivityManager_1.ActivityManager.GetActivityController(this.CNe.Type).NewJourneyRequest()
  }
  OnRefreshView() {
    this.gNe?.RefreshByData(this.CNe.AllBeginnerTargetList, () => {
      var i = this.gNe?.GetLayoutItemList(),
        t = this.ActivityBaseData;
      for (const e of i) e.SetEnableJump(t.GetEnableJump(e.DataId)), e.SetFinish(t.GetFinishState(e.DataId))
    })
  }
  pNe(i) {

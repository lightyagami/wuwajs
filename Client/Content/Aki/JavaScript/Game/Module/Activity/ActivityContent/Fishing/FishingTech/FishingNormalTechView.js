...
exports.FishingNormalTechView = undefined;
const UE = require("ue");
...
const FishingNormalTechCostItem_1 = require("./FishingNormalTechCostItem");
const FishingNormalTechViewLevelUpItem_1 = require("./FishingNormalTechViewLevelUpItem");
const FishingTechAreaItem_1 = require("./FishingTechAreaItem");
const FishingTechNodeItem_1 = require("./FishingTechNodeItem");
const TECH_NODE_AREA_ONE = 1;
const TECH_NODE_AREA_TWO = 2;
const TECH_NODE_AREA_THREE = 3;
class FishingNormalTechView extends UiTabViewBase_1.UiTabViewBase {
  constructor() {
    ...
    this.z9_ = 0;
    this.jbe = (i, e) => {
      if (!this.z9_ || ModelManager_1.ModelManager.FishingModel.GetFishingTechUnlock(this.z9_)) {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIItem], [1, UE.UIItem], [2, UE.UIItem], [3, UE.UIItem], [4, UE.UIItem], [5, UE.UIHorizontalLayout], [6, UE.UIItem], [7, UE.UIItem], [8, UE.UIScrollViewWithScrollbarComponent], [9, UE.UIItem]];
  }
  async OnBeforeStartAsync() {
    ...
    this.$a_ = new FishingNormalTechViewLevelUpItem_1.FishingNormalTechViewLevelUpItem();
    i.push(this.$a_.CreateThenShowByActorAsync(this.GetItem(7).GetOwner()));
    this.Xa_ = new GenericLayout_1.GenericLayout(this.GetHorizontalLayout(5), this.sGe);
    await Promise.all(i);
  ...
  OnStart() {
    ...
    var s = ModelManager_1.ModelManager.FishingModel.GetLastNode();
    this.Ka_?.RefreshNode(s);
    let h = undefined;
    if ((h = this.ExtraParams ? ModelManager_1.ModelManager.FishingModel.GetNormalTechNodeById(this.ExtraParams) : ModelManager_1.ModelManager.FishingModel.GetFirstUnlockNode()) && (h.Area === FishingDefine_1.FISHING_TECH_FIRST_NODE_AREA ? this.ja_.SelectNode() : h.Area === FishingDefine_1.FISHING_TECH_LAST_NODE_AREA ? this.Ka_.SelectNode() : (this.Ha_?.FindAndSelectNode(h), this.Wa_?.FindAndSelectNode(h), this.Qa_?.FindAndSelectNode(h)), h.Area >= FishingDefine_1.FISHING_TECH_BEHIND_NODE_AREA)) {

...
exports.ScratchTicketCellItem = undefined;
const UE = require("ue");
...
class ScratchTicketCellItem extends GridProxyAbstract_1.GridProxyAbstract {
  constructor() {
    ...
    this.Hvl = () => {
      this.Hqe(this.inl);
      this.UiLevelSequence.PlaySequence("RevaelB");
    };
    this.lRo = () => {
      if (this.rMt) {
        this.rMt(this.inl);
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIButtonComponent], [1, UE.UISprite], [2, UE.UIItem], [3, UE.UIItem]];
  ...
  async OnBeforeStartAsync() {
    ...
    await this.sft.CreateThenShowByResourceIdAsync("UiItem_ItemBaseB", this.GetItem(2));
  ...
  RefreshByResultData(e, t) {
    this.inl = e;
    if (t.SequenceType !== 6 && t.SequenceType !== 7) {
      this.Hqe(this.inl);
    }

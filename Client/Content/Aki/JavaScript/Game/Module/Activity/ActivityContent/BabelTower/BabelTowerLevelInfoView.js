...
exports.BabelTowerLevelInfoView = undefined;
const UE = require("ue");
...
class BabelTowerLevelInfoView extends UiViewBase_1.UiViewBase {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIItem], [1, UE.UIButtonComponent], [2, UE.UIArtText], [3, UE.UIItem], [4, UE.UIText], [5, UE.UIItem], [6, UE.UIItem], [7, UE.UIItem], [8, UE.UIItem], [9, UE.UIButtonComponent]];
    this.BtnBindInfo = [[9, this.L3e], [1, this.koc]];
  }
  async OnBeforeStartAsync() {
    ...
    await this.Hoc.CreateThenShowByActorAsync(this.GetItem(5).GetOwner());
    this.Hoc.OnClickBtnCallBack = this.zal;
    this.$oc = new BabelTowerLevelBuffItem_1.BabelTowerLevelBuffItem();
    await this.$oc.CreateThenShowByActorAsync(this.GetItem(6).GetOwner());
  ...
  D5t() {
    ...
    var t = ConfigManager_1.ConfigManager.InstanceDungeonConfig.GetConfig(i)?.FightFormationId;
    var t = ConfigManager_1.ConfigManager.EditBattleTeamConfig.GetFightFormationConfig(t);
    var r = ModelManager_1.ModelManager.RoleModel.GetRoleList();
    var o = [];
    var a = BabelTowerController_1.BabelTowerController.GetBabelTowerData().RoleLockData.get(this.joc.BabelTowerLevelId) ?? [];
    for (const f of r) {
      if (f.GetRoleId() !== 0) {
        e = f.GetRoleConfig().WeaponType;
        e = MultiTeamRoleSelectView_1.MultiTeamRoleGridData.Phrase(f, false, false, a.length > 0 && !a.includes(e));
        o.push(e);
      }
    }
    var s = [];
    ...
    var n = [];
    for (const M of ConfigManager_1.ConfigManager.RoleConfig.GetMainRoleByGender(t === 1 ? LoginDefine_1.ELoginSex.Boy : LoginDefine_1.ELoginSex.Girl)) {
      n.push(M.Id);
    }
    for (const g of r) {
      var l = ConfigManager_1.ConfigManager.RoleConfig.GetTrialRoleConfigByGroupId(g);
      if (!ModelManager_1.ModelManager.RoleModel.IsMainRole(l.ParentId) || !!n.includes(l.ParentId)) {
        if (l = ModelManager_1.ModelManager.RoleModel.GetRoleDataById(l.Id)) {
          s.push(MultiTeamRoleSelectView_1.MultiTeamRoleGridData.Phrase(l, false, false));
        }
      }
    }
    ...
    var r = new Array();
    if (s.length > 0) {
      h = MultiTeamRoleSelectView_1.MultiTeamRoleData.Phrase("BossRushTrailRole", s);
      r.push(h);
    }
    r.push(t);
    var h = this.joc.RoleList;
    var _ = [];
    for (const u of h) {
      if (u !== 0) {
        _.push(u);
    ...
    t = MultiTeamRoleSelectView_1.MultiTeamRoleSelectData.Phrase(5, 3, _, undefined, this.mEc, this.X4t, undefined, r, [], "BabelTeamTips_1");
  ...
  aqe() {
    ...
    var e;
    var i = ConfigManager_1.ConfigManager.BabelTowerConfig.GetBabelTowerLevelConfig(this.joc.BabelTowerLevelId);
    var i = ModelManager_1.ModelManager.BabelTowerModel.CalculateDifficultyConfigByStarNum(i.ActivityId, this.joc.StarNumber);
    if (i) {
      e = UE.Color.FromHex(i.TextBgColor);
      this.GetItem(3).SetColor(e);
      LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(4), i.DifficultyTextKey);
    }
  }
  Og() {
    this.XSn();

...
class ActivityBlackCoastData extends ActivityData_1.ActivityBaseData {
  constructor() {
    super(...arguments), this.bja = new Map, this.ROe = new Map, this.qja = 0, this.Oja = 0, this.y7s = (t, e) => t.Goal - e.Goal
  }
  OnInit(t) {
  ...
  PhraseEx(t) {
    ...
    if (t) {
      ...
      for (const r of t.rM_) {
        ...
        e ? e.Achieved = !0 : Log_1.Log.CheckWarn() && Log_1.Log.Warn("Activity", 37, "[BlackCoastActivity] 奖励Id不存在", ["RewardId", r])
      }
    }
  }
  ...
  InitProgressReward() {
    ...
    var t = ConfigManager_1.ConfigManager.ActivityBlackCoastConfig.GetActivityConfig(this.Id),
      t = (this.qja = t.ItemId, ConfigManager_1.ConfigManager.ActivityBlackCoastConfig.GetAllRewardConfigByActivityId(this.Id));
    for (const r of t) {
      var e = new ActivityBlackCoastDefine_1.BlackCoastProgressRewardData;
      e.Id = r.Id, e.Goal = r.Active, e.DropId = r.DropId, e.GetCurrentGoal = () => this.GetProgressItemCount(), this.Oja = Math.max(this.Oja, e.Goal), this.bja.set(r.Id, e)
    }
  }
  ...
  GetAllAvailableProgressRewardIds() {
    var t = [];
    for (const e of this.GetAllProgressRewardData()) 0 === e.GetState() && t.push(e.Id);
    return t
  }
  ...
  HasProgressRewardRedDot() {
    for (const t of this.bja.values())
      if (0 === t.GetState()) return !0;
    return !1
  }
  SetTaskRewardGot(t, e) {
    this.ROe.get(t).SetTaskRewardGot(e), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RefreshCommonActivityRedDot, this.Id)
  }
  Gja() {
    this.ROe.clear();
  ...
  StageUpdate(t) {
    for (const r of t) {
      ...
      e ? e.StageUpdate(r) : Log_1.Log.CheckWarn() && Log_1.Log.Warn("Activity", 37, "[BlackCoastActivity] 活动Stage不存在", ["Id", r.s5n])
    }
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RefreshCommonActivityRedDot, this.Id)
  }
  HasStageRewardRedDot() {
    for (const t of this.ROe.values())
      if (t.GetRewardState()) return !0;
    return !1
  }
  HasAnyNewStageRedDot() {
    for (const t of this.ROe.values())
      if (1 === t.StageState)
        if (!ModelManager_1.ModelManager.ActivityModel.GetActivityCacheData(this.Id, 0, t.StageId, 0, 0)) return !0;
    return !1
  }
  HasNewStageFlag(t) {
    return 1 === this.ROe.get(t).StageState && !ModelManager_1.ModelManager.ActivityModel.GetActivityCacheData(this.Id, 0, t, 0, 0)
  }
  SaveNewStageFlag(t) {
  ...
  GetCurrentLockQuestId() {
    for (const t of this.GetAllStages())
      if (!t.IsUnlock) return ConfigManager_1.ConfigManager.ActivityBlackCoastConfig.GetStageConfig(t.StageId).QuestionId
  }
  GetPreviewWeaponDataList() {
    var t = [],
      e = ConfigManager_1.ConfigManager.ActivityBlackCoastConfig.GetActivityConfig(this.Id);
    if (e)

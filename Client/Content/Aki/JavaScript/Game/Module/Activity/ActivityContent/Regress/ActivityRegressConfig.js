...
class ActivityRegressConfig extends ConfigBase_1.ConfigBase {
  GetRegressSignRewards(e, r) {
    ...
    return void 0 === i && Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "回归活动->获取回流签到奖励配置失败,请检查配置表RegressSignReward", ["activityId:", e], ["grade:", r]), i
  }
  GetRegressQuestionnaireConfig(e) {
    var r = LauncherNetworkDetectionController_1.LauncherNetworkDetectionController.IsGlobalPlayer(),
      i = RegressInvestigationByInvestigationTypeAndIfGlobal_1.configRegressInvestigationByInvestigationTypeAndIfGlobal.GetConfig(e, r);
    return void 0 === i && Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "回归活动->获取回流调查问卷配置失败,请检查配置表RegressInvestigation", ["type:", e], ["ifGlobal:", r]), i
  }
  GetRegressBaseConfigListByType(e) {
    var r;
    if (4 !== e) return void 0 === (r = RegressBaseByEntryType_1.configRegressBaseByEntryType.GetConfigList(Number(e))) && Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "[回流活动]ActivityRecallConfig.GetRecallBaseConfigByType->", ["获取回流基础配置失败,请检查配置表RegressBase: entryType:", e]), r;
    Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "[回流活动]ActivityRecallConfig.GetRecallBaseConfigByType->", ["获取回流基础配置失败,请检查传入参数,请传入新角色1类型来获取配置，EntryType:", e])
  }
  GetRewardConfig(e) {
    var r = RewardConfigById_1.configRewardConfigById.GetConfig(e);
    if (void 0 !== r) return r;
    Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "[回流活动]ActivityRecallConfig.GetRewardConfig->", ["获取回归奖励配置失败,请检查配置表奖励档次|RewardConfig: rewardGroupId:", e])
  }
  ...
  GetSortedOpenRegressEntryConfigList() {
    ...
    return e.sort((e, r) => {
      ...
      const i = ModelManager_1.ModelManager.ActivityRegressModel.CheckIfEntryOpen(e)[0] ? 1 : 0,
        t = ModelManager_1.ModelManager.ActivityRegressModel.CheckIfEntryOpen(r)[0] ? 1 : 0;
      return t === i ? e.Id - r.Id : t - i
    }), e
  }
  GetSortedOpenRegressBaseConfigList() {
    var e = [];
    for (const r of ModelManager_1.ModelManager.ActivityRegressModel.GetLastestRegressBaseConfigList(3)) ModelManager_1.ModelManager.ActivityRegressModel.CheckIfEntryOpen(r)[0] && e.push(r);
    return e
  }
  ...
  GetRegressEntryConfigByType(e) {
    if (4 === e) Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "[回流活动]ActivityRecallConfig.GetRegressEntryConfigByType->", ["获取回流配置失败,请检查传入参数,请传入新角色1类型来获取配置,RegressEntry:entryType:", e]);
    else {
      var r = RegressEntryByEntryType_1.configRegressEntryByEntryType.GetConfigList(Number(e));
      if (void 0 !== r) return r;
      Log_1.Log.CheckError() && Log_1.Log.Error("ActivityRecall", 63, "[回流活动]ActivityRecallConfig.GetRegressEntryConfigByType->", ["获取回流配置失败,请检查配置表RegressEntry: entryType:", e])
    }
  }
  GetUnlockRegressEntryViewConfigList() {
    var e, r = [],
      ...
      [n] = ModelManager_1.ModelManager.ActivityRegressModel.CheckIfEntryOpen(i);
    let o = !1;
    return void 0 !== t && ([e] = ModelManager_1.ModelManager.ActivityRegressModel.CheckIfEntryOpen(t), o = e), (n || o) && (n && r.push(i), !n) && o && r.push(t), r
  }

...
class ScratchTicketData extends ActivityData_1.ActivityBaseData {
  ...
  InitData(t) {
    this.Lo = ConfigManager_1.ConfigManager.ActivityScratchTicketConfig.GetScratchTicketConfig(this.Id), void 0 !== this.Lo && (this.wol(t.EM_), this.Bol(t.IM_))
  }
  wol(t) {
    if (!(0 < this.xol.length)) {
      ...
      this.xol.sort((t, i) => t.Config.PreRoundId - i.Config.PreRoundId), this.UpdateAllRoundState()
    }
  }
  UpdateAllRoundState() {
    for (let t = 0; t < this.xol.length; t++) {
      var i, r = this.xol[t];
      0 === t ? r.UpdateRoundState(2) : (i = this.xol[t - 1], r.UpdateRoundState(i.GetRoundState()))
    }
  }
  UpdateCellReward(t, i) {
    t = this.GetRoundDataById(t);
    void 0 !== t && (t.UpdateCellDataReward(i.TM_), t.UpdateRemainReward(i.MM_), this.UpdateAllRoundState())
  }
  Bol(t) {
  ...
  qol(t) {
    for (const i of this.Pol)
      if (i.Id === t) return i;
    Log_1.Log.CheckError() && Log_1.Log.Error("ScratchTicket", 58, "ScratchTicketData无法获取条件id", ["id", t])
  }
  ...
  GetFirstProgressRoundDataIndex() {
    ...
    if (i <= 0) return -1;
    for (let t = 0; t < i; t++)
      if (2 !== this.xol[t].GetRoundState()) return t;
    return i - 1
  }
  GetRoundDataIndex(i) {
    for (let t = 0; t < this.xol.length; t++)
      if (this.xol[t] === i) return t;
    return -1
  }
  ...
  IsAllRoundFinish() {
    for (const t of this.xol)
      if (2 !== t.GetRoundState()) return !1;
    return !0
  }
  HasRoundInProgress() {
    for (const t of this.xol)
      if (1 === t.GetRoundState()) return !0;
    return !1
  }
  GetRemainCount() {

...
class DockyardInteractViewModel {
  constructor() {
    this.Yzt = void 0, this.ConfigId = -1, this.ActionIncId = -1, this.IsComplete = !1, this.BackpackPanelModel = new DockyardInteractBackpackPanelModel_1.DockyardInteractBackpackPanelModel, this.InteractPanelModel = new DockyardInteractPanelModel_1.DockyardInteractPanelModel, this.JXl = void 0, this.CloseClick = () => {
  ...
  RotateClick() {
    this.BackpackPanelModel.IsOutOfRange ? this.InteractPanelModel.IsOutOfRange || this.InteractPanelModel.Panel.RotateClick() : this.BackpackPanelModel.Panel.RotateClick()
  }
  ConfirmClick() {
    var t, i;
    this.JXl && (this.JXl.CanConfirm() ? this.JXl.IsOverlap() ? this.JXl.Panel.HandleOverlapConfirm() : (i = this.LXl(), t = this.qr_(), i = {
      InteractId: this.ConfigId,
      LeftDataList: i,
      RightDataList: t,
      ActionIncId: this.ActionIncId,
      Callback: (t, i) => {
        this.IsComplete = i, t && this.JXl?.Panel.HandleFinishConfirm(), i && this.Yzt?.OpenInteractFinishTipsView()
      }
    }, ControllerHolder_1.ControllerHolder.FishingController.RequestFishingHandIn(i)) : this.JXl.ShowScrollingTips())
  }
  HandleDragBegin() {
    this.InteractPanelModel.LastCanTickState ? this.JXl = this.InteractPanelModel : this.BackpackPanelModel.IsOutOfRange || (this.JXl = this.BackpackPanelModel)
  }
  HandleDragResult() {
    this.BackpackPanelModel.IsOutOfRange ? this.InteractPanelModel.IsOutOfRange ? (this.InteractPanelModel.LastCanTickState ? (this.JXl = this.InteractPanelModel, this.InteractPanelModel.Panel) : this.JXl?.Panel).HandleDragFail() : (this.JXl = this.InteractPanelModel, this.InteractPanelModel.Panel.HandleDragSuccess()) : (this.JXl = this.BackpackPanelModel, this.BackpackPanelModel.Panel.HandleDragSuccess())
  }
  BackpackTick(t) {
  ...
  LXl() {
    ...
    return this.JXl === this.InteractPanelModel && (i = this.BackpackPanelModel.Panel.InSelectItemBlock.GetData(), t = this.JXl.Panel.GetLeftTopPanelPos(i.Data.ItemId), i = DockyardPanelUtil_1.DockyardPanelUtil.CreateFishingItemInfo(i.Data, i.Rotate, t), e.push(i)), e
  }
  qr_() {
  ...
  TXl() {
    var t, i = [];
    for (const e of this.InteractPanelModel.ShowItemMap.values()) e.Data.IncId !== this.BackpackPanelModel.InSelectedBlockId && (t = DockyardPanelUtil_1.DockyardPanelUtil.CreateFishingItemInfo(e.Data, e.Rotate, e.Pos), i.push(t));
    return i
  }
}

"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.BusinessTravelRoleItem = undefined;
const UE = require("ue");
const AudioSystem_1 = require("../../../../../../../../Core/Audio/AudioSystem");
const Protocol_1 = require("../../../../../../../../Core/Define/Net/Protocol");
const ConfigManager_1 = require("../../../../../../../Manager/ConfigManager");
const ModelManager_1 = require("../../../../../../../Manager/ModelManager");
const LevelSequencePlayer_1 = require("../../../../../../Common/LevelSequencePlayer");
const GridProxyAbstract_1 = require("../../../../../../Util/Grid/GridProxyAbstract");
const LguiUtil_1 = require("../../../../../../Util/LguiUtil");
class BusinessTravelRoleItem extends GridProxyAbstract_1.GridProxyAbstract {
  constructor() {
    super(...arguments);
    this.RoleId = 0;
    this.SequencePlayer = undefined;
    this.kva = i => {
      if (i === "Action03") {
        this.Nva();
        this.Fva(false);
        this.Vva(false);
        this.iTa(false);
        this.Hva();
        this.dfa(false);
      }
    };
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.SpineSkeletonAnimationComponent], [1, UE.UIItem], [2, UE.UIText], [3, UE.UIItem], [4, UE.UIItem], [5, UE.UIItem], [6, UE.UIItem], [7, UE.UIText], [8, UE.UIText], [9, UE.UIItem]];
  }
  OnStart() {
    this.SequencePlayer = new LevelSequencePlayer_1.LevelSequencePlayer(this.RootItem);
    this.SequencePlayer.BindSequenceCloseEvent(this.kva);
    this.GetItem(3)?.SetUIActive(false);
    this.GetItem(9)?.SetUIActive(false);
    this.GetItem(4)?.SetUIActive(false);
    this.GetItem(5)?.SetUIActive(false);
    this.GetItem(6)?.SetUIActive(false);
    this.GetItem(1)?.SetUIActive(false);
  }
  OnBeforeDestroy() {
    this.SequencePlayer.Clear();
  }
  VAn(i, t) {
    this.GetSpine(0).SetAnimation(0, i, true)?.SetMixDuration(t);
  }
  FAn(i) {
    this.GetItem(1)?.SetUIActive(true);
    LguiUtil_1.LguiUtil.SetLocalTextNew(this.GetText(2), i);
  }
  Nva() {
    this.GetItem(1)?.SetUIActive(false);
  }
  Fva(i) {
    this.GetItem(3)?.SetUIActive(i);
  }
  Vva(i) {
    this.GetItem(4)?.SetUIActive(i);
  }
  iTa(i) {
    this.GetItem(5)?.SetUIActive(i);
  }
  jva(i) {
    var t = ModelManager_1.ModelManager.MoonChasingBusinessModel.GetEditTeamDataById(this.RoleId);
    if (t.Level > i) {
      this.GetItem(6)?.SetUIActive(true);
      this.GetText(7)?.SetText(i.toString());
      this.GetText(8)?.SetText(t.Level.toString());
    }
  }
  Hva() {
    this.GetItem(6)?.SetUIActive(false);
  }
  dfa(i) {
    this.GetItem(9)?.SetUIActive(i);
  }
  Refresh(i) {
    this.RoleId = i;
  }
  async RefreshAsync() {
    var i = ConfigManager_1.ConfigManager.BusinessConfig.GetEntrustRoleById(this.RoleId);
    await this.SetSpineAssetByPath(i.SmallSpineAtlas, i.SmallSpineSkeletonData, this.GetSpine(0));
    this.GetSpine(0).SetAnimation(0, "idle", true);
  }
  PlayStartAction() {
    AudioSystem_1.AudioSystem.PostEvent("play_ui_zuiyuejie_loading");
    this.SequencePlayer.PlayLevelSequenceByName("Action01");
    this.VAn("working", 0.1);
    this.dfa(true);
  }
  PlayRunFinishAction(i, t) {
    AudioSystem_1.AudioSystem.ExecuteAction("play_ui_zuiyuejie_loading", 0);
    this.SequencePlayer.PlayLevelSequenceByName("Action02");
    var e = ConfigManager_1.ConfigManager.BusinessConfig.GetEntrustRoleById(this.RoleId);
    if (i === Protocol_1.Aki.Protocol.oA_.qGs) {
      AudioSystem_1.AudioSystem.PostEvent("play_ui_zhuiyuejie_positive");
      this.FAn(e.SuccessDialog);
      this.VAn("happy", 0);
      this.Fva(true);
    } else if (i === Protocol_1.Aki.Protocol.oA_.Proto_Bad) {
      AudioSystem_1.AudioSystem.PostEvent("play_ui_zhuiyuejie_passive");
      this.FAn(e.FailDialog);
      this.VAn("fail", 0);
      this.Vva(true);
    } else {
      AudioSystem_1.AudioSystem.PostEvent("play_ui_haoping");
      this.VAn("idle", 0);
      this.iTa(true);
    }
    this.jva(t);
  }
  PlayEndAction() {
    this.SequencePlayer.PlayLevelSequenceByName("Action03");
  }
  StopRunToLastFrame() {
    this.SequencePlayer.StopSequenceByKey("Action02", true, true);
  }
  GetRoleId() {
    return this.RoleId;
  }
}
exports.BusinessTravelRoleItem = BusinessTravelRoleItem;
//# sourceMappingURL=BusinessTravelRoleItem.js.map
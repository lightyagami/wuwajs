...
exports.DangoMonopolyRollDiceView = undefined;
const UE = require("ue");
...
const DangoMonopolyDiceBuffPanel_1 = require("./DangoMonopolyDiceBuffPanel");
...
class DangoMonopolyRollDiceView extends DangoMonopolyViewBase_1.DangoMonopolyViewBase {
  constructor() {
    ...
    this.IsRollDiceFinish = false;
    this.DiceBuffPanel = undefined;
    this.AutoWaitTime = 1000;
    this.RVc = () => {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIButtonComponent], [1, UE.UIText], [2, UE.UIButtonComponent], [3, UE.UIItem], [4, UE.UITexture], [5, UE.UIText], [6, UE.UIItem]];
    this.BtnBindInfo = [[0, this.RVc], [2, this.kVc]];
  }
  Es_() {
  ...
  async OnBeforeStartAsync() {
    ...
    this.DiceBuffPanel = new DangoMonopolyDiceBuffPanel_1.DangoMonopolyDiceBuffPanel();
  ...
  OnBeforeDestroy() {
    ...
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DangoMonopolyMoveStart);
    EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.DangoMonopolyMoveStepStartOrEnd, true);
  ...
  async PlayRollDice(t) {
    ...
    await i.Run();
    i.DiceOutlineBp?.SetActorHiddenInGame(false);
...
${t}`;
  ...
  DiceAnimEnd() {
    ...
    this.ActivityData.RollDice.DiceOutlineBp?.SetActorHiddenInGame(true);
    this.CloseMe();
  }
  async CheckTriggerBuff() {
    var t = this.OpenParam?.TriggerBuffId;
    if (t && this.ActivityData.BuffIsAfterRollDiceFire(t)) {
      this.ActivityData.OpenViewDangoTips(t);
      await this.PlayBuffEffect(t);
    }
  }
  ...
  async WaitAutoTime() {
    await TimerSystem_1.GameplayTimerSystem.Wait(this.AutoWaitTime);
  }
}

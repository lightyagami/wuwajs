...
class ActivityRegressDoubleDropChallengeItem extends UiPanelBase_1.UiPanelBase {
  ...
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [[0, UE.UIText], [1, UE.UIText], [2, UE.UIItem], [3, UE.UIItem], [4, UE.UIText], [5, UE.UIItem], [6, UE.UIText], [7, UE.UIItem]];
  }
  OnStart() {
  ...
  Hli() {
    ...
    if (this.EntryType === 1) {
      ...
      a = e?.HintText ?? "";
      r = ModelManager_1.ModelManager.AdventureGuideModel.GetIsDetectionPreOpenByDungeonType(21);
    }
    if (this.EntryType === 2) {
      e = t.WeekUnLock;
      t = ConfigManager_1.ConfigManager.ActivityRegressConfig.GetConditionGroup(e);
      a = t?.HintText ?? "";
      r = ModelManager_1.ModelManager.AdventureGuideModel.GetIsDetectionPreOpenByDungeonType(7);
    }

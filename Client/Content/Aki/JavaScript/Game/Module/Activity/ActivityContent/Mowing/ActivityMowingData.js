...
exports.ActivityMowingData = undefined;
...
class ActivityMowingData extends ActivityData_1.ActivityBaseData {
  constructor() {
    ...
    this.v2e = undefined;
    this.M2e = undefined;
    this.MowingLevelInfoDict = undefined;
    this.SNe = (t, e) => {
      var [, i] = this.E2e(t.RewardState);
      var [, r] = this.E2e(e.RewardState);
      if (i === r) {
        return t.Id - e.Id;
      } else {
        return i - r;
  ...
  IsNewInstanceOpen() {
    if (this.IsUnLock() && this.GetPreGuideQuestFinishState()) {
      for (var [, t] of this.MowingLevelInfoDict.entries()) {
        ...
        if (!ModelManager_1.ModelManager.ActivityModel.GetActivityCacheData(this.Id, 0, t.s5n, 0, 0) && e) {
          return true;
        }
      }
    }
  ...
  ReadNewInstance() {
    for (var [, t] of this.MowingLevelInfoDict.entries()) {
      if (t.Mps <= TimeUtil_1.TimeUtil.GetServerTime()) {
        ModelManager_1.ModelManager.ActivityModel.SaveActivityData(this.Id, t.s5n, 0, 0, 1);
      }
    }
  ...
  UpdateLevelRewards(t) {
    for (const r of t.Ips) {
      var e;
      var i;
      if (this.MowingLevelInfoDict.get(r.s5n)) {
        this.MowingLevelInfoDict?.set(r.s5n, r);
        e = r;
        i = KillMonstersScoresByInstanceID_1.configKillMonstersScoresByInstanceID.GetConfig(e.s5n);
        e = this.S2e(e, i);
        (i = this.M2e.get(r.s5n)).RewardState = e.RewardState;
        i.RewardButtonText = e.RewardButtonText;
      } else if (Log_1.Log.CheckDebug()) {
        Log_1.Log.Debug("Activity", 49, "后端推了一个不存在的割草副本数据", ["id", r.s5n.toString()]);
      }
    }
  }
  GetLevelDiffIndex(t) {
    ...
    if (e) {
      return KillMonstersScoresByInstanceID_1.configKillMonstersScoresByInstanceID.GetConfig(t).DifficultyOptions.indexOf(e.z6n);
    } else {
      if (Log_1.Log.CheckError()) {
        Log_1.Log.Error("Activity", 49, "当前[击杀积分|KillMonstersScores]没有割草活动副本数据");
      }
      return 0;
    }
  }
  GetLevelDiffRecommendLevel(t) {
    t = this.MowingLevelInfoDict.get(t);
    if (t) {
      return TakeWeedsDifficultyById_1.configTakeWeedsDifficultyById.GetConfig(t.z6n)?.RecommendedLevel ?? 0;
    } else {
      return 0;
    }
  }
  GetTotalPoint() {
    let t = 0;
    for (var [, e] of this.MowingLevelInfoDict) {
      t += e.Eps;
    }
    return t;
  }
  ...
  L2e(t) {
    ...
    var e = this.jNe(t);
    var t = TimeUtil_1.TimeUtil.GetCountDownDataFormat2(t, e[0], e[1]).CountDownText ?? "";

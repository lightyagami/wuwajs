"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.ActivityTurntableController = undefined;
const Protocol_1 = require("../../../../../Core/Define/Net/Protocol");
const Net_1 = require("../../../../../Core/Net/Net");
const EventDefine_1 = require("../../../../Common/Event/EventDefine");
const EventSystem_1 = require("../../../../Common/Event/EventSystem");
const ConfigManager_1 = require("../../../../Manager/ConfigManager");
const ControllerHolder_1 = require("../../../../Manager/ControllerHolder");
const ModelManager_1 = require("../../../../Manager/ModelManager");
const RewardItemData_1 = require("../../../ItemReward/RewardData/RewardItemData");
const ActivityControllerBase_1 = require("../../ActivityControllerBase");
const ActivitySubViewTurntable_1 = require("./ActivitySubViewTurntable");
const ActivitySubViewTurntableLock_1 = require("./ActivitySubViewTurntableLock");
const ActivityTurntableData_1 = require("./ActivityTurntableData");
const ActivityTurntableDefine_1 = require("./ActivityTurntableDefine");
class ActivityTurntableController extends ActivityControllerBase_1.ActivityControllerBase {
  constructor() {
    super(...arguments);
    this.DSe = (t, r) => {
      ModelManager_1.ModelManager.ActivityModel.GetCurrentActivitiesByType(Protocol_1.Aki.Protocol.uks.Proto_TurnTableActivity).forEach(e => {
        e.OnQuestStateChange(t, r);
      });
    };
    this.qdi = (t, r) => {
      ModelManager_1.ModelManager.ActivityModel.GetCurrentActivitiesByType(Protocol_1.Aki.Protocol.uks.Proto_TurnTableActivity).forEach(e => {
        e.OnCommonItemCountAnyChange(t, r);
      });
    };
    this.ZTl = t => {
      ModelManager_1.ModelManager.ActivityModel.GetCurrentActivitiesByType(Protocol_1.Aki.Protocol.uks.Proto_TurnTableActivity).forEach(e => {
        if (t.zS_) {
          e.RefreshTask(t.zS_, false);
        }
      });
    };
  }
  OnRegisterNetEvent() {
    Net_1.Net.Register(27638, this.ZTl);
  }
  OnUnRegisterNetEvent() {
    Net_1.Net.UnRegister(27638);
  }
  OnAddEvents() {
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.OnQuestStateChange, this.DSe);
    EventSystem_1.EventSystem.Add(EventDefine_1.EEventName.OnCommonItemCountAnyChange, this.qdi);
  }
  OnRemoveEvents() {
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.OnQuestStateChange, this.DSe);
    EventSystem_1.EventSystem.Remove(EventDefine_1.EEventName.OnCommonItemCountAnyChange, this.qdi);
  }
  OnOpenView(e) {}
  OnGetActivityResource(e) {
    if (e.GetPreGuideQuestFinishState()) {
      return "UiItem_ActivityTurntable";
    } else {
      return "UiItem_ActivityTurntableLock";
    }
  }
  OnCreateSubPageComponent(e) {
    return new (e.GetPreGuideQuestFinishState() ? ActivitySubViewTurntable_1.ActivitySubViewTurntable : ActivitySubViewTurntableLock_1.ActivitySubViewTurntableLock)();
  }
  OnCreateActivityData(e) {
    return new ActivityTurntableData_1.ActivityTurntableData();
  }
  OnGetIsOpeningActivityRelativeView() {
    return false;
  }
  static RequestTurntableRun(a) {
    var e = new Protocol_1.Aki.Protocol.sfs();
    e.w6n = a;
    Net_1.Net.Call(19942, e, e => {
      if (e) {
        if (e.Q4n !== Protocol_1.Aki.Protocol.Q4n.KRs) {
          ControllerHolder_1.ControllerHolder.ErrorCodeController.OpenErrorCodeTipView(e.Q4n, 19267);
        }
        var t = ModelManager_1.ModelManager.ActivityModel.GetActivityById(a);
        if (t) {
          var r = [];
          for (const i of Object.keys(e._vs)) {
            var n = [{
              IncId: 0,
              ItemId: Number.parseInt(i)
            }, e._vs[i]];
            r.push(n);
          }
          t.SetRunResult(e.N6n, r);
          EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.TurntableStartRun, e.N6n);
          EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.RefreshCommonActivityRedDot, a);
        }
      }
    });
  }
  static ShowTurntableItemObtain(e, t) {
    if (e.length !== 0) {
      var r = [];
      for (const i of e) {
        var n = new RewardItemData_1.RewardItemData(i[0].ItemId, i[1], i[0].IncId);
        r.push(n);
      }
      e = ConfigManager_1.ConfigManager.ItemRewardConfig.GetRewardViewFromSourceConfig(ActivityTurntableDefine_1.TURNTABLE_RESULT_DISPLAY_ID)?.RewardViewId;
      if (e) {
        ControllerHolder_1.ControllerHolder.ItemRewardController.OpenCommonRewardView(e, r, t);
      }
    }
  }
}
exports.ActivityTurntableController = ActivityTurntableController;
//# sourceMappingURL=ActivityTurntableController.js.map
...
class TsAnimNotifyStateRotateToPlayer extends UE.KuroAnimNotifyState {
  ...
  K2_NotifyBegin(t, i, s) {
    this.Initialize();
    var t = t.GetOwner();
    return t instanceof TsBaseCharacter_1.default && !t.AbilitySystemComponent?.HasAnyGameplayTag(this.屏蔽标签列表) && !!(t = t.CharacterActorComponent?.Entity)?.Valid && (this.ParamsMap.has(t.Id) ? this.ParamsMap.get(t.Id).Update(0, s) : this.ParamsMap.set(t.Id, new AnsRotateParam(s)), true);
  }
  K2_NotifyTick(t, i, s) {
    t = t.GetOwner();
  ...
  UpdateRotationRole(t, i) {
    ...
    let r = s.NowTime * s.TotalDurationReciprocal;
    if (this.Curve) {
      e = this.Curve.GetFloatValue(MathUtils_1.MathUtils.Clamp(e, 0, 1));
      r = this.Curve.GetFloatValue(MathUtils_1.MathUtils.Clamp(r, 0, 1));
    }
    a = this.GetRotationRoleAngle(t) * MathUtils_1.MathUtils.Clamp((r - e) / (1 - e), 0, 1) / i;
    s = Math.sign(a) * MathUtils_1.MathUtils.Clamp(Math.abs(a), 0, this.旋转速度) * i;
    MathUtils_1.MathUtils.CommonTempQuat.DeepCopy(t.ActorQuatProxy);
    MathUtils_1.MathUtils.CommonTempRotator.Set(0, s, 0);
    MathUtils_1.MathUtils.CommonTempQuat.Multiply(MathUtils_1.MathUtils.CommonTempRotator.Quaternion(), this.TmpQuat);
    this.TmpQuat.Rotator(MathUtils_1.MathUtils.CommonTempRotator);
    t.SetActorRotation(MathUtils_1.MathUtils.CommonTempRotator.ToUeRotator(), "TsAnimNotifyStateRotate", false);
  ...
  GetRotationRoleAngle(t) {
    ...
    if (Global_1.Global.BaseCharacter?.IsValid()) {
      ...
      s.Normalize();
      return GravityUtils_1.GravityUtils.GetAngleOffsetInGravityForActor(t, i, s);
    } else {
      return 0;
    }
  }

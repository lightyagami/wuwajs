...
exports.UiViewFloatContainer = undefined;
...
class UiViewFloatContainer extends UiViewContainer_1.UiViewContainer {
  ...
  async $Oe(e, i) {
    return !!(await this.CloseViewImplementAsync(i)) && (Log_1.Log.CheckInfo() && Log_1.Log.Info("UiFloatContainer", 10, "界面关闭成功,显示中关闭", ["区域", e], ["当前界面", i.Info.Name]), true);
  }
  Rcr(e) {
  ...
  Ucr(e) {
    ...
    if (i) {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("UiFloatContainer", 10, "从队列中获取要显示的界面", ["区域", e], ["界面", i.Info.Name]);
      }
  ...
  ClearContainer() {
    for (const a of this.Ecr.values()) {
      a.Clear();
    }
    var e;
    var i;
  ...
  StartWaitingNormalView() {
    for (const e of Array.from(this.Ecr.keys())) {
      if (!this.Scr.has(e)) {
        this.Ucr(e);
      }
    }
  ...
  bcr(e, i) {
    if (e.OpenPromise?.IsPending()) {
      e.SetLoadingFinishOperation(() => {
        this.qcr(e, i);
      });
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("UiFloatContainer", 10, "界面在打开中", ["view", e.Info.Name], ["bActive", i]);
      }
    } else {
      this.qcr(e, i);
    }
  ...
  qcr(e, i) {
    if (i) {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("UiFloatContainer", 10, "界面唤醒界面动画", ["view", e.Info.Name], ["bActive", i]);
      }
      e.SetActive(true);
    } else {
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("UiFloatContainer", 10, "界面暂停界面动画", ["view", e.Info.Name], ["bActive", i]);
      }
      e.SetActive(false);
    }
  }
  ...
  RefreshByPureModeChanged() {
    for (const e of this.Scr.values()) {
      this.RefreshParentUiItem(e);
    }
    for (const i of this.ycr.values()) {
      this.RefreshParentUiItem(i);
    }
  }
  RefreshParentUiItem(i) {
    ...
    if (t.OnlyShowInMain) {
      let e = undefined;
      if (e = t.HideInPureMode && ModelManager_1.ModelManager.BattleUiModel?.PureModeData?.IsOpen ? UiLayer_1.UiLayer.GetPureModeFloatUnit(UiLayerType_1.ELayerType.BattleFloat) : UiLayer_1.UiLayer.GetFloatUnit(UiLayerType_1.ELayerType.BattleFloat, t.RootItemIndex)) {
        i.SetParentUiItem(e);
      }
    }
  }

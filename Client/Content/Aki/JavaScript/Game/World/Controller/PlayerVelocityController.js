...
class PlayerVelocityController extends ControllerBase_1.ControllerBase {
  ...
  static BEa(t) {
    var e;
    this.bEa.Start(), t <= 0 ? (ModelManager_1.ModelManager.CreatureModel.EnableEntityLog && Log_1.Log.CheckInfo() && Log_1.Log.Info("World", 60, "计算玩家移动速度时检测到间隔时间异常", ["delta", t]), this.PEa()) : t > MAX_DELTA_TIME ? (ModelManager_1.ModelManager.CreatureModel.EnableEntityLog && Log_1.Log.CheckInfo() && Log_1.Log.Info("World", 60, "计算玩家移动速度时检测到间隔时间过长", ["delta", t]), this.av()) : ((e = this.xEa()).Subtraction(this.BKs, this.qEa), this.qEa.DivisionEqual(t), this.BKs.DeepCopy(e), e = MathCommon_1.MathCommon.Clamp(6 * t, 0, 1), this.wEa.MultiplyEqual(1 - e), this.qEa.MultiplyEqual(e), this.wEa.AdditionEqual(this.qEa)), this.GEa(), this.bEa.Stop()
  }
  static GEa() {
    var t = this.wEa.SizeSquared(),
      e = t > HIGH_SPEED_SQUARED;
    e !== this.tWo && (this.tWo = e, ModelManager_1.ModelManager.CreatureModel.EnableEntityLog && Log_1.Log.CheckInfo() && Log_1.Log.Info("World", 60, "高速模式切换", ["IsHighSpeed", this.tWo], ["AvgVelocity", t]), this.OEa(), EventSystem_1.EventSystem.Emit(EventDefine_1.EEventName.OnHighSpeedModeChanged, this.tWo))
  }
  static OEa() {
    this.tWo ? this.kEa(Math.floor(this.REa / 2)) : this.kEa(this.REa)
  }
  static kEa(t) {
    this.NEa[0] = IMPOSTER_UPDATE_BATCH, this.NEa[1] = t, UE.KismetSystemLibrary.ExecuteConsoleCommand(GlobalData_1.GlobalData.World, this.NEa.join(" "))
  }

...
class BattleNetController {
  static async RequestCaptureEntity(e) {
    ...
    if (ModelManager_1.ModelManager.CreatureModel.GetEntity(t)) {
      ...
      return !!(r = await Net_1.Net.CallAsync(21698, r)) && (r.G9n === 0 || !(Log_1.Log.CheckWarn() && Log_1.Log.Warn("Level", 29, "幻象收复失败", ["ErrCode", r.G9n]), 1));
    } else {
  ...
  static async RequestBatchCaptureEntity(e) {
    var t = Time_1.Time.Now;
    if (t - this.iku < REQUEST_TIME_GAP) {
      return [];
    }
    this.iku = t;
    var r = [];
    ...
    for (const _ of e) {
      ...
      if (i) {
        if (ModelManager_1.ModelManager.CreatureModel.GetEntity(i)) {
          ...
          r.push(Net_1.Net.CallAsync(21698, a));
          o.set(i, _);
...
BattleNetController.iku = 0; //# sourceMappingURL=BattleNetController.js.map
...
class CombatDebugBlueprintFunctionLibrary extends UE.BlueprintFunctionLibrary {
  ...
  static AddPassiveSkillForDebug(t, e) {
    ...
    t?.Valid && t.LearnPassiveSkill(Number(e), {
      CombatMessageId: -1n
    })
  }
  ...
  static IsDrawEntityBoxEnabled() {
    return CombatDebugDrawController_1.CombatDebugDrawController.IsDrawEntityBoxEnabled ?? !1
  }
  ...
  static IsDrawEntityBoxInfoEnabled() {
    return CombatDebugDrawController_1.CombatDebugDrawController.IsDrawEntityBoxInfoEnabled ?? !1
  }
  ...
  static SetDebugPrintOpened(t, e) {
    e ? CombatLog_1.CombatLog.DebugCombatInfo.add(t) : CombatLog_1.CombatLog.DebugCombatInfo.delete(t)
  }
  ...
  static GetBuffComponent(t, e) {
    ...
    return t?.GetBuffByHandle(e) ? t : t && (0, RegisterComponent_1.isComponentInstance)(t, 190) && t.GetFormationBuffComp()?.GetBuffByHandle(e) ? t.GetFormationBuffComp() : void 0
  }
  ...
  static GetDebugBuff(t, e) {
    ...
    return r || (!r && (0, RegisterComponent_1.isComponentInstance)(t, 190) ? t.GetFormationBuffComp().GetBuffByHandle(e) : void 0)
  }
  ...
  static GetAttributeDebugString(t) {
    var e = EntitySystem_1.EntitySystem.GetComponent(t, 22),
      r = EntitySystem_1.EntitySystem.GetComponent(t, 173);
    if (!e || !r) return "";
    ...
    if (s)
      for (const y of s) u[y.tSs] = y;
    for (let t = 1; t < CharacterAttributeTypes_1.ATTRIBUTE_ID_MAX; t++)
      if (!(CharacterAttributeTypes_1.attributeIdsWithMax.has(t) || a.has(t) || CharacterAttributeTypes_1.attrsAutoRecoverSpeedMap.has(t) || n.has(t) || o.has(t))) {
        var [C, l] = [r.GetBaseValue(t), r.GetCurrentValue(t)], b = l.toFixed(0), l = l === C ? "" : (C < l ? "(+" : "(") + (l - C).toFixed(0) + ")";
        const s = u[t];
        var [C, c] = [s?.eSs ?? 0, s?.y6n ?? 0], m = c.toFixed(0), c = c === C ? "" : (C < c ? "(+" : "(") + (c - C).toFixed(0) + ")";
  ...
  static GetStateAttributeDebugString(t) {
    ...
    for (let t = 1; t < CharacterAttributeTypes_1.ATTRIBUTE_ID_MAX; t++)
      if (CharacterAttributeTypes_1.attributeIdsWithMax.has(t) || CharacterAttributeTypes_1.attrsAutoRecoverSpeedMap.has(t)) {
        ...
        const n = o[t];
        var s, u, C, l, b = (n?.eSs ?? 0).toFixed(0);
        CharacterAttributeTypes_1.attrsAutoRecoverSpeedMap.has(t) ? (s = CharacterAttributeTypes_1.attrsAutoRecoverSpeedMap.get(t) ?? 0, l = CharacterAttributeTypes_1.attrsAutoRecoverMaxMap.get(t) ?? 0, u = r.GetCurrentValue(l).toFixed(0), l = (o[l]?.y6n ?? 0).toFixed(0), C = r.GetCurrentValue(s).toFixed(0), s = (o[s]?.y6n ?? 0).toFixed(0), a += `#${t} undefined C:${i}/${u} (${C}/s) | S:${b}/${l} (${s}/s)
`) : CharacterAttributeTypes_1.attributeIdsWithMax.has(t) && (u = CharacterAttributeTypes_1.attributeIdsWithMax.get(t) ?? 0, C = r.GetCurrentValue(u).toFixed(0), l = (o[u]?.y6n ?? 0).toFixed(0), a += `#${t} undefined C:${i}/${C} | S:${b}/${l}
`)
      } return a.trim()
  }
  static GetFormationAttributeDebugString(t) {
    ...
    var t = t.ServerDebugInfo?.M6n,
      r = new Array;
    if (t)
      for (const l of t) r[l.E6n] = l;
    for (const b of FormationPropertyAll_1.configFormationPropertyAll.GetConfigList()) {
      var a = b.Id,
        n = FormationAttributeController_1.FormationAttributeController.GetValue(a),
        o = FormationAttributeController_1.FormationAttributeController.GetMax(a),
        i = FormationAttributeController_1.FormationAttributeController.GetSpeed(a),
        s = r[a],
        u = s?.y6n.toFixed(0) ?? "???",
        C = s?.I6n.toFixed(0) ?? "???",
        s = s?.L6n.toFixed(0) ?? "???";
      e += `#${a} C:${n?.toFixed(0)}/${o?.toFixed(0)} (${i?.toFixed(0)}/s)` + ` | S:${u}/${C} (${s}/s)
`
    }
    return e.trim()
  }
  static GetPassiveDebugString(t) {
    ...
    if (0 < e.GetAllPassiveSkills().length) {
      var n = EntitySystem_1.EntitySystem.GetComponent(t, 206);
      a += "----- 客户端被动技能 -----\n";
      for (const u of e.GetAllPassiveSkills()) {
        ...
        var o = PassiveSkillById_1.configPassiveSkillById.GetConfig(u.SkillId),
          i = n?.GetPassiveSkillCdInfo(u.SkillId)?.CurRemainingCd?.toFixed(2) ?? "【无CD组件】";
...
`
  ...
  static GetTagsDebugString(t) {
    const o = EntitySystem_1.EntitySystem.GetComponent(t, 205)?.TagContainer;
    t = EntitySystem_1.EntitySystem.GetComponent(t, 22);
    if (!t || !o) return "";
    ...
    if (e) {
      var a = i.get("实体");
      for (const s of e) r.set(s.m5n, r.get(s.m5n) ?? 0 + s.m9n), a.set(s.m5n, s.m9n)
    }
    if (t) {
      var n = i.get("编队");
      for (const u of t) r.set(u.m5n, r.get(u.m5n) ?? 0 + u.m9n), n.set(u.m5n, u.m9n)
    }
    return ("【客户端】\n" + [...o.GetAllExactTags()].map(t => {
      ...
      for (const n of o.GetAllChannels()) {
        ...
        a && (r += CharacterTagContainer_1.channelDebugName[n] + ` x ${a} `)
      }
      return r.trimEnd() + ")\n"
  ...
  static GetCueDebugString(t, e = "") {
    ...
    for (const n of t.GetAllCurrentCueRef()) 0 < a.length && !a.some(t => String(n.CueConfig.Id).includes(t)) || (r += `CueId: ${n.CueConfig.Id} Handle: ${n.Handle} BuffId: ${n.BuffId}
`);
    return r
  }
  static LoadDataTable(t) {
    if (Info_1.Info.IsPlayInEditor) return ResourceSystem_1.ResourceSystem.Load(t, UE.DataTable)
  }
}

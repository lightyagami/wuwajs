...
class PostProcessTriggerStateOutsideToInside extends PostProcessTriggerStateBase_1.default {
  ...
  OnUpdate(s) {
    this.Timer > this.Owner.TransitionTime ? this.StateMachine.Switch(0) : (this.Timer += s / 1e3, s = MathUtils_1.MathUtils.Clamp(this.Timer / this.Owner.TransitionTime, 0, this.GetTargetDefaultValue()), this.Owner.GetPostProcessComponent().BlendWeight = s)
  }
}

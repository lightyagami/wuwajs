...
class EffectBlueprintFunctionLibrary extends UE.BlueprintFunctionLibrary {
  ...
  static SetMaterialControllerDataSync(e, t, a) {
    if (!(t.length <= 0) && t !== "None") {
      var r;
      var o = ModelManager_1.ModelManager.CreatureModel.GetEntityById(e)?.Entity;
      const c = o?.GetComponent(3)?.Actor?.CharRenderingComponent;
      if (c) {
        if (a) {
          ResourceSystem_1.ResourceSystem.LoadAsync(t, UE.PD_CharacterControllerDataGroup_C, e => {
            if (e) {
              c?.AddMaterialControllerDataGroup(e);
            } else if (Log_1.Log.CheckError()) {
              Log_1.Log.Error("Battle", 4, "无法找到材质效果组", ["materialDataPath", t]);
            }
          });
        } else {
          ResourceSystem_1.ResourceSystem.LoadAsync(t, UE.PD_CharacterControllerData_C, e => {
            if (e) {
              ...
              Log_1.Log.Error("Battle", 4, "无法找到材质效果", ["materialDataPath", t]);
            }
          });
        }
        ...
        r.sVn.nVn = t;
        r.sVn.aVn = a;
        CombatMessage_1.CombatNet.Send(25173, o, r);
      } else if (Log_1.Log.CheckError()) {
  ...
  static ChangeMaterialTextures(a, e) {
    if (a) {
      ResourceSystem_1.ResourceSystem.LoadAsync(e, UE.KuroChangeMaterialsTextures, t => {
        if (t?.IsValid()) {
          let e = a.GetComponentByClass(UE.KuroChangeSkeletalMaterialsComponent.StaticClass());
          (e = e || a.AddComponentByClass(UE.KuroChangeSkeletalMaterialsComponent.StaticClass(), false, undefined, false)).ChangeMaterialsWithDataAsset(t);
        } else if (Log_1.Log.CheckError()) {
          Log_1.Log.Error("RenderCharacter", 25, "ChangeMaterialTextures失败，因为asset无效", ["Path", e], ["actor", a?.GetName()]);
        }
      });
    }
  }

...
class InputController extends ControllerBase_1.ControllerBase {
  ...
  static InitializeEnvironment() {
    Info_1.Info.UseFastInputCallback && cpp_1.FKuroInputInterface.InitializeEnvironment(), Info_1.Info.AxisInputOptimize ? UE.KismetSystemLibrary.ExecuteConsoleCommand(GlobalData_1.GlobalData.World, "Kuro.Input.AxisOptimize 1") : UE.KismetSystemLibrary.ExecuteConsoleCommand(GlobalData_1.GlobalData.World, "Kuro.Input.AxisOptimize 0")
  }
  static get Model() {
      ...
      switch (e) {
        case 1:
          ...
          for (const s of this.Model.GetHandlers()) {
            var o = s.GetInputFilter();
            if (o.BlockAction(t)) break;
            o.ListenToAction(t) && s.HandlePressEvent(t, r)
          }
          break;
        case 2:
          var i = n.get(t);
          ...
          for (const u of this.Model.GetHandlers()) {
            var p = u.GetInputFilter();
            if (p.BlockAction(t)) break;
            p.ListenToAction(t) && u.HandleReleaseEvent(t, a)
          }
      }
    }
  }
  static SetMoveControlEnabled(t, e, n, r) {
    if (this.GMe = t, this.NMe = e, this.OMe = n, this.kMe = r, Info_1.Info.AxisInputOptimize && (this.Model.NextFrameRefreshAxisValues(), !(t && e && n && r))) {
      var o = this.Model.GetAxisValues(),
        i = this.Model.GetHandlers();
      if (!t || !e) {
        o.set(InputEnums_1.EInputAxis.MoveForward, 0);
        for (const a of i) a.ClearSingleAxisInput(InputEnums_1.EInputAxis.MoveForward, !1)
      }
      if (!r || !n) {
        o.set(InputEnums_1.EInputAxis.MoveRight, 0);
        for (const p of i) p.ClearSingleAxisInput(InputEnums_1.EInputAxis.MoveRight, !1)
      }
    }
  }
  static InputAxis(t, e, n = !0) {
    var r = this.Model.GetAxisValues();
    if (Info_1.Info.AxisInputOptimize) {
      if (n && this.j$a.add(t), r.get(t) === e) return
    } else if (0 === e && r.has(t)) return;
    if (ModelManager_1.ModelManager.InputModel.IsOpenInputAxisLog && 3 === t && Log_1.Log.CheckInfo() && Log_1.Log.Info("Input", 10, "[InputLog][InputController]开始接收输入", ["axis", t], ["value", e]), t === InputEnums_1.EInputAxis.MoveForward) {
      if (!this.GMe && 0 < e) return;
      if (!this.NMe && e < 0) return
    }
    if (t === InputEnums_1.EInputAxis.MoveRight) {
      if (!this.kMe && 0 < e) return;
      if (!this.OMe && e < 0) return
    }
    r.set(t, e), ModelManager_1.ModelManager.InputModel.IsOpenInputAxisLog && Log_1.Log.CheckInfo() && Log_1.Log.Info("Input", 10, "[InputLog][InputController]完成接收输入", ["axisSet", r])
  }
  static PreProcessInput(t, e) {
    if (this.Model)
      for (const n of this.Model.GetHandlers()) n.PreProcessInput(t, e)
  }
  static PostProcessInput(t, e) {
    if (this.Model) {
      var n, r, o = this.Model.GetHandlers();
      ...
      for ([n, r] of this.Model.GetAxisValues())
        for (const _ of o) {
          ...
          i.ListenToAxis(n) && (ModelManager_1.ModelManager.InputModel.IsOpenInputAxisLog && 3 === n && Log_1.Log.CheckInfo() && Log_1.Log.Info("Input", 10, "[InputLog][InputController]开始处理轴输入", ["axis", n], ["value", r]), _.HandleInputAxis(n, r))
        }
      InputController.FMe.Stop();
      var a, p, s = Time_1.Time.WorldTimeSeconds;
      InputController.VMe.Start();
      for ([a, p] of this.Model.GetPressTimes()) {
        var u = this.qMe(p, s);
        if (u !== KEY_RELEASED_TIME)
          for (const I of o) {
            var l = I.GetInputFilter();
            if (l.BlockAction(a)) break;
            l.ListenToAction(a) && I.HandleHoldEvent(a, u)
          }
      }
      InputController.VMe.Stop(), InputController.HMe.Start();
      try {
        for (const f of o) f.PostProcessInput(t, e)
      } catch (t) {
        t instanceof Error ? Log_1.Log.CheckError() && Log_1.Log.ErrorWithStack("Json", 10, "PostProcessInput", t, ["msg", t.message]) : Log_1.Log.CheckError() && Log_1.Log.Error("Json", 10, "PostProcessInput", ["error", t])
      } finally {
        InputController.HMe.Stop()
      }
      if (Info_1.Info.AxisInputOptimize) {
        for (const E of this.j$a) {
          this.Model.GetAxisValues().has(E) && this.Model.GetAxisValues().delete(E);
          for (const C of o) C.ClearSingleAxisInput(E, !0)
        }
        this.j$a.clear()
      } else this.Model.GetAxisValues().clear();
      ModelManager_1.ModelManager.InputModel.IsOpenInputAxisLog && Log_1.Log.CheckInfo() && Log_1.Log.Info("Input", 10, "[InputLog][InputController]开始输入处理完成")
    }
  }
  ...
  static GetKeyDownTime(t) {
    t = this.Model.GetPressTimes().get(t);
    return t && t !== KEY_RELEASED_TIME ? Math.max(0, Time_1.Time.WorldTimeSeconds - t) : 0
  }
  static qMe(t, e) {
    return void 0 === t || t === KEY_RELEASED_TIME ? KEY_RELEASED_TIME : e - t
  }
  static SetForceFeedbackConfig(t, e) {
    UE.BasePlayerController.SetKuroForceFeedbackConfig(t, e)
  }
  static AddInputLayer(t, e) {
    e.UnitId = t, this.Model.AddInputLayer(t, e)
  }
...
exports.InputController = InputController, (_a = InputController).HMe = Stats_1.Stat.Create("InputController.PostProcessInput"), InputController.FMe = Stats_1.Stat.Create("InputController.HandleInputAxis"), InputController.VMe = Stats_1.Stat.Create("InputController.HandleHold"), InputController.GMe = !0, InputController.NMe = !0, InputController.OMe = !0, InputController.kMe = !0, InputController.wMe = (t, e, n) => {
  var n = n.GetInputAxis(),
    r = (InputController.InputAxis(n, e, !1), Global_1.Global.CharacterController);
  r && 0 < e && n !== InputEnums_1.EInputAxis.Zoom && Info_1.Info.IsInKeyBoard() && !r.bShowMouseCursor && InputManager_1.InputManager.MoveCursorToCenter()
}, InputController.BMe = (t, e, n) => {
  n = n.GetInputAxis();
  InputController.InputAxis(n, e, !1)
}, InputController.bMe = (t, e, n) => {
  e = 0 === e ? 1 : 2, n = n.GetInputAction();
  InputController.InputAction(n, e)
}, InputController.AMe = () => {
  for (var [t] of InputController.Model.GetPressTimes()) InputController.InputAction(t, 2)
}, InputController.jJa = () => {
  1 !== Info_1.Info.PlatformType && "IOS" !== Platform_1.Platform.CloudGamePlatform && "Mac" !== Platform_1.Platform.CloudGamePlatform || (UE.KuroStaticLibrary.SetInputKeyDeadZone(Global_1.Global.CharacterController, 0, new UE.Key(new UE.FName("Gamepad_LeftY")), 0), UE.KuroStaticLibrary.SetInputKeyDeadZone(Global_1.Global.CharacterController, 0, new UE.Key(new UE.FName("Gamepad_RightX")), 0), UE.KuroStaticLibrary.SetInputKeyDeadZone(Global_1.Global.CharacterController, 0, new UE.Key(new UE.FName("Gamepad_RightY")), 0), UE.KuroStaticLibrary.SetInputKeyDeadZone(Global_1.Global.CharacterController, 0, new UE.Key(new UE.FName("Gamepad_LeftX")), 0))
}, InputController.PMe = t => {
  t && Log_1.Log.CheckInfo() && Log_1.Log.Info("Input", 7, "强制释放所有按键", ["Reason", t]);
  for (var [e] of InputController.Model.GetPressTimes()) InputController.InputAction(e, 2)
}, InputController.xMe = t => {
  var e, n = ModelManager_1.ModelManager.InputDistributeModel;
  for ([e] of InputController.Model.GetPressTimes()) {
    var r = n.GetActionInputDistributeTagName(InputEnums_1.EInputAction[e]);
    !r || n.IsTagMatchAnyCurrentInputTag(r) || InputController.InputAction(e, 2)
  }

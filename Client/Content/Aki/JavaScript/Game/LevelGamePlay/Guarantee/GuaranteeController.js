...
class GuaranteeController extends ControllerBase_1.ControllerBase {
  ...
  static ExecuteActions(e, t) {
    if (t) {
      for (const o of e) {
        var n = o.Name;
        var r = GuaranteeActionCenter_1.GuaranteeActionCenter.GetGuaranteeAction(n);
        if (r) {
          if (Log_1.Log.CheckInfo()) {
            Log_1.Log.Info("LevelEvent", 39, "执行保底行为：", ["actionName", n], ["ActionInfo", o]);
          }
          r.Execute(o, t);
        }
      }
...
GuaranteeController.rIe = (e, t, n, r = false) => {
  _a.aIe(e, t, true, n, r);
};
GuaranteeController.nIe = (e, t, n, r = false) => {
  _a.aIe(e, t, false, n, r);
};
GuaranteeController.aIe = (e, t, n, r, o) => {
  var a;
  if (t && t.Type !== 7 && r && r.Name) {
    t = GuaranteeActionCenter_1.GuaranteeActionCenter.GetActionFilterMode(r.Name);
    a = ModelManager_1.ModelManager.LevelGeneralModel;
    if (n) {
      if (!a.HasSceneGuaranteeActionInfo(r, t)) {
        a.AddSceneGuaranteeActionInfo(r);
        if (Log_1.Log.CheckInfo()) {
          Log_1.Log.Info("LevelEvent", 39, "添加场景保底行为：" + r.Name, ["触发行为", e], ["ActionInfo", r]);
        }
      }
    } else {
      a.PopSceneGuaranteeActionInfo(r);
      if (Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("LevelEvent", 39, "移除场景保底行为：" + r.Name, ["触发行为", e], ["ActionInfo", r]);
      }
    }
  }

...
class CharacterSplineMoveTask extends SplineMoveTaskBase_1.SplineMoveTaskBase {
  constructor(e, t, i) {
    ...
    this.B7 = undefined;
    this.xsa = (e, t) => {
      if (this.EntityHandle?.Entity?.Valid && t !== CharacterUnifiedStateTypes_1.ECharPositionState.Climb && (t = this.EntityHandle.Entity.GetComponent(48)) && t.IsMovingToLocation()) {
  ...
  OnStartTask() {
    ...
    if (this.EntityHandle.Entity.GetComponent(0)?.IsNpc() && this.gLe?.NpcFollow) {
      this.EntityHandle.Entity.GetComponent(199)?.PauseAi("StartMoveWithSpline");
      ...
      if (e = this.EntityHandle.Entity.GetComponent(328)) {
  ...
  OnEndTask(e) {
    ...
    if (this.gLe?.MoveTarget.Type === "Player") {
      s = this.EntityHandle.Entity.GetComponent(48);
      t = this.EntityHandle.Entity.GetComponent(111);
      if (s) {
        s.StopMove(false);
        i = t?.MoveState;
        s.ResetMaxSpeed(i);
      }
      if (this.gLe?.CheckClimb && (s = this.EntityHandle.Entity.GetComponent(36)) && t && t.PositionState === CharacterUnifiedStateTypes_1.ECharPositionState.Climb) {
        s.KickWallExit();
      }
      (i = this.EntityHandle.Entity.GetComponent(67))?.ClearMoveVectorCache();
      i?.SetActive(true);
    }
    this.EntityHandle.Entity.GetComponent(48).IsSpecialMove = false;
    if (this.EntityHandle.Entity.GetComponent(0)?.IsNpc() && this.gLe?.NpcFollow) {
      this.EntityHandle.Entity.GetComponent(199)?.ResumeAi("StartMoveWithSpline");
    }
    this.B7?.(e);
  }
  nKl(e, t) {
  ...
  sKl(e) {
    ...
    if (h[0]) {
      ...
      i.AdditionEqual(t);
      s = (h = this.EntityHandle.Entity.GetComponent(188)).GetMeshTransform();
      r.SetActorLocation(i.ToUeVector(), "MoveWithSplineDetectClimb", true);
      h.SetModelBuffer(s, 10);
      (r = t).UnaryNegation(r);
      return this.EntityHandle.Entity.GetComponent(111)?.PositionState === CharacterUnifiedStateTypes_1.ECharPositionState.Climb || (this.EntityHandle.Entity.GetComponent(36)?.DetectClimbWithDirect(false, r.ToUeVector(), true) ?? false);
    } else {
      if (Log_1.Log.CheckWarn()) {
  ...
  ODe() {
    ...
    var o = this.EntityHandle.Entity.GetComponent(48);

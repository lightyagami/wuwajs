      ...
      if (Global_1.Global.BaseCharacter?.IsValid()) {
        ...
        switch (r.ResetType.Type) {
          ...
          case "ResetToFixedDirection":
            var s = r.ResetType;
            LevelEventResetPlayerCameraFocus.mce.Set(s.Direction.Y ?? 0, s.Direction.Z ?? 0, s.Direction.X ?? 0), CameraController_1.CameraController.FightCamera.LogicComponent.PlayCameraEulerRotatorWithCurve(LevelEventResetPlayerCameraFocus.mce, o, void 0, t, l)
        }
        Global_1.Global.BaseCharacter && ModelManager_1.ModelManager.CreatureModel.GetEntityById(Global_1.Global.BaseCharacter.EntityId)?.Entity?.GetComponent(62)?.InterruptAutoMoving("进入相机调整ResetPlayerCameraFocus", !0), Log_1.Log.CheckInfo() && Log_1.Log.Info("Event", 57, "结束恢复相机调整")

...
class LevelEventVehicleMoveWithPathLine extends LevelGeneralBase_1.LevelEventBase {
  ...
  ExecuteWhenEntitiesReady() {
    this.puc(this.OPt.TargetVehicle), this.Cuc()
  }
  puc(e) {
    switch (e.Type) {
      case "Current":
  ...
  guc() {
    ...
    Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 50, "获取玩家载具失败，找不到全局玩家角色")
  }
  Cuc() {
    if (this.Jh) switch (this.OPt.ControlType.Type) {
      case "EnterPathMoving":
        var e = this.OPt.SplineEntityId,
          t = this.Jh.GetComponent(109);
        t?.SetExtraMoveParams(this.OPt.ControlType.ControlParams), t?.StartSplineMove(e, this.OPt.ControlType.Pattern);
        break;
      case "ExitPathMoving":
        t = this.OPt.SplineEntityId, e = this.Jh.GetComponent(109);
        e?.ResetExtraMoveParams(), e?.EndSplineMove(t)
    } else Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 50, "设置载具控制状态失败，无法找到目标实体")
  }
}

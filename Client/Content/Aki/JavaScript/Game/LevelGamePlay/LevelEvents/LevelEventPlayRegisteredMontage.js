...
class LevelEventPlayRegisteredMontage extends LevelGeneralBase_1.LevelEventBase {
  constructor() {
    ...
    this.ej_ = () => {
      ...
      this.sDe?.Entity?.GetComponent(199)?.ResumeAi(KEY);
      this.FinishExecute(true);
    };
  }
  ExecuteInGm(e, t) {
  ...
  ExecuteWhenEntitiesReady() {
    ...
    if (t) {
      if ((this.sDe = t).Entity.GetComponent(50)?.IsAiDriver && Log_1.Log.CheckDebug()) {
      ...
      if (StringUtils_1.StringUtils.IsEmpty(i)) {
        ...
        var s = t.Entity?.GetComponent(49);
        if (s) {
          var n = {
            InitStateName: e?.InitState,
            EndStateName: e?.EndState
          };
          const v = t.Entity?.GetComponent(199);
          v?.PauseAi(KEY);
          if (this.IsAsync) {

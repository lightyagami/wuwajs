...
class LevelEventRemoveBuffFromCreature extends LevelGeneralBase_1.LevelEventBase {
  ...
  ExecuteNew(e, t) {
    if (e) {
      if (this.gLe = e, this.fLe = [], void 0 !== e.EntityId && this.fLe.push(e.EntityId), e.EntityIds?.length)
        for (const o of e.EntityIds) this.fLe.push(o);
      this.CreateWaitEntityTask(this.fLe)
    } else Log_1.Log.CheckError() && Log_1.Log.Error("Event", 33, "参数类型错误"), this.FinishExecute(!1)
  }
  ExecuteWhenEntitiesReady() {
    for (const o of this.fLe) {
      var e = ModelManager_1.ModelManager.CreatureModel.GetEntityByPbDataId(o);
      if (e?.IsInit) {
        var t = e.Entity.GetComponent(174);
        for (const s of this.gLe.BuffIds) t.RemoveBuff(s, -1, "LevelEventRemoveBuffFromCreature")
      }
    }
  ...
  ExecuteInGm(e, t) {
    ...
    } else Log_1.Log.CheckError() && Log_1.Log.Error("Event", 33, "执行行为时:参数类型错误", ["EventType", this.Type]), this.FinishExecute(!1)
  }
}

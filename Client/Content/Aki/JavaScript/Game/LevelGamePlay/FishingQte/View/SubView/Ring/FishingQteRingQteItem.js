...
class FishingQteRingQteItem extends UiPanelBase_1.UiPanelBase {
  constructor(t, e, i) {
    super(), this.GameInfo = t, this.RingInfo = e, this.RingConfig = i, this.$t_ = void 0, this.y7_ = new Map, this.Qfl = new Map, this.jfl = []
  }
  OnRegisterComponent() {
  ...
  OnBeforeDestroy() {
    this.$t_ = void 0, this.y7_.clear(), this.Qfl.clear(), this.jfl.length = 0
  }
  Yt_(t) {
    let e = 3;
  ...
  async zt_(t, e, i, s) {
    let h = this.jfl.pop();
    h || (h = new FishingQteRingQteSingleItem, n = LguiUtil_1.LguiUtil.CopyItem(this.$t_, this.RootItem), await h.CreateByActorAsync(n.GetOwner()));
    ...
    switch (h.SetRotation(-n), h.SetFill(s / FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT), h.SetType(e), e) {
      case 1:
        this.RingInfo.AddQteArea(t, r, a), this.y7_.set(t, h);
        break;
      case 2:
        this.RingInfo.AddPerfectArea(t, r, a), this.Qfl.set(t, h), h.GetRootItem().SetAsLastHierarchy()
    }
    h.SetUiActive(!0)
  }
  async S7_(e, i, s) {
    if (await this.zt_(e, 1, i, s), this.rvl(0, 100) <= this.GameInfo.PerfectAppearRate) {
      var s = Math.min(s, this.RingConfig.PerfectSize),
        h = i,
        r = (i + s) % FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT;
      let t = i;
      t = h <= r ? this.rvl(h, r - s + 1) : this.rvl(h, h + s - 1 - s + 1) % FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT, await this.zt_(e, 2, t, s)
    }
  }
  SpawnAreaAtValidArea(t, e) {
    ...
    s <= h ? this.S7_(t, i, s) : i <= e ? (e = this.rvl(i, e - h + 1), this.S7_(t, e, h)) : (e = this.rvl(i, i + s - 1 - h + 1) % FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT, this.S7_(t, e, h))
  }
  Jt_() {
    var t = this.RingInfo.GetQteAreas().get(0)?.StartCellIndex ?? 0,
      e = (this.ResetAllArea(), FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT / this.RingConfig.MultiBoxGroup);
    if (Number.isInteger(e)) {
      var i, s = this.RingConfig.MultiBoxGroup,
        h = this.rvl(1, FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT);
      if (1 === s && t) i = this.Yt_(this.RingConfig.RandomArea), t = (t + FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT / 2) % FishingQteDefine_1.FISHINGQTE_RINGCELLCOUNT, this.S7_(0, t, i);
      else
        for (let t = 0; t < s; t++) {
    ...
    } else Log_1.Log.CheckError() && Log_1.Log.Error("SceneGameplay", 37, "[FishingQte]格子扩展配置有误：不可被等分", ["配置组数", this.RingConfig.MultiBoxGroup])
  }
  Zt_() {
    this.ResetAllArea();
  ...
  SpawnContinuousArea(t, e = 1) {
    if (this.RingInfo.IsWholeRing) switch (this.RingConfig.RefreshType) {
      case 0:
        break;
      case 1:
        this.Jt_();
        break;
      case 2:
        this.ResetAreaInLink(t, e), 0 === this.RingInfo.GetQteAreas().size && this.Jt_()
    } else {
...
class FishingQteRingQteSingleItem extends UiPanelBase_1.UiPanelBase {
  constructor() {
    super(...arguments), this.awl = new UE.FName("Progress"), this.LevelSequencePlayer = void 0
  }
  OnRegisterComponent() {
    this.ComponentRegisterInfos = [
      [0, UE.UITexture],
  ...
  OnBeforeDestroy() {
    this.LevelSequencePlayer?.Clear(), this.LevelSequencePlayer = void 0, this.awl = void 0
  }
  Cc_() {
    var t = [];
    return t.push(this.GetTexture(0)), t.push(this.GetTexture(1)), t.push(this.GetTexture(2)), t.push(this.GetTexture(3)), t.push(this.GetTexture(4)), t
  }
  SetRotation(t) {
    for (const e of this.Cc_()) e.SetUIRelativeRotation(Rotator_1.Rotator.Create(0, t, 0).ToUeRotator())
  }
  SetType(t) {
    switch (t) {
      case 1:
  ...
  SetFill(t) {
    for (const e of this.Cc_()) e.SetFillAmount(t), e.SetCustomMaterialScalarParameter(this.awl, t)
  }
  PlayAnim(t) {
    this.LevelSequencePlayer.GetCurrentSequence() === t ? this.LevelSequencePlayer.ReplaySequenceByKey(t) : (this.LevelSequencePlayer.StopPlayingSequence(!1, !0), this.LevelSequencePlayer.PlayLevelSequenceByName(t, !1))
  }
}

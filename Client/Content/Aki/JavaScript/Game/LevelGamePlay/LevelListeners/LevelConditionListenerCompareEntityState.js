...
class LevelConditionListenerCompareEntityState extends LevelListenerBase_1.LevelListenerBase {
  ...
  OnListen(t, e, i) {
    if (t.Type === "CompareEntitySelfState") {
      this.uXd = LevelGamePlayUtils_1.LevelGamePlayUtils.GetEntityHandle(undefined, i)?.Entity;
      this.mXd();
  ...
  mXd() {
    if (this.uXd?.Valid) {
      if (!EventSystem_1.EventSystem.HasWithTarget(this.uXd, EventDefine_1.EEventName.OnSceneItemStateChange, this.Zge)) {
        EventSystem_1.EventSystem.AddWithTargetUseHoldKey(this, this.uXd, EventDefine_1.EEventName.OnSceneItemStateChange, this.Zge);
      }
    } else if (Log_1.Log.CheckError()) {

...
class LevelAimLineController extends ControllerBase_1.ControllerBase {
  ...
  static uye() {
    ...
    this.Cye.filter(e => !this.gye.includes(e)).forEach(e => {
      if (e?.Valid) {
        e.Entity.GetComponent(141).SetIsBeingTargeted(false);
      }
    });
    this.gye.filter(e => !this.Cye.includes(e)).forEach(e => {
      if (e?.Valid) {
        e.Entity.GetComponent(141).SetIsBeingTargeted(true);
      }
    });
    this.Cye = this.gye;
    this.gye = [];
...
LevelAimLineController.oye = (e, t) => {
  ...
  } else if ((t = ModelManager_1.ModelManager.SceneInteractionModel.GetEntityByActor(t)) && t.Entity.GetComponent(165)?.Valid && !_a.gye.includes(t)) {
    _a.gye.push(t);
  }
};

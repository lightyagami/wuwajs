...
function calculateCellSize(t, s) {
  if (t <= s) {
    ...
    return exports.BIGSTUFFEDDOLL_RINGCELLCOUNT - t + 1 + s;
  }
}
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.ContinuousArea = exports.Area = exports.BigStuffedRingInfo = exports.BigStuffedGameInfo = exports.calculateCellSize = exports.SINGLECELL_ANGLE = exports.BIGSTUFFEDDOLL_RINGCELLCOUNT = undefined;
...
class BigStuffedGameInfo {
  ...
  UpdateCurrentArrowStayInfo() {
    if (this.ofl.length !== 0) {
      ...
      for (const e of this.ofl) {
        t += e.ValidAreas.length;
        if (this.CurrentArrowStayTotalIndex < t) {
          this.CurrentArrowStayRingId = e.RingId;
          this.CurrentArrowStayRelativeValidAreaIndex = this.CurrentArrowStayTotalIndex - s;
          break;
        }
        s = t;
      }
  ...
  GetValidAreaNum() {
    let t = 0;
    for (const s of this.ofl) {
      t += s.ValidAreas.length;
    }
    return t;
  }
  OnArrowDirectionReverse() {
    for (const t of this.ofl) {
...
class BigStuffedRingInfo {
  ...
  OnArrowDirectionReverse() {
    ...
    for (const r of this.ValidAreas) {
      r.OnArrowDirectionReverse();
    }
    for (var [, t] of this.Sbl) {
      t.OnArrowDirectionReverse();
    }
    for (var [, s] of this.Mbl) {
      s.OnArrowDirectionReverse();
    }
    for (var [, e] of this.ybl) {

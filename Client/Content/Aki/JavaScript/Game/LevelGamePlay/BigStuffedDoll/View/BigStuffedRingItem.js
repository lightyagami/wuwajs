...
class BigStuffedRingItem extends UiPanelBase_1.UiPanelBase {
  constructor(t, i) {
    super(), this.Id = -1, this.Pfl = void 0, this.Ebl = void 0, this.wfl = void 0, this.Bfl = void 0, this.bfl = void 0, this.qfl = void 0, this.LevelSequencePlayer = void 0, this.Yrn = void 0, this.Gfl = !1, this.cce = Rotator_1.Rotator.Create(), this.XZh = 0, this.YZh = 0, this.Nfl = !1, this.LDl = !1, this.RDl = 0, this.Ibl = -1, this.owt = t => {
      switch (t) {
        case BLANK_QTE_ANIM:
  ...
  async InitAsync() {
    ...
    t && (0 < t.length && this.RootItem.SetAnchorOffsetX(this.Pfl.Offset[0]), 1 < t.length) && this.RootItem.SetAnchorOffsetY(this.Pfl.Offset[1])
  }
  OnRegisterComponent() {
  ...
  async OnBeforeStartAsync() {
    var t = this.GetItem(3),
      t = (await this.Bfl.CreateThenShowByActorAsync(t.GetOwner()), this.GetItem(4)),
      t = (await this.bfl.CreateThenShowByActorAsync(t.GetOwner()), this.GetItem(5));
    await this.qfl.CreateThenShowByActorAsync(t.GetOwner()), this.Yrn = this.GetItem(2), this.Yrn.SetUIRelativeRotation(Rotator_1.Rotator.Create().ToUeRotator()), this.Yrn.SetUIActive(!1), this.GetTexture(0).SetAlpha(0), this.LevelSequencePlayer = new LevelSequencePlayer_1.LevelSequencePlayer(this.RootItem), this.LevelSequencePlayer.BindSequenceStartEvent(this.owt), this.LevelSequencePlayer.BindSequenceCloseEvent(this.yct)
  }
  OnBeforeDestroy() {
  ...
  OnAreaClick(t, i) {
    ...
    this.LevelSequencePlayer.StopCurrentSequence(!0, !0), e && this.LevelSequencePlayer.PlayLevelSequenceByName(e), s && this.LevelSequencePlayer.PlayLevelSequenceByName(s)
  }
  OnArrowEnter() {
    this.Gfl = !0, this.Yrn?.SetUIActive(!0), this.LevelSequencePlayer.StopCurrentSequence(!0, !0), this.LDl ? this.LevelSequencePlayer.PlayLevelSequenceByName(SWITCH_IN_CURRENT) : (this.LevelSequencePlayer.PlayLevelSequenceByName(CURRENT_RING_SHOW), this.LDl = !0)
  }
  OnArrowExit() {
    this.Gfl = !1, this.Yrn?.SetUIActive(!1), this.LevelSequencePlayer.StopCurrentSequence(!0, !0), this.LDl ? this.LevelSequencePlayer.PlayLevelSequenceByName(SWITCH_OUT_CURRENT) : (this.LevelSequencePlayer.PlayLevelSequenceByName(NOT_CURRENT_RING_SHOW), this.LDl = !0)
  }
  OnArrowStayAreaUpdate(t) {
    if (this.Gfl) {
      var i = this.Ebl.GetValidAreas();
      if (i)
        if (t < 0 || t >= i?.length) Log_1.Log.CheckError() && Log_1.Log.Error("SceneGameplay", 18, "[BigStuffedDoll]初始化Arrow时：相对有效区域索引越界", ["relativeValidAreaIndex", t]);
        else {
          i = i[t];
          switch (this.XZh = -Math.max(i.StartCellIndex - 1, 0) * BigStuffedDefine_1.SINGLECELL_ANGLE, this.YZh = -i.EndCellIndex * BigStuffedDefine_1.SINGLECELL_ANGLE, this.YZh >= this.XZh && (this.YZh -= 360), i.ArrowDirection) {
            case 0:
      ...
      else Log_1.Log.CheckError() && Log_1.Log.Error("SceneGameplay", 18, "[BigStuffedDoll]初始化Arrow时：找不到有效区域", ["id", this.Id])
    }
  }
  OnTick(e) {
    ...
    if (this.Gfl && this.Pfl && 2 === s.GetGameStage()) {
      ...
      if (h) {
        ...
        e = e / 1e3;
        let i = this.cce.Yaw;
        this.Nfl ? i = this.cce.Yaw + t * (h * e) : ((i = this.cce.Yaw + t * (h * e)) < this.YZh && (i = this.YZh, s.ArrowDirectionReverse()), i > this.XZh && (i = this.XZh, s.ArrowDirectionReverse())), this.cce.Yaw = i, this.Yrn?.SetUIRelativeRotation(this.cce.ToUeRotator())
      }
    }
  }
  ...
  GetCurrentArrowStayCellIndex() {
    var t;
    return this.Gfl ? (t = Math.abs(this.cce.Yaw) % 360, Math.floor(t / BigStuffedDefine_1.SINGLECELL_ANGLE) + 1) : 0
  }
}

...
class LevelConditionCheckBattleRoleIsNot extends LevelGeneralBase_1.LevelConditionBase {
  Check(e, o) {
    var n;
    return 0 === e.LimitParams.size ? (Log_1.Log.CheckError() && Log_1.Log.Error("LevelCondition", 16, "配置错误！条件的参数不应该为空", ["inConditionInfo.Id", e.Id]), !1) : (n = parseInt(e.LimitParams.get("RoleId"))) ? ModelManager_1.ModelManager.RoleModel.GetBattleTeamFirstRoleId() !== n : (Log_1.Log.CheckError() && Log_1.Log.Error("LevelCondition", 16, `配置错误！条件${e.Id}的RoleId参数不符合条件类型${LevelGeneralDefine_1.ELevelGeneralCondition.CheckBattleRole}的定义`), !1)
  }
}

      ...
      if (1 === l.Type && l.ClientExecuteActions)
        if (t.EntityIds && 0 !== t.EntityIds.length)
          for (const r of t.EntityIds) {
            ...
            o?.Valid ? ControllerHolder_1.ControllerHolder.CreatureController.SetEntityEnable(o, t.Visible, "LevelEventSetClientEntityVisible") : Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 31, "目标Entity不存在", ["PbDataId", r])
          } else Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 31, "目标Entity未配置");
        else Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 26, "不是场景引用的帧事件");
    else Log_1.Log.CheckError() && Log_1.Log.Error("LevelEvent", 31, "参数类型错误")
  }
}

...
class SeekTraceContentPanel extends UiPanelBase_1.UiPanelBase {
  constructor() {
    super(...arguments);
    this.tod = [];
    this.iod = UE.NewArray(UE.BuiltinInt);
    this.rod = UE.NewArray(UE.BuiltinInt);
    this.ood = UE.NewArray(UE.BuiltinInt);
    this.sdd = UE.NewArray(UE.BuiltinInt);
    this.sod = new UE.FName("GridArray");
    this.aod = new UE.FName("StateArray");
    this._od = undefined;
    this.hql = undefined;
    this.hod = [5, 6];
    this.lod = [9, 10];
    ...
    this._dd = [0, 0];
    this.udd = [0, 0];
    this.fod = Vector2D_1.Vector2D.Create();
    this.GIl = undefined;
    this.god = undefined;
    this.Cod = () => {
      var t = new Map();
      for (const U of this.hdd) {
        var i = this.GetUiNiagara(U).NiagaraComponent;
        if (!i) {
          TimerSystem_1.TimerSystem.Next(this.Cod);
          return;
        }
        t.set(U, i);
      }
      ...
      for ([e, s] of t) {
        ...
        switch (e) {
          case 5:
          case 6:
            UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(s, this.sod, this.iod);
            UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(s, this.aod, this.rod);
            break;
          case 9:
          case 10:
            UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(s, this.sod, this.sdd);
            UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(s, this.aod, l);
        }
      }
    };
    this.tcd = t => {
    ...
    this.cdd = t => {
      ...
      if (!!t && !(a = ModelManager_1.ModelManager.SeekTraceModel).SelectedItem) {
        if (this.bad && (i = this.Q_t, LguiUtil_1.LguiUtil.ConvertPointerPositionToLguiCenterPosition(t.pointerPosition, i), t = i.X / this.cod + this.uod, e = -i.Y / this.cod + this.uod, (s = this.LLe)[0] = Math.ceil(t), s[1] = Math.ceil(e), ControllerHolder_1.ControllerHolder.SeekTraceController.SelectItem(s))) {
          h = s[0];
          r = s[1];
          this._dd[0] = h;
          this._dd[1] = r;
          ...
          this.dod[0] = h - a[0];
          this.dod[1] = r - a[1];
          this.fod.X = -(t - s[0] + 0.5) * this.cod;
          this.fod.Y = (e - s[1] + 0.5) * this.cod;
          this.Iod(true);
          this.ddd(i, true);
          this.GIl?.();
        }
    ...
    this.vKe = t => {
      ...
      if (t && ModelManager_1.ModelManager.SeekTraceModel.SelectedItem) {
        ...
        (t = this.LLe)[0] = Math.ceil(i.X / this.cod + this.uod) - this.dod[0];
        t[1] = Math.ceil(-i.Y / this.cod + this.uod) - this.dod[1];
  ...
  async OnBeforeStartAsync() {
    ...
    this.Eod();
    this._od = new GenericLayout_1.GenericLayout(this.GetGridLayout(0), this.Mod, this.GetItem(1).GetOwner());
    await this._od.RefreshByDataAsync(ModelManager_1.ModelManager.SeekTraceModel.EnableGridList);
    this.hql = new GenericLayout_1.GenericLayout(this.GetGridLayout(2), this.Sod, this.GetItem(3).GetOwner());
    await this.hql.RefreshByDataAsync(this.tod);
  }
  OnStart() {
    var t;
    ...
    if (i === 0) {
      ...
      this.cod = t / i;
      this.uod = i / 2 - 1;
      this.ldd = this.uod + 0.5;
      (t = this.GetDraggable(7)).OnPointerBeginDragCallBack.Bind(this.cdd);
      t.OnPointerDragCallBack.Bind(this.vKe);
      t.OnPointerEndDragCallBack.Bind(this.mdd);
      t.OnPointerDownCallBack.Bind(this.cdd);
      t.OnPointerCancelCallBack.Bind(this.mdd);
      t.OnPointerUpCallBack.Bind(this.mdd);
  ...
  ResetView() {
    this.Iod(false);
    this.fdd(13, NIAGARA_DURATION, () => {
      for (const i of this.hod) {
        var t = this.GetUiNiagara(i).NiagaraComponent;
        UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(t, this.sod, this.sdd);
      }
    }, () => {
      this.V7e();
      this.fdd(12, NIAGARA_DURATION);
  ...
  OnInputControllerChange() {
    if (ModelManager_1.ModelManager.SeekTraceModel.SelectedItem) {
      this.yod(true, Info_1.Info.IsInGamepad());
    }
    this.icd();
  }
  OnSeekTraceSucceed() {
    this.fdd(14, NIAGARA_DURATION);
  }
  icd() {
    ...
    if (t) {
      ...
      if (!this.ecd) {
        this.ecd = new UiSequencePlayer_1.UiSequencePlayer(e);
        this.ecd.BindOnEndSequenceEvent(this.tcd);
      }
      this.udd[0] = 0;
      this.udd[1] = 0;
      this.gdd(0, 0);
  ...
  UpdateKeyBoardSelectFrame() {
    ...
    if (!!Info_1.Info.IsInKeyBoard() && !(s = ModelManager_1.ModelManager.SeekTraceModel).SelectedItem) {
      if ((t = Global_1.Global.CharacterController) && (t = t.GetCursorPosition())) {
        ...
        t = Math.ceil(i.X / this.cod + this.uod);
        i = Math.ceil(-i.Y / this.cod + this.uod);
        e = s.PanelWidth;
        s = s.PanelHeight;
        if (!(t < 0) && !(e <= t) && !(i < 0) && !(s <= i)) {
          this.gdd(t, i);
  ...
  gdd(t, i) {
    var e;
    if (t !== this.Zdd[0] || i !== this.Zdd[1]) {
      this.ecd?.StopPrevSequence(false, true);
      this.ecd?.PlaySequencePurely("Float");
      this.Zdd[0] = t;
      this.Zdd[1] = i;
      (e = this.Q_t).X = (t - this.ldd) * this.cod;
      e.Y = -(i - this.ldd) * this.cod;
      this.GetItem(11).SetAnchorOffset(e.ToUeVector2D());
    }
  }
  Bud() {
    if (this.Zud && !ModelManager_1.ModelManager.SeekTraceModel.SelectedItem) {
      this.ecd.StopPrevSequence(false, true);
      this.ecd.PlaySequencePurely("Move");
    }
  }
  ...
  pdd() {
    ...
    if (ModelManager_1.ModelManager.SeekTraceModel.SelectedItem) {
      t = this.udd[0];
      i = this.udd[1];
      (e = this.Q_t).X = (t - this.ldd - this._dd[0]) * this.cod;
      e.Y = -(i - this.ldd - this._dd[1]) * this.cod;
      this.GetItem(8).SetAnchorOffset(e.ToUeVector2D());
      (e = this.LLe)[0] = t - this.dod[0];
      e[1] = i - this.dod[1];
  ...
  GamePadSelectItem() {
    ...
    if (this.bad) {
      if ((t = ModelManager_1.ModelManager.SeekTraceModel).SelectedItem) {
        this.yod(false, true);
      } else if (ControllerHolder_1.ControllerHolder.SeekTraceController.SelectItem(this.udd)) {
        e = this.udd;
        ...
        this.Iod(true);
        this.ecd.StopPrevSequence(false, true);
        this.ecd.PlaySequencePurely("Sle");
        this.GIl?.();
      }
    }
  }
  ...
  yod(t, i) {
    ...
    if (e) {
      ...
      if (t === 0) {
        ...
        if (i) {
          this.Cdd(this._dd[0], this._dd[1]);
        }
      }
      e = this.Zdd;
      e[0] = -1;
  ...
  Iod(t) {
    if (t) {
      ...
      for (let t = 0; t < this.ood.Num(); t++) {
        var h = s.has(t) ? e : 0;
        this.ood.Set(t, h);
      }
    }
    var r = t ? this.ood : this.sdd;
    for (const o of this.lod) {
      var a = this.GetUiNiagara(o).NiagaraComponent;
      UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(a, this.sod, r);
    }
  }
  ddd(t, i) {
    t.AdditionEqual(this.fod);
    var e = (UiLayer_1.UiLayer.UiRootItem.GetWidth() - this.cod) / 2;
    var s = (UiLayer_1.UiLayer.UiRootItem.GetHeight() - this.cod) / 2;
    var h = this._dd[0] * this.cod;
    var r = this._dd[1] * this.cod;
    t.X = MathUtils_1.MathUtils.Clamp(t.X, -e, e);
    t.Y = MathUtils_1.MathUtils.Clamp(t.Y, -s, s);
    if (this.Zud && (this.GetItem(11).SetAnchorOffset(t.ToUeVector2D()), i)) {
      this.ecd.StopPrevSequence(false, true);
      this.ecd.PlaySequencePurely("Sle");
    }
    t.X -= h;
  ...
  V7e() {
    ...
    for (const i of this.hod) {
      var t = this.GetUiNiagara(i).NiagaraComponent;
      UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(t, this.sod, this.iod);
      UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(t, this.aod, this.rod);
    }
    this.hql?.RefreshByData(this.tod);
  }
  Eod() {
    var i = this.tod;
    var e = this.iod;
  ...
  vdd(t, i) {
    ...
    if (t !== 12 && t !== 13 && t !== 14) {
      ...
      UE.NiagaraDataInterfaceArrayFunctionLibrary.SetNiagaraArrayInt32(e, this.sod, this.iod);
      i();
    } else {

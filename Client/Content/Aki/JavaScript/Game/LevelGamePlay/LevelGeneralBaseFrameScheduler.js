...
class LevelGeneralBaseFrameScheduler {
  ...
  static PushActionToFrameScheduler(e, t, i, _) {
    ...
    if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
      Log_1.Log.Info("LevelEvent", 79, "LevelEvent行为是否分帧", ["EventName", n], ["是否分帧", n && this.Nwm.has(n)], ["普通分帧任务队列长度", this.Vwm.length], ["低优先级分帧队列长度", this.jwm.length]);
    }
    if (!n || !this.Nwm.has(n)) {
      return false;
    }
    ...
    if (r.EntityId && EntitySystem_1.EntitySystem.GetComponent(r.EntityId, 0)?.GetPbModelConfig()?.EntityType === "MusicListener") {
      if (this.jwm.length > this.EAm) {
        ...
        if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
          Log_1.Log.Info("LevelEvent", 79, "LevelEvent行为进入低优先级分帧队列", ["EventName", n], ["低优先级分帧队列长度", this.jwm.length]);
        }
        return true;
      }
    }
    if (this.Vwm.length > MAX_ACTION_QUEUE_LENGTH) {
      if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckWarn()) {
        Log_1.Log.Warn("LevelEvent", 79, "LevelEvent分帧任务超过最大容量, 需要关注", ["EventName", n]);
      }
      return false;
      ...
      if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("LevelEvent", 79, "LevelEvent行为进入普通分帧队列", ["EventName", n], ["分帧队列长度", this.Vwm.length]);
      }
      return true;
    }
  }
  static UpdateActionFrameScheduler() {
    while (this.Vwm.length > 0 && this.Hwm < this.Gwm) {
      this.Vwm.shift()?.();
      this.Hwm++;
      if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("LevelEvent", 79, "执行普通分帧队列任务LevelEvent行为", ["当前帧已执行任务数", this.Hwm], ["每帧最大任务数", this.Gwm]);
      }
    }
    while (this.jwm.length > 0 && this.Hwm < this.Gwm) {
      var e = this.jwm.shift();
      if (!e) {
        return;
      }
      if (Time_1.Time.Frame - e.FrameEnqueued > LOW_PRIORITY_ACTION_ALIVE_FRAME) {
        ...
        this.Hwm++;
        if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
          Log_1.Log.Info("LevelEvent", 79, "执行低优先级分帧队列任务LevelEvent行为", ["当前帧已执行任务数", this.Hwm], ["每帧最大任务数", this.Gwm]);
        }
      }
    }
    this.Hwm = 0;
  ...
  static ClearActionFrameScheduler() {
    if (this.FrameSchedulerLogEnabled && Log_1.Log.CheckInfo()) {
      Log_1.Log.Info("LevelEvent", 79, "离开场景前清理积压的LevelEvent分帧任务", ["分帧任务队列长度", this.Vwm.length]);
    }
    this.Hwm = 0;
    this.Vwm.length = 0;
  }
  static OnSettingFrameRateChanged(e) {
    if (e <= FRAME_LIMIT_LEVEL_1) {
      this.Fwm = MAX_ACTION_PER_FRAME_LEVEL_1;
    } else if (e <= FRAME_LIMIT_LEVEL_2) {
      this.Fwm = MAX_ACTION_PER_FRAME_LEVEL_2;
    } else {
      this.Fwm = MAX_ACTION_PER_FRAME_LEVEL_3;
    }
  }
...
(exports.LevelGeneralBaseFrameScheduler = LevelGeneralBaseFrameScheduler).Nwm = new Set([EventDefine_1.EEventName.CheckMusicBeatsEvent]);
LevelGeneralBaseFrameScheduler.Vwm = [];
LevelGeneralBaseFrameScheduler.jwm = [];
LevelGeneralBaseFrameScheduler.Fwm = 0;
LevelGeneralBaseFrameScheduler.EAm = MAX_ACTION_QUEUE_LENGTH;
LevelGeneralBaseFrameScheduler.Hwm = 0;

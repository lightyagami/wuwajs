...
class TimeUtil {
  ...
  static SetServerTimeStamp(t) {
    Time_1.Time.SetServerTimeStamp(Number(MathUtils_1.MathUtils.LongToBigInt(t))), this.InitNextDayTimeStamp()
  }
  static Tick(t) {
    this.kde && Time_1.Time.ServerTimeStamp >= this.kde && this.InitNextDayTimeStamp()
  }
  static InitNextDayTimeStamp() {
    var t = new Date(Time_1.Time.ServerTimeStamp);
    t.getHours() >= TimeUtil.CrossDayHour && t.setDate(t.getDate() + 1), t.setHours(TimeUtil.CrossDayHour, 0, 0, 0), this.kde = t.getTime() + Math.random()
  }
  ...
  static GetTimeString(t) {
    var e;
    return t < 0 ? "" : (e = (e = t % this.Minute) < 10 ? "0" + e : e.toString(), ((t = Math.floor(t / this.Minute)) < 10 ? "0" + t : t.toString()) + ":" + e)
  }
  ...
  static CalculateDayGapBetweenNow(t, e) {
    var i = Time_1.Time.ServerTimeStamp / TimeUtil.InverseMillisecond,
      ...
      i = e ? t - i : i - t,
      t = (i < 0 && Log_1.Log.CheckDebug() && Log_1.Log.Debug("Mail", 27, "时间非法"), i / 86400);
    let n = t;
    return t < 2 && (n = e ? r.getMonth() < a.getMonth() ? 1 : a.getDate() - r.getDate() : r.getMonth() > a.getMonth() ? 1 : r.getDate() - a.getDate()), parseInt(n.toFixed(0))
  }
  ...
  static CalculateRemainingTime(e, i = 1) {
    if (!(e <= 0)) {
      ...
      for (var r = {
        ...
        if (a) return r.TimeValue = a[0], r.TextId = CommonDefine_1.remainTimeTextId[t], r.RemainingTime = a[1] + TimeUtil.TimeDeviation, r;
        --t
      }
      return r
    }
  ...
  static GetRemainTimeDataFormat3(t) {
    let e = [0, 0];
    return t > CommonDefine_1.SECOND_PER_DAY ? e = [3, 2] : t > CommonDefine_1.SECOND_PER_HOUR ? e = [2, 1] : t > CommonDefine_1.SECOND_PER_MINUTE && (e = [1, 0]), TimeUtil.Vde(t, 1, e[0], e[1])
  }
  static GetRemainTimeDataFormat4(t) {
    var e = [1, 1];
    return t < CommonDefine_1.SECOND_PER_MINUTE ? TimeUtil.Vde(CommonDefine_1.SECOND_PER_MINUTE, 1, e[0], e[1]) : TimeUtil.Vde(t, 1, e[0], e[1])
  }
  ...
  static GetCurrentCrossDayStamp() {
    var t = new Date(Time_1.Time.ServerTimeStamp);
    return t.getHours() < TimeUtil.CrossDayHour && t.setDate(t.getDate() - 1), t.setHours(TimeUtil.CrossDayHour, 0, 0, 0), t.getTime()
  }
...
}(exports.TimeUtil = TimeUtil).OneDayHourCount = 24, TimeUtil.Hour = 3600, TimeUtil.Minute = 60, TimeUtil.OneDaySeconds = 86400, TimeUtil.Millisecond = .001, TimeUtil.InverseMillisecond = 1e3, TimeUtil.CrossDayHour = 4, TimeUtil.kde = 0, TimeUtil.TimeDeviation = .1, TimeUtil.Fde = {
  [0]: t => {
    if (0 < t) return [t = Math.floor(t), t]
  },
  1: t => {
    if (t >= CommonDefine_1.SECOND_PER_MINUTE) return [(t - (t = t % CommonDefine_1.SECOND_PER_MINUTE)) / CommonDefine_1.SECOND_PER_MINUTE, t]
  },
  2: t => {
    if (t >= CommonDefine_1.SECOND_PER_HOUR) return [(t - (t = t % CommonDefine_1.SECOND_PER_HOUR)) / CommonDefine_1.SECOND_PER_HOUR, t]
  },
  3: t => {
    if (t >= CommonDefine_1.SECOND_PER_DAY) return [(t - (t = t % CommonDefine_1.SECOND_PER_DAY)) / CommonDefine_1.SECOND_PER_DAY, t]
  }
};

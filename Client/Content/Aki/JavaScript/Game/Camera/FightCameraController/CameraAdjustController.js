...
exports.CameraAdjustController = undefined;
...
class CameraAdjustController extends CameraControllerBase_1.CameraControllerBase {
  constructor() {
    ...
    this.DefaultAdjustFadeCurve = undefined;
    this.tle = false;
    this.ile = false;
    this.ole = 0;
    this.rle = 0;
  ...
  ApplyCameraAdjust() {
    ...
    var l;
    var n;
    if (this.IsActivate && this.Camera.TargetEntity && this.Camera.IsTargetLocationValid && (h = this.Camera.PlayerLocation, t = this.Camera.TargetLocation, l = this.Camera.CurrentCamera.ArmRotation, s = Vector_1.Vector.Create(), l.Vector(s), t.Subtraction(h, this.lle), h = !this.lle.IsNearlyZero() && !s.IsNearlyZero(), i = this.lle.CosineAngle2D(s), a = this.lle.Size(), r = this.lle.Z < 0 ? this.CheckLowAdjustPitchMin : this.CheckTallAdjustPitchMin, n = this.lle.Z < 0 ? this.CheckLowAdjustPitchMax : this.CheckTallAdjustPitchMax, e = a < this.CheckAdjustNearerRange ? this.CheckAdjustNearerYawAngleMin : this.CheckAdjustFartherYawAngleMin, a = a < this.CheckAdjustNearerRange ? this.CheckAdjustNearerYawAngleMax : this.CheckAdjustFartherYawAngleMax, this.ile = l.Pitch < r || l.Pitch > n, this.nle = !h || i > Math.cos(e * MathUtils_1.MathUtils.DegToRad) || i < Math.cos(a * MathUtils_1.MathUtils.DegToRad), !(r = this.Camera.CheckPositionInScreen(t, this.CheckInScreenMinX, this.CheckInScreenMaxX, this.CheckInScreenMinY, this.CheckInScreenMaxY)) || this.ile || this.nle) && (this.Camera.CameraAutoController.EnableForce(this), this.nle = true, this.tle = true, this.hle = 0, this.nle && (l = this.lle.SineAngle2D(s), this.sle = this.Camera.CurrentCamera.ArmRotation.Yaw, n = Rotator_1.Rotator.Create(), this.lle.Rotation(n), this.ale = n.Yaw, l > 0 ? !r || i > Math.cos(e * MathUtils_1.MathUtils.DegToRad) ? this.ale += e : i < Math.cos(a * MathUtils_1.MathUtils.DegToRad) ? this.ale += a : this.ale = this.sle : !r || i > Math.cos(e * MathUtils_1.MathUtils.DegToRad) ? this.ale -= e : i < Math.cos(a * MathUtils_1.MathUtils.DegToRad) ? this.ale -= a : this.ale = this.sle, this.sle - this.ale > 180 ? this.ale += 360 : this.ale - this.sle > 180 && (this.ale -= 360)), this.ile)) {
      this.ole = MathUtils_1.MathUtils.StandardizingPitch(this.Camera.CurrentCamera.ArmRotation.Pitch);
      this.rle = MathUtils_1.MathUtils.StandardizingPitch(this.Camera.AdjustPitch(this.lle));
    }
  }
  UpdateInternal(t) {
    return;

...
class TsTaskMoveToTarget extends TsTaskAbortImmediatelyBase_1.default {
  ...
  InitTsVariables() {
    this.IsInitTsVariables && !GlobalData_1.GlobalData.IsPlayInEditor || (this.IsInitTsVariables = !0, this.TsMoveMode = this.MoveMode, this.TsMoveState = this.MoveState, this.TsIsFollow = this.IsFollow, this.TsIsFly = this.IsFly, this.TsDistance = this.Distance, this.TsTargetEntityId = this.TargetEntityId, this.TsTargetPos = Vector_1.Vector.Create(this.TargetPos.X, this.TargetPos.Y, this.TargetPos.Z), this.LastLocation = Vector_1.Vector.Create(), this.TargetLocation = Vector_1.Vector.Create(), this.TmpVector = Vector_1.Vector.Create())
  }
  ReceiveExecuteAI(t, i) {
    this.InitTsVariables();
    var s, e = t.AiController;
    e ? (s = (e = e.CharActorComp).Entity, this.MoveComp = s.GetComponent(45), this.GetMoveToTargetPosition(this.TargetLocation) ? this.TryFindPathToTarget(this.TargetLocation) ? (this.LastTime = Time_1.Time.WorldTime, this.LastLocation?.Reset(), this.LastLocation?.DeepCopy(e.ActorLocationProxy)) : this.Finish(!0) : this.FinishExecute(!0)) : (Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "错误的Controller类型", ["Type", t.GetClass().GetName()]), this.FinishExecute(!0))
  }
  ReceiveTickAI(t, i, s) {
    var t = t.AiController;
    !t || (t = t.CharActorComp, this.CheckMoveFailed(t, s)) ? this.Finish(!0) : this.TsIsFollow && (this.GetMoveToTargetPosition(this.TmpVector) ? Vector_1.Vector.DistSquared(this.TmpVector, this.TargetLocation) > this.TsDistance * this.TsDistance && (this.TryFindPathToTarget(this.TmpVector) ? this.TargetLocation.DeepCopy(this.TmpVector) : this.Finish(!0)) : this.Finish(!0))
  }
  OnClear() {
    this.MoveComp && (this.MoveComp.StopMoveNew(), this.MoveComp = void 0), this.LastTime = 0, this.MoveFailedCountDown = 0, this.LastLocation?.Reset(), this.TargetLocation?.Reset()
  }
  CheckMoveFailed(t, i) {
    ...
    if ((Time_1.Time.WorldTime - this.LastTime) * MathUtils_1.MathUtils.MillisecondToSecond * MIN_MOVE_SPEED >= s) {
      if (this.MoveFailedCountDown += i, this.MoveFailedCountDown >= MOVE_FAILED_TIME) return t.SetActorLocation(this.TargetLocation.ToUeVector(), "[TsTaskMoveToLocation]长时间处于某点", !1), !0
    } else this.MoveFailedCountDown = 0;
    return this.LastTime = Time_1.Time.WorldTime, this.LastLocation.DeepCopy(t.ActorLocationProxy), !1
  }

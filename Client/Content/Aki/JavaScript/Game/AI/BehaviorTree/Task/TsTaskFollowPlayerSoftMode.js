...
class TsTaskFollowPlayerSoftMode extends TsTaskAbortImmediatelyBase_1.default {
    ...
    if (!this.IsInitTsVariables || GlobalData_1.GlobalData.IsPlayInEditor) {
      ...
      if (this.RotateInfo) {
        ...
        for (let t = 0; t < i.Num(); t++) {
          this.TsNotSyncAxisList.push(i.Get(t));
        }
      } else {
        this.TsRotateSpeed = 2000;
        this.TsRotateType = 0;
  ...
  ReceiveExecuteAI(t, i) {
    ...
    if (h) {
      if ((s = h.CharActorComp)?.Valid) {
        ...
        this.MoveComp = h.CharActorComp?.Entity?.GetComponent(48);
    ...
    if (h) {
      ...
      if (r?.Valid) {
        ...
        if (t?.Valid) {
          ...
          if (!this.TsKeepDistanceMin && s < this.TsMinSquared) {
            o = this.TempTarget.Z - e.Z;
            if (Math.abs(o) >= START_MOVE_HEIGHT_LIMIT) {
              this.TempVector.Set(0, 0, o);
              this.LerpMove(this.TempVector, h);
            }
          } else {
            e.Subtraction(this.TempTarget, this.TempVector);
            Vector_1.Vector.VectorPlaneProject(this.TempVector, t.ActorUpProxy, this.TempDirection);
            this.TempDirection.Normalize();
            this.TempDirection.MultiplyEqual(this.TsDistanceMin);
            this.TempTarget.AdditionEqual(this.TempDirection);
            if (s < this.TsSweepMoveSquared) {
              this.TempTarget.Subtraction(e, this.TempVector);
              this.LerpMove(this.TempVector, h);
            } else if (s < this.TsDistanceForceMove * this.TsDistanceForceMove) {
              r.SetActorLocation(this.TempTarget.ToUeVector(), "FollowPlayerSoftMode", true);
            } else if (!this.CheckObstacle(this.TempTarget, this.TempTarget) || !(o = r.DefaultRadius * 2, this.TempTarget.Subtraction(e, this.TempVector), h = this.TempVector.Size(), (h -= o) <= 0 || (this.TempVector.Normalize(), this.TempVector.MultiplyEqual(h), e.Addition(this.TempVector, this.TempTarget), this.CheckObstacle(this.TempTarget, this.TempTarget)))) {
              r.SetActorLocation(this.TempTarget.ToUeVector(), "FollowPlayerSoftMode", false);
            }
          }
        } else {
          this.FinishExecute(false);
  ...
  LerpMove(t, i) {
    ...
    if (!(h < 0)) {
      h = (s = t.Size()) / h;
      if ((h = MathUtils_1.MathUtils.Lerp(0, this.TsMaxMoveSpeed, h) * i) < s) {
        t.Normalize();
        t.MultiplyEqual(h);
      }
      this.MoveComp?.MoveCharacter(t, 1, "FollowPlayerSoftMode");
    }
  }
  UpdateRotate(s, h) {
    if (this.MoveComp) {
      ...
      switch (this.TsRotateType) {
        case 1:
        case 2:
          if (this.TsRotateEntityIdBlackboardKey !== "" && (e = ControllerHolder_1.ControllerHolder.BlackboardController.GetEntityIdByEntity(o.Id, this.TsRotateEntityIdBlackboardKey))) {
            t = EntitySystem_1.EntitySystem.Get(e);
          }
          break;
        case 3:
          t = o.GetComponent(43)?.SkillTarget?.Entity;
      }
      let i = undefined;
      if ((i = t?.Valid ? t.GetComponent(1) : i) && this.TsRotateType === 2) {
        ...
        if (i) {
          ...
          this.TempTarget.FromUeVector(Global_1.Global.CharacterCameraManager.D_GetCameraLocation());
          this.TempCameraForward.FromUeVector(Global_1.Global.CharacterCameraManager.GetActorForwardVector());
          this.TempTarget.AdditionEqual(this.TempCameraForward.MultiplyEqual(this.TsCameraDistance));
        }
      ...
      if (!(this.TsNotSyncAxisList.length <= 0)) {
        ...
        if (r) {
          ...
          Quat_1.Quat.FindBetween(Vector_1.Vector.UpVectorProxy, this.MoveComp.GravityUp, this.TempQuat);
          GravityUtils_1.GravityUtils.GetRotatorInNormal(this.TempRotator, this.TempQuat, this.TempInvertRotator1);
          GravityUtils_1.GravityUtils.GetRotatorInNormal(s.ActorRotationProxy, this.TempQuat, this.TempInvertRotator2);
        }

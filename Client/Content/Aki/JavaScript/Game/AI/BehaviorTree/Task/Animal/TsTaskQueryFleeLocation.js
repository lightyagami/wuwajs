...
const queryExtent = new UE.VectorDouble(100, 100, 1000);
class QuatNode {
  constructor(t, e) {
...
class TsTaskQueryFleeLocation extends TsTaskAbortImmediatelyBase_1.default {
  constructor() {
    ...
    this.VectorCache1 = undefined;
    this.VectorCache2 = undefined;
    this.VectorCache3 = undefined;
    this.QuatNodeQueue = undefined;
  ...
  static InitStaticVariables() {
    ...
    TsTaskQueryFleeLocation.QuaternionQueue.push(Quat_1.Quat.Create(0, 0, 0, 1));
    var e = MathUtils_1.PI_DEG / ANGLE_INTERVAL;
    for (let t = 1; t < e; ++t) {
      var i = t * ANGLE_INTERVAL * 0.5 * MathUtils_1.MathUtils.DegToRad;
      var s = Quat_1.Quat.Create(0, 0, Math.sin(i), Math.cos(i));
  ...
  ReceiveExecuteAI(t, e) {
    ...
    if (i) {
      ...
      if (s && Log_1.Log.CheckInfo()) {
        Log_1.Log.Info("AI", 6, "AnimalDebug SwitchAnimalState2", ["Tree", this.TreeAsset?.GetName()], ["WorldTimeSeconds", Time_1.Time.WorldTimeSeconds], ["CdInternal", this.CdInternal]);
      }
      if (Time_1.Time.WorldTimeSeconds < this.CdInternal) {
        this.Finish(false);
      } else {
        ...
        if (this.FindTarget()) {
          if (this.QueryFleeLocation()) {
            ...
            ControllerHolder_1.ControllerHolder.BlackboardController.SetVectorValueByEntity(this.Character.Entity.Id, BLACKBOARD_KEY_FLEE_LOCATION, i.X, i.Y, i.Z);
            if (s && Log_1.Log.CheckInfo()) {
              Log_1.Log.Info("AI", 6, "AnimalDebug QueryFlee4", ["FleeLocation", i]);
            }
            this.Finish(true);
          } else {
            if (s && Log_1.Log.CheckInfo()) {
              Log_1.Log.Info("AI", 6, "AnimalDebug QueryFlee3 QueryFleeLocation Failed");
            }
            this.Finish(false);
          }
        } else {
  ...
  FindTarget() {
    if (this.TsTargetKey) {
      ...
      if (t) {
        ...
        if (t) {
          this.TargetCharacter = t.GetComponent(3);
          return true;
        }
      }
    }
  ...
  QueryFleeLocation() {
    ...
    t.Subtraction(this.TargetCharacter.ActorLocationProxy, this.VectorCache1);
    ...
    this.FoundPath = false;
    var e = this.Character.ActorForwardProxy;
    for (const a of this.QuatNodeQueue) {
      ...
      a.Cost = (1 - e.DotProduct(this.VectorCache2)) * 0.5;
      a.VectorCache.AdditionEqual(t);
    }
    let i = TURN_COST_WEIGHT;
    var s = this.VectorCache3.DotProduct(e);
    if (s > TURN_COST_DIVIDING_LINE_3) {
      i = TURN_COST_WEIGHT_3;
    } else if (s > TURN_COST_DIVIDING_LINE_2) {
      i = TURN_COST_WEIGHT_2;
    }
    this.QuatNodeQueue.sort((t, e) => i * (t.Cost - e.Cost) + (1 - i) * (t.CostBase - e.CostBase));
    this.DebugDraw2();
    for (const h of this.QuatNodeQueue) {
      var r = (0, puerts_1.$ref)(undefined);
      if (UE.NavigationSystemV1.D_K2_ProjectPointToNavigation(GlobalData_1.GlobalData.World, h.VectorCache.ToUeVector(), r, undefined, undefined, queryExtent)) {
        var r = Vector_1.Vector.Create((0, puerts_1.$unref)(r));
        var o = AiContollerLibrary_1.AiControllerLibrary.NavigationFindPath(GlobalData_1.GlobalData.World, t.ToUeVector(), r.ToUeVector(), this.NavigationPath);
        if (o && (this.FoundPath = o, Log_1.Log.CheckWarn())) {
          Log_1.Log.Warn("Test", 6, "FoundPath", ["Actor", this.Character?.Actor.GetName()], ["target", r], ["self", t]);
        }
        if (this.FoundPath) {
          if (GlobalData_1.GlobalData.IsPlayInEditor && this.DebugMode) {
      ...
      if (GlobalData_1.GlobalData.IsPlayInEditor && this.DebugMode) {
        this.DebugDraw(h.VectorCache.ToUeVector(), ColorUtils_1.ColorUtils.LinearRed);
      }
    }
...
TsTaskQueryFleeLocation.QuaternionQueue = undefined;
exports.default = TsTaskQueryFleeLocation; //# sourceMappingURL=TsTaskQueryFleeLocation.js.map
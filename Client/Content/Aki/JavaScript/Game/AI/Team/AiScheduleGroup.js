...
class AiScheduleGroup {
  ...
  Use() {
    ...
    for (const _ of this.dse) {
      ...
      if (_.CharActorComp.Entity.CheckGetComponent(217).HasTag(-1503953470)) {
      ...
      if (t.HasAttack) {
        if (t.NextScheduleTimeAttack < Time_1.Time.WorldTime && (!_.CharActorComp.Entity.CheckGetComponent(217).HasTag(-1371021686) || t.NextScheduleTimeOut < Time_1.Time.WorldTime)) {
          AiScheduleGroup.Pse.push(_);
        }
      } else if (t.NextScheduleTimeNoAttack < Time_1.Time.WorldTime) {
        AiScheduleGroup.Pse.push(_);
      } else if (_.CharActorComp.Entity.CheckGetComponent(217).HasTag(-1371021686)) {
        t.HasAttack = true;
    ...
    if (i > 0) {
      ...
      for (const a of this.fse) {
        ...
        for (const c of a) {
          ...
          if (s && s.InZone && !s.IsAttacker && !c.CharActorComp.Entity.CheckGetComponent(217).HasTag(-1371021686)) {
  ...
  Ese() {
    var e = this.Gsn;
    var t = this.Target.Entity.GetComponent(67);
    let i = 0;

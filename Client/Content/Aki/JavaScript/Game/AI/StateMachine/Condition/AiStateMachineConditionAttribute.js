...
class AiStateMachineConditionAttribute extends AiStateMachineCondition_1.AiStateMachineCondition {
  constructor() {
    super(...arguments), this.ine = 0, this.one = 0, this.rne = 0, this.oie = (t, i, s) => {
      this.ResultSelf = i >= this.one && i <= this.rne
    }
  }
  OnInit(t) {
    this.ine = t.CondAttribute.AttributeId, this.one = t.CondAttribute.Min, this.rne = t.CondAttribute.Max, this.Node.AttributeComponent.AddListener(this.ine, this.oie, "AiConditionEvent");
  ...
  ToString(t, i = 0) {
    super.ToString(t, i), t.Append(`属性 [${this.ine}, ${this.one}, ${this.rne}]
`)
  }
}

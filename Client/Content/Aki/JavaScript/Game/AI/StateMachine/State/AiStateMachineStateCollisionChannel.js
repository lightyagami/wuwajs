...
class AiStateMachineStateCollisionChannel extends AiStateMachineState_1.AiStateMachineState {
  ...
  OnInit(t) {
    this.IgnoreChannels = [];
    for (const e of t.BindCollisionChannel.IgnoreChannels) this.IgnoreChannels.push(e);
    return !0
  }
  OnActivate() {
    for (const t of this.IgnoreChannels) this.Node.ActorComponent.Actor.CapsuleComponent.SetCollisionResponseToChannel(t, 0)
  }
  OnDeactivate() {
    for (const t of this.IgnoreChannels) this.Node.ActorComponent.Actor.CapsuleComponent.SetCollisionResponseToChannel(t, 2)
  }
}

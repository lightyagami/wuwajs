...
class AiConfig extends ConfigBase_1.ConfigBase {
  constructor() {
    super(...arguments), this.CommonStateMachineJsonObject = void 0, this.Ete = new Set
  }
  OnInit() {
    var e = AiStateMachineConfigById_1.configAiStateMachineConfigById.GetConfig(commonStateMachine);
    if (!e) return Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 30, "AiConfig Init fail, no commonStateMachine config"), !1;
    this.CommonStateMachineJsonObject = JSON.parse(e.StateMachineJson);
    for (const r of BlackboardWhiteListAll_1.configBlackboardWhiteListAll.GetConfigList()) this.Ete.add(r.Key);
    return !0
  }
  OnClear() {
  ...
  LoadAiConfig(e, r, i = !1) {
    ...
    e.AiBase = AiBaseById_1.configAiBaseById.GetConfig(r), e.AiBase ? (e.AiBase.SubBehaviorConfigs && this.Ste(e, i), e.AiBase.StateMachine && (e.StateMachineConfig = AiStateMachineConfigById_1.configAiStateMachineConfigById.GetConfig(e.AiBase.StateMachine))) : Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiBase配置", ["Id", r], ["Target", o.Actor.GetName()], ["CreatureId", o.CreatureData.GetOwnerId()])
  }
  yte(e) {
    var r = e.CharActorComp,
      i = r.CreatureData?.GetPbEntityInitData();
    i && (i = (0, IComponent_1.getComponent)(i.ComponentsData, "AiComponent"))?.InitState && 1 === i.InitState.Type && i.InitState.Wander && !e.AiWanderRadiusConfig && (e.AiWanderRadiusConfig = AiWanderRadiusConfigById_1.configAiWanderRadiusConfigById.GetConfig(i.InitState.Wander), !e.AiWanderRadiusConfig) && Log_1.Log.CheckWarn() && Log_1.Log.Warn("BehaviorTree", 57, "缺少AiWander的范围配置", ["Id", i.InitState.Wander], ["Target", r.Actor.GetName()], ["CreatureId", r.CreatureData.GetOwnerId()])
  }
  Ste(e, r = !1) {
    ...
    if (n && (e.AiWanderInfos || (e.AiWanderInfos = new AiWanderInfos_1.AiWanderInfos), e.AiWanderInfos.AiWander = AiWanderById_1.configAiWanderById.GetConfig(Number(n)), e.AiWanderInfos.AiWander || Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiWander配置", ["Id", n], ["Target", i.Actor.GetName()], ["CreatureId", i.CreatureData.GetOwnerId()])), n = e.AiBase.SubBehaviorConfigs.get("AiBattleWander")) {
      e.AiWanderInfos || (e.AiWanderInfos = new AiWanderInfos_1.AiWanderInfos);
      var a = AiBattleWanderById_1.configAiBattleWanderById.GetConfig(Number(n));
      if (a)
        if (0 === a.GroupIds.length) e.AiWanderInfos.AiBattleWanderGroups = new Array;
        else {
          var t = new Array;
  ...
  Ite(e) {
    ...
    for (const t of e.BaseSkill.RandomSkills)
      for (const A of t.ArrayInt) i.has(A) || (i.add(A), r = AiSkillInfosById_1.configAiSkillInfosById.GetConfig(A), o.push(r));
    i.clear();
    var n, a = new Array;
    for (const d of o) e.SkillInfos.set(d.Id, d), i.has(d.SkillPreconditionId) || (i.add(d.SkillPreconditionId), n = AiSkillPreconditionById_1.configAiSkillPreconditionById.GetConfig(d.SkillPreconditionId), a.push(n));
    for (const f of a) e.SkillPreconditionMap.set(f.Id, f);
    e.InitTagMap()
  }
  LoadAiTeamConfigNew(e, r) {
    if (e.AiTeamLevel = AiTeamLevelNewById_1.configAiTeamLevelNewById.GetConfig(r), e.AiTeamLevel) {
      e.AiTeamAreas = [];
      ...
      for (const t of e.AiTeamAreas) {
        ...
        n ? e.AiTeamAttacks.push(n) : (n = AiTeamAttackById_1.configAiTeamAttackById.GetConfig(t.AttackWeightId)) ? (o.set(t.AttackWeightId, n), e.AiTeamAttacks.push(n)) : Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "没有配置AiTeamAttack", ["AttackWeightId", t.AttackWeightId], ["LocationId", t.Id])
      }
    } else Log_1.Log.CheckError() && Log_1.Log.Error("AI", 6, "没有配置AiTeamLevel", ["Id", r])
  }
    ...
    if (e && e.SubBehaviorConfigs) {
      ...
      if (e) {
        ...
        if (!(e.length <= r)) {
          ...
          if (i) return i;
          Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiPatrol配置", ["Id", e[r]])
        }
      }
    }
  }
  LoadAiPatrolConfigById(e) {
    ...
    if (r) return r;
    Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiPatrol配置", ["Id", e])
  }
  Tte(e, r, i) {
    if (r) {
      var o = AiSenseGroupById_1.configAiSenseGroupById.GetConfig(Number(r));
      if (o) {
        if (0 === o.AiSenseIds.length) return new(AiConfig.AsyncAiPerception ? AsyncAiPerception_1.AsyncAiPerception : AiPerception_1.AiPerception)(e, o, new Array);
        let r = 0;
        var n = [],
          a = new Array;
        for (const d of o.AiSenseIds) {
          let e = -1;
          0 === r && i ? (e = i.FirstAiSenseId, n.push(i.FirstAiSenseId)) : (e = d, n.push(d));
      ...
      Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiSenseGroup配置", ["Id", r], ["Actor", e.CharActorComp.Actor.GetName()])
    }
  }
  LoadAiSense(e) {
    var r = AiSenseById_1.configAiSenseById.GetConfig(Number(e));
    if (r) return r;
    Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiSense配置", ["AiSenseId", e])
  }
  LoadSpecialHateAndSenseConfig(e) {
    e = e.CharActorComp.Entity.GetComponent(0).GetMonsterComponent();
    let r = void 0;
    return e?.SpecialHateAndSenseConfig && !(r = SpecialHateAndSenseById_1.configSpecialHateAndSenseById.GetConfig(e.SpecialHateAndSenseConfig)) && Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少SpecialHateAndSense配置", ["Id", e.SpecialHateAndSenseConfig]), r
  }
  LoadAiHate(e) {
    var r;
    if (e) return (r = AiHateById_1.configAiHateById.GetConfig(Number(e))) || Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiHate配置", ["Id", e]), r
  }
  ...
  LoadAiAlert(e) {
    var r;
    if (e) return (r = AiAlertById_1.configAiAlertById.GetConfig(Number(e))) || Log_1.Log.CheckError() && Log_1.Log.Error("BehaviorTree", 6, "缺少AiAlert配置", ["Id", e]), r
  }
}(exports.AiConfig = AiConfig).AsyncAiPerception = !1, AiConfig.CppAsyncAiPerception = !1;

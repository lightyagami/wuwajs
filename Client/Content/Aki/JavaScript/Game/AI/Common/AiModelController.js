...
class AiModelController extends ControllerBase_1.ControllerBase {
  ...
  static OnTick(o) {
    ...
    for (var [, r] of this.Model.ActiveAiTeams) 0 < r.TeamMemberToGroup.size && r.Tick();
    this.Dte()
  }
  static Dte() {
    for (var [, o] of this.Model.HatredGroups)
      for (var [, r] of o)
        for (const s of r) s.CharAiDesignComp?.Valid && s.CharAiDesignComp.Entity.Valid || r.delete(s);
